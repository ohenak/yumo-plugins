...
| **M-7f** | **The untyped phases reach M-7e's halt path.** Phase CR calls the *same* `reviewLoop` as every document-typed phase, passing `docType: null`, so an exhausted Phase CR window writes `POSTMORTEM-CR-{feature}.md` by the same inline prompt — the post-mortem write is not conditioned on a document type anywhere on that path. Recorded because `REQ-RCV-01` O-10 owes a leg asserting a Phase CR halt creates **no** `## Reset Region` (N-7), and that leg is vacuous unless the halt writes a file at all. | `pdlc/workflows/orchestrate-dev.js:4721-4733` (the Phase CR call, `docType: null` at `:4724`), the halt trigger at `:1933-1937` |
...
| `Iterations (budget {MAX_REVIEW_ROUNDS}, rounds run {k})` | that exact render, two decimal integers ≥ 0, `{k}` the rounds the halting **entry** dispatched | the halt path's post-mortem section list | RCV-01 | The post-mortem's Iterations section. **Replaces** the baseline literal `` `Iterations (${MAX_REVIEW_ROUNDS} — limit reached)` `` (M-1c), which carries one number and a *"limit reached"* phrase that is false on `REQ-RCV-01` AC-1.5(1)'s zero-round halt. Two labelled integers because AC-1.3's whole point is that the budget and the rounds run are **not** the same number and must not be conflated where the operator reads them; labelling them is also what lets the test assert an equality rather than a substring match. |
...
| `Iterations (budget …, rounds run …)` | AC-1.3's declared render, added at round 1 of the reset Phase R window (v2.7). Produced by **O-14**, asserted by **O-10**. It is this REQ's only operator-facing string; the refusal strings are `REQ-RCV-07` §6's. |
