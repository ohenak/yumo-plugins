...
**Why the force fixture is *no prior post-mortem → force → zero-round halt → force again*** (`REQ-RCV-01` O-10, relocated 2026-08-01 round 1 of the reset window): it is the path that reaches a first force with **no unconsumed clearance present**. A resolved post-mortem with `A < H` grants a window on that entry, so the forced phase would dispatch and the leg would assert nothing about the cap.

**Leg 4, the derived start below `W`** (`REQ-RCV-01` AC-1.5(2); relocated from that REQ's O-10, 2026-08-01 round 1 of the reset window, nothing changed in meaning). Both legs are stated by the **highest existing round**, since *"start"* and *"highest + 1"* read alike and the pair must discriminate. *Granting leg:* `W = 4`, the clearance already spent (`A = H`, so this entry grants nothing and `W` is read from a durable region), every cross-review file of that document type removed ⇒ **no** highest existing round, derived start 1, origin wins ⇒ the entry starts at **4**, dispatches, overwrites nothing. *Control:* the same `W = 4` with highest existing round **6** ⇒ derived start **7**, past the window end `W + 2 = 6` ⇒ **halts on the budget path**, no dispatch. A control at highest **5** would *dispatch* — round 6 is the window's last slot — so it discriminates nothing.

**The two Iterations-section legs** (`REQ-RCV-01` AC-1.3/AC-1.4; relocated from that REQ's O-10, same date and round). (i) The section asserted as an **equality on the declared render** (baseline §3), both integers — not a substring match, which any rendering satisfies — on **three named fixtures**: the **creating** halt, the **re-halt** on a post-mortem whose section reads `rounds run {k}`, `k > 0` (asserted to read `rounds run 0` after), and a **re-halt on a post-mortem carrying no Iterations section at all**, asserted to gain exactly one, appended per `REQ-RCV-01` AC-1.4 clause 3's not-found disposition. The equality is honest on all three because the render is the **loop's** output on every halt in scope, not an authoring agent's; a fixture whose expected string a test double was told to write would prove nothing. The second fixture is the point of the leg: it is the case AC-1.3 exists for. (ii) The **re-halt leg**: **0** authoring dispatches, and only what AC-1.4's three clauses mandate changed — stated as **byte-equality against the expected file, the prior bytes with clauses 1, 2 and 3's edits applied**, plus the dispatch count, since a body an agent rewrites identically satisfies neither alone. *"A byte comparison with the region span excised"* would be unsatisfiable on every reachable fixture: a re-halt is reachable only through a post-mortem the operator resolved (M-7a), so every such fixture carries an unfenced `RESOLVED: yes` **outside** the region which clause 2 removes. (iii) A leg for **either halt-path write left unconfirmed**, in **two fixtures that must be discriminated by the ❌ text**, not merged (`REQ-RCV-01` v3.1): both refuse the phase with no answering line, `H` and `A` unmoved, **`notice` empty** — row B's *unconfirmable-append* variant, an IO fault of the loop being no state of the region, so no S-16 — and **no `RESOLVED:` line is stripped**. *Fixture A, the lost `HALT-REASON:` append:* clause 3 has already landed, so the expected file carries **this entry's Iterations render** (pinned, not incidental) and the ❌ text is `Refused — halt line unconfirmed at {path}`; nothing is stripped because clauses 1 and 2 are **one update** and the update did not land (`REQ-RCV-01` AC-1.4, split §5.8). *Fixture B, the failed Iterations write:* the entry ends before clause 1, so the region is **byte-unchanged** and the ❌ text is `Refused — iterations section unconfirmed at {path}`. A single leg asserting the generalised render passes either way and falsifies nothing, which is why the ❌ text is the discriminating conjunct here; catalogue §4's act 1 differs between the two, and on fixture B does not apply at all.
...
### 5.8 Why AC-1.4's clauses 1 and 2 are one update, not two ordered writes

Relocated from `REQ-RCV-01` AC-1.4 *the ordering and its report* (2026-08-01, round 3 of the reset
window) so the REQ keeps the rule and this file keeps its reason; **nothing changed meaning in the
move**. The rule itself — *after a halt in scope there is no reachable state in which this halt's
`HALT-REASON:` line is present and an unfenced `RESOLVED:` line survives* — stays in AC-1.4, as does
the single confirmation over both edits and the disposition of its failure.

**The reason.** Were the strip separately losable, an unconfirmed strip after a landed append would
leave `H` incremented, `A` unmoved and a **readable** `RESOLVED:` marker in the file. `REQ-RCV-01`
AC-1.5(4)'s gate reads exactly those two things — marker readable, `A < H` — so the next entry would
grant a window no operator cleared, and, because only the strip ever removes a marker, a persistent
write fault would re-grant one on **every** subsequent halt while it lasted, with no notice, no ❌ row
and nothing in the run report. That inverts the single outcome `REQ-RCV-01` exists to deliver, and it
is the invariant §5.4's enumerated points assert as a leg — *one clearance granting exactly one
window*. It is also not reachable through O-5: a `RESOLVED:` line is never a region line (`REQ-RCV-01`
AC-1.4 clause 2, catalogue §1), so O-5's fail-closed disposition for a lost region does not quantify
over it. Making the two edits one update removes the state rather than dispositioning it; a **torn**
single update remains `REQ-RCV-07` AC-7.5's, as for the append alone.

**What it buys the recovery.** Because a landed-but-unconfirmed halt-path update moves exactly one of
`H`/`A` by one and no reachable state moves both, catalogue §4's act 1 can be stated as a **count
test** the operator can take from the region (`H > A` for the `halt line` source, `A = H` for the
`answering line` source) rather than as an instruction to recognise an unconfirmed line by
inspection, which is not an observable property of the file.
