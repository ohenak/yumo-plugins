...
the 37 tasks fall into 18 batches. Nothing else in this document declares batches; §5 explains the
...
|---|---|---|---|---|---|---|
| A-00 | **Repair the wave-gate command (§5.1).** Rewrite `.claude/pdlc.config.json` → `implementation.testCommand` to the restated form that preserves jest's configured `testPathIgnorePatterns` instead of replacing it. Verified defect: the shipped form re-collects `__tests__/helpers/` and `__tests__/fixtures/` as test suites and jest fails a suite with no tests. No source or test file changes. | — | `.claude/pdlc.config.json` | 1 | — | ⬚ |
