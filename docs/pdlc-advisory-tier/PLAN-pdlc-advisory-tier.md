...
| 7–17 | 5–15 | A-18 … A-32 | **Green.** Each task un-skips exactly the block(s) §3 names for it — A-23, A-24 and A-31 each un-skip **two**, one in their own seam file and one in `advisoryDriver.test.js` (§8.2) — every un-skipped block passes in full, and every still-skipped block is still skipped. That last conjunct is mechanical, not nominal: comparing `perFile` from `/tmp/adv-gate-w{n-1}.json` and `/tmp/adv-gate-w{n}.json`, for each block this task un-skips, `pending` falls from *k* to 0 and `passed` rises from 0 to *k* — where *k* is that block's own wave-(n−1) `pending` count, so no expected case count has to be recorded anywhere — and **every other block in the file is unchanged in all three counters**. The whole suite is green under the script-owned aggregate run. From executor batch 6 onward every wave also runs `postWaveCommand` (`node pdlc/workflows/build-runtime.mjs`) and commits `pdlc/workflows/dist/`, so `runtimeBundle.test.js` is part of "the whole suite is green" at every step. |
| 18 | 16 | A-33 | **Green + the D-6 comparison + zero remaining skips.** `advisoryDisabled.test.js` is un-skipped and passes against the transcribed literal, with the fixture's scenario header re-asserted first. This is the last un-skipper, so the gate also asserts that **no `describe.skip` block remains** in any `advisory*.test.js` file — a case left skipped is a case that never ran. **Where that check lives, and why it is two checks:** (a) a **case** in `advisoryDisabled.test.js` (A-16 🔴 / A-33 🟢) that reads every `advisory*.test.js` in `pdlc/workflows/__tests__/` **as text** — no jest-in-jest — with the glob **unqualified, i.e. including `advisoryDisabled.test.js` itself** (safe: none of the regexes below matches this case's own source, which contains no `describe`-then-dot sequence, so self-inclusion costs nothing and closes the one file check (b) alone would otherwise cover) — and asserts no match for `/\b(describe|it|test)\s*\.\s*skip\b/`, `/\bx(describe|it|test)\b/`, or a binding assigned from any of those; and (b) the wave agent's own reading of the targeted `--json` run, where `perFile` reports `pending === 0` for every block of every `advisory*.test.js` path. (a) is the durable regression that ships; (b) is the direct behavioural observation and catches an alias (a) could not name. A bare grep for the literal `describe.skip` is sufficient for neither. |
