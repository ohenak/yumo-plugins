...
| 1.3 | 2026-08-04 | **Phase PR erratum round** — three targeted edits, no scope change. (1) §4.3, §5.5, §6.3 and §7.2: A1 and A3 declare **`verifyGate: null`**, not `async () => ({ passed: true })`. With `permittedActions: []` the driver never reaches step 6 and `resolved` is unreachable at those seams, so the trivially-passing stub — the mutation FSPEC T-03-6(b)'s gate-exclusivity criterion treats as falsifying — no longer appears as a shipped implementation; the typedef's `verifyGate` is correspondingly nullable, and PLAN's A-31 `verifyGate == null` assertion is now what the TSPEC specifies. (2) §4.3, §4.5 and §8.2 name `waitMs`'s reporting surface: `SeamOps` stays at **nine** members and the **driver** accumulates the check-rollup wait, owning the counter and passing a `recordWait(ms)` sink into seam construction that only A5 calls; the asserted surface is the `waitMs` argument the driver passes to `budgetExceeded`, which is what PROP-BUD-03 pins. (3) §11.1 restates the "one check, at one place" assertion with its matcher: the three sites do not share a literal spelling, so the scan is `/\.enabled\b/` over `orchestrate-dev.js` and `orchestrate-queue.js` only (never the `dist/*.bundle.js` copies), expecting exactly three matches — driver early return, config-notice gate, distil guard — with `parseAdvisoryConfig`'s body sliced out before counting, matching PROPERTIES PROP-DIS-06 verbatim. |
