...
| T-08 | shared code vs. two implementations (FSPEC §14.1). **Decided**: two implementations (§13.1 row 6). The *evidence* holding them equal is narrowed deliberately to the **predicate**; the enumeration half is held by inspection, not by a test, with its divergence classes recorded as accepted exposure (§10.4) | §7.1, §13.1 row 6 | AT-P7 (differential, L4) — a differential over `classifyCorpus`'s two-region predicate against the hook's `:41` predicate, both fed the same basename list. It does **not** falsify the enumeration pair, and §12.2 says so rather than implying coverage it has not got |
