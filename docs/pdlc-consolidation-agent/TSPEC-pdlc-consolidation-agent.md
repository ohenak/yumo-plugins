...
| T-08 | one corpus, one **predicate** — narrowed deliberately; the *enumeration* half is **not** held by a test, see below | §7.1 | AT-P7 (differential, L4) — a differential over `classifyCorpus`'s two-region predicate against the hook's `:41` predicate, both fed the same basename list. It does **not** falsify the enumeration pair, and §12.2 says so rather than implying coverage it has not got |
