...
| `consolidationCredential.test.js` | L2 | AT-K1, AT-K2, AT-K3, AT-K4, AT-K5, AT-K6, AT-K7. **AT-K3b is deliberately not here**: the split is by subject, and its subject is the pass's corpus handling, which `consolidationPass.test.js` owns — so the AT-K family spans two files, which the id→file equality permits (it obliges exactly one file per id, not one file per id prefix) |
