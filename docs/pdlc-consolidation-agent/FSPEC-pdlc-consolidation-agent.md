...
|---|---|---|---|---|
| pdlc | draft | Claude | 9.0 | 2026-08-06 |
...
**No fixture in §13 covers that two-action-one-subject pass** — the rows named here are the two
...
or partitions on PR-opening rather than on action multiplicity (AT-Q7, AT-Q7c likewise), and the
rows that place a `revise` or `retire` beside an earlier `promote` (AT-F9, AT-F10, AT-F18) do so
**across passes**, while the merge scoped here is intra-pass — so it is named **PROPERTIES-owned per
