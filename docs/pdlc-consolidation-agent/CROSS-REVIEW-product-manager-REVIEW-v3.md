...
- **G-01 was fixed by narrowing the rule, not by deleting it — and the control proves which.** The easy version of this fix drops the derivation and lets everything read `promoted`. What landed instead filters on one flag set at one site (`:952`, `:799`) and ships a degraded-pass control asserting `promoted-degraded` still fires (`consolidationOperatorChannels.test.js:571-608`). The test comment says out loud that it exists so "the derivation is not merely removed". That is the shape of fix I trust on a re-review.
- **The fix argued from the specs rather than from my review.** The code comment at `:945-951` cites vocabularies §1's join and `duplicate-suppressed` as the controlling precedent, which is the argument I made — but it is now recorded at the line, where the next reader meets it, rather than only in a cross-review file that harvest will distil. Same for `:791-795` at the rejection site. A later maintainer who wonders why a bar decline does not degrade gets the answer without archaeology.
- **G-02 was answered with the operator's response, not with a label.** The heading is `# Declined at the AC-2.3 pattern bar (not a degraded promotion)` (`:2492`), and the comment above the split states the product distinction exactly — one is "residue to retry", the other "a coincidence to judge", "so they are never interleaved" (`:2477-2480`). The author read the finding as a question about what the human does next, which is what it was.
- **G-03's fix reads the corpus grammar instead of guessing at it.** `matchesFeatureToken` matches the `LEARNINGS-{feature}.md` slot that FSPEC §3.1 fixes (`:1090-1091`) and only falls back to a bounded token match outside that grammar (`:1093`). Narrower and more honest than a tightened substring, and the fallback-to-all arm I asked to keep is untouched (`:1073`).
- **A finding was answered by recording a fact rather than by inventing a caller.** TE F-14 flagged `result.writeSet` as having no production reader. `4ef6fe71` does not wire a fake consumer to turn the row green; it states that AT-M5 is the only reader, that step 15 reads `state.writeSet` directly, and names the wiring that would retire the finding if item 8 ever surfaces the write set (`:1233-1237`). Under DC-07's dead-config rule that is the correct disposition for a lineage field on an operator-facing return, and it leaves the next reviewer better off than a silent close would.
- **The two Low fixes riding along were both narrowings with controls.** `REPOSITORY_UNRESOLVED_RE` dropped its bare `/not found/` alternative — which subsumed the specific `repository not found` beside it and matched git's DNS stderr — and gained a transport-class row containing the phrase (`consolidationReport.test.js:469-484`). That is the E-23 → E-22 misclassification the surrounding comment claims to avoid, now actually tested rather than asserted.
- **Nothing in the approved surface regressed.** `dist/` is in sync on all five rows, the suite's only red is the documented untracked-file false-red on both HEADs, and the propose-only guarantee, the non-lossy degradation story, the duplicate-suppression precedent and the four ACs wired in round 1 are all intact on HEAD. The three round-2 fixes touched status derivation, one renderer and one matcher, and nothing beyond them.
