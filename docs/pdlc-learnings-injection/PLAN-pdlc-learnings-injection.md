...
| Field | Value |
|---|---|
| Upstream | REQ → FSPEC → TSPEC → **PLAN** — `TSPEC-pdlc-learnings-injection.md` (v0.9); `FSPEC-pdlc-learnings-injection.md` (v0.14); `REQ-pdlc-learnings-injection.md` (v0.10); `DECISIONS-pdlc-learnings-injection.md` (v0.5) |
...
|---|---|---|---|---|
| pdlc | Draft | Claude | 1.3 | 2026-08-21 |
...
test files scheduled by the batch ladder**: the seven jest suites TSPEC §T.5 assigns, `learningsSuiteMap` (the partition's
...
script that captures it. Fourteen is the **ladder's** count, not the tree's: the DoD remediation
commit `2fc6fcd3` landed five further test-side files after batch 13, so at `09c7c62f` the tracked
`learnings*` test-side set is **eighteen files** — fourteen `learnings*.test.js` suites and three
helpers under `pdlc/workflows/__tests__/`, plus `pdlc/engine/__tests__/learnings-config-example.test.js`
(§File-ownership manifest → Post-batch remediation).
...
Behaviour lives in REQ v0.10 / FSPEC v0.14 / TSPEC v0.9 and is referenced by id (`AC-`, `BR-`,
...
| `pdlc/workflows/package.json` | exists; at `2fc6fcd3` the `c8` block reads `allow-external: true`, an `include` of four `**/`-anchored entries (`**/pdlc/workflows/orchestrate-dev.js`, `**/pdlc/workflows/orchestrate-queue.js`, `**/pdlc/workflows/build-runtime.mjs`, `**/scripts/capture-learnings-baseline.mjs`) and a three-glob `exclude` for the capture tests' worktrees | **no LI task** — modified post-ladder by `2fc6fcd3` (§File-ownership manifest → Post-batch remediation); the capture script is **inside** the per-file gate, §Verification DoD 11/12 |
...
| LI-08 | **RED block/material suite** (L1): `LI-AT-05` (the rendered form transcribed literally from TSPEC §OQ.1, not keyword-matched), `LI-AT-11` (section-set equality over what BR-6 selected, taken over a fixture that carries at least one **non-canonical heading form** beside the corpus's usual `## N. Title` shape — the un-numbered `## Cross-Feature Patterns`, the un-glossed `## Rejected Proposals`, a `###` sub-heading that must read as body text, and one near-miss (`## Process Findings`) that must **not** match — so F-O-1's **second** rule is pinned by the test rather than assumed from the fixture builder's shape (TE F-03). The rule is TSPEC §D.3's: exactly two `#`, an optional ordinal stripped and discarded (it is not the priority), an optional trailing gloss, and otherwise exact case-sensitive comparison against `BR6_SECTION_NAMES`. The variant shapes are declared in **LI-02**'s spec surface, not built ad hoc here; LI-08's existing `Deps` on LI-02 carries the edge). `LI-AT-12` (the character-safe cut of §D.5 — an **ASCII** fixture so the expected count is the bound exactly, plus a separate multi-byte case pinning `≤`). Expected byte counts are hand-computed from the fixture over **material only**, ignoring every delimiter (§D.5). **Amendment note (v0.7, superseding v0.5's).** `pdlc/workflows/__tests__/learningsBlock.test.js` and `__tests__/helpers/learningsFixtures.js` exist at HEAD from earlier dispatches of these two rows (`1920f281` for LI-02, `5e522a52` for LI-08). Their `Status` cells still read ⬚ because that column is the **dispatcher's** ledger and this note does not write it; the two records are about different things — file existence is a fact about HEAD, `Status` is a fact about the wave's bookkeeping — and neither contradicts the other (TE F-01). What the landed corpus *renders* is the bare-title form throughout: no shipped builder passes `ordinal` or `gloss`. The knobs themselves already exist — `renderSection` accepts `ordinal` and `gloss`, both unexercised by any landed suite, and a free-form `body` the landed suites already use (`learningsBlock.test.js` passes `body:` on all six of its section specs, and `learningsSelect.test.js` passes it on the non-BR-6 section) — so the amendment adds **callers** for the two unexercised knobs and reuses an already-exercised one, plus the near-miss title and the `###`-as-body case, not four new knobs (PM F-04). The heading-form cases are therefore an amendment to those landed files, taken by their **existing owners** in a follow-up commit: ownership does not move, the single-writer manifest is unchanged, and the expected-red rows that commit owes are named in §The three gate wordings under **Amendment commits on landed suites**, which is what P-A-7 requires be committed before the run it governs (v0.6 erratum) | `__tests__/learningsBlock.test.js` | — | 3 | LI-02 | ⬚ |
...
| LI-16 | **GREEN the pure selection core** (TSPEC §I.3, §D.3–§D.6): `looksLikeLearningsDocument` (deliberately weak — a truncated document stays eligible), `parseHarvestDate` (`\b`-anchored ISO prefix, string comparison, **no `Date` object**), `extractInjectableMaterial` (BR-6 priority order, character-safe cut, `bounded` decided at the cut — **and TSPEC §D.5's zero-bound short-circuit, whose production half this task owns: `maxBytes <= 0` is tested _before_ the cut and returns `{material: "", bounded: false, bytes: 0, sections: []}` for every `text`, including one carrying all five sections; `bounded` is `false` because nothing was taken, so nothing was cut**. Named here because no other task's enumerated edits reach it: LI-12 is a test task and LI-21 edits only `main()` and `buildFinalReport` (PM F-01, E-36)), `orderCorpus` (§D.4's comparator with `Buffer.compare` as the tiebreak, not `<`/`>`), and `selectLearnings`, whose `rejected[]` is **total over `entries`** and tests `excluded` **before** `readOk` so a self document is never mis-reported as `RSN-UNREADABLE`. `selectLearnings` also owns the drop side of the same rule: a document whose extraction yields `sections: []` is dropped `RSN-NO-MATERIAL` **before** the count and total bounds are applied, so the drop consumes no `maxDocuments` slot. That is one branch keyed on *yields no material*, covering both of §T.7's disjuncts — the structural one (E-33) and the zero-bound one (E-36) — with **no** zero-bound special case in the selector (TSPEC §D.5). Greens `learningsSelect.test.js` | `__tests__/learningsSelect.test.js` | `pdlc/workflows/orchestrate-dev.js` | 8 | LI-15, LI-07 | ⬚ |
...
- `pdlc/workflows/package.json` — **modified by `2fc6fcd3`**, no LI task (its row lives in
  §Post-batch remediation below). Earlier versions of this document declared it unmodified with
  `scripts/capture-learnings-baseline.mjs` deliberately exempt from `c8.include`; that was true when
  written and is false at HEAD. `2fc6fcd3` set `allow-external: true`, re-anchored every `include`
  entry as a `**/` path, **added `**/scripts/capture-learnings-baseline.mjs` to the include set**,
  and added a three-glob `exclude` for the worktrees the capture tests materialise (CODE_REVIEW v1
  F4, second round: under `allow-external`, bare cwd-relative basenames stop matching, so the
  script's entry alone would have silently dropped the three workflow modules from the report). The
  former exemption is retired; the script is inside the per-file branch floor, and
  `coverageInstrumentation.test.js` fails if any included module stops resolving
  (§Verification DoD 11/12).
...
The DoD remediation commit `2fc6fcd3` (CODE_REVIEW v1 findings F1/F7/F12, F8, F9, F10, F11/F12, and
F4's second round) landed after batch 13. This subsection is derived from
`git show --name-status 2fc6fcd3`, not from any prior description of the commit: it lists **45
paths** — 18 added fixture prompts under `PIPELINE-NON-AUTHORING-PROMPTS/` (collapsed into the
fixtures row below, as the ladder's own row collapses them), **five added test-side files**, nine
modified files under `pdlc/workflows/__tests__/`, one modified pre-existing engine suite
(`docs-uniqueness.test.js`), four modified production/configuration files
(`orchestrate-dev.js`, `scripts/capture-learnings-baseline.mjs`, `pdlc/workflows/package.json`,
`pdlc/workflows/.gitignore`), the regenerated `dist/pdlc-cli.mjs`, and seven pipeline/document
files (`REQ`, `FSPEC`, `TSPEC`, `CLAUDE.md`, `pdlc/OPERATIONS.md`, `pdlc/README.md`,
`.claude/pdlc.config.example.json`) that are pipeline artifacts, not manifest surfaces. Every
new file is owned by no LI task; every write to a ladder-owned file is a **second write**, which
is exactly the case P-A-5 says must be recorded as a manifest amendment — **one added row per
file** — rather than left in a completion note (TE v12, PM v12). The root `.gitignore` (LI-04's
file) is **not** in the commit; the `.gitignore` it touches is `pdlc/workflows/.gitignore`, a
different file with no prior row. `dist/pdlc-cli.mjs` keeps no row here for the same reason it has
none above: it is regenerated, not written.
...
| `pdlc/engine/__tests__/learnings-config-example.test.js` | new — no LI owner; the fifth added file, the one that brings the tracked `learnings*` test-side set to eighteen at `09c7c62f` | `2fc6fcd3` | F9 — the example-config expectation, in its own file so a config-example edit cannot red `ci-arrangement.test.js`'s delivery-blocking scope |
| `pdlc/workflows/__tests__/fixtures/learnings-baseline/` (incl. `MANIFEST.json`; +18 `PIPELINE-NON-AUTHORING-PROMPTS/*.txt`) | **second writer** after LI-06 (batch 4) | `2fc6fcd3` | F7 — re-capture extending the matrix to the whole non-authoring dispatch set |
...
| `pdlc/workflows/__tests__/learningsCaptureScript.test.js` | **second writer** after LI-03 (batch 2) | `2fc6fcd3` | F4 (second round) — `captureFixturesFromWorktree` driven directly, tracking the capture script's rework |
| `pdlc/workflows/__tests__/learningsSelect.test.js` | **second writer** after LI-07 (batch 3) | `2fc6fcd3` | tracks `selectLearnings`' signature change (below) |
| `pdlc/workflows/__tests__/learningsCorpus.test.js` | **second writer** after LI-09 (batch 3) | `2fc6fcd3` | same signature change, corpus-shell callers |
| `pdlc/workflows/__tests__/learningsDispatchSet.test.js` | **second writer** after LI-11 (batch 5) | `2fc6fcd3` | F8 — adopts `helpers/learningsComposition.js`'s two-invocation composition |
| `pdlc/workflows/__tests__/learningsConfig.test.js` | **second writer** after LI-12 (batch 5) | `2fc6fcd3` | config-read-error arm and property cases added alongside the F10 disclosure family |
| `pdlc/workflows/__tests__/learningsArmInventory.test.js` | **second writer** after LI-23 (batch 5) | `2fc6fcd3` | tracks the same `selectLearnings` signature change |
| `pdlc/workflows/__tests__/coverageInstrumentation.test.js` | pre-existing suite, no prior row | `2fc6fcd3` | F4 (second round) — drives c8 against the rewritten `c8` block and fails if an included module stops resolving |
| `pdlc/engine/__tests__/docs-uniqueness.test.js` | pre-existing suite, no prior row | `2fc6fcd3` | accounts for the branch's document set |
| `pdlc/workflows/orchestrate-dev.js` | **second writer** after LI-15…LI-22 (batches 7–14) | `2fc6fcd3` | F1 — `selectLearnings` drops its `feature` parameter, its caller is updated, and the `_log` emitter is wired into `main()` (15 insertions, 6 deletions) |
| `scripts/capture-learnings-baseline.mjs` | **second writer** after LI-05 (batch 3) | `2fc6fcd3` | F1/F7 — capture rework (74 insertions, 19 deletions) |
| `pdlc/workflows/package.json` | first write — previously owner-less **and unmodified**; both halves of that description now dated | `2fc6fcd3` | F4 (second round) — the `c8` block rewrite §Production and generated describes |
| `pdlc/workflows/.gitignore` | no prior row (not LI-04's root `.gitignore`, which the commit does not touch) | `2fc6fcd3` | one ignore line for the capture tests' artifacts |

**The single-writer premise survives, and here is why it is not merely asserted.** The
second-writer rows name ladder-owned files — LI-06's fixtures and guard, six ladder-owned suites,
and the three production surfaces of LI-15…LI-22, LI-05 and the `c8` configuration — so on the
manifest's own terms every one is a second write to a landed batch's surface. They race nothing:
`2fc6fcd3` is one serial commit landing **after batch 13**,
with no batch in flight and no concurrent agent holding any of these files, so there is no batch in
which two writers coexist — which is the property batch-safety rule 2 actually protects, and it is
preserved rather than excepted. The same argument covers every row above identically; none needs a
separate case. Each ladder task remains the ladder owner of its files; this commit did not
move ownership, and a future in-ladder write to any of them still belongs to its ladder task.
...
together by construction. No two rows share a batch and a file. This arithmetic is taken over the **two dispatcher-parsed tables only**; §Post-batch remediation's nineteen rows are outside the ladder, carry no `Owner` cell, and are excluded from every count in this paragraph by construction (TE v12). Reconciling the tree instead: at `09c7c62f` the tracked `learnings*` test-side set is eighteen files — the ladder's thirteen (twelve suites plus `helpers/learningsFixtures.js`; the fourteenth test row is the fixture subtree, a directory), plus `2fc6fcd3`'s five added files, all rowed above (PM v13, TE v13).
...
| document **yields no material** ⇒ `RSN-NO-MATERIAL`, dropped before the bounds, no slot consumed (BR-9/D-12 as FSPEC v0.14 restates them). **Two disjuncts, one branch** (TSPEC §T.7): no `BR6_SECTION_NAMES` heading present (E-33), **or** `maxBytesPerDocument: 0` admits none (E-36) | AT-28 (structural disjunct); AT-30 case 3 (zero-bound disjunct) | LI-07 / LI-16 (structural); LI-12 (red) / **LI-16** (production green) / LI-21 (config plumbing only) (zero-bound) |
...
Twelve arms, twelve entering tasks — **thirteen** entering cases, because the `RSN-NO-MATERIAL` arm is entered by two disjuncts and TSPEC §T.7 keeps them as one row, one branch. The arm count is therefore unchanged by FSPEC v0.14's E-36, and so is LI-23's set equality, which is taken over **reason codes**, not over disjuncts: the zero-bound disjunct is driven by LI-12's third `LI-AT-30` case rather than by LI-23's inventory, and greened by **LI-16**, which owns both of §D.5's functions — the `maxBytes <= 0` short-circuit in `extractInjectableMaterial` and the no-slot `RSN-NO-MATERIAL` drop in `selectLearnings`. LI-21 is in the cell for the config plumbing that makes `maxBytesPerDocument: 0` reachable at run level, not for the branch itself; a reader taking the earlier `LI-12 / LI-21` pair as the production owner would have found neither function in either task's enumerated edits (PM F-01), and LI-22's row-by-row walk cross-checks both entering-task columns. **The table is the map; `learningsArmInventory.test.js` (LI-23) is the oracle** (TE F-07): it drives all twelve arms and asserts the observed **non-`null`** `corpusOutcome` values, `rejected[].reason` values and notice ids **set-equal** to `LEARNINGS_CORPUS_OUTCOMES`, `LEARNINGS_REJECT_REASONS` and `LEARNINGS_NOTICES`, so an arm that silently stops being entered reds a test rather than surviving a reading. The non-`null` scoping on the corpus-outcome domain is TE F-01's: `null` is the healthy value of that field (TSPEC §D.2) and the three threshold/self arms cannot be driven without observing it, while `null` is not a member of the frozen catalogue — it is asserted instead by the record suite's per-dispatch rows. LI-22's row-by-row walk remains as the human cross-check that this table and that suite still name the same twelve arms — a diluted file-level percentage would have shown neither.

### TSPEC obligations and open questions → where they land

| Obligation | Landing |
|---|---|
| F-O-1 … F-O-7 (discharged in TSPEC) — **F-O-1 owns two heading-recognition rules** since FSPEC v0.14: the BR-3 document-shape predicate (`LEARNINGS_HEADING_RE`) **and** the rule by which a heading counts as one of BR-6's named sections (`BR6_SECTION_NAMES`, exactly two `#`, optional ordinal, optional gloss, otherwise exact) — both discharged in TSPEC §D.3, both bytes-only and model-free | implemented by LI-15 (F-O-3 registration, F-O-4), LI-16 (**both** F-O-1 rules, in `looksLikeLearningsDocument` and `extractInjectableMaterial`; the second rule is pinned red by LI-08's `LI-AT-11` heading-form cases), LI-17 (F-O-2), LI-19 (F-O-3 serialisation), LI-06 (F-O-5), LI-20 (F-O-6), LI-02 (F-O-7) |
...
| 1, 4, 6 — **green-terminal** | The batch's new suite is **green on authoring**: `learningsPremises.test.js` over HEAD's premises (batch 1), `learningsBaselineGuard.test.js` over the capture it was written from (batch 4), `learningsSuiteMap.test.js` statically parsing the `learnings*.test.js` directory, whose six AT-bearing suites all exist at the end of batch 5 and whose other six matching files register `LI-T-` titles only, so the closure is already equal to six at authoring (batch 6; TE F-03 — the same phrasing §Traceability's batch ladder carries, since the suite reads twelve files and its greenness depends on six of them not contributing). None has a red episode, and none may be *given* one by inventing a symbol for it to miss — **and** every pre-existing test's status is unchanged from the measured baseline, the same conjunct the other three rows carry (TE F-02). Without it these batches would have no oracle for a regression they can cause, and batch 4 is the one that commits a whole new fixture subtree into a tree `coveredViolations` walks in full |
...
named here, ahead of the run they govern, in the three cases that can arise (A, B and C below):

| Case | When the follow-up commit lands | Expected-red rows it adds |
|---|---|---|
| A — **the scheduled case** | before batch 9 (which includes batches 7 and 8) | **None.** `LI-AT-11`'s heading-form cases are inside `learningsBlock.test.js`, which the ledger already lists as a **whole suite** red after batches 7 and 8, red for the specified reason that `extractInjectableMaterial` does not implement F-O-1's second rule yet. They green with the rest of the suite when LI-17 lands at batch 9, and the ledger's row for batch 9 — which drops `learningsBlock` entire — is already correct for them. No row is added, and none may be dropped early. The window is derived, not just asserted: a commit landing in batches 2–6 is also "before batch 9", and those batches carry **no ledger at all** — batches 2, 3 and 5 are RED-terminal and batches 1, 4 and 6 green-terminal — so there is no live table for it to amend, and its red reason there is the earlier one (`extractInjectableMaterial` is not defined yet), not batches 7–8's (it is defined and does not implement F-O-1's second rule). Either way the outcome is the same: no row (PM v9 F-01, TE v9 F-02). The *When* cell reads **before batch 9** rather than "before batch 7" so that a commit landing *during* batches 7 or 8 is claimed by a case header and not only by this derivation: those two batches are exactly the ones whose ledger already lists `learningsBlock` as a whole-suite red, so the derivation above decides them and yields no row, and case B — which begins where the greening batch first matters — starts at batch 9 with no gap between them (PM v11 F-02, TE v11 F-02) |
| B — **after LI-17 has greened the suite, with a greening batch still ahead** (batch 9 through batch 12) | the commit re-reds committed green code | The ledger gains, for **every batch from the one the commit lands in through the batch that greens them** — a span that is well-formed only while a greening batch remains ahead, which is why case C exists — the named row `learningsBlock` → **`LI-AT-11`'s heading-form cases only** (the un-numbered `## Cross-Feature Patterns`, the un-glossed `## Rejected Proposals`, the `###` sub-heading that must read as body text, and the `## Process Findings` near-miss that must not match). Stated in test names, not the suite name, exactly as the split-suite rows are. The amendment is an edit to **this** PLAN, committed **before** that batch runs; a re-red landing without it is a gate failure, not a red |
| C — **batch 13 or later, the case that is live at HEAD** | any commit landing in batch 13 or after, including batch 14 — LI-21 (`e7fa8d87`) has landed, so batch 13's pre-LI-21 slot is closed and cannot recur; the domain is stated by batch number rather than by LI-21's commit so that no batch falls between case B's upper bound (12) and this case (PM v11 F-01, TE v11 F-02) | **None — and the amendment landed green; this row records an outcome, not a pending expectation (TE v12).** LI-16 (`be2456c8`), LI-17 (`a4998e13`) and LI-21 (`e7fa8d87`) are all landed, so no batch remains that greens anything: batch 14 is LI-22's REFACTOR-and-close, which adds no assertions and carries the **unqualified** full-suite-green gate. Case B's span therefore has no terminus here and does not apply. The terminating condition in its place is the gate itself: **the ledger stays empty and the amendment must be green at the commit that lands it — and it was.** At `09c7c62f`, `npm test -- __tests__/learningsBlock.test.js __tests__/learningsSelect.test.js` reports **26 passed, 26 total** with a `test.skip`/`describe.skip` count of **0** over both files. All four of `LI-AT-11`'s heading-form cases (the un-numbered `## Cross-Feature Patterns`, the un-glossed `## Rejected Proposals`, the `###` line that must read as body text, and the near-miss `## Process Findings` that must not match) and both Group D amendments to the landed `learningsSelect.test.js` (`PROP-ORDER-06`, `PROP-CORPUS-09`) are inside that count. So **no `learningsBlock` ledger row was owed and none exists, and the fix-before-batch-14 obligation was never triggered** — case C's failure limb below is **unexercised, not waived**, and PROPERTIES §C.4 records the same discharge. That is not a hope — the production half F-O-1's second rule needs is already at HEAD (`canonicalSectionName` strips an optional ordinal via `SECTION_HEADING_RE`, strips an optional trailing gloss, compares case-sensitively against `BR6_SECTION_NAMES`, and returns null for a `###` line, which `^##[ \t]+` never matches), so the heading-form cases assert behaviour that is shipped and the near-miss `## Process Findings` is already unmatched. If such an amendment nonetheless lands **red**, it has found a real defect, not staged a TDD red: there is no later batch to name in a ledger row, so the fix commit is owed **before batch 14 runs**, and a red surviving into batch 14 is a gate failure. The same rule governs any other amendment to a landed suite arriving from here on — including the PROPERTIES-driven re-reds §C.4 of PROPERTIES routes to this PLAN (PROP-BOUND-03's `maxBytesPerDocument <= 0` case, PROP-BOUND-05/07/08, and the Group D amendments to the landed `learningsSelect.test.js`): under case C they owe no ledger row, and they owe green — which PROPERTIES §C.4 records as discharged, PROP-BOUND-03's `maxBytesPerDocument <= 0` case having landed green on its first exercise through the `extractInjectableMaterial` seam |
...
11. Coverage, stated against the measured prior value (TE F-09): run as `npx c8 npm test -- --runInBand` under the arrangement's four `--testPathIgnorePatterns`, **both stages exit 0**, and `orchestrate-dev.js`'s per-file branch coverage is **≥ 85 %** — measured 88.14 % at HEAD, so the region may consume at most 3.14 points of headroom. The bare `npm run test:coverage` is not the gate: it inherits the two `documentOracles` failures in stage 1 and never reaches stage 2. **Since `2fc6fcd3` the stage-2 per-file set is four modules, not three**: the `c8.include` rewrite (§File-ownership manifest → Production and generated) adds `**/scripts/capture-learnings-baseline.mjs`, so the ≥ 85 % branch floor applies to it too, and §The measured baseline's three-row table is the pre-`2fc6fcd3` measurement it says it is, not the set the gate measures at HEAD. The per-file floor is **not** claimed as this region's oracle (§Traceability, DoD 3).
12. Coverage exemption — **retired at `2fc6fcd3`** (TE F-08 originally; superseded). This clause once exempted `scripts/capture-learnings-baseline.mjs` from `c8.include` on the ground that the block resolved relative to `pdlc/workflows/`; `2fc6fcd3` re-rooted exactly that — `allow-external: true` plus `**/`-anchored entries — and **added the script to the include set**, so it now sits inside DoD 11's per-file branch floor and no exemption exists for a verifier to check. The oracles this clause named — `LI-T-IGNORE`'s three conjuncts, `LI-T-WORKTREE`'s two, and the baseline guard — remain in force as behavioural oracles, no longer as stand-ins for a missing floor, and `coverageInstrumentation.test.js` fails if any included module stops resolving.
13. **LI-01's completion note exists and carries both of its written records** (PM Q-07, P-A-8): the one-time pre-flight measurement of the §Overview change-surface table's four absence claims at HEAD, and the H-2 engine-failure triage citing the CI run that decides it. Neither is a test — the absence claims are falsified on schedule by this PLAN's own tasks between batches 1 and 4, so nothing standing can assert them afterwards — and this clause is what preserves them.
14. **The four POSTMORTEM-D remediations riding on this branch are consciously carried, and are named here rather than left for a reader to discover from the diff** (CR round 1, PM F-10; re-filed as PM v2 F-01). None of them traces to an acceptance criterion of this feature's REQ, and none is owned by an `LI-*` task row; they are process repairs for the two erratum rounds that halted on this branch, landed here because this branch is where they were diagnosed. They are, with their subjects at HEAD: (a) the **erratum restatement retry** — the one re-dispatch a non-approving zero-`FINDING:` delta confirmation is granted before failing closed, inside `erratumRound`, tested by `erratumProtocol.test.js`'s `RT-1g-a…e`; (b) the rewritten **`findingGrammarClause()`**, the prompt clause the three review skills' `## Delta-Confirmation Findings` skeleton requirement is stated through; (c) **`pdlc/hooks/scripts/check-finding-grammar.sh`**, registered as a `PostToolUse: Write|Edit` hook in `pdlc/hooks/hooks.json` and behaviourally tested — warn, no-warn and missing-tag arms, driven by bare path through the real PostToolUse envelope — by `hookCompatibility.test.js`'s `CR round 1 (PM F-09)` block; (d) the `## Delta-Confirmation Findings (erratum rounds)` sections added to `pdlc/skills/{pm,se,te}-review/SKILL.md`. **REQ G-5 is narrowed by (a), not satisfied by it.** G-5 promises that erratum routing is unchanged and that "this feature changes what an author is told, never what the pipeline requires of what they produce"; (a) changes how many dispatches an erratum round may make, so G-5 holds for the injection region and does **not** hold for this branch as a whole. That is a scope statement this PLAN can record but cannot authorise — the authoritative amendment is REQ's, and is routed as an erratum. Until it lands, this clause is the branch's disclosure: no reader of the diff has to infer which changes are the feature and which are the remediations, and no future feature may cite (a)–(d) as precedent for deciding product scope inside engineering artifacts. What this clause does **not** do is widen the DoD: (a)–(d) carry their own tests, named above, and clauses 1–13 remain the injection region's bar.
...
| P-A-6 | Does Phase P have a scheduling dependency on this wave — hold the PROPERTIES commit until batch 14, or land it in one commit after LI-21? (PM Q-06) | **Author whenever; commit at the first point the suite is green, which in practice is after LI-21 (batch 13).** P-A-3's bar is about the *commit*, not the authoring: a property over an arm this wave has not built yet is honestly red, and a red suite outside the ledger breaks the batches 7–13 gate. So yes — Phase P has a scheduling dependency on this wave, and it is recorded here: the PROPERTIES **document** may land at any time (it is prose, no jest test), the PROPERTIES **suite** lands in one commit once green, or else its rows are handled under **P-A-7's governing case** — which at HEAD is case C, where no ledger remains to amend into and the obligation is green-at-landing; the amend-into-the-ledger-by-name route is case B's, and case B closed at batch 12 (TE v11 F-03). No task row in this PLAN owns either | 
| P-A-7 | If a PROPERTIES suite does land red before batch 14, is the ledger amended in this document, or recorded elsewhere? (TE Q-01) | **In this document, before the batch it applies to** — the same rule as P-A-5, and for the same reason. The expected-red ledger and the §File-ownership manifest are both live tables a dispatcher reads to decide whether a batch passed; an amendment that lives anywhere else enforces nothing. One rule covers both: **a live table is amended by an edit to this PLAN, committed before the run it governs.** | 
| P-A-8 | LI-01's completion note carries two written records that no DoD clause mentions. Should DoD require them? (PM Q-07) | **Yes, and DoD 13 now does.** Neither record is a test — the four absence measurements at HEAD stop being checkable after batch 4 by construction (that is exactly why they left the standing suite, PM F-07), and the H-2 engine-failure triage is a CI reading. Without a DoD clause the only thing preserving them is a batch-1 habit, so DoD gains one: LI-01's completion note exists and carries both records | 
...
| 0.5 | 2026-08-20 | Round 7 upstream-cascade confirmation (PM F-01…F-04; TE F-01…F-04), absorbing **FSPEC v0.12 → v0.13** and **TSPEC v0.7 → v0.9**. LI-12's `LI-AT-30` becomes **three** cases — `maxBytesPerDocument: 0` (E-36) added — with a three-conjunct positive oracle (key present with empty BR-8 rows; `rejected[]` set-equal to every non-self corpus path at reason `RSN-NO-MATERIAL`; **no** document carrying `RSN-COUNT`, the no-slot conjunct that falsifies a slot-consuming zero-byte cut), so a mutation of §D.5's `maxBytes <= 0` short-circuit can no longer survive the suite (PM F-01, TE F-01). The fail-open arm table's `RSN-NO-MATERIAL` row is broadened to TSPEC §T.7's two-disjuncts-one-branch form and routes the zero-bound disjunct to AT-30 case 3 / LI-12 / LI-21; the arm count is unchanged, and LI-23's set equality — taken over reason codes, not disjuncts — is untouched (PM F-02, TE F-02). F-O-1's **second** heading-recognition rule is mapped: the obligations row names both rules, LI-08's `LI-AT-11` gains non-canonical heading forms and a near-miss, and LI-02's spec surface gains the declared-heading-form knob (TE F-03). DoD 4's byte-identity scope is restated as dispatches **outside BR-1's rule** rather than "non-authoring", so Phase CR's authoring-classified non-C-1 optimizer round is inside the promise (PM F-04). ERR-3 and ERR-7 are recorded **CLOSED** (FSPEC v0.11/v0.12; TSPEC §A.2 implements BR-1 directly), and **three** stale version pins now read FSPEC v0.13 / TSPEC v0.9 — the 0.1 changelog row is a historical record and correctly keeps the pins it was written against, so "four" in this row's original wording overcounted (corrected in v0.7, PM F-05-low) (PM F-03, TE F-04). No task moved batch, no `Deps` edge changed, no fixture was invalidated |
| 0.6 | 2026-08-20 | **Erratum round (Phase PR; TE, PM, SE — same item).** LI-08's v0.5 amendment note assigned the heading-form follow-up to the landed files' existing owners without naming the expected-red rows for the re-red, which P-A-7 requires be committed as an edit to this PLAN before the run it governs. §The three gate wordings gains an **Amendment commits on landed suites** paragraph naming them in both cases: **A** — the commit lands before batch 7 and adds **no** row, because `learningsBlock.test.js` is already ledgered as a whole suite red after batches 7–8 and greens entire at LI-17/batch 9; **B** — the commit lands at batch 9 or later and re-reds green code, so the ledger gains `learningsBlock` → **`LI-AT-11`'s heading-form cases only**, stated in test names, for every batch from the landing batch through the batch that greens them. `learningsFixtures.js`'s consumers (`learningsSelect.test.js`, `learningsCorpus.test.js`, the rest) carry **no** row: the declared-heading-form knob is additive over the landed helper's existing ordinal/gloss rendering, so their statuses do not move — and if a later helper amendment is not additive, the suites it moves enter the ledger by name first. LI-08's note now points at that paragraph. Nothing else changed: no task moved batch, no `Deps` edge changed, no AT partition or fixture was touched |
| 0.7 | 2026-08-20 | Round 8 cross-review (PM F-01…F-05; TE F-01, F-02). **LI-16 is named the owner of TSPEC §D.5's zero-bound production half** — the `maxBytes <= 0` short-circuit in `extractInjectableMaterial` and the no-slot `RSN-NO-MATERIAL` drop in `selectLearnings` — and the fail-open arm table's zero-bound cell is re-split `LI-12 (red) / LI-16 (production green) / LI-21 (config plumbing only)`; the previous `LI-12 / LI-21` pair pointed at two tasks whose enumerated edits reach neither function (PM F-01). **LI-AT-30 conjunct (iii) gains its fixture precondition**: the third case's corpus carries more eligible non-self documents than the `maxDocuments` in force (≥ 6 at REQ §4.1's default of 5), below which no document could carry `RSN-COUNT` under any implementation and the conjunct passes for free against the slot-burning mutant (PM F-02). **§Open questions records ERR-8** (FSPEC Step 5, items 15–16, open at HEAD and addressed to this author) with the finding that LI-16 and LI-12 already encode TSPEC's corrected ordering, so no task moves (PM F-03). LI-08's amendment note moves to the **end** of the AT enumeration — it previously orphaned `LI-AT-12` mid-list — reconciles with the `Status` column by naming the two landed commits (`1920f281`, `5e522a52`) and the column's dispatcher ownership (TE F-01), and drops the false "bare-title form only" claim: `renderSection` already carries unexercised `ordinal`, `gloss` and `body` knobs, so the amendment adds callers, not knobs (PM F-04). LI-02 states that the `###` variant is emitted as **body text** through the existing `body` knob, since `renderSection` hardcodes the two-`#` prefix and grows no `level` parameter (TE F-02). The 0.5 row's "four stale version pins" is corrected to three (PM F-05-low). No task moved batch, no `Deps` edge changed, no AT partition, fixture or manifest row was touched |
| 0.8 | 2026-08-21 | **Erratum round (Phase PR; PM, TE, SE — same item, six raises).** P-A-7's case table terminated case B's ledger span at *"the batch that greens them"*, but LI-16 (`be2456c8`), LI-17 (`a4998e13`) and LI-21 (`e7fa8d87`) are all landed and LI-22/batch 14 adds no assertions, so an amendment to `learningsBlock.test.js` arriving now has no greening batch to bound its rows — the span had no terminus at HEAD. Case B is re-scoped to **batches 9–12** (the window where a greening batch still remains) and new **case C** governs the post-batch-13 case that is live at HEAD: the ledger stays **empty** and the amendment **is expected to land green**, because the production half F-O-1's second rule needs is already shipped (`canonicalSectionName` strips ordinal and gloss, compares case-sensitively against `BR6_SECTION_NAMES`, and `SECTION_HEADING_RE` never matches a `###` line) — answering PM Q-02. The terminating condition in place of the span is batch 14's unqualified gate: a red that does land is a real defect owed a fix **before batch 14 runs**, and one surviving into batch 14 is a gate failure, not a ledger entry. Case A's "before batch 7" window is derived rather than asserted, covering batches 2–6 (no ledger exists there) and naming their earlier red reason (PM v9 F-01, TE v9 F-02). The closing "no row of their own" sentence is scoped to **this** follow-up commit rather than reading as a standing exemption, and the PROPERTIES re-reds §C.4 routes here (PROP-BOUND-03's zero case, PROP-BOUND-05/07/08, the Group D `learningsSelect.test.js` amendments) are routed to case C (TE v9 F-01). Nothing else changed: no task moved batch, no `Deps` edge changed, no AT partition, fixture or manifest row was touched, and the batches 7–13 ledger is byte-identical |
| 0.9 | 2026-08-21 | **Erratum round (Phase PR; PM, TE, SE).** Two targeted corrections, no structural change. P-A-7's lead-in still read *"in the two cases that can arise"* above the table v0.8 grew to **three** rows; it now reads "in the three cases that can arise (A, B and C below)" (TE v11 F-01; this row originally credited a PM v10 erratum — corrected in v1.3, PM v13). LI-08's amendment note's claim that `renderSection`'s `ordinal`, `gloss` and `body` are "all three unexercised by any landed suite" is false for `body` at HEAD — `learningsBlock.test.js` passes it on all six section specs and `learningsSelect.test.js` passes it on the non-BR-6 section — so the note is restated as two unexercised knobs plus one already-exercised one; the conclusion (the amendment adds callers, not knobs) is unchanged and strengthened (TE v10 F-01, PM v10 F-01). Nothing else changed: no task moved batch, no `Deps` edge changed, no AT partition, fixture or manifest row was touched, the batches 7–13 ledger is byte-identical, and the case A/B/C table's own text is untouched |
| 1.0 | 2026-08-21 | **Phase CR round 1 (PM F-10 / TE F-05).** Two items, no structural change. **New DoD 14** names the four POSTMORTEM-D remediations riding on this branch — the erratum restatement retry, the rewritten `findingGrammarClause()`, `check-finding-grammar.sh`, and the three review skills' `## Delta-Confirmation Findings` sections — as consciously carried, with the test that owns each, and states plainly that REQ G-5 holds for the injection region but not for this branch as a whole; the authoritative REQ amendment is routed as an erratum, since a PLAN can disclose scope but cannot authorise it. No task row was added for them: they are process repairs, not feature work, and clauses 1–13 remain the injection region's bar. **DoD 6's `dist/` clause was found violated at HEAD and repaired in code, not in this document** — `build-runtime.mjs --check` printed `STALE pdlc/workflows/dist/pdlc-cli.mjs` after LI-22's `isLearningsSelfPath` reflow was never re-bundled; the rebuild is committed and `consolidationBuild.test.js`'s `T32` is green on the branch as committed. Nothing else changed: no task moved batch, no `Deps` edge changed, no AT partition, fixture or manifest row was touched |
| 1.1 | 2026-08-21 | **Round 11 delta confirmation (PM F-01…F-03; TE F-01…F-03).** Six Low findings, four of them distinct; no High, no Medium, no structural change. TE F-01 (the P-A-7 lead-in's stale "two") was already applied at v0.9 and needed no write. P-A-7's case headers now tile the batch line with no gap: case A's *When* reads **before batch 9 (which includes batches 7 and 8)** with the derivation stating why those two batches yield no row (PM F-02, TE F-02), and case C's reads **batch 13 or later**, stated by batch number rather than by LI-21's commit, so batch 13's pre-LI-21 slot — unreachable at HEAD since `e7fa8d87` landed — is claimed rather than left between B's bound of 12 and C (PM F-01, TE F-02). Case B's parenthetical aside now closes its em dash before "the named row", so the gate contract parses on first read (PM F-03). P-A-6's PROPERTIES fallback stops offering case B's amend-into-the-ledger route unconditionally and instead routes to **P-A-7's governing case**, which at HEAD is C: no ledger remains, the obligation is green-at-landing (TE F-03). No task moved batch, no `Deps` edge changed, no AT partition, fixture or manifest row was touched, and the batches 7–13 ledger is byte-identical |
| 1.2 | 2026-08-21 | **Erratum round (Phase PR; PM, TE).** Four items, no structural change: no task moved batch, no `Deps` edge changed, no AT partition or fixture was touched, and the batches 7–13 ledger is byte-identical. **(1)** P-A-7 case C cited `92b7ea0c`, `d462ddd8` and `2cbacada`, all pre-rebase and unreachable from HEAD; re-pinned to the commits PROPERTIES already uses — LI-21 `e7fa8d87`, LI-16 `be2456c8`, LI-17 `a4998e13` (PM v12). The same triple in the v0.8 and v1.1 changelog rows is re-pinned with it: those rows assert what is landed *at HEAD*, so a pin a reader cannot resolve falsifies the claim rather than dating it. The commits are the same commits under post-rebase identity; no historical claim changed. **(2)** Case C's verdict cell read *"the amendment is expected to land green"* — a pending expectation for something already discharged. It now records the outcome: at `09c7c62f` both suites report **26 passed / 26 total, 0 skips**, covering all four `LI-AT-11` heading-form cases and both Group D `learningsSelect.test.js` amendments (`PROP-ORDER-06`, `PROP-CORPUS-09`), so no ledger row was owed and none exists, and the failure limb is **unexercised, not waived** (TE v12). The rule itself is unchanged — only its tense. **(3)** §File-ownership manifest listed fourteen test rows against eighteen `learnings*` files tracked at `09c7c62f`; the four `2fc6fcd3` remediation files — `helpers/learningsBaselineScenarios.js`, `helpers/learningsComposition.js`, `learningsDisclosure.test.js`, `learningsErratumBinding.test.js` (CODE_REVIEW v1 F1/F7/F12, F8, F10, F11/F12) — are now recorded (TE v12). **(4)** The second-owner rows P-A-5 requires for that commit's re-capture of `fixtures/learnings-baseline/**` and `learningsBaselineGuard.test.js` are recorded alongside them (PM v12). Items 3 and 4 land in a new **§Post-batch remediation** subsection whose rows carry a **landing commit** rather than an `Owner` cell: the dispatcher parses `Owner` as a task id and no LI task owns these files, so a `2fc6fcd3` or `none` cell would be the same contract violation §Production and generated already states in prose for `dist/pdlc-cli.mjs`. The two dispatcher-parsed tables are therefore byte-unchanged, and §The arithmetic is scoped to them explicitly. Batch-safety rule 2 is preserved, not excepted — `2fc6fcd3` is one serial commit landing after batch 13, so no batch ever holds two writers on those files, and LI-06 remains their ladder owner |
| 1.3 | 2026-08-21 | **Erratum delta re-confirmation round (Phase PR; PM v13, TE v13 — POSTMORTEM-PR).** Ten findings, all mechanical corrections to this document; no task moved batch, no `Deps` edge changed, no AT partition, fixture or ledger row was touched, and the batches 7–13 ledger is byte-identical. **(1) §Post-batch remediation re-derived from `git show --name-status 2fc6fcd3`, not from the routed list** (PM/TE High, delta): the commit lists **45 paths**, not "six test-side surfaces"; the subsection now carries **nineteen rows** — the five added files including `pdlc/engine/__tests__/learnings-config-example.test.js` (the eighteenth tracked `learnings*` test-side file, PM Medium), P-A-5 second-owner rows for the six ladder-owned suites (LI-03/LI-07/LI-09/LI-11/LI-12/LI-23) and for `orchestrate-dev.js` (LI-15…LI-22), `scripts/capture-learnings-baseline.mjs` (LI-05), plus first-write rows for `pdlc/workflows/package.json`, `pdlc/workflows/.gitignore`, `coverageInstrumentation.test.js` and `docs-uniqueness.test.js` — and states that LI-04's root `.gitignore` is not in the commit. The single-writer argument is generalised over all rows, unchanged in substance (te-review: "only the enumeration is short"). **(2) The `package.json` premise corrected in the three places it appeared** (PM/TE High, inherited): §Overview's change-surface row, §Production and generated's bullet and DoD 11/12 now state HEAD — `allow-external: true`, four `**/`-anchored includes with the capture script among them, a three-glob `exclude`; the DoD 12 exemption is recorded as **retired at `2fc6fcd3`** and DoD 11 names the four-module stage-2 set. **(3) §Overview's "fourteen new test files" qualified as the ladder's count** with the eighteen-at-`09c7c62f` tree count beside it (TE Low). **(4) Changelog repairs** (PM Lows): rows 0.5/0.6 restored to monotone order; row 0.9's lead-in credit corrected to TE v11 F-01; case A's derivation now quotes "before batch 9", matching its *When* cell. **(5) Pin refresh, not cascade** (TE Low): header and prose pins read REQ v0.10 / FSPEC v0.14 / DECISIONS v0.5 (LI-12, the `RSN-NO-MATERIAL` arm row, §Overview and the F-O-1 obligations row); te v13 verified substance unchanged at the dispatched versions |
