...
|----|----------|-----------|----------|---------|----------------|
| F-01 | Medium | delta | local | The erratum restated D-12 over "yields material" but left D-9 reading "Does the per-document byte bound bind? → **yes → bounded flag** / no". At `maxBytesPerDocument: 0` the bound binds and **no** bounded flag is emitted — the document is dropped with `RSN-NO-MATERIAL` and has no BR-8 row to flag — so D-9 and D-12 answer the same fixture differently. BR-6's "Either way the document's row carries the bounded flag" has the same unqualified form, corrected only by the sentence that follows it; the decision table has no following sentence, and FSPEC:296 makes that table the DC-05 branch catalogue PLAN transcribes. Qualify D-9's yes-branch ("→ bounded flag, except where the bound is zero (D-12)"). | §Behavioral Flow → decision table D-9 (vs D-12); §Business Rules → BR-6 "How the per-document bound binds" |
| F-02 | Low | delta | local | The `maxBytesPerDocument: 0` half of REQ AC-4.4 is now decided in **BR-6** and reasoned in **BR-9**, but the reverse-trace row at FSPEC:178 still reads `AC-4.4 \| BR-5, BR-14 \| AT-30`. The reverse trace is the map a test author uses to find the rule behind an AT; for AT-30's third leg it points at neither owning rule. Add BR-6 (and BR-9) to the AC-4.4 row. | §Linked Requirements → reverse-trace row AC-4.4 |
| F-03 | Medium | inherited | nonlocal | BR-15 (FSPEC:710) compares both sides as **sets of paths, not counts** — "a document opened more than once neither adds a member nor changes the verdict (REQ AC-5.2)" — but AT-33 (FSPEC:973), which transcribes it and which PLAN LI-11 reads, still says "exactly one attempt per report-named document other than the `RSN-SELF` ones". A production path that opens a selected document twice is green under the rule and red under the oracle. Raised as v14 F-01; not routed by this erratum. Restate AT-33's expected set as a set of paths, or make at-most-one-open-per-path an explicit conjunct of BR-15. | §Acceptance Tests → AT-33 (vs §Business Rules → BR-15) |
| F-04 | Medium | inherited | nonlocal | BR-11, AT-03 and AT-29 quantify over "every dispatch prompt outside BR-1's rule — whether it fails the authoring conjunct or the C-1 target-document conjunct", while the upstream clause the traceability row `AC-4.3 \| BR-11 \| AT-03, AT-29` sends a reader to still reads "**non-authoring** dispatch prompts stay byte-identical to the disabled run" (REQ:367), echoed at AC-6.1 (REQ:412). The FSPEC's reading is the correct one — it follows AC-1.2, which AC-4.3 itself cites — but the cited clause no longer says it the same way and the weaker wording is what a later reader resolves against. REQ unchanged since v14 (same hash), so this remains the REQ's to fix. | §Business Rules → BR-11; §Linked Requirements traceability row AC-4.3 |
| F-05 | Low | inherited | nonlocal | A-2 (FSPEC:1029) reads "If a future phase introduces a dispatch that satisfies **neither** conjunct … BR-1 excludes it by construction". BR-1 excludes dispatches failing **at least one** conjunct; "neither" describes a strict subset and omits the authoring-classified/non-C-1-target case. Say "fails either conjunct". Raised as v14 F-03; untouched by this erratum. | §Open Questions → Assumptions (A-2) |

FINDING: Medium | delta | local | §Behavioral Flow → decision table D-9 (vs D-12) | D-12 was restated over "yields material" but D-9 still reads "yes → bounded flag"; at maxBytesPerDocument: 0 the bound binds and no bounded flag is emitted (the document is dropped with RSN-NO-MATERIAL and has no BR-8 row), so the two rows answer the same fixture differently and the DC-05 branch catalogue PLAN transcribes promises a flag the run does not emit — BR-6's unqualified "Either way ... carries the bounded flag" has the same defect, corrected only by the sentence after it
FINDING: Low | delta | local | §Linked Requirements → reverse-trace row AC-4.4 | the zero-per-document-bound half of AC-4.4 is now decided in BR-6 and reasoned in BR-9, but the reverse-trace row still names only BR-5 and BR-14, so AT-30's third leg traces to neither of its owning rules
FINDING: Medium | inherited | nonlocal | §Acceptance Tests → AT-33 (vs §Business Rules → BR-15) | BR-15 compares both sides as sets of paths and not counts, but AT-33 — the side PLAN LI-11 transcribes — still says "exactly one attempt per report-named document", so rule and oracle disagree on a document opened twice; raised as v14 F-01 and not routed by this erratum
FINDING: Medium | inherited | nonlocal | §Business Rules → BR-11; traceability row AC-4.3 | BR-11/AT-03/AT-29 quantify over dispatches outside BR-1's two-conjunct rule while the cited upstream clause REQ AC-4.3 (echoed at AC-6.1) still says "non-authoring dispatch prompts"; the FSPEC's reading follows AC-1.2 and is correct, but the REQ wording is still unfixed at HEAD and should be routed back to the REQ
FINDING: Low | inherited | nonlocal | §Open Questions → Assumptions (A-2) | A-2 says BR-1 excludes a dispatch that "satisfies neither conjunct" where the exclusion set is dispatches failing at least one conjunct, so as written it omits the authoring-classified/non-C-1-target case
