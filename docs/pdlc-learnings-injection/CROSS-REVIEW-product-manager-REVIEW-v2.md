...
| F-04 | Medium | Local | **The new `PROP-ISOLATE-02` digest manifest is baselined on the post-change tree, and its rationale states the opposite of what the branch does.** `5e952bdb` added `learningsDispatchSet.test.js`'s SKILL.md digest test, and its construction is the right one — real `git ls-files -- 'pdlc/skills/**'`, set-equality over paths against the manifest keys (never containment), a non-vacuity control on the enumerated set, and hand-transcribed digests never regenerated inside the assertion. The defect is the baseline, not the mechanism: the manifest was transcribed at authoring time from *this branch's* tree, and its own comment claims "This feature's own scope boundary (§Scope boundary) touches no file under `pdlc/skills/**`". That is false at HEAD — `git diff $(git merge-base main HEAD)..HEAD -- pdlc/skills/` returns three changed files (`pm-review`, `se-review`, `te-review` SKILL.md, +42/−9), the `## Delta-Confirmation Findings` remediations F-01 above already tracks. So the test proves *stability since authoring*, which is worth having, but it cannot prove the BR-16 paraphrase in its own title ("no SKILL.md text moves") — text did move, and the manifest records the moved bytes as expected. Fix: transcribe the three review-SKILL digests from the merge-base instead and assert those three as known-changed with a named reason, or retitle the test and delete the scope-boundary sentence so the comment matches DoD 14's disclosure. Medium, not High: the movement is disclosed and accepted in PLAN DoD 14, and FSPEC BR-16's actual text is about required sections, headings and verdict grammar (`FSPEC-pdlc-learnings-injection.md:743-750`), not about file bytes. | FSPEC BR-16; REQ AC-5.3, G-5 |
