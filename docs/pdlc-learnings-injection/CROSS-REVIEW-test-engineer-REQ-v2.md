...
| ID | Question |
|----|---------|
| Q-04 | When the corpus listing fails (`RSN-UNLISTABLE`), does the report still carry an AC-3.1 row set per authoring dispatch (empty, per AC-3.1's "empty set of rows, not a missing field") plus the run-level unlistable record? A test needs to know whether the two coexist or whether unlistable replaces the rows. |
| Q-05 | Does AC-2.2's byte-order tiebreak run over the full repository-relative path, so that `docs/completed/{p}/...` sorts ahead of `docs/{p}/...`? That is what the shipped enumeration produces, and it means archived features outrank live ones under an absent ordering key — worth stating as intended rather than incidental. |

## Positive Observations

- The C-1 rewrite is the right kind of fix: it replaces a hand-counted set with the taxonomy the
  code already carries, so the oracle survives a run with no DECISIONS phase and a run with five
  optimizer rounds without the REQ being re-edited. AC-1.1's "a fixed count of six is not the
  oracle" makes the intent unmissable to whoever writes the test.
- AC-3.2's reason set is now seven catalogued ids with a stated set-equality obligation, and each of
  `RSN-TRUNCATED`, `RSN-UNREADABLE`, `RSN-UNPARSEABLE` maps to a fixture a TE can build in one line.
  F-10 asks where one id is recorded, not whether the set is closed.
- AC-2.2 converted an entirely deferred oracle into a total tiebreak plus two properties that are
  testable today (mtime permutation, directory rename). The mtime property in particular is the one
  that catches a real regression class, and it is stated as a property rather than an example.
- AC-5.1b is a genuine addition rather than a restatement: distinguishing an operator typo from a
  deliberate disable is exactly the case that would otherwise be byte-identical and silent, and
  AC-6.2 pins it in CI.
- §7.1 pastes the stopping rule rather than citing it, including the "grows while the count does
  not fall" clause — the form DC-09 asks for.

## Recommendation

**Approved with minor changes**

All five v1 High findings are closed and the revision introduced none. F-10 through F-13 are
implementability and report-shape refinements that FSPEC/TSPEC can carry as named entry
obligations, which is the disposition §7.1's stopping rule prescribes for exactly this class.

Recommended carry-downstream obligations for FSPEC/TSPEC:
1. Where `RSN-UNLISTABLE` is recorded, and the scope of the AC-3.2 set-equality test (F-10, Q-04).
2. Whether AC-3.3's reproduction inputs sit inside or beside AC-3.1's closed enumeration (F-11).
3. Which readers the shared corpus definition binds, and which stay literally pinned (F-12).
4. That AC-1.2's byte-identity comparison is asserted under AC-6.1's fixture harness (F-13).
