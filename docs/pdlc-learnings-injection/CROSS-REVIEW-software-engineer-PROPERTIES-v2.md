...
| ID | Question |
|----|---------|
| Q-01 | *(carried from v1 Q-02, unanswered in the text and not gating.)* PROP-DISPATCH-07 keeps `_recordDocType` plumbed through five hops in production code permanently, and PROP-DISPATCH-03 now depends on that probe for its operand — the rescoping made the probe load-bearing for two properties rather than one. Was a pure-predicate alternative weighed: export `injectHere`'s predicate and assert the set equality over it directly, with the run-level check reduced to "every `(dispatchKind, docType)` pair reaching the site is in the predicate's domain"? §O.4 argues convincingly that the *report* is the wrong operand; it still does not compare the probe against a pure predicate. If the answer is "the probe is the only thing that can observe the Phase CR `null` path end-to-end", one clause saying so closes it permanently. |
| Q-02 | PROP-DISPATCH-08's two-call-site set equality is keyed by `(enclosing named function, call-site form)` over "a static parse of `orchestrate-dev.js`". Both call sites are arrow/async-function bodies nested inside `reviewLoop` and `main` respectively — the enclosing binding for one is `const wrapped = (…) =>` and for the other `async function wrappedDispatch(…)`, two different syntactic shapes. Is the intended instrument a real parse (an AST walk resolving enclosing scope) or a regex over the file text? The two have very different failure modes on a 15,311-line module, and LI-11's row does not say which it owns. |
| Q-03 | PROP-RECORD-09 moved to "green on authoring" at LI-14, so it has no red predecessor and its subject is the suite rather than production code. But LI-21 is the task that introduces `runMirror` into the report, ten batches later. Between LI-14 and LI-21 the property is green over a corpus of suites in which the field does not exist yet. Is that understood — that its first *meaningful* evaluation is at LI-21, and that the batch-14 full-suite green is what actually certifies it — or should LI-21's row carry an explicit re-assertion? |

## Positive Observations

- **The three High fixes are structural, not cosmetic.** PROP-DISPATCH-08 is the best example: what I asked for was "convert four tautological conjuncts into one invariant", and the property that came back asserts the two-call-site set equality *and* requires each of the four outside families to be shown calling `agentFn` directly — which is the half that makes the invariant falsifiable rather than merely true. Both halves re-measured correct at HEAD, including the `waveImplementPrompt` call site the revision cites by name.
- **§O.1's new paragraph generalises the fix instead of patching one property.** "The three static-scan absences share one instrument and one positive control" identifies that a static absence over *source text* has a different vacuity mode from an absence over a run — a scan that located nothing, not an unexercised branch — and states the obligation once for all three. That is the right altitude for an oracle section, and it is why F-03 above is a one-clause tightening rather than a re-litigation.
- **PROP-BOUND-08 is a genuinely new defeater, not a restatement.** The observation that a matcher written to a wrong heading spelling greens on every synthetic fixture written to the same wrong spelling — and that no synthetic fixture can falsify it — is the kind of reasoning that finds the bug six months early. It came with the measurement to back it (9 of 9, 0 of 9 bare), it carries the positive-presence conjunct, and it correctly declines to invent the recognition rule, routing it as an erratum instead.
- **§O.7 handled the inherited figure exactly right.** Rather than deleting `87 of 89` or defending it, the revision marks it inherited, names its two-repository basis, acknowledges the conflicting TE re-derivation, and then states the locally checkable form the argument actually rests on (9 of 9, 19,340–50,695 bytes against a 6,000-byte bound). The claim got weaker and the argument got stronger.
- **§C.2's bidirectionality rule is a durable improvement even with F-01 open.** Striking five padded rows and writing down the rule that prevents them is worth more than the five rows; padding an AC row with a property that does not claim the AC is how a coverage matrix stops meaning anything. The rule now exists in the document, and F-01 is the last three rows catching up to it.
- **§C.4's file enumeration now reconciles against PLAN's own arithmetic.** Fourteen rows over fourteen files, naming each owning task, matching PLAN §File-ownership manifest's "fourteen test rows over fourteen files" — and separating the two *existing* files this document names (the pin's subject and the seam helpers) from the new ones, which is the distinction that made PROP-CORPUS-01 misleading at v1.
