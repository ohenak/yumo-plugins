...
| Field | Value |
|---|---|
| Upstream | REQ → FSPEC → TSPEC → PLAN → **PROPERTIES** — `REQ-pdlc-learnings-injection.md` (v0.9); `FSPEC-pdlc-learnings-injection.md` (v0.13 — this revision is grounded on the v0.13 erratum: E-36, BR-6's material-only accounting basis and zero-bound clause, F-O-1's two heading rules); `TSPEC-pdlc-learnings-injection.md` (v0.9 — §D.3's section matcher and extent-assembly rule absorbed here); `PLAN-pdlc-learnings-injection.md` (**v1.3** — re-pinned from v0.8 at this revision (PM v12 F-01, SE v12 F-01); LI-12's three-case AT-30 still matches PROP-CONFIG-09; the *Amendment commits on landed suites (P-A-7)* table was added at **v0.6** with two cases and grew to **three** at v0.8, whose changelog re-scopes case B to batches 9–12 and adds **case C** — quoted verbatim at HEAD as *"batch 13 or later, the case that is live at HEAD"* — as the case that governs the amendments this document routes; v0.7 named LI-16 the owner of TSPEC §D.5's zero-bound production half, gave LI-AT-30 conjunct (iii) its fixture precondition, recorded ERR-8 and relocated LI-08's amendment note; all absorbed in §C.4 and §G.3); `DECISIONS-pdlc-learnings-injection.md`. **v0.8 re-pins every commit anchor to HEAD `09c7c62f`** (PM v11 F-01): the branch was rebased and every anchor this document carried was pre-rebase, and re-measuring at HEAD showed the branch had also advanced past the reviewed commit — §C.4's seven `learningsBlock.test.js` absence claims are reversed, P-A-7 case C is recorded **discharged green** rather than pending, and the four unowned CODE_REVIEW v1 remediation files are recorded as §G.2 gap 5 and routed to PLAN in §G.3. No property, oracle, fixture, AT mapping or coverage row moves. **v0.9 re-pins the PLAN reference v0.8 → v1.3** (PM v12 F-01, SE v12 F-01). The pin was taken at v0.8 and PLAN has since passed through v0.9, v1.0, v1.1, v1.2 and v1.3; the rulings re-verified against PLAN at v1.3 in that pass — P-A-7's three cases and their windows — are present unchanged, and for those the finding was pin freshness, not fidelity. The claim is **scoped to the rulings actually re-checked**, not to every ruling this document cites: **v1.0 extends the sweep to P-A-6**, which PLAN did move (v1.1 rewrote its PROPERTIES fallback route; see §C.4). Two consequences of re-pinning were taken at v0.9, because a pin at HEAD may not carry a quotation that is no longer verbatim at HEAD: case C's case name is re-quoted as *"batch 13 or later, the case that is live at HEAD"*, and §G.3's *"PLAN at HEAD"* sentence restates case A's window as **before batch 9** and case C's outcome as recorded-green. Attributions of the form *"PLAN v0.8"* elsewhere in §C.4 and in §G.3's struck bullets are **provenance, not pins**, and are left unchanged (SE v12). No property, oracle, fixture, AT mapping or coverage row moves at v0.9 either. **v1.0 lands round 14's findings** (PM v14 F-01…F-06, SE v14 F-01…F-04): §C.4's statement of P-A-6's fallback route is restated on PLAN v1.1's rewritten form (`PLAN-pdlc-learnings-injection.md`, P-A-6: *"P-A-7's governing case — which at HEAD is case C, where no ledger remains to amend into and the obligation is green-at-landing; the amend-into-the-ledger-by-name route is case B's, and case B closed at batch 12"*), §G.3's newly-routed manifest item is struck as answered by PLAN v1.2 items (3)/(4) and v1.3 item (1), and §G.2 gap 5's file count is re-derived. No property, oracle, fixture, AT mapping or coverage row moves at v1.0 either. |
