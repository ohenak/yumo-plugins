...
|----|----------|-------|---------|----------------|
| F-01 | **High** | Local | **The re-pin to PLAN v1.3 asserts a fidelity this document does not have: P-A-6's fallback route was rewritten at PLAN v1.1, and this document still offers the retired one.** Lines 1178–1181 tell the reader that the PROPERTIES suite *"may be committed as soon as it is green — or, if it lands red, its rows are amended into the ledger by name first, under the same P-A-7 rule"*, and line 1186 calls **P-A-6** *"(byte-unchanged at v0.8)"*. PLAN's P-A-6 at HEAD (`PLAN-pdlc-learnings-injection.md:663`) rules the opposite: the suite's rows *"are handled under **P-A-7's governing case** — which at HEAD is case C, where no ledger remains to amend into and the obligation is green-at-landing; **the amend-into-the-ledger-by-name route is case B's, and case B closed at batch 12**"*. PLAN's v1.1 changelog records the change as deliberate (TE v11 F-03). **This is delta-attributable, not merely inherited**: at v0.8 the passage was accurate as of its own pin; this delta re-pinned to v1.3 and added the sentence *"every ruling this document cites is still present at v1.3, so the finding was pin freshness, not fidelity"* (line 11), which is false for exactly this ruling — and the same header paragraph promises the sweep it did not finish (*"a pin at HEAD may not carry a quotation that is no longer verbatim at HEAD"*, *"two consequences of re-pinning are taken with it"*; there are four). The consequence is actionable and wrong: an implementer landing this document's own suite red is directed to amend a PLAN ledger that no longer exists, instead of treating the red as the real defect P-A-7 case C says it is. Fix: restate lines 1180–1181 as case C's obligation (green at landing; a red is a real defect owed a fix before batch 14), drop *"byte-unchanged at v0.8"* at line 1186 or re-date it to v1.1, and narrow the header's blanket sentence to the rulings actually re-verified | PLAN P-A-6, P-A-7 case C; DoD 6 |
| F-02 | Medium | Local | **§G.3 still routes to PLAN an item PLAN answered two versions ago — under a bullet the same delta re-pinned to v1.3.** Lines 1357–1367 (*"Newly routed this round — one item"*) hand PLAN the choice *"whether the four take task rows, a remediation row, or an explicit out-of-manifest note"*. PLAN v1.2 item (3) records all four in a new **§Post-batch remediation** subsection, item (4) adds the P-A-5 second-owner rows for `2fc6fcd3`'s re-capture (my v13 F-05), and v1.3 item (1) re-derives that subsection from `git show --name-status 2fc6fcd3` into **nineteen rows**. The delta re-pinned the struck bullets immediately above this one (line 1326) to v1.3 and stopped there. This document names the anti-pattern itself twice in the same section — *"re-raising it here would be the DEC-ERR-01 anti-pattern of routing a question already routed"*. **Non-gating**: a stale route misdirects attention; it falsifies no property, oracle or fixture. Fix: strike the bullet into the *Also answered — by PLAN* list, citing PLAN v1.2 items (3)/(4) and v1.3 item (1) | PLAN §Post-batch remediation; DEC-ERR-01 |
| F-03 | Medium | Local | **Gap 5's file count is not reproducible by the command §C.4 says it ran.** Line 1278 states the re-measurement *"finds **eighteen** `learnings*` files under `pdlc/workflows/__tests__`"*; at HEAD that directory holds **seventeen** (3 helpers + 14 test files). §C.4's table has eighteen *rows*, but its eighteenth is `fixtures/learnings-baseline/` — a directory, not a `learnings*` file, and itself 22 files. Separately, PLAN v1.3 now uses "eighteen" for a different set (*"the eighteenth tracked `learnings*` test-side file"* = `pdlc/engine/__tests__/learnings-config-example.test.js`, outside this document's stated scope). **Non-gating**: inherited, and the conclusion the count supports (PLAN's manifest under-counted) was true and is now discharged. Fix: say *"seventeen `learnings*` files plus the baseline fixture directory — eighteen inventory rows"*, and note that the engine-side file is out of this inventory's scope by construction | PLAN §File-ownership manifest |
| F-04 | Medium | Local | **Carried from v13 F-03, unaddressed: one of the four unowned files sits on a property's oracle path, which §C.4 and gap 5 do not say.** Both describe all four as artifacts *"no property of this document names"* — true as stated — but `helpers/learningsComposition.js:2` describes itself as *"the AC-2.5 / PROP-ORDER-05 composition"* and is imported by the task-owned `learningsDispatchSet.test.js:42`, so PROP-ORDER-05's two-process oracle executes through an unowned helper. The oracle has not moved and is still satisfied, so this is completeness, not fidelity. Fix: one clause in gap 5 | AC-2.5, PROP-ORDER-05 |
| F-05 | Low | Local | **Residual of v13 F-02: line 1340 still paraphrases case C as *"after batch 13"*.** PLAN v1.3's case C is *"batch 13 or later"*, one batch wider (PLAN v1.1 restated it by batch number so no batch falls between case B's bound of 12 and case C). The bullet is struck-through provenance about what PLAN v0.8 answered, and the conclusion (no ledger row) is identical under either width. Fix: one word, or leave and mark it as v0.8-era wording | PLAN P-A-7 case C |
| F-06 | Low | Local | **Carried from v13 F-04, unaddressed: the baseline fixture row attributes two capture events to one commit.** §C.4's `fixtures/learnings-baseline/` row reads `Added by 744311f7` while its own cell enumerates the `PIPELINE-NON-AUTHORING-PROMPTS/` arm of 18 files, which arrived in `2fc6fcd3`. Against the table's own convention (*"each row carries the commit that added the file"*), a reader auditing the arm at `744311f7` finds nothing. Fix: split the row or name both commits | PLAN §File-ownership manifest |

**One High.** Everything the delta *quoted* verifies character-exactly against PLAN at HEAD — all six
new fixed-string quotations, the case A/B/C windows, PLAN's version and changelog chain. What does
not hold is the delta's own generalisation over what it did not quote. The blocking item is that
generalisation and the one live instruction it makes wrong.
