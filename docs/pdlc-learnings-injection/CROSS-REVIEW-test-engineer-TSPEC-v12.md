...
|----|----------|-----------|----------|---------|----------------|
| F-01 | High | delta | local | TSPEC's only statement of when `RSN-NO-MATERIAL` fires is the structural condition "No BR-6 section present". FSPEC v0.13 restated it over *yields no material*, with two disjuncts — no priority section, **or** `maxBytesPerDocument: 0` admitting none — and E-36 requires the zero-bound document to be dropped before the bounds, consuming no slot. Under TSPEC as written a section-carrying document at a zero bound is **selected** with `bytes: 0` and takes a `maxDocuments` slot, so AT-30's third case ("BR-8 rows present and empty") reds. Restate the drop over the extractor's return value (`material: ""`, `sections: []`), which is also the more falsifiable condition. | §T.6 decision-branch map, "No BR-6 section present ⇒ `RSN-NO-MATERIAL`" |
| F-02 | High | delta | local | FSPEC v0.13 widened F-O-1 to **two** heading-recognition rules and assigned both to TSPEC: the document-shape predicate **and** the rule by which a heading counts as one of BR-6's named sections (numbered form, bare title, or a prefix). §D.3 discharges the first only, and §T.6's obligation map still glosses F-O-1 as the "presents as a LEARNINGS document" predicate. FSPEC explicitly refuses to decide the second ("Which heading forms count as which section is F-O-1's"), so the rule exists nowhere. AT-11's set equality over `sections` and AT-28's `RSN-NO-MATERIAL` oracle are both unwritable without it, and a conventional-form fixture is green under all three readings, pinning the choice nowhere. Add a §D.3-shaped subsection and re-gloss F-O-1. | §D.3 / §T.6 obligation map, F-O-1 |
| F-03 | Medium | delta | local | §I.2's AT-30 gloss enumerates two zeros (`maxDocuments: 0`, `maxTotalBytes: 0`); upstream AT-30 now enumerates three, adding `maxBytesPerDocument: 0`, plus the conjunct that every corpus document carries `RSN-NO-MATERIAL` (E-36). A PLAN author deriving `learningsConfig.test.js`'s fixtures writes two cases and E-36 ships named by no test. Transcribe the third case and state its oracle as three positive conjuncts (rows present and empty; `learningsInjection` key present; every corpus path carrying reason exactly `RSN-NO-MATERIAL`) — never an absence-only "no rows". | §I.2, "**AT-30 owns none of them**" |
| F-04 | Medium | delta | nonlocal | `bounded`'s value at `maxBytesPerDocument: 0` is undecided. §D.5 decides `bounded` "at the cut" and no cut occurs at a zero bound, yet the document is maximally abridged. T-O-6's owed property quantifies over **any** non-negative `maxBytes`, so a generator sampling `0` decides the question by accident. Decide it, or exclude `0` from the property's domain with a stated reason. | §I.3 `extractInjectableMaterial` contract; §D.5; T-O-6 |
| F-05 | Medium | inherited | nonlocal | §T.6's fixture list still carries three AT-02 dispatch run shapes; upstream AT-02 enumerates four, the fourth being the authoring-classified dispatch whose target is none of C-1's six types, and it carries the mutation obligation "reverting BR-1's second conjunct reds this test". Transcribe the fourth shape and name the file owning the mutation check. Unchanged since v11 — TSPEC's bytes have not moved. | §T.6, "AT-02 dispatch-universe set equality" |
| F-06 | Medium | inherited | nonlocal | `parseLearningsConfig` still returns `present` with no consumer in §I.3's gate (which drops it) and no behavioural oracle in §T.5. §OQ keeps it for AC-5.1a's report-key distinction, but no test reads it, so its removal would red nothing. | §I.3, §T.5 |
| F-07 | Low | inherited | nonlocal | §A.2's coincidence-invariant paragraph still cites `converge()`'s six `docType` sites as `:13766`, `:13774`, `:13807`, `:13874`, `:13893`, `:13996`; HEAD's sites are `:13908`, `:13916`, `:13949`, `:14016`, `:14035`, `:14138`. The set-equality claim itself is unaffected. Restate per DEC-DOC-01 (cite content, not line number). | §A.2 coincidence invariant |
| F-08 | Low | inherited | nonlocal | Seam anchors still stale at HEAD: P-7's `defaultGit` `:11658-11676` (HEAD `:11698`), P-8's `defaultReadFile` `:11513-11519` (HEAD `:11553`), §A.4's `defaultListFiles` `:11586-11605` (HEAD `:11626`), §A.5's run-scoped `notices` sink `:12110` (HEAD `:12150`). No cited proposition is falsified. DEC-DOC-01. | §Ground-truth P-7/P-8; §A.4; §A.5 |
| F-09 | Low | delta | nonlocal | The header's Upstream row reads FSPEC **v0.12**; HEAD is **v0.13**. It was one version behind at v11 and is now two, alongside the five passages still citing "FSPEC v0.9" (§A.5's loci sentence, §I.3's E-21…E-34 row-ownership sentence, §T.5's `learningsRecord.test.js` row, ERR-4, ERR-6). Every cited proposition is still verbatim-present upstream; the document simply names three upstream versions at once. | Header Upstream row; §A.5, §I.3, §T.5, ERR-4, ERR-6 |

FINDING: High | delta | local | §T.6 decision-branch map, "No BR-6 section present ⇒ RSN-NO-MATERIAL" | TSPEC fires RSN-NO-MATERIAL on the structural condition only; FSPEC v0.13 restated it over "yields no material", adding the maxBytesPerDocument-zero disjunct, and E-36 requires the zero-bound document dropped before the bounds consuming no slot — under TSPEC as written it is selected with bytes 0 and takes a slot, so AT-30's third case reds
FINDING: High | delta | local | §D.3 / §T.6 obligation map, F-O-1 | FSPEC v0.13 widened F-O-1 to two heading-recognition rules and assigned both to TSPEC; §D.3 discharges only the document-shape predicate and the obligation map still glosses F-O-1 as that one rule, so BR-6's heading-matching rule has an owner named upstream and no text in the owner — AT-11's sections set equality and AT-28's oracle are unwritable without it
FINDING: Medium | delta | local | §I.2, "AT-30 owns none of them" | §I.2 enumerates two admits-nothing zeros; upstream AT-30 now enumerates three (adding maxBytesPerDocument: 0) plus the every-document-carries-RSN-NO-MATERIAL conjunct (E-36), so a PLAN author derives two fixtures and E-36 ships named by no test
FINDING: Medium | delta | nonlocal | §I.3 extractInjectableMaterial contract; §D.5; T-O-6 | bounded's value at maxBytesPerDocument: 0 is undecided — §D.5 decides bounded "at the cut" and no cut occurs at a zero bound, while T-O-6's owed property quantifies over any non-negative maxBytes
FINDING: Medium | inherited | nonlocal | §T.6, "AT-02 dispatch-universe set equality" | fixture list carries three AT-02 run shapes; upstream AT-02 enumerates four, the fourth being the authoring-classified non-C-1 target carrying the mutation obligation, and no file is named as owning that mutation check
FINDING: Medium | inherited | nonlocal | §I.3, §T.5 | parseLearningsConfig still returns `present` with no consumer in §I.3's gate and no behavioural oracle in §T.5, so removing it would red nothing
FINDING: Low | inherited | nonlocal | §A.2 coincidence invariant | six converge() docType line anchors (:13766, :13774, :13807, :13874, :13893, :13996) are stale against HEAD (:13908, :13916, :13949, :14016, :14035, :14138); DEC-DOC-01
FINDING: Low | inherited | nonlocal | §Ground-truth P-7/P-8; §A.4; §A.5 | seam anchors stale at HEAD: :11658-11676 → :11698, :11513-11519 → :11553, :11586-11605 → :11626, :12110 → :12150; DEC-DOC-01
FINDING: Low | delta | nonlocal | Header Upstream row; §A.5, §I.3, §T.5, ERR-4, ERR-6 | header names FSPEC v0.12 where HEAD is v0.13, and five passages still cite FSPEC v0.9; no cited proposition is falsified

## Recommendation

**Needs revision.** Two High findings, both `delta` and both `local`: this TSPEC no longer
compresses FSPEC faithfully at the two places the v0.13 erratum moved. Neither asks FSPEC to move
again and neither reopens a settled decision — one bounded follow-up round on TSPEC discharges
both, restating §T.6's drop condition over *yields no material* (F-01) and adding a §D.3-shaped
subsection that states BR-6's heading-matching rule with F-O-1 re-glossed (F-02). F-03 should ride
along, since it is the fixture that would have caught F-01.
