...
| D-O-9 | TSPEC closes `OQ.2`, retires `ERR-4`, drops the `present`/`sectionMalformed` conjuncts from §I.3 and aligns `LEARNINGS_DEFAULTS` with REQ §4.1 — emitted as a TSPEC erratum with this revision (DEC-LI-07). PROPERTIES and PLAN read TSPEC, so this must land before `AT-31`/`AT-32` are authored against §I.3 | TSPEC |
