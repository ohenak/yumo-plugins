...
- **The High finding was closed at the root, not at the oracle.** The obvious fix for v1 F-01 was to add a test around two string literals. What landed instead removes the dead-constant class: `orchestrate-dev.js:2860`/`:2862` now assign `DECISION_LEDGER_CORPUS_OUTCOMES.UNLISTABLE`/`.EMPTY` rather than bare strings, so the catalogue is load-bearing in production. The test on top of that (`decisionLedgerInjector.test.js:455–516`) is two-sided in the sense the finding asked for: declared members set-equal a literal hand-transcribed from FSPEC F-6/F-7, and the values the injector *emits* across its F-6/F-7/F-10 arms set-equal that same literal. M1 and M2 confirm both directions red.
- **The F-10 arm is included honestly.** The emitted-set test runs a survivorless arm, asserts `corpusOutcome` is `null` there, and then filters it out of the set comparison (`:509–512`). A weaker test would have omitted the arm silently; this one states why it contributes nothing.
- **F-03's remediation is a positive/positive pair, not a negative.** The new test asserts creator and optimizer dispatches are **byte-identical to their flag-off counterparts** (`:516–517`), floors non-vacuity (`:514–515`), and adds an anchor conjunct that the flag-on run really did inject somewhere (`:520`). It never asserts "the block's bytes are absent". M7 shows it catches the exact leak class in isolation.
- **A second positive pair arrived unprompted.** The malformed-config test at `:528–546` gives REQ-DECLEDGER-05's notice half a positive through `main()` — set-equal to `{NTC-DECLEDGER-MALFORMED}`, not "contains" — pairing the flag-off empty-set conjunct that was previously absence-shaped on its own. The adjacent notice filter was also fixed (`:453–457`): it used to stringify the notice *object*, so `String(n).includes("NTC-DECLEDGER-")` matched nothing on any run and the conjunct could not fail. That is a false-green the round found itself.
- **F-04's replacement makes the claim the title makes.** `decisionLedgerFixtureGuard.test.js:238–253` reads Baseline v1.2's own `Verified at` row off disk and asserts the transcribed commit appears in it, with a `typeof verifiedAtRow === "string"` non-vacuity guard so a renamed row fails rather than passing trivially. M5 red.
- **The E-8 production fix is line-local and correctly gated.** The new pass (`orchestrate-dev.js:2733–2745`) fires only when the full block already exceeds `maxBytes`, so a corpus that fits is untouched — and that gate is reasoned in the comment rather than left implicit. It matches `TSPEC`:778–781's own text ("a single line that alone exceeds `maxBytes` is dropped whole by the same step and the loop continues over the rest"), so the shipped code moved *toward* §3.6, not away from it.
- **The E-8 change brought its own head-position anchor.** `decisionLedgerBounds.test.js` adds a case where the oversized record is `fullOrder[0]` — asserted explicitly at the top of the test — and pins `result.omitted` by **set equality on the whole list**, not containment. The pre-existing AT-15 anchor was also strengthened from `not.toContain` to full `toEqual` on the surviving ids. M3 reds on this anchor alone, which is the right blast radius for a rule this local.
- **The property was widened rather than weakened around the same change.** Rather than relaxing PROP-BND-04 to "subsequence", the property now derives framing bytes from the observed block and the model's own formatter and requires that any record dropped from *ahead* of a survivor be one that could not fit `maxBytes` alone (`:196–208`) — the E-8 exemption, and nothing broader. M4 confirms it still reds when a fitting line is dropped from the front.
- **No skip drift.** 70 skipped at v1, 70 skipped at HEAD, +5 passing tests — the round added oracles without parking any.
- **Bundle and tree are clean.** `build-runtime.mjs --check` reports `in-sync`; `git status --porcelain` is empty; `docs/.DS_Store` is untracked and `.gitignore:47` prevents its return.
