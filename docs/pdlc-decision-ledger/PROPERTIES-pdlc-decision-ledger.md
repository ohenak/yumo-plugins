...
report field's sole evidence (PM F-03, PM F-05). (5) **PROP-INV-09**'s hand-off corrected: the field's obligation now names `TSPEC`'s **two** homes, both inside §7.2's live arm — conjunct 3's key-set delta and the flag-on presence-and-shape assertion, both carried by PROP-WIRE-12 — replacing the retired routing to PROP-OFF-05 and PROP-WIRE-11, and recording that §7.6's AT rows are expressly not a home (PM F-05). (6) **PROP-OFF-05** retired its FX-BASELINE
...
non-referent note (PM F-03). (7) The INV rationale paragraph's "both operands set-equality-checked"
...
`CENSUS_TOKENS` (PM F-06, SE F-02, SE F-03). (8) Counts follow: INV 10 → 11, WIRE 11 → 12, partition
total 101 → **103**, pyramid integration 37 → 39. (9) §Gaps gained a second routed item —
...
| **PROP-WIRE-12** | **Paired-run key-set delta (live), the report field's sole evidence.** On the live composition-root arm's **own** paired runs — one `main()` run flag-off and one flag-on, both driven inside this arm — the **symmetric difference** between the `report` key set the flag-off run **itself returns** and the key set the flag-on run **itself returns** must be exactly `{decisionLedger}`, asserted as a set equality **in both directions** so a spuriously **added** key on either arm and a spuriously **dropped** key on either arm both fail. The referent is the paired runs, **never** FX-BASELINE: `TSPEC` §7.4's recording captures reviewer-prompt streams and no `report` keys at all (§7.2's referent-split note), so it cannot serve here. On the same flag-off run the emitted `NTC-DECLEDGER-*` notice set must be **set-equal to empty**. `TSPEC` §5.4 names this arm the field's **sole** evidence — no `FSPEC` AT row asserts `report.decisionLedger` — so deleting the arm deletes the field's only proof, which is what this property exists to make un-deletable. On the **flag-on** run of the same pair, this property additionally asserts `report.decisionLedger`'s **presence and shape** — the second of the two homes `TSPEC` §7.3's report-field paragraph names — so both homes are discharged here and nowhere else. PROP-WIRE-11 does not substitute for either: that is a unit-level contract on the conditional spread, asserted over the injector, not over two `main()` runs. | Integration |
...
| **PROP-OFF-05** | With the flag off, the emitted notice set must be **set-equal to empty** — not merely free of `NTC-DECLEDGER-*`, and not compared against a baseline notices array, since FX-BASELINE records reviewer-prompt streams only and holds no notices array and no `report` key set to be set-equal *to* (`TSPEC` §7.2's referent split; see §FX-BASELINE, *Recorded stream deliberately narrow*). The **report key-set** half of this obligation is **PROP-WIRE-12's**, on the live arm's own paired flag-off/flag-on `main()` runs, which is the only place a `report` key set exists to compare. | Observability |
...
| **PROP-INV-06** ✖ | **Source census.** Zero occurrences of any member of `DECISION_LEDGER_CENSUS_TOKENS` — `selectDecisions`, `recogniseDecisionRecords`, `renderDecisionLedgerBlock`, `gatherDecisionCorpus`, `DECISION_LEDGER_OMIT_REASONS`, `DECISION_LEDGER_CORPUS_OUTCOMES` — anywhere in `orchestrate-dev.js` **outside** the regions this feature owns: (a) the body of **every** member of the frozen `DECISION_LEDGER_OWNED_DECLS` — **fourteen** at `TSPEC` v1.0, the six functions plus the eight top-level constants — each sliced **from its own declaration line to the next top-level declaration of any name**, boundaries taken from **all** of the module's top-level declarations rather than from the owned subset (`loopEconomicsAnchorGuard.test.js`'s `bodyOf` over `allTopLevelDecls`); and (b) the `main()` wiring run between `// === DECISION LEDGER WIRING START ===` and `... END ===`. Slicing **every** owned declaration rather than a hand-picked three is what makes the census satisfiable: a token's own declaration line, and its uses by its sibling declarations, are then never in the remainder. Brace-matching is **not** the mechanism — it cannot slice a constant such as `DECISION_HEADING_RE` at all. | Contract |
| **PROP-INV-07** | **The partition, not export set-equality.** `DECISION_LEDGER_CENSUS_TOKENS` ∪ `DECISION_LEDGER_CENSUS_EXEMPT` must be **set-equal** to `DECISION_LEDGER_OWNED_DECLS`, and the two sub-sets **disjoint** — asserted in both directions over the same frozen fourteen-member owned list PROP-INV-06 slices, so a symbol added later must be classified into one sub-set or the other or the test reddens. Set equality of `DECISION_LEDGER_CENSUS_TOKENS` against **all** of the module's decision-ledger exports is the form `TSPEC` §7.3 **rejects as red by construction** (roughly a dozen are declared; only six carry decision-record data) and must not be asserted. | Contract |
...
| **PROP-INV-09** ✖ | The report field name `decisionLedger` must **not** be a census token. `TSPEC` §7.3 records why: the shipped `learningsInjectionField` analogue is threaded through `buildFinalReport` and named at every one of its call sites, all far outside `main()`'s wiring sentinels, so the census would red on conforming code. What the field is owed instead is behavioural, by **exactly two named homes, both inside `TSPEC` §7.2's live composition-root arm**: that arm's conjunct-3 paired-run key-set delta, and, on the flag-on path, that same arm's presence-and-shape assertion on the field — **PROP-WIRE-12** carries both. `FSPEC` §7.6's AT rows are expressly **not** a home (no AT row's Notes column mentions `report.decisionLedger`), and neither is PROP-OFF-05, which is T-02's FX-BASELINE guard, nor PROP-WIRE-11 alone, which is a unit-level contract on the conditional spread. Deleting the live arm therefore removes the field's sole proof. | Contract |
...
| **PROP-INV-11** | **Every owned declaration resolves to exactly one.** Each of the fourteen members of `DECISION_LEDGER_OWNED_DECLS` must resolve to **exactly one** top-level declaration of `orchestrate-dev.js` at HEAD — asserted as a count of `1`, so both a **zero**-resolution member (a rename or a deletion silently shrinking the exclusion, the defect the `TSPEC` v1.0 erratum round existed to fix) and a **two**-resolution member fail. This is `TSPEC` §7.3's red-on-rename conjunct; PROP-INV-08's non-emptiness check catches neither. | Contract |

**PROP-INV-06's two operands are both frozen, and each is kept honest by its own conjunct** — which
is the whole reason the census is implementable. The **token** operand is held by PROP-INV-07's
partition (union with `DECISION_LEDGER_CENSUS_EXEMPT` equals `DECISION_LEDGER_OWNED_DECLS`,
disjoint); the **owned-declaration** operand is held by PROP-INV-11's resolves-to-exactly-one and
PROP-INV-08's non-empty-slice, **not** by a set equality against the module's exports —
`TSPEC` §7.3 names that comparison red by construction. **Where the three census constants live**
is load-bearing for both: `DECISION_LEDGER_CENSUS_TOKENS`, `DECISION_LEDGER_CENSUS_EXEMPT` and
`DECISION_LEDGER_OWNED_DECLS` are declarations of `decisionLedgerCensus.test.js` **itself**, not of
`orchestrate-dev.js` (`TSPEC` §7.3, *Where the three census constants live*), which is why no
module-surface section declares them, why none of the three is a member of the owned list or of
either sub-set, and why the census never scans the file the three are declared in. `TSPEC` §7.3 also
records the earlier, unimplementable wording and why it failed: the token set was ubiquitous (`id` is one of the commonest identifiers in the file)
...
| (b) | `manifest.mergeBaseSha` asserted equal to a hand-transcribed `EXPECTED_MERGE_BASE_SHA` **literal in the test file**, and the capture must be **invoked with a resolved 40-hex sha**, never a symbolic ref: `runCaptureScript` records `mergeBaseSha` as whatever `mergeBaseRef` string the caller passed (`scripts/capture-learnings-baseline.mjs`, `runCaptureScript` forwarding `mergeBaseRef` to `_captureFixtures`), so an invocation with `origin/main` records a ref string and the transcribed literal and the recorded value stop being the same kind of thing. The guard must therefore also assert the recorded value matches `/^[0-9a-f]{40}$/` (PM Q-02) | Same reason. `git merge-base --is-ancestor {recorded sha} HEAD` is kept only as a documented **weaker second signal** — it cannot distinguish "pre-feature" from "mid-feature", since a later `main` commit is an ancestor of HEAD too. A form computing `git merge-base origin/main HEAD` **at test time** is specifically excluded: it makes a required check depend on a current local `origin/main` and can red on an unrelated push to `main`. Ancestry is resolved against **`HEAD`**, never `origin/main`, so the check needs no fetch and is hermetic in CI |
...
**Feeds:** PROP-OFF-01…06, PROP-WIRE-09, PROP-INV-03, ORC-04. It is **not** the referent for any
`report` key set or notices array — see PROP-WIRE-12, which owns that comparison against the live
arm's own paired runs.
