...
|----|----------|-----------|----------|-------------|----------------|
| F-01 | High | delta | local | The re-grounding's new partition asserts `DECISION_LEDGER_CENSUS_TOKENS` ∪ `DECISION_LEDGER_CENSUS_EXEMPT` (**nine**) = `DECISION_LEDGER_OWNED_DECLS` (**fifteen**), with the conjuncts "each member of `OWNED_DECLS` resolves to exactly one top-level declaration at HEAD" and "each slice asserted non-empty before counting". Fourteen of the fifteen have an owning green task that declares them in `orchestrate-dev.js` (T-13…T-18). The fifteenth, `DECISION_LEDGER_CENSUS_TOKENS`, has none: it is the census test's own operand — the analogue of the precedent's `ANCHOR_TOKENS`, declared at `loopEconomicsAnchorGuard.test.js:114` and absent from every `pdlc/workflows/*.js` — and TSPEC declares it in no module-surface section (all seven occurrences are in §7.3 or the changelog). Both conjuncts therefore red on a conforming implementation, putting the `[red]` T-11 that T-18 depends on back into the red-by-construction state round 9 repaired for `gatherDecisionCorpus` and §5.2's catalogues. The root is upstream (TSPEC §7.3:1297 lists the member, with a self-literal rationale that only holds for production code) and is routed as an erratum; the PLAN-side residue is that its frozen list requires a declaration no task in the PLAN creates, and the manifest that assigns the other two lists is silent on this one. | §Batches → `T-11` (PLAN:150); file-ownership manifest (PLAN:202); §Definition of Done census bullet (PLAN:488) |
