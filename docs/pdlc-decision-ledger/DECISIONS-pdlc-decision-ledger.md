...
| **DEC-DECLEDGER-16** | Every byte literal in TSPEC carries a provenance class — upstream decision, measurement at Baseline v{N} (`M-xx`), or budget ceiling (unwritten) — and a ceiling may enter a claim only where substituting the true, smaller drafted value preserves it (so `10,859 + 1,200 ≤ 12,500` and `10,859 ≤ 12,500 − 1200` are equally admitted), never as a term in an equality. Binds assertions and pinned expected values, and prose stating a figure as a standing fact; prose recounting a retired figure or a labelled worst-case upper bound is out of scope | §3.6, §4.3, §7.3 / D-9, D-10 | POSTMORTEM-D: rounds 6–7 each shipped a ceiling inside an asserted equality (`12,059`), red on conforming implementations and invisible at prose altitude | Easy — an authoring rule; relaxing it re-admits the round-6/-7 defect class |
...
| PROPERTIES | DEC-DECLEDGER-16 binds **assertions and pinned expected values** (TSPEC prose is bound only where it states a figure as a standing fact, not where it recounts a retired figure or a labelled worst case). A pinned equality may transcribe only upstream decisions and Baseline measurements. DEC-DECLEDGER-12's 1,200 — and any figure derived by adding it — may enter a property only in a comparison that survives substituting the true, smaller drafted value: `10,859 + 1,200 ≤ 12,500` and `10,859 ≤ maxBytes − 1200` are both admitted, `10,859 + 1,200 = 12,059` is not. The two halves of `12,059 ≤ 12,500` are pinned separately where each is measurable, per TSPEC §7.3's conjuncts |
