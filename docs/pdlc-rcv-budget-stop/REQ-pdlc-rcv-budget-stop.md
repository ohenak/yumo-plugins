...
**v2.7** (round 1) answered thirteen findings: pickup order **10 → 12 → 13 → 17 → 18** (§3.1, R-14, paired into `REQ-RCV-07` X-07/R-16); the second `forcePhases` blocked by the step-G refusal; the zero-round halt not re-authoring an existing post-mortem; a derived start below `W` decided; the Iterations render declared; O-10's **0-call** and interim-only legs, property obligation and DC-03 ledger; O-13/O-14; DC-09's stopping rule pasted into §9.
**v2.6** (operator-directed) removed all implementation contracts to TSPEC altitude, with no id, AC, threshold or semantic changed; **v2.5** and **v2.4** (rounds 10, 9) relocated O-10's fixtures to split §5.4 and deferred the consistency check, not the word; **v1.6 … v1.2** addressed rounds 5 … 1. **On size:** the hard ceiling is binding, so every revision since v2.5 pays for its additions by compressing prose or relocating to a shared file — v2.8 paid by relocating three rationales to split **§5.4**, **§5.5** and **§5.6**, and by compressing §§1–10.
...
`iterations` is the **budget**, not the rounds run (M-1c). Because AC-1.5(1) makes a **zero-round** halt the commonest new case, the Iterations section additionally states the **rounds this
...
**The scope of "every halt".** The rule below is quantified over **every halt that writes `POSTMORTEM-{phase}-{feature}.md` for a document-typed review-loop phase** (AC-1.1's scope, M-7e) — **not** over the pipeline's other halt classes (creator-agent failure, branch guard, listing failure, Phase PUB/CI, Phase DOD), none of which writes a post-mortem at HEAD or is asked to start (N-4), nor the phases N-7 excludes. So `H` counts **post-mortem-writing halts of this phase for this document** — the only halts leaving a marker for a clearance to clear, which makes the pairing exact. No exception within that scope:
...
4. **the reset is anchored and consumed, in the POSTMORTEM, by the loop.** The **reset region** is read as two counts: `H`, the number of `HALT-REASON:` lines, and `A`, the number of
   ...
   **So `WINDOW-START:` values are non-descending on every path** — normative, and what makes §4.1's
...
| **N-7** (shared) | Applying these mechanisms to Phase CR or Phase DOD. | Restated because AC-1.1 says so explicitly: untyped loops keep a per-invocation budget, take the shared constant's new value, and get no reset region. |
| **NB-1** | The fixed-point test, the zero-delta test, how a round's blocking count is read. | `pdlc-rcv-fixed-point-stop`'s. This REQ states only the window they are evaluated inside and the halt path they halt on. A finding that this document never says *when* the loop compares two rounds is **correct and known**. |
...
| # | Obligation | Owner |
|---|---|---|
| **O-5** | Specify the mechanism by which AC-1.4's **reset region survives every halt** — loop-owned state, **not** a prompt clause. The outcome is fixed here: the first halt creates the region, a later halt preserves every line and appends its own, unfenced `RESOLVED:` lines are stripped, and a region lost or unwritable is **reported fail-closed** rather than proceeded past on a silently widened window. Partial outcomes land fail-closed too (`W` = 1, or S-16 and a sanctioned repair). | TSPEC |
