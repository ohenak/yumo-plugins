...
**Consequence for sequencing.** This REQ is deliverable alone as a **requirement**: its window, budget, halt path and clearance accounting are determined without any successor. `pdlc-rcv-reset-region` is queued at **`Order 18`**, and the net pickup order after this row is **12 → 13 → 17 → 18** — `orchestrate-queue`'s `precheckDependencies` blocks only on a dependency *present* in the table with a non-`done` status, falling through to readiness triage for one that is absent, and the candidate walk is in `Order` order; so row 13 is pickable and row **17** precedes row 18. QUEUE.md's gloss *"10 → 12 → 18, with 18 ahead of 17"* inverts that comparison and is superseded here. (A queue-driver claim, not an `orchestrate-dev.js` control-flow claim, so NB-4's `M-*`-only discipline does not reach it; the function is named so the next reader need not re-derive it.) **R-14 states the two consequences** — a three-feature interim, and row 17's machine-written `WINDOW-RESUMED:` lines landing in unvalidated regions — both reasons X-06's interim must not be able to refuse.
...
1. **the reset region exists after the halt, and it carries this halt's line.** A halt finding **no existing post-mortem** — the first halt of a phase, creating the file —
   ...
   **The append carries the same confirmation obligation AC-1.5(4) puts on the answering line, and it is a confirmation *that this halt's line is present in the region*, not that the file exists** — on a re-halt the file always exists, so an existence-shaped check is satisfied whether or not the line landed, and that is exactly the path that matters. **Fail-closed on failure:** the halt is not recorded as taken, the entry **refuses the phase** and reports it, leaving the operator a state they can act on. This closes the otherwise-terminal failure: a lost `HALT-REASON:` under-counts `H`, so `A = H` holds forever, no clearance is ever unconsumed (AC-1.5(4)'s gate), the operator's `RESOLVED: yes` grants nothing and every later entry is a zero-round halt. What a **partially written** line leaves behind is `REQ-RCV-07` AC-7.5's, as for the answering line (NB-3).
...
| **NB-6** | Changing what the **authoring agent** writes into a post-mortem created by a **zero-round** halt. | **Correct and known, accepted.** The first halt of a phase authors its post-mortem from HEAD's prompt (M-7e), which asks for Phase, Iterations, Reviewers, Pattern of Disagreement, Best-Guess Root Cause and Recommendation over the phase's cross-review files. On a zero-round entry no reviewer of *this* window ran, so those sections are written from the **previous** window's files or left thin — and the operator still gets the two things the `RESOLVED: yes` decision needs, both **loop-written and pinned**: AC-1.3's Iterations render (`rounds run 0`, so the vacuity is visible rather than disguised) and the region's `HALT-REASON:` line naming S-4. Changing the prompt is a change to *what a halt is* (N-4) and a new operator-facing artifact, hence a new REQ; the cost is one authoring dispatch on the **first** halt only, since AC-1.4 re-authors none thereafter. |
