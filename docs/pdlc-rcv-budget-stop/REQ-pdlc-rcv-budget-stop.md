...
| Shared split record | `docs/_constraints/pdlc-rcv-split.md` — the v2.0 altitude split, what moved, and the **paired edges** this REQ and `REQ-RCV-07` must revise together (§5), the shared arguments — *why the validation conjunct is unwired* (§5.1), *why validation is a conjunct of the gate* (§5.2), *why a refusal is not a halt* (§5.3) — and the catalogue delegation (§6). |
...
| **X-06** | **read from** `pdlc-rcv-reset-region` REQ-RCV-07 AC-7.1 — the ordered validation algorithm behind AC-1.5(4)'s *region validates* predicate, and AC-7.2/AC-7.5's refusal | AC-1.5(4)'s third gate conjunct, and everything that follows a failure of it | This is a **forward** edge, not a `depends-on`: the predicate's *meaning* and its fail-closed disposition are fixed below, so nothing here is under-determined. But the conjunct is **not wired into the gate at this REQ's own ship** — `REQ-RCV-07` wires it in the same commit that lands AC-7.1's decision procedure. Until then AC-1.5(4)'s gate is its **two decidable conjuncts** (a readable `RESOLVED: yes`, and `A < H`), and `W` is read as §4.1 states without the validation guard. The **injection point** exists — the interim composition accepts the seam as a parameter — so row 18 adds a call rather than rebuilding a call graph, and **no call site is emitted at this ship**: the interim composition calls it **0 times** (O-10). So there is no production default to get wrong, and no interim entry is refused that AC-1.5(1)–(3) would have admitted: **no refusal and no S-16 on any branch, region or none, leaving every branch on the path AC-1.1–AC-1.5(3) and (5) already put it on.** **Why the conjunct is not shipped early — stated once for both ends of the edge at `pdlc-rcv-split.md` §5.1**, in one line: the refusing horn refuses on **non-emptiness**, which is exactly *the phase has halted* (AC-1.4 clause 1); the granting horn is the fail-open below; and a **narrower** procedure must still disagree with AC-7.1 on ordering and highest round, unclearable in the **refusing** direction until AC-7.4. Leaving it unwired is the only interim whose behaviour is **today's**. **What is *not* deferred:** §6's S-13/S-14 **grammar** is in force at this ship — the predicate's grammar layer is this REQ's, its **analysis** layer (ordering, highest round, `H − A ∈ {0, 1}`) is AC-7.1's, so that step re-checks rather than first-checks the grammar (§4.1, O-10 leg 3). **The cost** — R-10's hand-edited-region fail-open, open for one queue interval and no wider than HEAD's — is stated at §5.1 and R-14. Wiring *valid* once the conjunct **is** wired is the fail-open AC-1.5(4) exists to close: a defect, not a shortcut (`REQ-RCV-07` O-10). |
...
REQ's criteria read has a durable home and **there is no in-process-only row**; the full map is
`pdlc-rcv-baseline.md` **§3.2** (relocated there at round 9, read by the whole family). The two rows
this REQ's own clearance gate turns on stay here:
...
4. **the reset is anchored and consumed, in the POSTMORTEM, by the loop.** The **reset region** is read as two counts: `H`, the number of `HALT-REASON:` lines, and `A`, the number of
   `WINDOW-START:` **plus** `WINDOW-RESUMED:` lines — both counted **by line prefix, whatever the value**, so a malformed value contributes no origin (§6) but still answers a halt. A clearance is **unconsumed** exactly when all three hold: `checkPostmortem` reads a `RESOLVED: yes`, `A < H`, **and the region
...
| **O-12** | Specify how `W` reaches `deriveRoundWindow` as a **resolved value** — a decimal integer computed before the call — so that function and `windowEnd` keep their synchronous, seam-free contract (AC-1.2), with the async `_readFile` that reads the post-mortem living in the caller. **The answering-line append and its byte confirmation are `REQ-RCV-07` O-12's**, and the halt path's read-modify-write is O-5's. Also specify the **injected *validate* seam** O-10's contract leg asserts against — its parameter name, per this repo's `_`-prefixed injection convention (`_agent`, `_readFile`, …), its **arity 2** (the region, the branch listing) and a **result carrying the verdict and, when the verdict is false, AC-7.1 step 4's `{reason}` from the closed set of three** — not a bare boolean, which cannot carry the `{reason}` AC-1.5(4)'s false disposition requires the run report to render. **Who emits:** the **caller's run-report builder** emits the single `reset-region-corrupt: {reason}` notice (S-16, catalogue §2); the seam **selects** the reason and never holds a report sink — that selection is `REQ-RCV-07` AC-7.1 step 4's, so the two statements agree. The result is single-valued and total exactly as AC-1.5(4) requires (DC-01). This is the **target signature**, so the contract leg O-10 asserts today survives row 18 rather than breaking at it; it is a constraint on `REQ-RCV-07`'s implementation and is **carried to that REQ's O-12 in this revision** (§10's paired-edge rule covers X-06/R-14 ↔ X-07/R-16 only, so this edge is carried deliberately). At this ship the seam is accepted by the interim composition and not called; `REQ-RCV-07` AC-7.1 supplies its implementation. Specify too that origin resolution takes the greatest **well-formed** `WINDOW-START:` value (§6), falling back to `1` when none is, so `W` is a decimal integer on every path. | TSPEC |
