...
4. **the reset is anchored and consumed, in the POSTMORTEM, by the loop.** The **reset region** is read as two counts: `H`, the number of `HALT-REASON:` lines, and `A`, the number of
   ...
   **not** hold here: the file is not byte-unchanged (a partial append may have landed), and the
   ratchet's *same reason next entry* has no reason to be stable. **Three** outcomes, all safe: the
   line landed whole ⇒ `A = H` and the next entry grants nothing; nothing landed ⇒ the next entry
   re-observes the clearance and grants the one window paid for, never two; the write **tore** (a
   truncated line, a value without its newline) ⇒ the next entry reads a step-2 value fault ⇒ S-16
   `invalid-window-start`/`invalid-window-resumed` ⇒ the *corrupt-region* refusal, its texts, and the
