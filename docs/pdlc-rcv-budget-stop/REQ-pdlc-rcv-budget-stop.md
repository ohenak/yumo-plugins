...
**On every halt in scope, including one that finds an existing post-mortem.** The quantifier is *"the loop halts on the budget"*, with no exception for a re-halt, and the re-halt is the case this criterion exists for: a second entry into an exhausted window is by construction a zero-round halt, so the operator who reads `rounds run 3` there — the *previous* halt's number, left as that halt wrote it — reads exactly the conflation this criterion prohibits, on the commonest new case. **So the section is refreshed by the halt, and AC-1.4 carves it out of its byte-unchanged rule.** Refreshing it is not re-authoring: it is a fixed two-integer render the loop computes from the constant and its own round count, with no agent and no `## Recommendation` involved, which is why it costs nothing AC-1.4's cost argument protects.
