...
**v2.6** (operator-directed) removed all implementation contracts to TSPEC altitude, no id, AC, threshold or semantic changed; **v2.5** and **v2.4** (rounds 10, 9) relocated O-10's fixtures to split §5.4 and deferred the consistency check, not the word; **v1.6 … v1.2** addressed rounds 5 … 1. **On size:** the hard ceiling binds, so every revision since v2.5 pays for its additions by compressing prose or relocating to a shared file — v2.8 paid by relocating three rationales to split **§5.4**, **§5.5** and **§5.6**, and by compressing §§1–10.
...
**in force at this ship**, not deferred with the consistency checks (X-06).
