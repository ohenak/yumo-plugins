...
**refusal** string of its own. It mints exactly **one** operator string, added at v2.7: AC-1.3's
Iterations render, declared as a baseline §3 row and repeated below. Two grammars are restated below because §4.1 and AC-1.5(4) depend on
their being **in force at this ship** rather than deferred with the consistency checks (X-06).

| Row | Owned / read | Default and note |
|---|---|---|
| `MAX_REVIEW_ROUNDS` | owned | **3** (was 5); baseline §3 for the derivation, §3.1 for this REQ's note. **Exactly one declaration repo-wide** (AC-1.2), test code included; the reachability mechanism and the width-5 assertion inventory are O-13's |
| **`Iterations (budget {MAX_REVIEW_ROUNDS}, rounds run {k})`** | owned | **That exact render**, two decimal integers ≥ 0, in the post-mortem's Iterations section. **New at v2.7** and stated here because AC-1.3 depends on it being a fixed string rather than a shape: it **replaces** the baseline literal, whose single number and *"limit reached"* phrase are false on the zero-round halt (M-1c). `{k}` is the rounds the halting **entry** dispatched — `0` on AC-1.5(1)'s halt. Baseline §3 carries the row and §3.1 the note; O-14 produces it, O-10 asserts it |
...
| **NB-5** | Restoring a reset region that `harvest-learnings` has deleted, or making the window survive Phase H. | The post-mortem is the region's durable home and harvest deletes it once `LEARNINGS-{feature}.md` exists (§4.1). Within a feature this is benign — Phase H runs after every review phase — and a post-harvest `forcePhases` re-entry finding no region gets `W = 1`, `H = A = 0`: the same default as a feature that never halted, granting nothing the operator did not ask for by forcing. A finding that a post-harvest re-entry has no window history is **correct and known**; a durable-across-harvest home would be a new artifact and therefore a new REQ. |
...
| **O-12** | Specify **how `W` is resolved before the round window is computed**, and how the *region validates* predicate is supplied to the clearance gate; the halt path's region maintenance is O-5's. **The seam's own contract is fixed in exactly one place, and it is `REQ-RCV-07` O-12** — one-directional, not reciprocal: this REQ's TSPEC adopts whatever that obligation fixes and states none of it, and `REQ-RCV-07` O-12 is corrected in this same revision to stop citing this one (the two previously pointed at each other, so no document fixed it). What this REQ owes the seam is the **interim composition's observable**: the predicate is reachable from the clearance gate and is consulted **zero times** while the conjunct is unwired (X-06) — the 0-call leg O-10 owns. That is a fact about behaviour, not about a signature, which is why it can be stated here at all. This REQ fixes only the observables: the gate is **fail-closed** — a region that does not validate refuses the phase and consumes nothing; the run report carries **exactly one** `reset-region-corrupt: {reason}` notice, `{reason}` drawn from `REQ-RCV-07`'s closed set of three; and `W` falls back to **1** whenever no well-formed `WINDOW-START:` value exists (§6, §4.1). | TSPEC |
...
| **O-10** | Properties and tests for this requirement, at the level of observable outcomes. **The region's maintenance (AC-1.4):** the first halt of a phase creating `## Reset Region` with exactly one `HALT-REASON:` line, and the operator's first clearance then granting a window; a later halt preserving the region and stripping the spent marker, so the phase reads as unresolved again; a **fenced** `RESOLVED: yes` surviving the strip while an unfenced one is removed; region lines **appended, not prepended**; a **Phase CR halt creating no `## Reset Region`** (N-7) — non-vacuous because the untyped phases reach the same halt path and do write a post-mortem (**M-7f**), so the leg asserts a file that exists lacks the section, not that no file exists. **The window (AC-1.1–AC-1.3, AC-1.5(1)–(3)):** the budget and the reported iterations asserted **over the constant, never a literal** (the `rounds 4..6 of 3` shape is illustrative), with rounds-run `0` on the zero-round halt; **row C** asserted cell by cell with **zero reviewer dispatches asserted positively** — a dispatch count of `0` alongside the absence of any new cross-review file, since a test double that writes nothing satisfies absence either way; a **forced** phase on an exhausted document halting rather than re-reviewing, and a second force changing nothing. **The gate (AC-1.5(4)–(5)):** one clearance granting **exactly one** window (`A < H` grants, `A = H` grants nothing); an S-11 clearance writing `WINDOW-RESUMED:`, leaving `W` unchanged, with a subsequent convergence halt then **not** auto-cleared. **The validation-dependent legs are deferred to `REQ-RCV-07` O-10** (X-06): the refusal, its renders and *a region that does not validate consuming no clearance* belong to the REQ under which the conjunct is in force and can be exercised on a production path.

**Four legs this REQ keeps rather than defers, because nothing else can carry them.** The fixtures are stated once, for both ends of the split, at `pdlc-rcv-split.md` **§5.4**; what follows is which legs are owed here and why.

1. **The 0-call contract leg.** On split §5.4's **leg 1** fixture — a well-formed non-empty region that *grants* — the interim composition consults the *region validates* predicate **exactly 0 times**, asserted as a count, not as an absence. Leg 1 is the fixture that carries it because it is the only interim fixture defeating both decidable conjuncts, hence the only one a **wired** implementation would answer differently. **This is the interim ship's only falsifiable oracle for X-06**: without it, *"the conjunct is deliberately not consulted"* and *"the conjunct is wired with an ad-hoc interim procedure"* — the granting horn `pdlc-rcv-split.md` §5.1 rejects — pass every other leg identically. It is a **contract** leg, replaced (not deleted) when row 18 wires the call.
2. **Legs 1 and 2 of split §5.4 themselves** — the granting region and the no-region-at-all region — each with **≥ 1** reviewer dispatch asserted positively.
3. **The interim-only malformed-value leg** (split §5.4 **leg 3**): a malformed `WINDOW-START:` value still counts toward `A` (it is a line of that prefix) but contributes no origin, so `W` = **1**, never a non-numeric value reaching the window arithmetic. Decidable from §6's grammar alone, which is why it is this REQ's. It **inverts at row 18**, where AC-7.1 refuses that fixture with `invalid-window-start`; it is marked interim-only so that commit rewrites it rather than deleting an assertion it cannot explain.
4. **The derived start below `W`** (AC-1.5(2)): a region granting `W = 4` with every cross-review file for the document type removed, so `highest + 1 = 1` — the entry starts at **4**, not 1, dispatches a round, and writes no file that overwrites an existing one; and the control, `highest + 1 = 6` with `W = 4`, halting on the budget path.

**Two legs the revision of AC-1.3 and AC-1.4 adds.** The post-mortem's Iterations section asserted as an **equality on §6's declared render**, both integers, with `rounds run 0` on the zero-round halt — not a substring match, which any rendering satisfies. And the **re-halt leg**: a halt finding an existing post-mortem makes **0** authoring dispatches and leaves every byte outside the reset region unchanged — asserted as a byte comparison of the file with the region span excised, plus the dispatch count, since a body an agent happens to rewrite identically would satisfy neither alone.

**One property-based obligation, not only the enumerated points.** The window-origin and counts resolution is a parser over an ordered line sequence, and the interleaving of `HALT-REASON:`, `WINDOW-START:` and `WINDOW-RESUMED:` lines is exactly what the enumerated fixtures cannot cover. Over **generated** region line sequences (arbitrary order, arbitrary well-formed and malformed values, arbitrary `RESOLVED:` state) assert the invariants this REQ already states: `W` is always a decimal integer ≥ 1; `W` never exceeds the greatest well-formed `WINDOW-START:` value present; no absent or malformed value ever **widens** the window; and at most one answering line is written per entry. The fail-closed direction is the property — a generator that produces a region granting a window wider than the budget must fail.

**DC-03 routing, and the ledger's lifecycle.** Every assertion above whose failure is the **only** signal of its defect — the 0-call count, the budget and `iterations` asserted over the constant, one clearance granting exactly one window, the zero-dispatch conjunct on row C, the re-halt byte comparison — is load-bearing under `DOMAIN-CONSTRAINTS.md` DC-03 and must pass the falsification cycle, recorded in `docs/pdlc-rcv-budget-stop/FALSIFICATION-LEDGER.md`: the mutation named in writing before the run, the red test ids recorded, the revert re-verified green, and any assertion with no nameable mutation filed as a **Residual** with its reason rather than counted. **Lifecycle (DC-10):** the ledger is a feature artifact, not a process artifact — `harvest-learnings` deletes `CROSS-REVIEW-*`, `CODE_REVIEW-*` and `POSTMORTEM-*` and **not** this file, which stays in `docs/pdlc-rcv-budget-stop/` as the durable record of which assertions were proven to fail. | PROPERTIES |
| **O-13** | Own the **budget-width change's blast radius**, which AC-1.2 states as an outcome and does not resolve. Two parts, both due before implementation starts rather than discovered inside it. (a) **Decide how test code obtains the effective budget** so that AC-1.2's "exactly one declaration repo-wide" and AC-1.3's "asserted over the constant, never the literal" are simultaneously satisfiable — today they are not: the constant is deliberately unexported and the suite therefore keeps its own copy of the value, which is the second declaration AC-1.2 forbids and which would silently disagree with the module the moment 5 becomes 3. Exporting it, or any other single-source mechanism, is TSPEC's call; keeping two declarations is not, because it is the defect. (b) **Enumerate, before the change, every existing assertion that encodes today's width of five** — the round-window derivation suite, the pacing-wrapper suite (including an acceptance-test *title* that names the width) and the review-loop suite all carry them — and state for each whether it is re-expressed over the constant or is a genuine width-5 expectation that must change value. The enumeration is the deliverable: an implementer who discovers these mid-PLAN re-decides (a) under time pressure. | TSPEC |
| **O-14** | Produce AC-1.3's Iterations render — **both** quantities, at §6's declared string. The baseline section is a fixed literal carrying one number and a *"limit reached"* phrase (M-1c) that is false on the zero-round halt, so this is a replacement, and the rounds this entry ran must be threaded to wherever that section is produced. Also specify the **no-re-author** path AC-1.4 requires: on a halt finding an existing post-mortem the Iterations section of the existing file is what the operator reads, so state whether it is updated in place or left as the prior halt wrote it — this REQ requires only that the file outside the reset region is byte-unchanged, which fixes the answer to *left as written* unless FSPEC states otherwise and amends that clause. | FSPEC → implementation |
