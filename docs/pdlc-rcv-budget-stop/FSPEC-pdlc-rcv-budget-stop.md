...
| **E-1b** | **Migration, the pre-feature post-mortem case.** A post-mortem that predates this feature survives on the branch — `## Iterations (5 — limit reached)`, a readable unfenced `RESOLVED: yes`, and **no `## Reset Region`**. (`docs/pdlc-rcv-budget-stop/POSTMORTEM-R-pdlc-rcv-budget-stop.md` is exactly this shape at HEAD, so this is the ordinary migration path, not a corner) | the region reads `H = A = 0` (B-REG-1), so the gate's `A < H` conjunct is false and **B-CLR-4 grants nothing**: the entry proceeds to §4, admits zero rounds, takes the budget halt, creates the region with `H = 1` and **strips the marker**. The operator's *second* `RESOLVED: yes` then meets `A = 0 < H = 1` and grants the window. **Accepted cost, stated: two clearances for the first window, once per pre-feature post-mortem, per document.** The pre-feature marker is **not** honoured as a clearance — honouring it would mean minting an `H` the region never recorded, i.e. granting a window with no recorded halt behind it, which is precisely what BR-10 and BR-12 forbid. This is the same shape as **E-14** and is accepted for the same reason. It is bounded (one extra clearance, never recurring on that document), operator-visible (the first entry emits row C naming S-4, and §8.1's render shows `rounds run 0`), and needs **no migration script** | B-REG-1, B-CLR-4 |
...
| **E-14b** | A **creating** halt whose **clause-3** write is unconfirmed | the same two-clearance cost as E-14, reached by the other write, plus one surprise: clause 3 runs **after** the authoring dispatch, so a **fully authored** post-mortem is left on the branch with **no `## Reset Region`**, `H = 0` and no `RESOLVED:` marker. The next entry meets the shipped step-G refusal, so the operator clears a halt the accounting never recorded and then clears the re-halt that records it. **Accepted**, bounded, and fail-closed for the same reason as E-14 | B-HALT-4 |
...
**The dispatch outcome of a granted window is fixed for every row in this table.** In each row that
grants a window, **an approval is reached before the window's last round** — the phase completes
inside the window — unless the row states otherwise. **No row in this table states otherwise.**

*Wording, because the root is overloaded:* this clause deliberately does **not** say the rounds
"converge". §6.1 uses **convergence halt** for the `fixed-point:` / `no-revision:` non-convergence
stop — a **halt**, and the opposite outcome to the one fixed here. Where a row says *convergence
halt* it means §6.1's halt, and such a halt is **not** an exception to this clause: no row grants a
window and then reaches one.

Without this clause every clearance row's negative conjuncts (*no second
`HALT-REASON:` is appended*, *the marker is not stripped*, *nothing is appended on either entry*)
would be true or false depending on a fact no Given pins: a granted window whose rounds run to
exhaustion budget-halts at its end, appending exactly that line and stripping exactly that marker, so
the conjunct would fail on a **correct** implementation. The clause exists so those conjuncts
discriminate the branch they are written for — AT-CLR-02a's pair is the whole evidence that `D > E`
resolves to a fresh window rather than a resume — instead of false-redding for an unrelated reason.
It fixes the **outcome**, not the fixture: which verdict sequence produces convergence is PROPERTIES'
(O-10).
...
| **AT-CLR-02** | B-CLR-2 | `W = 1`, `H = 1`, `A = 0`, a readable `RESOLVED: yes`, last `HALT-REASON:` beginning `no-revision:`, **highest existing round 1** — so `D = 2 ≤ E = 3`, the window still has rounds | the phase is entered | **exactly one** `WINDOW-RESUMED: 1` appended; `W` **unchanged at 1**; rounds already spent stay spent; **≥ 1** dispatch, at round **2**. (The conjunct *"a later convergence halt is not auto-cleared"* is **deliberately dropped**: §11.4's clause fixes an approval inside the granted window, so that antecedent never arises here and the conjunct could not fail. *One clearance grants exactly one window* is carried by AT-CLR-04 and AT-CLR-08.) **[target state — no path emits S-11 at this ship]** |
...
| **AT-CLR-04** | B-CLR-4 | `A = H = 1`, a readable `RESOLVED: yes`, **`W = 1` and highest existing round 1 — so the window is open** (`D = 2 ≤ E = 3`) and neither entry budget-halts. **Entry 2's precondition is stated, because the row asserts across two entries:** entry 1's dispatched rounds reach an approval at round **2** (§11.4's clause), which completes the phase, so entry 2 is a **`forcePhases` re-entry** — the only route back into a completed phase — with the region as entry 1 left it | the phase is entered, and re-entered under `forcePhases` | **nothing** is appended and **nothing** granted on either entry; `W` is unchanged at **1**; both entries reach the gate, observe `A = H`, and proceed to §4 dispatching normally — one clearance grants **exactly one** window. The arithmetic that makes entry 2 realisable rather than constructed: entry 1's approval leaves highest existing round **2**, so `D = 3 ≤ E = 3` and entry 2 still has a round to dispatch, while `A = H = 1` still closes the gate and `forcePhases` overrides the recorded **approval** only — it grants no window and moves no count (§4.3, **B-WIN-6**), and because `D ≤ E` here the forced entry is admitted rounds rather than taking B-WIN-6's zero-round halt. Every conjunct the row exists for survives. (The window must be pinned open: on an exhausted window the first entry budget-halts, appends a `HALT-REASON:` and strips the marker, so the second entry meets B-CLR-5's step-G refusal and "nothing appended" is false for reasons that are not this branch's; with the window open, §11.4's convergence clause is what keeps entry 1's dispatched rounds from budget-halting and reintroducing the same failure one round later). The Given is a **constructed** state, not one driven through production: `WINDOW-START: 1` is unreachable at this ship — a budget halt needs `D ≥ 4`, so every granting line carries at least 4 — and `WINDOW-RESUMED: 1` is B-CLR-2, which no path emits until `pdlc-rcv-fixed-point-stop` lands. The row is evidence about the **gate**, not about production reachability; B-CLR-4 must hold over hand-edited regions too (E-13) |
