...
grants a window, **the granted window's dispatched rounds converge before the window's last round**
— an approval is reached inside the window — unless the row states otherwise. **No row in this table
states otherwise.** Without this clause every clearance row's negative conjuncts (*no second
