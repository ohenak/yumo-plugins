...
- **The blocker was closed by removing the state, not by dispositioning it.** I offered two options and said either would clear my side. The author took the stronger one and took it properly: clauses 1 and 2 become **one update of one file**, stated as an unreachability claim over the file's content — "no reachable state in which this halt's `HALT-REASON:` line is present and an unfenced `RESOLVED:` line survives" — rather than as a mechanism. That is the right altitude for a REQ (an outcome over the artifact, not a write protocol), and it is falsifiable: a fixture that reaches the state falsifies it. Closing a fail-open by making it unreachable is strictly better than giving it a refusal render.
- **The count tests in catalogue §4 act 1 are the best thing in this revision, and they exist *because* of the one-update rule.** The previous act 1 told the operator to "delete the trailing unconfirmed region line" — but *unconfirmed* is not an observable property of a file, so the instruction was undecidable at the point of use. It is now a count the operator takes from the region itself: `H > A` for `halt line`, `A = H` for `answering line`, and **act 1 does not apply** for `iterations section`. §5.8 states exactly what makes the tests total — "a landed-but-unconfirmed write moves exactly one of `H`/`A` by one, and no reachable state moves both" — which is a consequence of the one-update rule, so the rule pays for itself twice. I checked the arithmetic on the creating halt, the ordinary re-halt and the skipped-act-1 residue; the tests hold on all three.
- **The `{which}` discriminator was carried to every consumer in the same revision, and I checked each end at HEAD rather than the claim.** Catalogue §4's ❌ cell (three literals, each attributed to its owning AC), catalogue §3's variant paragraph, `REQ-RCV-07` AC-7.6's *Given* and its two-column cell table, AC-7.5's *one exception* paragraph (now scoped "on the `answering line` source … and only when `A = H`"), the REQ's §4 ownership line, *the ordering and its report*, and split §5.4 leg (iii) all read the same three literals. The §4 cell also carries its own **dated revision history** — generalised, then re-discriminated, with the reason for each — which is how a shared constraint file stays reviewable.
- **Split §5.4 leg (iii) was widened into two fixtures that a single leg could not discriminate, and the leg says why.** "A single leg asserting the generalised render passes either way and falsifies nothing, which is why the ❌ text is the discriminating conjunct here" — and fixture A pins *this entry's Iterations render* as expected content precisely because clause 3 has already landed. That is a test-design consequence of the clause order being stated, and it was derived rather than asserted.
- **My Q-02 was answered by widening the risk register, not by arguing it away.** R-14 went from two residuals to three and named the new one against this ship rather than a future one ("**from this ship**, so do AC-1.4 clause 1's own `HALT-REASON:` appends"). A revision that adds a residual to its own risk table in response to a reviewer's aside is doing the thing risk tables are for.
- **The compressions I spot-checked were honest.** R-14's dropped pickup chain is at §3.1; O-13's dropped rationale is at split §5.7's *Prose that states the number* bullet; §5.8 carries a relocation stamp with the date, round and a "nothing changed meaning in the move" claim I verified against my own v3 text, which it restates faithfully. The one stale summary (F-04) is the exception, not the rule.
- **Blocking count reached zero on the round after the pre-commitment, without the scope creeping.** v1: 1 High + 4 Medium. v2: 1 High + 1 Medium. v3: 1 High. v4: none. Every finding of every round is closed or held by an explicit pre-commitment, and this round's three Lows are all consequences of this round's edit rather than newly discovered ground.
