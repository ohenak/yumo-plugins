...
| **TE F-10** | Fixed — T-07 exports the whole skip inventory (TSPEC §1.3 ∪ PROPERTIES §11.1) as frozen records plus a `REGISTERED_SKIPS` set. "Zero unexpected skips" is now a comparator, not an eyeball pass. **Superseded in Phase DOD (DOD-02):** the comparator runs run-wide from jest `globalTeardown`, not from T-01, and checks structure/agreement rather than closure — see §9 and T-07's row. |
...
## 0c. Revision note — v2.2 (2026-07-29) — Phase CR remediation ledger

Addresses `CROSS-REVIEW-se-codebase-v1.md` (round 1, 19 findings, "Needs revision", remediated in
`62c9bf8`) and `CROSS-REVIEW-se-codebase-v2.md` (round 2, verdict **approved**, 0H / 3M / 4L).
Upstream REQ/FSPEC remain **untouched**; TSPEC and this PLAN carry corrections only.

**The rule this section exists to enforce (SE G-07, round 2).** Round 1's F-01 fix was recorded as
"fixed, suite green" — and round 2 measured that it was *green by accident*: reverting the fix
(`trap 'exit 0' ERR EXIT` back to `ERR`) leaves all eight `driftC1Absent.test.js` tests green, so
nothing in the suite was actually binding the fix. A green suite after a fix proves only that the
fix did not break anything; it does not prove the fix is load-bearing. So, from v2.2 on:

> **Every remediation row that claims a *detector* must record, in the row itself: the mutation
> applied (precisely enough to re-apply), and the test ids that went red under it.** A row that
> cannot name a mutation is not a detector row — it is filed below as documentation-only, with its
> supporting measurement cited instead. This is §3.1's falsification discipline applied to
> remediation rather than to properties, and it is the same non-vacuity rule the project already
> runs on: a green result must be shown to be a real zero, not a broken-oracle zero.

### Round 1 — detector rows (red-first evidence recorded)

| ID | Mutation applied | Test ids that went red | Revert |
|---|---|---|---|
| **SE F-06** (post-run re-seat of `retiredPresent[].supersedingState`) | Neutralise the step-7 re-seat loop in `pdlc/hooks/scripts/sync-workflows.sh` (`:561`) so the array keeps its post-copy value — applied as an in-place self-assignment no-op, i.e. the loop body assigns `_PDLC_C3_RETIRED_STATE[$i]` its own current value instead of `${PDLC_STATE[$j]}`. Deliberately a *different* mutation from the one the implementing agent used (a loop-bound change), so the evidence shows the suite detects the **behaviour**, not one particular edit. | 4, all in `driftSync.test.js` → `PROP-MTM-04 (sync half)`: `conjunct 1 — R "stale" + artifact-copy-corrupt …`, `conjunct 1 — R "local-edit" + --force + artifact-copy-corrupt …`, `conjunct 3, sub-case (a) …`, `conjunct 3, sub-case (b) …`. The five agreement cases stayed green, which is correct — they are the compositions where the two passes cannot diverge. | Reverted; suite green (53 tests) and `shasum` back to `1da2fb95cc253d877ee656f186b3b593d394fb26`. |

### Round 2 — detector rows (red-first evidence recorded)

| ID | Mutation applied | Test ids that went red | Revert |
|---|---|---|---|
| **SE G-02** (the `EXIT` arm of `check-workflow-drift.sh:43`'s `trap 'exit 0' ERR EXIT` is load-bearing for AC-2.4) | The pre-existing eight `driftC1Absent` tests could not bind the `EXIT` arm at all: the availability gate `exit 0`s *before* any code that can reach a `set -u` fatal, so `ERR` alone satisfied them. The new detector removes that shielding — it installs a **complete-but-broken** C1 in which all 16 gated names are defined (so the gate passes) but `pdlc_load_manifest() { unset PDLC_ROWS_ID; }`, driving the hook into `_pdlc_n_rows=${#PDLC_ROWS_ID[@]}` and an unbound-variable fatal. Under `set -u` that fatal fires `EXIT`, never `ERR`. Two independent mutations were then run against it: **(1)** the implementing agent's — `ERR EXIT` → `ERR`; **(2)** an orchestrator re-break using a *different* edit — `ERR EXIT` → `ERR HUP`, which keeps a trap installed and removes only the `EXIT` arm, so it isolates that arm rather than merely "a trap exists". | Both mutations produced the **same single red**: `driftC1Absent.test.js` → `G-02 / AC-2.4 — the EXIT trap arm keeps the hook at exit 0 through a `set -u` fatal` › `exits 0 when a complete-but-broken C1 makes the hook die of an unbound variable`, failing on `expect(run.status).toBe(0)` with `Expected: 0 / Received: 1`. The other 8 stayed green under both. | Both reverted; suite green (9/9) after each, and `shasum` back to `e1d72c5f560ba3841985f0fbdaa4abb439313443` with the execute bit intact (`-rwxr-xr-x`). |
| **SE G-01** (`packagingViolations` is **total** over arbitrary JSON — it must never throw, whatever bytes sit at the manifest path) | Two layers. **(1)** The implementing agent wrote the new cases against unmodified production first (red: 25 failed / 4 passed of 57), then re-broke each guard individually to show every one is load-bearing. That per-guard sweep found a genuinely **unfalsifiable** line: the `try/catch` backstop had *no* detector — re-breaking it alone produced 0 failures. Rather than leave an unprovable line, the agent found a real input that reaches it (a well-formed manifest naming a packaged file that exists but is mode `000`) and added a test gated on `itOrSkip(…, "uid-nonroot", …)`; re-breaking the backstop alone now gives 1 failure. **(2)** An orchestrator fuzz, written independently of the agent's case table, drives 22 manifests through the oracle — `null`, `true`, `42`, `[]`, a bare string, empty, garbage, non-array `rows`/`entries`, null row/entry elements, non-array `retires`, a `__proto__` key, a BOM-prefixed body, whitespace-only, unicode escapes incl. a lone NUL, `{"rows":[[[[1]]]]}`, a **directory** at the manifest path, and an absent manifest — asserting only that an array comes back. | The fuzz is non-vacuous, measured against `HEAD`'s pre-fix module run outside the repo: **6 of 22 threw** — `null` (`Cannot read properties of null (reading 'entries')`), `nested-null-row` and `pluginPath-object` and `deep-array-nest` (`The "path" argument must be of type string`), `entries-null-elem`, and `retired-number` (`number 5 is not iterable`). Post-fix: **22/22 return an array, 0 throw.** `deep-array-nest` was *not* in the agent's enumerated cases and is fixed anyway, so the guards generalise beyond the table that motivated them. | Nothing to revert — the fuzz runs from a copy outside the working tree, so the repo was never mutated. `build-runtime.mjs --check` exits 0: `lib/document-oracles.mjs` is not inlined into the bundles, so this change owes no `dist/` rebuild. |
| **SE G-05** (the gate's last-position probe, and C1's last definition) | Two assertions, two mutations each named before running. **(a) The fixture.** The implementing agent deleted `pdlc_msg_w7` from the hook's probe list (`check-workflow-drift.sh:80`). The orchestrator re-broke it differently — **duplicating `pdlc_msg_w6` over `pdlc_msg_w7`**, keeping the list 16 names long so only the final probe changes, which isolates "the probe reaches position 16" from "the list got shorter". **(b) The last-definition assertion.** `lib/pdlc-drift.sh` is read-only to the agent, so it appended `pdlc_zzz_probe() { :; }` to a copy outside the repo and temporarily repointed the `readFileSync`. | **(a)** Both mutations red the same single case: `F-01 / AC-2.4 … › exits 0 and says something useful — C1 almost complete (only pdlc_msg_w7 missing)` — the agent's with `expect(received).not.toBeNull() — Received: null` (gate silently passes, reporting nothing missing), the orchestrator's failing the same case with 13 others green. **(b)** `G-05 … › the last \`^pdlc_...()\`-shaped definition in lib/pdlc-drift.sh is pdlc_msg_w7` red with `Expected: "pdlc_msg_w7()" / Received: "pdlc_zzz_probe()"`. | All reverted. `check-workflow-drift.sh` back to `523c8a93535b094edd3c55abefcf14de5803987b` with the execute bit intact; `lib/pdlc-drift.sh` untouched throughout at `604199b7758da8a4e09356ec65ad8442a4387e3f` (the value the cross-review recorded); no `/tmp` path left behind in the suite; 14/14 green. Independently re-measured at the shell: the last `^pdlc_[a-z0-9_]*()` match in C1 really is `pdlc_msg_w7`. |
| **SE G-03** (message catalogue) | Ran the new catalogue assertions against the **unmodified** shells before touching them — i.e. the assertions were written first and allowed to fail. | 8 failed / 1 passed of 9 in `driftC1Absent.test.js`, from two distinct real causes: (a) `expectRemediationClass: class "pluginUpdate" requires text to name update the plugin — not found in "Reinstall or update the pdlc plugin; …"`, fixed by the rewording; (b) `Expected: "pdlc_load_manifest" / Received: "pdlc_probe_hash_tool"` on the corrupt-C1 cases, which was a **bug in the assertion, not the code** — that fixture defines `pdlc_load_manifest`. Fixing it by parameterising the expected name as a third `C1_CASES` element turned the defect into an extra guarantee: the gates' **probe order** is now pinned. | Green after: 9/9. |

### Round 1 — documentation-only rows (no detector; measurement cited)

These correct prose, not behaviour, so there is no mutation to name. Each is backed by a measurement
recorded at the edit site rather than by a test.

| ID | What was corrected | Measurement standing behind it |
|---|---|---|
| **SE F-03** | TSPEC §11.3 row 2 and `driftBackups.test.js`'s describe claimed a locale-sensitivity the code cannot exhibit. Corrected to a scope statement. The reviewer's proposed replacement was **rejected as unbuildable**: `pdlc_backup_parse`'s tail rule plus the constant-`$id` filter mean every string entering the sort varies only in digits, so the ordering is locale-invariant *by grammar* — a non-digit name fails the parser and never reaches `matched[]`. | C and `en_US.UTF-8` produce identical output for both locale-exposed constructs on Darwin, so no detector for the claimed hazard exists on this platform at all. The `LC_ALL=C` export is retained on cross-libc grounds, and is labelled at the site as reasoning, not measurement. |
| **SE F-15** | The consequence attached to the recursion claim was wrong at four sites (`.gitignore`, TSPEC §10.1 ×2, TSPEC §16, PLAN L-01). Round 2's G-04 found my first pass fixed *which patterns recurse* but kept the false consequence; corrected at all four. | Two independent counts, both measured (git 2.50.1): an added ignore pattern cannot change AT-23, because `listAllFiles()` walks with `readdirSync` and never consults git; and it cannot untrack the fixture, because ignore rules do not apply to already-tracked paths. The surviving justification is narrower and is now the one stated. |
| **SE F-18** | PLAN L-09's gate was documented as reading `"green"`. | Measured to be `{ skipped: S_NOTHING_STAGED }` at that point; L-09 now records the measured value and warns against tightening the comparison to `=== "green"`. |

### Round 2 (`CROSS-REVIEW-se-codebase-v2.md`) — disposition

| ID | Sev | Disposition |
|---|---|---|
| **G-01** | M | **Fixed, with a detector — see the G-01 row in the round-2 detector table below.** The oracle is now total over arbitrary JSON: every malformed shape becomes a `6.2(a)` violation whose `detail` names the specific defect, rather than a thrown `TypeError`. A `describeJsonShape` helper keeps the message honest about `typeof null === "object"`, and the read is split out of the `JSON.parse` try so an unreadable manifest says "could not be read" instead of the misleading "not valid JSON". **The two oracles' divergent handling of "I cannot judge" is now documented where a future reader will trip over it** — a block comment between the §10.2 and §10.3 headers explains that both must refuse to throw, but that an *inapplicable question* skips while *malformed input that is itself the defect* becomes a violation, so the apparent inconsistency is not "fixed" by mistake. `pdlc/RELEASE-CHECKLIST.md` §1's "present but unreadable" paragraph was corrected — it had enumerated two F-05 cases as if they were the complete set. |
| **G-02** | M | **Fixed, with a detector — see the G-02 row in the detector table below.** |
| **G-03** | M | **Fixed.** The two C1-availability gate messages are now catalogued as **N-9** (hook) and **N-10** (sync) rather than matched by ad-hoc `/^pdlc: /m` greps. FSPEC §8.3 gains both rows plus the rationale for `N-` over a §8.2 `W-`: §8.2 is an ordered, exhaustive warning taxonomy over a baseline status and a set of classified rows, and a gate-stopped run produced *neither*, so there is no state for a §8.2 warning to describe and no position in its ordering — and admitting them would drag them into distinctness sets S1/S2/S3, which are defined over row-state and baseline-reason renderings. Severity therefore lives in the exit code (hook `0` per AC-2.4, sync `3` per FSPEC §5.8), not the message class. Both gate messages were reworded from "update the **pdlc** plugin" to "update the **plugin**" so they satisfy `CLASSES.pluginUpdate`'s literal `"update the plugin"` matcher; the `pdlc:` line prefix already supplies the product context. `splitStderrLines` was promoted to an export because `driftC1Absent.test.js` spawns a *relocated* copy of an entrypoint and so cannot use `runScript`'s `RunResult`. **TSPEC §7.2 was updated in the same round** to carry the N-9/N-10 rows verbatim, restoring the invariant `driftHarness.js`'s `MESSAGES` docstring asserts ("verbatim from TSPEC §7.2"); the two rows are now byte-identical in both files, checked with `diff`. |
| **G-04** | L | **Fixed** — see the SE F-15 row above; corrected at all four sites, and `coveredViolations(process.cwd())` re-measured at 0 afterwards, so the prose edits introduced no pattern violation. |
| **G-05** | L | **Fixed, with detectors — see the G-05 row in the round-2 detector table below.** A third `C1_CASES` row, `C1 almost complete (only pdlc_msg_w7 missing)`, defines the union of both gates' probe lists minus `pdlc_msg_w7` alone, so it is carried through the existing hook and sync loops and the `expectC1UnavailableNotice` helper rather than adding an ad-hoc stderr grep. The last-definition claim is now an assertion over the real `lib/pdlc-drift.sh` text (with a `definitions.length > 0` guard so a regex that matches nothing cannot pass), not a comment. |
| **G-06** | L | **Fixed** — a comment at the `eval "declare -a ${_pdlc_hook_arr}=()"` site records that the safety property is that `_pdlc_hook_arr` iterates the **local literal list above, never data sourced from a manifest, env var, or other input** — that invariant, not the `eval`, is what must never change — and notes bash 3.2's lack of namerefs as the reason the `eval` is there at all. Comment only; the diff against the pre-G-06 file is exactly four added lines, verified with `diff`. |
...
| T-13 | `[Fake first]` **C6 (3/3) — O-19(d) and the gate wiring.** Export `readDriftStateSafely(readFileFn, path)` = `try { return await readFileFn(path); } catch { return null; }`. Wire the gate inside `main` **before** the existing `QUEUE.md` read at `orchestrate-queue.js:504…523`, so a blocked drift state costs no queue work; return `buildQueueReport({outcome:"blocked", …})` on `blocked`. **The injected read is `await`ed at the call site** and the pre-existing unwrapped `await readFileFn(queuePath)` is **left alone** (FSPEC §6.1; residual R-6). Add **O-19(c)**'s code comment recording that the seam is LLM-mediated and that `runtime-adapter.js:85–96` propagates (no `try`/`catch`), so the wrapper is load-bearing — a comment with no assertable observable (R-7); it is a review item on the diff. Rebuild both bundles in the same commit. **Deviation (orchestrator-authorized, batch 6):** wiring the mandatory gate before every `main()` path broke 9 pre-feature `orchestrateQueue.test.js` fixtures (`makeFs()`, ~lines 307–316) that carried no drift-state entry, since the gate fail-closes to row-1 `blocked` on an absent read (FSPEC §6.2 row 1). The orchestrator authorized a fixture-only fix: `makeFs()` now injects a shape-valid, `resolved`/all-in-sync drift-state record at `.claude/workflows/.pdlc-drift-state.json` by default (overridable per-test), so those legacy tests keep exercising pipeline-selection behavior under the new precondition — no legacy assertion changed, no test deleted, `queueDriftGate.test.js` untouched. | `__tests__/queueDriftGate.test.js` | `pdlc/workflows/orchestrate-queue.js`, `pdlc/workflows/dist/**` | 6 | T-04, T-12 | ✅ |
...
| # | Task | Test File | Source File | Batch | Deps | Status |
|---|---|---|---|---|---|---|
| T-31 | **C1 layer 1 — seams and primitives.** The **idempotent-source guard** at the top of the file (`[[ -n ${PDLC_DRIFT_LIB_SOURCED:-} ]] && return 0; readonly PDLC_DRIFT_LIB_SOURCED=1`) **before** any `readonly` assignment (PROPERTIES §8.0, SE F-05); `export LC_ALL=C` (§2.5); the **`PDLC_FAULT_TOKENS` bash array** declared once here, adjacent to `pdlc_fault_active`, in TSPEC §5.2's table order, **sixteen** members, with **no** JS mirror and **no** generated side-artifact (PROPERTIES §8.0); `pdlc_fault_active <token> [scopeKey]` per §5.1/§5.1.1 (argument 1 always a bare literal at call sites; no trim, no case-folding; unrecognised spec ⇒ N-7 once with the whole spec text); `pdlc_trace <phase> <op> <rowId> <arg>` — tab-delimited, percent-encoded, **always exits 0** (`{ printf … >>"$PDLC_TRACE_FILE"; } 2>/dev/null \|\| true`); `pdlc_probe_json_tool`, `pdlc_json_read` (0/10/11/12), `pdlc_probe_hash_tool`, `pdlc_sha1`; and the backup grammar `pdlc_backup_format` / `pdlc_backup_parse` (fixed 24-byte tail) / `pdlc_prune_backups <dir> <knownIds…>` (**always 0**, TSPEC §11.1). | `__tests__/driftFault.test.js`, `driftOrdering.test.js`, `driftLadder.test.js`, `driftBackups.test.js` | `pdlc/hooks/scripts/lib/pdlc-drift.sh` | 6 | T-18, T-25, T-27, T-29, T-30 | **DISCOVERED BLOCKER (flagged for follow-up, not fixed here — out of T-31's file scope):** `__tests__/helpers/bin/lib-probe.sh`'s `SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"` line depends on an external `dirname` binary that is absent from `driftProbe.js`'s `PROBE_PATH_TOOLS` sandbox allow-list (`bash, git, python3, shasum, sha1sum, mv, rm, date, printf`); under that constructed PATH, `dirname` resolves to "command not found", `cd "$(dirname …)"` silently no-ops (empty command substitution), `SCRIPT_DIR`/`C1_PATH` resolve to garbage, and the tolerant `source "$C1_PATH" 2>/dev/null || true` fails silently — so **every** `lib-probe.sh`-driven case in `driftFault.test.js`'s layer-1 sourced-probe block and all of `driftBackups.test.js` reports `err\tunknown-function` regardless of C1's correctness (confirmed by direct reproduction: manually adding `dirname` to the sandbox PATH, or patching a scratch copy of `lib-probe.sh` to use `${BASH_SOURCE[0]%/*}` instead of `dirname`, both make the identical cases pass). This is a pre-existing T-39 test-helper defect, not a C1 defect — C1's `pdlc_*` functions are independently verified correct via `driftHelpers.test.js`'s 40/40 green (T-01 clause (d), a different driver, `backup-grammar.sh`, unaffected) and via manual `source`+invoke checks. Needs a follow-up task to add `dirname` to `PROBE_PATH_TOOLS` in `driftProbe.js`, or change `lib-probe.sh`'s `SCRIPT_DIR` line to `${BASH_SOURCE[0]%/*}` (bash-native, no external dependency). | ✅ |
...
`docs/completed/pdlc-workflow-distribution/FALSIFICATION-LEDGER-{task}.md`, one file per task, never a shared
...
| L-09 | **Landing verification, then the single landing commit.** Run the checklist against the *uncommitted* tree first: `node pdlc/workflows/build-runtime.mjs --check` exits 0 (bundles **and** manifest fresh); full `cd pdlc/workflows && npm test` green with **zero unexpected skips** — every skip printed carries a named unverified invariant; `coveredViolations(LIVE_ROOT)` is `[]`; `packagingViolations(LIVE_ROOT)` is `[]`; `advertisedVersionViolation(LIVE_ROOT)` is `"green"`; `indexMode` is `100755` for all five scripts and `100644` for `lib/pdlc-drift.sh`; the `covered-violations` fixture is tracked (`git ls-files --error-unmatch` over the **13** paths — SE F-13; the fixture is 7 expected-violation files plus **6** exempt entries, not 5). **Then commit — once — every landing change** (§1, §8's Phase-CR check). Post-commit, re-run: `advertisedVersionViolation(LIVE_ROOT) !== "red"` — **measured post-landing (SE F-18, Phase CR): the value is `{ skipped: S_NOTHING_STAGED }`, §10.3's branch (a), not the string `"green"`.** The weaker `!== "red"` is written that way precisely because the post-commit reading is a *skip*, not a pass: with `dist/` committed and clean, `git status --porcelain` reports no change beneath it, so there is nothing to advertise and the oracle declines to judge. Anyone tightening this row to `=== "green"` on the assumption that post-commit resembles L-05's pre-commit gate will turn a correct landing red. (Measured with 26 unrelated files dirty from Phase-CR remediation; all lie outside `pdlc/workflows/dist/`, so the `dist/`-scoped probe is unaffected and the reading is the clean-`dist/` one.) `git status --porcelain` shows no unintended untracked file under `.claude/workflows/` beyond the four now-ignored consumer copies, and the full suite is green. | *(whole suite)* | *(verification + the landing commit)* | 22 | L-05 | ✅ |
...
**Where it is recorded.** `docs/completed/pdlc-workflow-distribution/FALSIFICATION-LEDGER.md`, a tracked file.
...
### 6.2 The nine live `coveredViolations` files (planned 2026-07-28 as seven; re-measured at L-06 as nine — L-06's work list)
...
| 8 | `pdlc/workflows/build-runtime.mjs` | `.claude/workflows/orchestrate-{dev,queue}.js` (the `RETIRES_BY_ID` literals) |
| 9 | `pdlc/workflows/lib/document-oracles.mjs` | all five `COVERED_PATTERNS` literals, plus the doc-comment |

> **Rows 8–9 are self-matches, added by this feature (SE F-11, Phase CR).** The 2026-07-28
> measurement predates T-09, so the scanner did not yet exist to scan itself, and `build-runtime.mjs`
> did not yet carry retirement literals. Both files state the retired paths as *data* — the scanner's
> own patterns and the builder's `RETIRES_BY_ID` — so the scan matched its own source. **Resolved by
> fragment assembly, not by exemption:** each literal is split across a concatenation
> (`".claude/" + "workflows/"`, `"managed " + "manually"`) so it no longer appears contiguously in the
> file text. A fifth `EXEMPTIONS` entry was explicitly rejected — it would permanently blind the scan
> to two real code files. `EXEMPTIONS` remains the frozen four-member literal (TE F-10: widening it is
> itself a red test).

Everything under `docs/orchestrate-dev-workflow/` and `docs/completed/pdlc-workflow-distribution/` also matches
the raw patterns and is **exempt (ii)** (the directory contains `REQ-<X>.md`). P0-00 re-measures this
set; the re-measurement above is that clause firing as designed. The fixture stays at 7 — it is a
fixed corpus with its own expected count and does not track this live set.
...
`docs/completed/pdlc-review-loop-hardening/REQ-pdlc-review-loop-hardening.md` each reference
...
- [ ] `cd pdlc/workflows && npm test` is green, with capability skips checked **mechanically**, not by eye (TE F-10). Because jest gives every test *file* its own module registry and may run files in separate worker processes, a module-level `REGISTERED_SKIPS` set can only ever see its own file's skips — so an assertion over it inside `driftHelpers.test.js` would be vacuous. The check is therefore run-wide: `describeOrSkip`/`itOrSkip` append every skip they register to a run-scoped JSONL sink under the OS tmpdir (`__tests__/helpers/skipSink.js`, path published by jest `globalSetup`, one append-only line per record so concurrent workers are safe), and jest `globalTeardown` (`__tests__/helpers/skipSinkTeardown.js`) runs `validateSkipRecords()` over every worker's records against T-07's exported `SKIP_INVENTORY` (TSPEC §1.3 ∪ PROPERTIES §11.1), **failing the run** on any violation and naming the offending skip and the file that registered it. **Domain:** skips registered through `describeOrSkip`/`itOrSkip` — the capability-gated skips the inventory describes — *not* every skip jest reports (`it.skip` / `it.skip.each` written directly by a suite, e.g. `guardMatrix.test.js`'s rows, never reach the helpers and are not checked). **Enforced:** (C1) every registered skip has a non-empty name, a known capability, and a non-empty invariant list of non-empty strings; (C2) a skip whose name matches a `SKIP_INVENTORY` entry must carry that entry's capability and invariant list verbatim; (C3) the inventory itself is well-formed — unique non-empty names, known capabilities, non-empty invariant lists — checked on every run and so never vacuous. **Not enforced:** closure ("every registered uid-0 skip is a member of the inventory"), because `SKIP_INVENTORY` is spec-derived (TSPEC §1.3's table plus PROPERTIES §11.1's two leaves) while several suites legitimately gate other fixtures on `uid-nonroot` with their own invariant strings; asserting closure would fail the whole run on a root runner for skips that are correct, and widening the inventory to close the gap would be a spec change. `driftHelpers.test.js` clause (a) pins `validateSkipRecords()` itself on synthetic records, exercising each clause in both directions.
- [ ] All **40** acceptance tests (AT-1…AT-36 incl. AT-8a/8b, AT-14b, AT-18a/18b, AT-32(a)/32(b)) exist as real `it()` blocks in the files TSPEC §14 names — and the claim is **derivable from §2's task table**: every AT appears in exactly the task(s) owning the file(s) §14 places it in. v1.0 had one orphan, AT-7's `driftClassify.test.js` half, which §14 places in **both** files; it is now T-22's (classification + `--check` exit 2) with the queue half T-38's (TE F-01). The four split ATs are split in the tasks that own them: AT-5 → T-24 (a, b) + T-38 (c); AT-31 → T-38 (a, b); AT-34 → T-22 (×3); AT-14b → T-25 (×2).
