...
| **Blocking assertion — a predicate, not a count (v1.3).** The await-discipline scan `RLH-31` will encode reports **every** call site of FSPEC AT-19's closed thirteen-name set, resolved through `main()`'s destructuring aliases and through wrapper names that inherit the obligation, across `orchestrate-dev.js` and `orchestrate-queue.js` source; and **every such site that is not lexically preceded by `await` is classified by one of TSPEC §8.5's three rulings.** *That total classification, together with the lower bound below, is the whole of the blocking assertion.*
**Lower bound, so the classification cannot pass vacuously (v1.4, TE round-4 `F-01`):** the scan must report **at least one** call site in `orchestrate-dev.js` and **at least one** in `orchestrate-queue.js`; a scan reporting none in either file fails this gate. Total classification over an **empty** reported set is vacuously true, so without this conjunct a scanner whose alias regex, mask or call-site match silently returned zero sites — none of which throws, they all return the empty set — would pass this row, §12.3's DoD row and `RLH-AT-19` alike, the outcome TSPEC §8.5 names *"the worst possible failure for this test"*. **A lower bound, never an equality:** one-per-file is chosen because neither module's `main()` can run its pipeline without calling at least one injected seam, so no legitimate future source change can drive either file to zero and make the bound spuriously red — only a scan that has gone blind reports none — and unlike the exact figure that drifted across four rounds, a bound of one cannot drift upward. A site matching no ruling fails the gate and is blocking work before batch 2 — never a quiet fourth exemption (§11.3 `H-h`). **A site that is correctly exempt never fails this gate, whatever the total is**: a future correct arrow wrapper adds one to the number and nothing else. The gate exists so `RLH-AT-19`'s **empty** permitted-red window (§7.3 row 1) rests on a *checked* premise at batch 1 | see the advisory row below for the observed set. Nothing in this row depends on its size |
...
#### Test-engineer round-3 findings

| Id | Sev | Disposition |
|---|---|---|
| **F-01** | High | **Fixed by deletion of three of the four copies** — same remedy as PM `N-01`, see that row and the derivation above. TE's own admission ("I filed 'three' in round 2 and was also wrong — the author verified my number rather than re-deriving from the ruling") is the diagnosis v1.3 acts on: §14.2's `F-01` row is annotated to say the v1.2 disposition was **overstated**, that "completed at v1.2" in the audit table was wrong with it, and that the root cause was verification-in-place-of-derivation. The set above was derived from the predicate; the five agree with both reviewers, which is evidence and not the reason. |
| **F-02** | Medium | **Fixed** — see PM `N-03`. Stated as the property the set stood for; `race`/`any` withdrawn; v1.6's item-1 changelog row annotated in place to record the withdrawal rather than rewritten. |
| **F-03** | Medium | **Fixed — the mechanism is decided, owned, dependency-declared, and has its own oracle.** §9.2 gains a third item deciding it: a **masked bracket-depth walk**, file-local and unexported in `runtimeBundle.test.js` — (i) mask strings, templates, regex literals and comments; (ii) build the scan set with fixed-point wrapper resolution (§14.3's step 1); (iii) walk backward from each call site maintaining a stack of unclosed `(`/`[`/`{` to decide which §8.5 ruling, if any, applies. **No parser and no new dependency**: `pdlc/workflows/package.json` declares `jest` alone, `@babel/parser` and `esprima` are transitive only, and `H-n` halts the PLAN on a new dependency. **Owner: `RLH-31`**, whose §4 row now carries the mechanism and `RLH-SCAN-01`. **The tradeoff is stated on evidence**: the input is known-shaped (two files, unindented top-level functions, no nested combinator calls at HEAD), and an undecidable shape degrades to **unclassified** — a loud failure — never to a silent pass. Because the mechanism decides the answer, it gets its own red test: `RLH-SCAN-01` asserts the walk classifies each of the five derived sites under the expected ruling and reports an injected non-awaited, non-exempt site as unclassified. |
| **F-04** | Medium | **Fixed — the third erosion is closed by making the skip count an equality and the assertion state a three-valued check.** §12.2 step 2 now reads `1038 passed / 1 failed / **skipped exactly 70**` — an equality, not "or better" — and adds: **every in-window assertion must be *present* and either red-as-expected or green; `RLH-*` and `RLH-AT-*` are all executed, none is skipped, and a skip is not a green.** A skipped assertion was previously neither red nor green, satisfied "1038/1/70 or better", and was never consulted against §7.3's window — so `it.skip` on a permitted-red row would have passed the batch gate in silence. §12.2's immunity paragraph now names **three** erosions rather than two, with the reachability argument (70 skips are the skip-sink harness's, all accounted for, so any 71st is new). §12.3 gains a matching DoD row: no `RLH-*` or `RLH-AT-*` assertion is skipped, total skips exactly 70. |
| **F-05** | Medium | **Kept, with a mechanical check — not deleted.** The brief's preference was deletion "if a check would be contrived", and it is not contrived here: the check is a grep row in §12.3, **the exact construction §12.3 already uses for `selectMode`**, so it reuses a shipped precedent rather than inventing an instrument. It reds at HEAD (zero occurrences), so it is a real red-then-green rather than a tautology. And it catches the one *innocent* violation these documents make likely: TSPEC §7.1 edit 3 anchors the `endIndex` arithmetic inside `reviewLoop` while §11.5 relocates it to the gate, so an implementer following the TSPEC literally and the PLAN literally writes it twice. §11.5 states why the invariant earns an oracle rather than deletion; `H-q` keeps the halt because the halt now has a falsifying assertion. |
| **F-06** | Low | **Fixed — in the owning section, which required a third TSPEC clause.** The "an *anonymous* arrow has no name to inherit the exemption, and the obligation is then inherited by nobody" answer had been deleted from §9.2 at v1.2 in favour of a citation, but was never added to the cited section, so between v1.2 and v1.3 the citation pointed at nothing. The answer now lives in **TSPEC §8.5's returned-promise row (v1.7)**, with the shipped shape named (`batch.map((task) => agentFn(…))` at `orchestrate-dev.js:1867`) and the scope pinned: the exemption is unconditional on naming, only the *inheritance* depends on it. §14.1's `Q-02` entry is annotated to record that the claim pointed at nothing for one version and is corrected at v1.3. **Disclosure: this is a third §8.5 clause, one beyond the brief's two** — taken because `F-06` names §8.5 as the owning section, and the only alternative (answering it in the PLAN) would reinstate exactly the restatement v1.2 deleted and `N-01`/`F-01` are about. It narrows nothing and widens nothing; it states where an existing exemption stops. |
