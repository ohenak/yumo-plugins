...
| Cross-Reviews | `CROSS-REVIEW-product-manager-PLAN-v1.md`, `CROSS-REVIEW-test-engineer-PLAN-v1.md`, `CROSS-REVIEW-product-manager-PLAN-v2.md`, `CROSS-REVIEW-test-engineer-PLAN-v2.md`, `CROSS-REVIEW-product-manager-PLAN-v3.md`, `CROSS-REVIEW-test-engineer-PLAN-v3.md` (active; harvested into `LEARNINGS-pdlc-advisory-wave-gate.md` after Phase H) |
...
| 1.12 | 2026-08-20 | **Cross-review round 12 (PM, TE).** Re-walked TSPEC §5.1's file table against this document's file-ownership manifest with `comm` before touching the raised items (PM v2 Q-01) — that walk is what produced the round's High, and it is now a standing step of the re-grounding protocol here. **PM F-01 (High):** `pdlc/workflows/__tests__/advisoryWaveGateMain.test.js` — the DC-07 production-path suite whose `expect(result.haltAdvisory).toEqual({rootCause, diagnosis, repairApplied, repairPaths})` is a four-key set-equality over the *same* halt-fields object A6-18's green step widens — was owned by no task, so A6-18's green step would have reddened it under a gate declaring the suite green. It enters A6-18's test-file cell, the file-ownership manifest and the single-writer-per-batch walk (batch 6 only), and the four-key → five-key widening (`snapshotRef`, `null` on the escalation path) is named in A6-18's red step, with `haltReason`'s containment assertion explicitly left alone as AT-05-3's surviving oracle. The Overview's test-file count moves twelve → **thirteen** and the fixture becomes the fourteenth path. **PM F-02:** `advisoryEscalationLog.test.js`'s `expect(failed.notices).toHaveLength(2)` is an *exact* count on a real-temp-repo run where the capture succeeds, so the overwrite notice makes it **3** — named in A6-18's red step with its reason, on the same discipline A6-05 applies to its four bare row-count sites, and added as a DoD leg. **PM F-03:** the un-skip halt arm's overwrite notice now has a named owner. TSPEC §4.5's un-skip row says the seam has already returned by then, so the push is emitted at the un-skip halt site through the same `advisoryNotice` sink and the same `renderSnapshotOverwriteNotice` helper — that site and `waveExecution.test.js` are **A6-21's**, so A6-21 owns the push and its assertion (with a no-A6-fired negative companion), A6-18 owns the seam-level arm, and AT-06-4's traceability row now carries both arms and both owners rather than one. **PM F-04:** the lineage header's TSPEC label reads **v1.15**, matching the pinned bytes (`sha256:1f6ea486…`, re-computed at HEAD) and TSPEC's changelog head; REQ v1.16, FSPEC v1.7 and the DECISIONS hash were re-checked and are correct. **TE F-01 (Low):** the DoD leg for A6-10 and the AT-05-1 traceability row now name the *conjuncts* that falsify a restore-over-ignored-path implementation — TSPEC §5.2 case 4's positive-presence assertion paired with case 3's absent untracked file, plus case 5's ordering conjunct — instead of only the outcome, so the leg cannot be ticked from a hash-map assertion that structurally cannot fail. TE Q-01 (TSPEC §5.6's AT-02-1 row still reading "set-equal") is upstream's and is routed as an erratum, not folded in here. **No batch, wave or dependency edge moved**; one file-ownership cell gained one file — re-verified by re-running `parsePlanTasks` → `computeWaves` → `validatePlanContract` over the edited table: 11 tasks, 7 waves, `{ok: true}`. |
| 1.13 | 2026-08-20 | **Cross-review round 13 (PM, TE).** Re-grounded on upstream HEAD before touching the raised items (DEC-ERR-03): all four lineage digests re-computed locally and unchanged (REQ `f97f4f66…`, FSPEC `d602c440…`, TSPEC `1f6ea486…` — its version cell still reads `1.15` — DECISIONS `dc7a8d65…`), so no upstream decision needed absorbing this round. **TE F-01 (High): A6-18's `advisoryWaveGateMain.test.js` widening prescribed the wrong fifth value.** That suite's fixture drives an out-of-envelope refusal (`proposedAction: "E-2"`), not a capture failure, and its harness `_git` double answers `ok: true` to every verb `captureTreeSnapshot` issues (`add`, `rev-parse`, `write-tree`, `commit-tree` explicitly; `update-ref` through the `ok: true` fallthrough), so per A6-10 / TSPEC §3.5 the capture succeeds and per TSPEC §3.2 step 4 it runs at the call site on every applying wave — the fifth key is the wave-scoped ref `refs/pdlc/a6-snapshot-1` (the fixture's `haltReason` oracle pins wave 1), not `null`. The row now states that value, requires it be written spec-side as `"refs/pdlc/a6-snapshot-" + waveNum` under the same anti-echo rule AT-06-4's halves carry, and records the consequence: a non-`null` ref makes BR-14's overwrite notice due on that halt too, at no collateral cost because every notice oracle in the suite is a filtered count over `inapplicabilityStatements(logs)`, never a whole-array `toHaveLength`. The DoD widening leg carries the same value so verifier and implementer read one contract. **TE F-02 (Low):** the AT-06-4 DoD leg now requires a paired negative on **both** arms — AT-06-4b on the seam side, and on the un-skip side the no-A6-fired halt (positively pinned by outcome, `haltReason` and `a6.calls.length === 0`) where the `advisory` argument is omitted and no overwrite notice appears anywhere in `notices` — so a positive-only un-skip arm cannot tick it. **PM F-01 (Low):** batch-safety rule 2's file-by-file walk is restated as a **set-equality over the file-ownership manifest** — every owned `(file, batch)` pair is enumerated, so a path added to the manifest without a clause is a visible hole rather than a silent pass; the previously unenumerated single-owner paths are named. **No task row, batch, wave, dependency edge or file-ownership cell changed** — 11 tasks, 7 waves, DAG unchanged. |
...
| A6-18 | `pdlc/workflows/__tests__/advisoryWaveGate.test.js`, `pdlc/workflows/__tests__/advisoryRecord.test.js`, `pdlc/workflows/__tests__/advisoryEscalationLog.test.js`, `pdlc/workflows/__tests__/advisoryWaveGateMain.test.js`, `pdlc/workflows/orchestrate-dev.js` |
