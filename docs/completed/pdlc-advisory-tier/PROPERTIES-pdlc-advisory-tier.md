...
| PROP-RUNG-04 | Given `MODEL_ADVISORY` is rejected as a model error, `resolveAdvisoryRung` must (a) emit an `ADVISORY_MODEL_FALLBACK` notice naming **both** the unresolvable value and the substitute, (b) record `{ model: MODEL_ADVISORY_FALLBACK, fallback: true }` on `_state.resolved` (v1.1 — the decision rides the per-run memo; the function's return value is the dispatch outcome, TSPEC §3.4), (c) re-dispatch the same prompt exactly once, and (d) proceed. The record's `Model` row and the report summary must both show the substitution. | Functional | Unit | AC-1.3, M-2, T-01-3, T-08-7 | `advisoryRung.test.js` |
...
`resolveAdvisoryRung` records `{ model: MODEL_ADVISORY, fallback: false }` on `_state.resolved`
(TSPEC §3.4, v1.1 contract) without any alias existing. PROP-RUNG-09 therefore asserts the *reporting* of a non-substituted
...
reachable **only** through that seam's declared `verifyGate`. **At the three seams that declare a gate
— A2, A4, A5 — two conjuncts, both required** (A1 and A3 declare none and take the two-conjunct form
stated below, which runs the mutation in the opposite direction):
...
*Category: Functional · Level: Unit · Traces: AC-4.5, AC-4.6, NFR-2, AC-3.6, BR-6, T-03-6(b), TSPEC §4.3, TSPEC §5.5 · Home: all five in
...
The A1 row of PROP-GATE-01…05 was the one place the upstream documents disagreed on what is being
asserted; TSPEC v1.3 and PLAN v1.6 settled it in favour of `null` at both gateless seams (§13.1
item 1, now a closure record). Conjunct 1's `post-action-verification-failed` disposition is asserted at
the **three** seams that can apply an action: **A2, A4 and A5**.

**A1 and A3 take a different form, and for the same reason.** Both supply `permittedActions: []`, so
neither ever reaches step 4 and neither declares a post-action gate to stub: TSPEC §4.3 says the seam
with "no permitted action (A1, A3) supplies `permittedActions: []` and an `apply` that is never
reached — the §5.1 gate refuses first" (`TSPEC:432-433`), and TSPEC §5.5's per-seam gate table now
gives A1's `verifyGate` as "**`null`** — A1 declares no post-action gate (§5.4's '—' row) …
Deliberately **not** `async () => ({ passed: true })`: that trivially-passing stub is what FSPEC
T-03-6(b) treats as the falsifying mutation, so it must not appear as a shipped implementation"
(`TSPEC:655`), beside A3's "**`null`** — same shape as A1: `permittedActions: []`, step 6 unreachable,
`resolved` never reached" (`TSPEC:657`). PLAN A-23 specifies the implementation to match — "A3's
`permittedActions: []` with throwing `apply`/`revert` stubs" (`PLAN:274`) — and PROP-A3-05's terminal
behaviour is a pipeline halt with the classification attached, never an applied action.

For **both** rows the assertion is therefore **two conjuncts, both required**:

1. **The behavioural conjunct — `resolved` is unreachable on *every* path**, and each path terminates
   in `escalated` or `no-action` with its own O-1 triple (exact outcome value, named reason byte-equal
   in the disposition, the `ADVISORY-{feature}.md` `Disposition` row and the `ESCALATIONS.md`
   `Refusal reason` row, and the seam's pre-advisory behaviour still having happened). This is a
   positive assertion per path, not an absence-only oracle.
2. **The structural conjunct — the seam's `SeamOps.verifyGate` is asserted `=== null`, directly and
   per seam.** This is the mutation control, and at these two seams it is the *only* one available:
   with `permittedActions: []` the driver never reaches step 6 (`TSPEC:434-439`), so a build that
   shipped `verifyGate: async () => ({ passed: true })` at A1 or A3 would be behaviourally
   indistinguishable from the correct build and **no** behavioural oracle could catch it. Installing
   that stub must make the case fail, and conjunct 2 is exactly what makes it fail.

The mutation therefore reads in opposite directions at the two families, which is the one thing an
implementer must not transpose: at A2/A4/A5 the mutation is to **replace** the declared gate with
`async () => ({ passed: true })`; at A1/A3 it is to **install** that same stub where the shipped seam
declares `null`. PLAN §8.2's T-03-6 row states this in both directions and cites these lines for the
rationale (`PLAN:869`, `PLAN:258`). Asserting conjunct 1 of the A2/A4/A5 form at A1 or A3 would
instead require stubbing a gate they never reach and observing a disposition they cannot produce — it
