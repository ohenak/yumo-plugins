...
| T11 | 🟢 `[Fake first]` **new** `_run-suite.mjs` — suite runner: mint `PDLC_TEST_RUN_ID`, empty the run dir, spawn `node --test --import=./__tests__/_bootstrap.mjs __tests__/`, then the assertion step; `scripts.test` becomes the runner. **The runner forwards its own unrecognised arguments through to the spawned `node --test`**, so `npm test -- --experimental-test-coverage` is a hermetic coverage run rather than a second, bootstrap-less spelling of the suite (§8's coverage item depends on this). **Forwarded node flags go in node-option position — before `--test`'s path list — and the oracle is positional, not presence-shaped:** measured on node v20.20.1 at HEAD, `node --test --experimental-test-coverage __tests__/` exits `0` and prints a coverage report, while `node --test __tests__/ --experimental-test-coverage` exits `1` because node reads the flag as a path. The test therefore asserts **set-equality over the child's whole argv** against the expected `["--test", "--experimental-test-coverage", "--import=./__tests__/_bootstrap.mjs", "__tests__/"]` shape with the forwarded flag's index pinned **below** the index of `__tests__/`; a containment assertion ("the flag reaches argv") is explicitly rejected, because an implementation that appends satisfies it while V5 and §8's coverage floor are both unrunnable | `pdlc/engine/__tests__/suite-spine.test.js` | `pdlc/engine/__tests__/_run-suite.mjs`, `pdlc/engine/package.json` | 3 | T01 | ⬚ |
...
`Files | Task | Batch`, is safe for a *different* reason — not "no id-like column", which
...
| O-ENG-T6 | does any task assert that **this PLAN's own §4 manifest parses**? (TE PLAN-v5 Q-01) | No, and none is added. The check exists — `validatePlanContract(parsePlanTasks(PLAN).tasks, parsePlanOwnership(PLAN).ownership)` — but its subject is a per-feature document, not shipped code, so a test over it would live in `pdlc/workflows/__tests__/` and go stale the moment this feature merges. `planOwnership.test.js` and `waveExecution.test.js` pin the parser against synthetic PLANs, which is the durable half; the live-file half belongs to Phase P's own self-parse, which runs `parsePlanTasks` + `computeTopologicalBatches` over this file and halts the phase on an unparseable table. The v1.2→v1.3 failure TE names was real but predates that gate being consulted here: this PLAN is now re-derived every round (54 tasks / 54 owning rows / 76 pairs / `{"ok":true}` / 17 waves) and the figures are carried in §0's changelog as re-runnable rows. **If the operator wants it mechanical rather than procedural**, the one-line attachment point is `documentOracles.test.js`'s `LIVE_ROOT` pattern — a `docs/*/PLAN-*.md` sweep asserting `validatePlanContract().ok` for every feature PLAN on the branch, which is the form that does not go stale. Not built here: it is repo infrastructure, not this feature's scope |
| O-ENG-T7 | is T04's throwables corpus pinned or left to the implementer? (TE PLAN-v5 Q-02) | Pinned, and T04 owns the fixture. The corpus is a named export from the same test file — strings, `null`, `undefined`, plain objects, an `Error` with a `cause` chain, and a non-`Error` object carrying a `message` property — because a hand-rolled generator emitting only `Error` instances passes the totality property while proving nothing, which is TE's point exactly. The property quantifies over that corpus **and** over generated values; the corpus is the floor, not the whole domain. Any later task needing throwables imports it rather than re-rolling one (§4 keeps `outcome.test.js` under T04's sole ownership, so there is one writer) |
| O-ENG-T8 | is §8's ≥85 % floor per-module or aggregate? (TE PLAN-v5 Q-03) | **Per-module over `pdlc/engine/lib/`**, not an aggregate. An aggregate 85 % with `lib/transport-cli.mjs` sitting below it satisfies the sentence while leaving the fallback transport — the least-exercised module in the feature and the one with no CI-observable happy path — effectively unmeasured. §8's coverage item is read as a floor every `lib/*.mjs` file must clear on its own; the aggregate is reported but is not the gate |
