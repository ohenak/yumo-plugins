...
consumed while the un-consolidated set stays non-empty. This document still described AC-1.4 as
having "two named causes". Absorbed at the three sites the erratum named: (1) §5.3's proposal-file
table gains a row for that cause (**no** proposal file) and its paragraph now reads three named
causes, with AC-4.3's degraded pass named as the cause AC-1.4 still does not enumerate; (2) §14.4's
ER-3 records the partial absorption and restates what remains open; (3) §15's AC-1.4 → AT map binds
**AT-K3b** (§13.4), a new row asserting the all-unreadable terminating `no-op` — empty consumed
pair, no proposal file, no minted reason code, discriminated by AC-7.1's empty consumed-by-basename
list beside a non-empty un-consolidated set. AT-R7's locator for the degraded row is restated by
name rather than by ordinal, since the new row shifts it. No AC, BR, NFR, E-row, vocabulary pin or
other AT changed. This round's insertions shifted the document's two self-locators; both re-derived
at HEAD here (§4.2 → §4.3 `:578-579`, AT-P7 → §15.3 `:2476`), superseding the v11.6 values.

**Round 15 (v11.6) — te L-01 only; both v15 reviews are PASS and se filed no finding. No rule, AC,
BR, AT row, fixture or oracle strength changes.** AT-Q7's containment parenthetical (§13.5) now
carries the same "as recorded at the implementing layer" phrase AT-Q7c gained in v11.5, so the two
adjacent rows state one bound in one form. te L-02 (AT-Q7c's literal is pinned to TSPEC §9.3's
current contents) is **accepted as recorded, not repaired** — the reviewer files it as a known drift
point already governed by the shape rule beside it. This round's insertion shifted the document's two
self-locators; both re-derived at HEAD here (§4.2 → §4.3 `:566-567`, AT-P7 → §15.3 `:2459`),
superseding the values the v11.5 note below records.

**Erratum round (Phase D), v11.5 — one erratum, plus the two Low locator findings of the v14 reviews
(both PASS approvals); no rule, AC, BR, AT or fixture changes shape.** (1) **AT-Q7c's invoking-tree
upper bound** (§13.5) spelled §6.5's pre-widening literal `{add, commit, read-branch, read-status}`
and called it "its permitted set"; TSPEC §9.3 has since recorded three non-mutating widenings against
that set under DEC-LAYER-01 (⊕ `read-object`, ⊕ `read-remote`, ⊕ `read-index`), and `read-index` is
observed on AT-Q7c's own `promoted` Given via §7.1's corpus enumeration — so an AT transcribing the
row as it stood was red on correct code. The row now states the bound as §6.5's frozen set ∪ the
widenings recorded at the implementing layer, and says explicitly that what it fixes is the *shape*
of the bound (obliged below, permitted above, no merge or branch verb in either), not a literal
frozen ahead of the widenings §6.5 itself invites. §6.5 is unchanged: it remains the frozen statement
TSPEC inherits and widens by recorded decision. (2) Two stale intra-document locators repaired:
§4.2's producers table cites §4.3 at `:557-558` (was `:511-512`), and AT-P7 cites the change register
as §15.3 `:2449` (was §14 `:2401`) — te L-01, se F-01.
...
| `pdlc/skills/consolidate-learnings/SKILL.md:61` | was "Every `docs/*/LEARNINGS-*.md` with a Date Completed after the last logged pass is in scope" — a **date** boundary | replaced by the §3.2 predicate, so the skill prose and the pass agree |
...
**Aging a released marker was considered and declined.** Nothing ever removes or expires a
`RELEASED:` line, so a repo that has run one pass carries the file in its working tree indefinitely
(git-ignored, §5.4). Reporting an old released marker as evidence of a long-idle repo was weighed
and rejected: the cadence datum in `.consolidation-log.md` already answers "how long since the last
pass" from a durable, committed source, and the marker's timestamp would be a second, weaker answer
to the same question — one lost on every fresh clone. The silence here is a decision, not an
oversight, and no acceptance test asserts anything about a released marker's age beyond AT-M11's
"free at any age". One consequence belongs in TSPEC's operator notes rather than here: the operator
question "is a pass running?" is answered by **reading the line**, never by the file's existence.
...
|---|---|---|
| Domain invariant future REQs must respect | append to `docs/_constraints/DOMAIN-CONSTRAINTS.md` | as today (`pdlc/skills/consolidate-learnings/SKILL.md:66`) |
| Architectural decision now project-level | `docs/_decisions/DECISIONS-{topic}.md` | the path shape is today's (`pdlc/skills/consolidate-learnings/SKILL.md:67`); **`{topic}`'s derivation is new and this feature changes the convention** — see below |
...
- The skill's own instruction at `pdlc/skills/consolidate-learnings/SKILL.md:67` states the
...
generalises (`SKILL.md:64`). Running on a cadence does not lower it (NFR-3) — the trigger decides
...
items in `docs/_decisions/.consolidation-log.md` (AC-2.4, `SKILL.md:69`) — under the §10 record
...
observable and both are asserted: the positive half by AT-K3 and the rows below, the **"only when"**
half by **AT-R7**, whose Given is a pass with no §5.3 cause at all. So the rule is stated once here
and not restated per status:
...
| `no-op` because every enumerated basename was unreadable, so nothing was consumed (AC-1.4's third cause, REQ §4b) | **none** | nothing was consumed, so there is nothing to propose; the basenames stay in the un-consolidated set and the next pass retries them |
...
AC-1.4's "exits successfully without opening a PR or writing a proposal file" describes its **three
named causes** (REQ v2.5: empty consumed set, all promotions duplicate-suppressed, and every
enumerated basename unreadable so nothing was consumed — REQ §4b), which are the first three rows.
The degraded-promotion row is AC-4.3's pass, which reaches `no-op` by a cause AC-1.4 still does not
enumerate. This FSPEC reads the two criteria as
...
(`pdlc/skills/consolidate-learnings/SKILL.md:80`): that shape remains the fallback's presentation,
...
| AT-M3 | operator | **two fixtures**: (a) a marker file that is present but **empty** — the state a write that died mid-flush leaves, which the existence seam reports as `file_empty` and not as `file_missing` — and (b) a marker carrying a line that is neither `IN-PROGRESS:` nor `RELEASED:` | a pass starts | on **both**, it is reclaimed (not refused), the pass proceeds, and the log carries `reclaimed-stale-lock` with the abandoned id reported `unknown`. Fixture (a) is the one this row exists for: it is reachable only because §4.1 releases by writing a `RELEASED:` sentinel rather than truncating, and the recorded reclamation is the only durable trace of a pass that died inside its own take. Its paired negative is AT-M11, which asserts that the *released* state is **not** recorded this way — without that pair, an implementation that recorded `reclaimed-stale-lock` on every take passes this row |
| AT-M11 | operator | a marker carrying `RELEASED: {passId} {ISO-8601}`, in two fixtures — one written seconds ago and one older than `staleLockMinutes` | a pass starts | on **both**, the marker is taken and the pass proceeds, and the log row carries **no** `reclaimed-stale-lock` and **no** `consolidation-in-progress`: a released marker is free at any age, so neither the refusal arm nor the reclamation arm may fire. The older fixture is what stops an implementation from routing every non-`IN-PROGRESS:` file through the stale-lock arm |
...
| AT-R7 | operator | **three fixtures**, with `docs/_decisions/` listed before and after each pass. **(a)** a `promoted` pass in which every promotion landed — the guard-set one on an opened PR, the rest in the §5.4 commit — with nothing degraded, nothing suppressed and no §9 widening: **no** §5.3 cause. **(b)** a `no-op` pass whose promotions were all duplicate-suppressed (§5.3's second table, row 2): also no cause. **(c)** the positive control — a pass whose only promotion degraded on an absent credential (§5.3's degraded-promotion row) | each pass ends | on **(a)** and **(b)**, the set of `docs/_decisions/CONSOLIDATION-PROPOSAL-*.md` files is **unchanged** by the pass — in particular none exists for that pass's `passId`; on **(c)** exactly one exists, named for that `passId`. The negative fixtures are the half this row exists for: §5.3's "when, and only when" was otherwise asserted in the *when* direction alone, and an implementation that wrote a proposal file on every pass — or on every `no-op` — was green everywhere. Fixture (b) sits beside (a) because the two reach "no cause" by different routes (nothing to propose vs. everything suppressed) and §5.3 decides both on causes rather than on the terminal status, which differs between them |
...
| AT-K3b | operator | a corpus whose enumerated basenames are **all unreadable** on disk (permissions or IO error), with nothing else to promote | the pass ends | terminal `no-op` (AC-1.4's third cause, REQ §4b) — the consumed pair is appended **empty**, no `CONSOLIDATION-PROPOSAL-*.md` exists for that `passId`, and **no** reason code is minted for the condition. The discriminator is asserted in already-enumerated values: AC-7.1's *LEARNINGS consumed by basename* list is empty **while** the un-consolidated set is non-empty — the pairing that separates this pass from AC-1.4's first cause, whose two sets are both empty |
...
| T-10 | **The spellings of the "unavailable" observables**, collected here so a downstream author finds them in one place rather than only at the point each arises: §8.3's unavailable **path** cell and, from §8.1's **§8.4 steps 2–3** reader row, the **unavailable-half rendering** — the question is still put to the harvest agent on the fields the record does carry, with the missing half stated as unavailable rather than guessed; §10.3's `suppressed-by:` unavailable-`passId` rendering (a rendering of the second spelling, not a third form) and §6.4's statement of the same; and §6.5's seam permitted-set widening. **§8.3's `phase` arm and §8.1's `failure-mode-id` arm are deliberately not here** (SE v10 F-01, TE v10 M-01; subject narrowed per SE v12 F-01, which observed that §8.4 steps 2–3's `phase` half *is* collected by this cell's first clause, field-agnostically, with the `artifact` and `symptom` halves of the same arm): neither produces a rendering for TSPEC to spell. A `phase`-short record's §8.3 row carries the verdict `insufficient-evidence` — a **§15.2 lexicon value this document pins**, and §10.4 item 5 fixes what that row renders — so treating it as an unavailable literal would take the row out of §8.7's `insufficient-evidence` streak and make `unmeasurable` unreachable on that path; an id-short record emits **no** row at all, so there is nothing to render. Both arms are §14.5 LD-5's, once. In every case **this layer fixes the observable and defers only the literal** | DEC-LAYER-01: the spelling of a value this document does not pin is TSPEC's, and §15.2's lexicon owns no such value. §14.5's register is PROPERTIES-owned *fixtures*; these are TSPEC-owned *literals* |
...
| ER-3 | REQ | **Partially absorbed at REQ v2.5** — AC-1.4 now enumerates a third cause (every enumerated basename unreadable, nothing consumed, REQ §4b), which §5.3's table and AT-K3b carry. What remains open is unchanged in kind: AC-1.4's enumeration is still exhaustive by its own wording while AC-4.3 produces a further cause (a pass whose only promotion degraded). §5.3 and §7.3 are reconciled **within** this document, so the FSPEC is self-consistent; the REQ's own enumeration is not | the exhaustiveness claim is the REQ's, and correcting a REQ acceptance criterion is its author's edit, not this layer's |
...
layer that pins literals, not of a fixture to the layer that writes fixtures. **§8.3's `phase` arm and §8.1's
`failure-mode-id` arm are this table's alone** (LD-5), not both registers' (SE v10 F-01, TE v10
M-01; narrowed per SE v11 F-01, which observed that §8.4 steps 2–3's `phase` half *does* render an
unavailable literal and is collected by T-10 with the `artifact` and `symptom` halves of the same
arm): neither renders an unavailable literal — the first carries the pinned §15.2 verdict
`insufficient-evidence`, the second emits no row — so there is no spelling for T-10 to own.
...
| AC-1.3 | §4.1, §4.2, §4.3, §4.4 | AT-M1, AT-M2, AT-M3, AT-M5, AT-M11, AT-K6 |
| AC-1.4 | §5.3, §8.5, §8.7, §12.1 | AT-K3, AT-K3b (the third cause: the all-unreadable terminating `no-op`), AT-L2, AT-F13, AT-R7 (the "only when" half of §5.3) |
...
| `pdlc/skills/consolidate-learnings/SKILL.md` | `:61` (was the `Date Completed` date boundary) now carries the §3.2 predicate; `:67`'s `DECISIONS-{topic}.md` route gains the §5.2 derivation (`{topic} = failure-mode-id`), so the manual entry point and the pass cannot diverge | §3.2, §5.2 |
...
| # | Rule | Section | AT |
|---|---|---|---|
| BR-13 | At most one pass holds `docs/_decisions/.consolidation-lock` at a time; a second pass observing a **fresh `IN-PROGRESS:`** marker terminates `refused` with `consolidation-in-progress`. A `RELEASED:` marker is not held, so it is never refused on at any age (BR-14a, §4.2). | §4.2 | AT-M1, AT-M11 |
| BR-14 | An `IN-PROGRESS:` marker older than `consolidation.staleLockMinutes` (default `60`) is reclaimed, and the reclaiming pass records `reclaimed-stale-lock` alongside its own status. | §4.2 | AT-M2, AT-M3 |
| BR-14a | The marker is released by an **in-place write** of `RELEASED: {passId} {ISO-8601}` — never by removing the file, which no seam can do — and a `RELEASED:` marker is taken like an absent one, at any age, with no reason code. | §4.1, §4.2 | AT-M11 |
...
| E-11 | Marker file present but **empty** (a write that died mid-flush), or carrying a line that is neither `IN-PROGRESS:` nor `RELEASED:` | **reclaimed, not refused**; the abandoned id is reported `unknown`. Reachable because §4.1 releases by writing a `RELEASED:` sentinel and never by truncating — a released marker is E-11b, not this row | §4.2 | AT-M3 |
| E-11b | Marker file carrying a `RELEASED:` line, of any age | taken like an absent file: the pass proceeds and records **no** reason code — a released marker is free, not stale | §4.1, §4.2 | AT-M11 |
