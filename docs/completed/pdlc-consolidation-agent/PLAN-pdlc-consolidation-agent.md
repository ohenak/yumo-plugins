...
|---|---|---|
| `REQ-pdlc-consolidation-agent.md` | **2.5** | header table `:18` (§4b: an unreadable enumerated basename is **not** consumed) |
| `FSPEC-pdlc-consolidation-agent.md` | **11.7** | header table `:12`; erratum note `:14-26`; AT-register erratum history `:67-68` |
| `TSPEC-pdlc-consolidation-agent.md` | **2.8** | header table `:12` |
...
2026-08-10. Where a count is stated, the command that produced it is stated beside it. The live pins
are this table, T05's row, §8.3's DoD row, §9.1's erratum rows and §10's risk row; the numbered
revision entries above record what each revision measured **at the time** and are history, not live
claims.
...
| T05 | 🟢 **Traceability set-equality (no skip; green once §9.1 errata 4 and 8 land — both landed, at TSPEC v2.0 and v2.8).** L3: parse the FSPEC's AT register and TSPEC §12.3's own table and assert **set equality in both directions** — every register id has exactly one file, no file claims an id the register does not carry. Ids are extracted by matching the `AT-…` token grammar over the whole cell and **de-duplicated**, so `(no FSPEC AT)` prose contributes nothing unless it names an id and the report row's deliberate citation of AT-L5 is idempotent. The parser takes an injected `root` (DC-04) and consults no ambient state. **The count is read, never hard-coded.** The oracle is the set equality itself; beside it sit two conjuncts that make a failure legible instead of merely red: (i) a **version pin** — FSPEC's `Version` cell reads `11.7` (`FSPEC:12`) and TSPEC's reads `2.8` (`TSPEC:12`), in the shape T24 pins the vocabularies cell at `1.4`, so a later erratum round fails as *"the register moved"* rather than as *"the code is wrong"*; (ii) a **non-vacuity floor** — the parsed register is non-empty and its size is reported in the failure message, so two empty parses cannot agree perfectly. Measurement of record, re-taken at HEAD 2026-08-10: enumerating `AT-…` tokens over FSPEC §13's register range (`:2116-2267`) de-duplicated gives **100** ids at FSPEC v11.7, against **99** at v11.5 and v1.0's transcribed 96 at v11.1 — the delta from 99 being **`AT-K3b`** (`FSPEC:2210`), the all-unreadable-corpus `no-op` minted by FSPEC v11.7. **Precondition, stated because it is the one thing that could halt a wave — and it is now met on both counts:** §9.1 erratum 4 has **landed** (TSPEC §12.3 assigns all three of `AT-M11`, `AT-Q13`, `AT-R7`), and §9.1 erratum 8 has **landed** at TSPEC v2.8, which assigns `AT-K3b` to `consolidationPass.test.js` and re-derives §12.3's own count to **100** (`TSPEC:2908`) — so the case is green the moment it is written. Both figures above are the pin's expected values, not a hard-coded cardinality: the count itself is still read from the register at run time. The erratum channel repairs upstream documents **after Phase P converges and before Phase I dispatches**, which is the ordinary path this row now sits on. If a wave nonetheless reaches T05 with §12.3 short of the register, the task **halts and reports the missing ids** — it does not weaken the oracle to containment, which is the assertion that would still pass with the ids missing. This row needs no module symbol, and it guards the table from batch 2 onward. | `pdlc/workflows/__tests__/consolidationTraceability.test.js` **(new)** | — | 2 | T00 | ⬚ |
...
| T20 | 🔴 **RED (`describe.skip`) — the pass, end to end (L2).** Two blocks. `T28 — marker predicates`: `parseMarker` and `markerVerdict` driven **directly** on literal inputs (pure, no `main`) — including **AT-M11's pure half**: `markerVerdict` returns `free` for a `RELEASED: {passId} {ISO-8601}` marker in **both** of AT-M11's fixtures (`FSPEC:2160`), one written seconds ago and one older than `staleLockMinutes`, the older one being what stops an implementation routing every non-`IN-PROGRESS:` file through the `reclaim` arm. `T31 — pass lifecycle`: AT-C1, AT-C1b, AT-C2 … AT-C8, AT-M1 … AT-M6, AT-M6b, AT-M9, **AT-M11**, **AT-K3b** through `main()` with every seam doubled; plus the **(no FSPEC AT)** obligation TSPEC §12.2 records — (i) below carries **two fixtures**, of which only the mixed one is unregistered, the all-unreadable one being AT-K3b's discharge (FSPEC v11.7, `FSPEC:2210`; assigned to this file by TSPEC §12.3 at v2.8), and (ii) is AT-M3's own fixture (a). **AT-M11's pass-level half** also sits in this block, beside AT-M3, which is the pairing that makes it an oracle rather than a second happy path: on both released-marker fixtures the marker is taken, the pass proceeds, and the log row carries **no** `reclaimed-stale-lock` and **no** `consolidation-in-progress` — against AT-M3's two fixtures in the same block — the empty one and the neither-verb one — which *do* record `reclaimed-stale-lock`. Without that pair an implementation recording `reclaimed-stale-lock` on every take passes AT-M3 (`FSPEC:2159`, whose row sits immediately above AT-M11 at `:2160`). Both AT-M11 halves live in this one file, so T05's "exactly one file per register id" is undisturbed. The two obligations: (i) The unreadable-corpus-entry case, **two fixtures in the one case, each the other's control** (TSPEC §12.2 at v2.8, absorbing REQ §4b at v2.5). **Fixture 1, the mixed corpus** — one unreadable member and one readable — asserts that the **un-consolidated count** counts **both**; that `renderConsumedPair`'s basename list is **set-equal to `{readable}`** — the readable basename present, the unreadable one **absent**, and no third name (set equality, not containment plus an absence: REQ §4b decides that an unreadable entry is *not* consumed and stays un-consolidated for the next pass, so an implementation that renders both marks a file consumed that contributed no evidence); and that the report body names the unreadable basename and **not** the readable one. The readable member is the control that stops the count and body conjuncts passing on a fixture where nothing was readable. **Fixture 2, the all-unreadable corpus** (two enumerated basenames, `_readFile` returning `null` for both) **discharges AT-K3b**: terminal status exactly `no-op` — not `failed` and not `refused` — the rendered consumed pair **empty**, `|un-consolidated|` **2** with both basenames named in the body as unread, **no** `CONSOLIDATION-PROPOSAL-{passId}.md` written and **no** reason code minted for the condition, the status pinned against §6.4's frozen terminal-status catalogue rather than a retyped literal. Fixture 1 keeps "pair empty" from passing on a pass that enumerated nothing; fixture 2 keeps fixture 1's status assertion from passing on an implementation that terminates every unreadable-touching pass `failed`. Both land inside this row's already-planned `T31 — pass lifecycle` block: **no new task, no new file, no batch change**. (ii) **AT-M3's fixture (a), the empty marker, written inside the AT-M3 case** beside AT-M11, because the pairing *is* the oracle: a `""` marker asserts a normal terminal status **and** `reclaimed-stale-lock` recorded with the abandoned id `unknown` (E-11), against AT-M11's `RELEASED:` fixtures in the same block, which record neither reason code at either age (E-11b). `fakeFs` supports it unchanged — a value trimming to `""` returns `{ok:false, reason:"file_empty"}` (`__tests__/helpers/seams.js:296-299`) while `_readFile` returns `""`, which is exactly the present-but-unparseable state TSPEC §7.3 routes to `reclaim`. **AT-M3's full *Given* is written**, both arms: FSPEC BR-14a releases by writing a `RELEASED:` sentinel rather than truncating, so the truncated arm is reachable (E-11) and nothing here is red on correct code. Note that this obligation is no longer unregistered — it is AT-M3's own fixture (a) — so the only remaining `(no FSPEC AT)` obligation in this row is **(i)'s mixed fixture**, (i)'s all-unreadable fixture now carrying `AT-K3b`. | `pdlc/workflows/__tests__/consolidationPass.test.js` **(new)** | — | 3 | T01, T02 | ⬚ |
| T21 | 🔴 **RED (`describe.skip`) — routing and the PR route (L2).** Three blocks. `T28 — routing predicates`: `routeOf` (over the **imported** `MERGE_GUARD_DEFAULTS`, read and never copied), `routeProposal`, `enactedByLog`, `enactedByPr`, driven directly. `T30 — clone and seams`: AT-Q1, AT-Q7, AT-Q7b, AT-Q7c under §11.3(a)'s spy — a recording wrapper classifying each `_git`/`_ghRun` call with **both of the module's own** classifiers (`resolveSeamDomain`, `resolveSeamVerb`), handed the directory `fakeMakeTempDir` returned; **four** set assertions (partition, containment `observed ⊆ permitted`, obligation `obliged ⊆ observed`, and AT-Q7c's two `∅` equalities), compared over a `Set` and never a multiset. `T31 — routes end to end`: AT-R1 … AT-R5, **AT-R7**, AT-Q2, AT-Q3, AT-Q4, AT-Q5, AT-Q6, AT-Q8, AT-Q9, AT-Q10, AT-Q11, AT-Q12, **AT-Q13**. The last two carry register ids as of FSPEC v11.5 and still at v11.7 and are **no longer "(no FSPEC AT)"** — `FSPEC:67-68` retired both labels ("AC-3.2's body obligation gains AT-Q13"; "§5.3's 'only when' negative half gains AT-R7"), and FSPEC §15 traces them to AC-3.2 (`:2396`) and AC-1.4 (`:2388`). **AT-Q13** (`FSPEC:2201`) is AC-3.2's three PR-**body** obligations on **both** of the register's fixtures — (a) a promotion derived from two named features, (b) a single-occurrence promotion cleared under AC-2.3's standing-invariant argument — asserting on each that the body names its source LEARNINGS by **feature name, set-equal** to the features the promotion was derived from, carries the targeted failure mode's `symptom` line verbatim, and carries the AC-2.3 evidence in the form that fixture cleared the bar with. Fixture (b) is the arm that stops an unconditional recurrence list; AT-Q2's oracle is the trailer set only, and a body carrying nothing but the three trailers is green under AT-Q2 and red here. Expected values are transcribed **from the fixture LEARNINGS corpus the pass was handed and never read off the produced record** (reading them off the record greens the case even when pass and renderer drop the same field together). **AT-R7** (`FSPEC:2181`) is FSPEC §5.3's "and only when" half over **three** fixtures with `docs/_decisions/` listed before and after each pass: (a) a `promoted` pass with no §5.3 cause and (b) a `no-op` pass whose promotions were all duplicate-suppressed both leave the set of `docs/_decisions/CONSOLIDATION-PROPOSAL-*.md` files **unchanged** — in particular none exists for that `passId` — while (c), the positive control, is a single promotion degraded on an absent credential and yields exactly one, named for that `passId`. (b) sits beside (a) deliberately: the two reach "no cause" by different routes and §5.3 decides both on causes rather than on terminal status. Asserted through the write double's recorded path set. `fakeGhRun` with the suite's own script map — **not** `passingGh`, and `GH_SURFACE_NAMES` does not grow. | `pdlc/workflows/__tests__/consolidationRoute.test.js` **(new)** | — | 3 | T01, T02 | ⬚ |
...
- [ ] `consolidationTraceability.test.js` reports set equality in both directions between the FSPEC
      v11.7 register and TSPEC §12.3's table, with the version pin (FSPEC `11.7`, TSPEC `2.8`) and
      the non-vacuity floor green. Measurement of record, re-taken at HEAD 2026-08-10: **100**
      register ids (FSPEC §13, `:2116-2267`, `AT-…` tokens de-duplicated), the delta from 99 being
      `AT-K3b`. §9.1 errata 4 and 8 have both landed — TSPEC §12.3 assigns all 100 at v2.8,
      `AT-K3b` among them — so the row is tickable.
...
| 4 | **TSPEC §12.3's traceability table omits three FSPEC register ids** — `AT-M11`, `AT-Q13`, `AT-R7`. All three were register rows in FSPEC v11.5 §13 and remain so at v11.7 (`:2160`, `:2201`, `:2181`) and all three are traced to an AC by FSPEC §15 (AC-1.3 `:2387`, AC-3.2 `:2396`, AC-1.4 `:2388` at v11.7). Two of them are the sole oracle for an acceptance criterion's negative half: AT-M11 is AT-M3's explicitly named paired negative (`FSPEC:2160`, the row itself: a released marker is free at any age, and the older fixture is what stops every non-`IN-PROGRESS:` file being routed through the stale-lock arm), and AT-Q13 is the only test that reads the PR **body** for AC-3.2 (`FSPEC:2201` — a body carrying nothing but the three trailers is green under AT-Q2) | `grep -c` over `TSPEC-pdlc-consolidation-agent.md` returned **0** for each of the three (at TSPEC v1.8) | T20 (AT-M11, both halves) and T21 (AT-Q13, AT-R7). **Landed at TSPEC v2.0**: all three ids are assigned in §12.3, so T05's set equality is green — it never degraded to containment |
| 5 | **TSPEC §12.3 fixes the register size at a stale measurement.** `TSPEC:2395` read "The FSPEC's AT register carries **96** ids, measured at v11.1" (at TSPEC v1.8; since v2.0 `TSPEC:2485` reads 99 at v11.3). FSPEC was at **v11.5** at that measurement, whose own erratum note records the two additions that made 96 wrong | re-enumerating `AT-…` tokens over FSPEC §13, de-duplicated, gave **99** at v11.5; re-taken at HEAD (FSPEC v11.7, `:2116-2267`) it gives **100** | T05, whose count is now **read** from the register rather than transcribed, with a version pin so a later move fails as "the register moved" — the pin now reads FSPEC `11.7` / TSPEC `2.8` and the expected cardinality is **100** |

**Erratum 8 (raised against this PLAN at round 7, absorbed here).** After the five above landed, the
upstream moved again: REQ v2.5 §4b decided that an unreadable enumerated basename is **not** consumed
(it is omitted from the consumed pair and stays un-consolidated), FSPEC v11.7 minted **`AT-K3b`**
(`FSPEC:2210`) for the all-unreadable corpus terminating `no-op`, and TSPEC v2.8 assigned it to
`consolidationPass.test.js` and re-derived §12.3's count to **100** (`TSPEC:2908`). Absorbed, not
re-raised: T20's obligation (i) now carries **two fixtures** — the mixed corpus, whose consumed-pair
assertion is set equality with `{readable}` (the pre-erratum "contains both basenames" reading is
overturned), and the all-unreadable corpus, which discharges `AT-K3b` inside the already-planned
`T31 — pass lifecycle` block; T05's pin and cardinality follow. No new task, no new file, no graph
change.
...
| an FSPEC or TSPEC erratum round moves the AT register mid-implementation and reds T05 inside a halt-on-red wave | T05's **version pin** (FSPEC `11.7`, TSPEC `2.8`), which fails first and names the cause — the failure reads "the register moved", not "the code is wrong". The repair is then a one-line pin bump plus whatever task the new id belongs to, decided by a reader rather than by a hunt |
