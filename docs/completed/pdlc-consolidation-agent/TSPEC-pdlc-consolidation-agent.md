...
| `pdlc/skills/consolidate-learnings/SKILL.md` | `:61` (was the `Date Completed` date boundary) now carries the block/legacy predicate; `:67`'s `DECISIONS-{topic}.md` route gains `{topic} = failure-mode-id` | FSPEC §3.2, §5.2 |
