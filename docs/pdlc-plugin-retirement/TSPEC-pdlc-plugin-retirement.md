...
| TT-3 (bare-path invocation) | the script is spawned **by path with no interpreter** (`spawnSync(scriptPath, [...])`, not `spawnSync("bash", [scriptPath])`) and its status is one of `0`/`3`/`4` — **never `126`**; plus the index mode bit is `100755` (`git ls-files -s`) and the on-disk file is executable in a fresh clone. Re-homes `bootstrap.test.js`'s §9.3 mode-bit block and its dedicated never-126 assertion (`:265`–`:301`), which the sweep deletes with that module (§4.4) | same |
