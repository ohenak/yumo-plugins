...
| 6 | 20 of M-8's 21 `*.test.js` modules deleted (all but `hookCompatibility.test.js`, incl. `bootstrap.test.js`) + `runtimeProvenanceWiring.test.js` + 6 helpers + `hookCompatibility.test.js` reduction (its ten `(hasBash ? it : it.skip)` sites converted to `itOrSkip`, adding **ten** `"bash"` `SKIP_INVENTORY` rows to `__tests__/helpers/driftCapabilities.js` — same file as the class-3 entry, so the two commits are serialised, never batched, §5.5) + `driftGenerators.js` reduction; `consolidationHookParity.test.js`'s AT-3.3 clause 2 assertion (§5.2) plus the conversion of its four capability ternaries (`:203`, `:227`, `:343` on `canRunDifferential`, `:382` on `hasBash`) to `itOrSkip`, which adds a fifth capability key `python` to `driftCapabilities.js` and to `KNOWN_CAPABILITY_KEYS` in `helpers/skipSink.js` plus four `SKIP_INVENTORY` rows — the **third** serialised edit to `driftCapabilities.js` after class 3's TT-1b row and the ten `"bash"` rows above, never batched with either (§5.5, TE TSPEC v9 Q-01); **no assertion is re-homed out of `orchestrateQueue.test.js`** (§4.4) |
