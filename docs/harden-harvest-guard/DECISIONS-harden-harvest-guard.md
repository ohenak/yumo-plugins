...
**Consequences / accepted residual risks:** the self-test case table lives inside the production script (inert on the hook path — argv-gated, evaluated before the stdin read, § 3.1 step 0); the jest binding must assert the printed case count against a floor, not just exit 0, or an emptied table passes silently (TE F2-04, CFD-6 below). No RR-register entry — the residual gate exposure closed by CFD-1 was assessed as two independent anomalies deep (harness fault emitting one exact string × stray user export) even before the fix. The closed gate is pinned by the CFD-8 supplementary test (TE DECISIONS F-03): a refactor that drops the env-clear (or the argc gate, if that mechanism is chosen) now fails the suite instead of silently re-opening the keyhole.
...
**Consequences / accepted residual risks:** the REQ matrix row M77's fixture *parenthetical* is now known-wrong and is scheduled for correction at the next REQ touch (PM v2 F2-02 — CFD-2 below); no behavior or reason-code change. The unpinned corrupt-ref × LEARNINGS-committed cell conformantly routes G4/G5 (documented in FSPEC-GUARD-03). No RR-register entry.
...
**Consequences / accepted residual risks:** none new — the enumeration remains closed, so everything outside it is still **RR-1** (undefended verbs) by D1; fd-duplication (`2>&1`, `N>&M`) remains never-destructive per the **FSPEC CF-1 `>&` lexical disambiguation rule** (FSPEC-GUARD-02 rule 1; the label TSPEC § 3.5.1 also uses — not this document's CFD-1) — M56, M62, M67 — and appends (`>>`, `N>>`) remain never-destructive per the separate REQ-GUARD-02 append exclusion (M15).
...
## Carry-Forward Notes — binding implementation dispositions (CFD-1–CFD-8)

Document-local carry-forwards are numbered **CFD-n** (Carry-Forward, DECISIONS) — renamed from CF-n at v1.2 (PM DECISIONS F-01 / TE DECISIONS F-05) to avoid colliding with the **FSPEC's** own CF-n namespace (CF ids restart per document in this pipeline; cross-document CF references must always be document-qualified, as DEC-06's consequences now do). CFD-1–CFD-7 fold in the iteration-2 TSPEC Lows (both reviews returned **Approved with minor changes** — PM: 2 Low; TE: 5 Low; overlap on the self-test gate); CFD-7 was amended and CFD-8 added by the iteration-1 DECISIONS reviews. None alters a decision, reason code, or matrix expectation; each is recorded here as the binding implementation-level disposition so se-implement absorbs them without re-review.

| CFD | Source | Disposition (binding for IMPL) |
|---|---|---|
| CFD-1 | PM v2 F2-01 + TE v2 F2-03 | **Self-test gate hardening (amends DEC-01):** the wrapper explicitly **unsets `GUARD_SELF_TEST`** from the Python's environment on the non-self-test (hook) path, closing the joint-sentinel keyhole (hook stdin literally `--self-test` × stray user export would otherwise divert a `PARSE_ERROR`-bound input to the self-test, exit 0). The § 3.1/§ 6.6 "settable only by this wrapper branch" prose is corrected accordingly. TE's argc-gate (`len(sys.argv) == 3` with a second wrapper-only sentinel) is the sanctioned equivalent if the env-clear proves awkward — either satisfies; pick one, don't stack both silently. The closed state is pinned by CFD-8 |
| CFD-2 | PM v2 F2-02; pointer corrected per PM DECISIONS F-02 / TE DECISIONS F-04 | **M77 REQ fixture parenthetical (relates to DEC-04):** the REQ row's "corrupt `.git/refs/remotes/origin/{branch}`" construction is empirically known-wrong (C18 — classifies as clean absence, routes G5). Correct at the **next REQ touch**: replace the parenthetical with the loose-object construction or a reference to the **FSPEC-GUARD-03** failure-vs-absence boundary (error-scenario row — the section that defines the boundary; FSPEC-GUARD-06 only maps the query-failure path to `NOT_COMMITTED`). No behavior change; TSPEC § 6.2's loose-object fixture (with builder self-check) is authoritative meanwhile. Do not open a REQ revision solely for this |
| CFD-3 | TE v2 F2-01; conjunct tightened per TE DECISIONS F-02 | **Residual REQ-GUARD-07 substring conjuncts:** the § 6.3 oracle table's mechanical `substrings` assembly additionally asserts, per reason: `NOT_COMMITTED` rows — the substring **`then commit`** (a bare `commit` is unfalsifiable: the template's state description "is not committed" contains it, so dropping the "then commit (and push)" instruction would still pass); `INDETERMINATE` rows — the fail-closed rationale (e.g. `ailing closed`, case-tolerant); `DEGRADED` rows — an `interpreter` substring alongside `python3`. One added conjunct per contract-table row |
| CFD-4 | TE v2 F2-02 | **D3 `{operand}` interpolation for stream-indeterminate operands (M21):** an `xargs`-wrapped deletion verb has no operand token — its operands are the piped input stream. Decision: `emit_block` interpolates the fixed marker **`piped input`** for `{operand}` in the D3 template when the indeterminate operand is a pipe stream (mechanically assemblable; M21's MATRIX `substrings` entry asserts `piped input`). One sentence added to § 3.7's D3 template at implementation |
| CFD-5 | TE v2 F2-03 | Folded into CFD-1 (same finding, TE spelling) |
| CFD-6 | TE v2 F2-04 | **Self-test binding asserts the case count (amends DEC-01 consequences):** the jest binding parses the stdout case count printed on self-test success and asserts it **≥ a named floor constant** (`SELF_TEST_MIN_CASES`, set near the § 6.6 enumerated coverage areas; never below 1) — an emptied `SELF_TEST_CASES` table or a gate bug exiting 0 without running cases now fails loudly instead of passing silently |
| CFD-7 | TE v2 F2-05; convention replaced per TE DECISIONS F-01 | **Bespoke-row convention (§ 6.3) — dispatch-map rule:** rows needing dedicated `it()` orchestration **keep their `MATRIX` entry** with a `bespoke: true` marker; the `it.each` parameterization filters marked entries out; the dedicated `it()` blocks are **generated mechanically from an `id → orchestration function` dispatch map** (each generated block carries the row-id title and consumes the marked entry's config — no hand-written free-standing blocks). The § 6.3 self-audit gains a meta-check asserting the **dispatch map's key set is set-equal to the set of `bespoke: true` ids in `MATRIX`** (fourth conjunct of meta-test 1, or a meta-test 4). This closes both directions the bare-marker convention left un-audited: a marked row with no dispatch entry fails the set-equality (zero-execution — a generated block cannot be "forgotten", it exists iff the map entry does), and an unmarked row with a dispatch entry also fails it (double-execution). Meta-test 1's id-set + length equality and the REQ-GUARD-05 exactly-one-asserting-test invariant remain intact — a row is never executed by both paths. **This dispatch-map rule is the binding disposition for se-implement: TSPEC § 6.3's bespoke convention is implemented per this rule, superseding the v1.1 bare-marker wording** |
| CFD-8 | TE DECISIONS F-03 | **Self-test gate pinning test (amends CFD-1 / DEC-01 consequences):** the § 6.6 supplementary set (outside matrix accounting, consistent with DEC-01's "no matrix row" disposition) gains one mechanically writable case covering **both** sanctioned CFD-1 mechanisms identically (env-clear and argc-gate are observationally equivalent): `runGuard(fixture, {stdinRaw: "--self-test", env: {GUARD_SELF_TEST: "1"}})` → `expectBlock(res, "PARSE_ERROR", ["unparseable"])`. Without it, a refactor dropping the `unset` (or the argc check) passes the suite while the fail-open keyhole re-opens |

---

## Review Disposition — DECISIONS iteration 1 (v1.1 → v1.2)

Reviews: [CROSS-REVIEW-product-manager-DECISIONS.md](CROSS-REVIEW-product-manager-DECISIONS.md) (Approved with minor changes — 2 Low) and [CROSS-REVIEW-test-engineer-DECISIONS.md](CROSS-REVIEW-test-engineer-DECISIONS.md) (Needs revision — 1 Medium, 4 Low). All findings addressed.

| Finding | Severity | Disposition |
|---|---|---|
| TE F-01 | Medium | **Fixed — CFD-7 replaced.** The bare `bespoke: true` marker convention re-opened the silent coverage-drop hole (a marked row's dedicated block was audited by nothing; delete or forget it and all meta-tests still pass with zero executions). CFD-7 now mandates dedicated `it()` blocks generated from an `id → orchestration function` dispatch map whose key set is asserted **set-equal** to the `bespoke: true` id set in `MATRIX`, closing both the zero-execution and double-execution directions. Noted as the binding disposition for se-implement: TSPEC § 6.3's bespoke convention is implemented per this dispatch-map rule |
| TE F-02 | Low | **Fixed — CFD-3 tightened.** The `NOT_COMMITTED` conjunct now asserts `then commit` (falsifiable) instead of bare `commit`, which the template's own "is not committed" state description satisfies vacuously |
| TE F-03 | Low | **Fixed — CFD-8 added.** Pinning test for the CFD-1 gate hardening: `runGuard(fixture, {stdinRaw: "--self-test", env: {GUARD_SELF_TEST: "1"}})` → `expectBlock(res, "PARSE_ERROR", ["unparseable"])`, in the § 6.6 supplementary set. DEC-01 consequences cross-reference it |
| PM F-02 / TE F-04 | Low | **Fixed — CFD-2 repointed.** The M77 correction target is the **FSPEC-GUARD-03** failure-vs-absence boundary (error-scenario row), not FSPEC-GUARD-06 (which only maps the query-failure path to `NOT_COMMITTED`); the document now agrees with DEC-04's Context |
| PM F-01 / TE F-05 | Low | **Fixed — namespace de-collided.** Document-local carry-forwards renamed **CFD-1..CFD-8**; DEC-06's consequences now cite the **FSPEC CF-1 `>&` lexical disambiguation rule** (FSPEC-GUARD-02 rule 1) explicitly and no longer fold M15 into it — the `>>`/`N>>` append exclusion is cited as the separate REQ-GUARD-02 rule it is |
| PM Q-01 | Question | **Confirmed as intended.** RR-2/RR-4/RR-W carry their rationale in the REQ Residual Risk Register and correspond to no rejected-alternative deliberation beyond what the register rows record; DEC entries exist only where a rejected alternative is likely to be confidently re-litigated. Should the G2/G3 HEAD-fallback choice attract an actual "fix to permanent blocking" attempt, that event is the trigger for promoting it to a DEC entry |
...
| 1.2 | 2026-07-02 | DECISIONS iteration-1 feedback (PM: 2 Low; TE: 1 Medium, 4 Low): CFD-7 bespoke convention replaced with the dispatch-map rule + set-equality meta-check (TE F-01, binding for se-implement's § 6.3 implementation); CFD-3 `NOT_COMMITTED` conjunct → `then commit` (TE F-02); CFD-8 gate pinning test added and cross-referenced from DEC-01 consequences (TE F-03); CFD-2 repointed at FSPEC-GUARD-03 (PM F-02 / TE F-04); document-local carry-forwards renamed CF-n → CFD-n and DEC-06's FSPEC CF-1 citation qualified with the M15 fold-in removed (PM F-01 / TE F-05); disposition table added; lineage extended with the DECISIONS cross-reviews |
