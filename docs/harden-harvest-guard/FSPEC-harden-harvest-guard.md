...
| Cross-Reviews | [CROSS-REVIEW-software-engineer-FSPEC.md](CROSS-REVIEW-software-engineer-FSPEC.md), [CROSS-REVIEW-test-engineer-FSPEC.md](CROSS-REVIEW-test-engineer-FSPEC.md), [CROSS-REVIEW-software-engineer-FSPEC-v2.md](CROSS-REVIEW-software-engineer-FSPEC-v2.md), [CROSS-REVIEW-test-engineer-FSPEC-v2.md](CROSS-REVIEW-test-engineer-FSPEC-v2.md), [CROSS-REVIEW-software-engineer-FSPEC-v3.md](CROSS-REVIEW-software-engineer-FSPEC-v3.md), [CROSS-REVIEW-test-engineer-FSPEC-v3.md](CROSS-REVIEW-test-engineer-FSPEC-v3.md) |
| LEARNINGS | docs/harden-harvest-guard/LEARNINGS-harden-harvest-guard.md |

# FSPEC — harden-harvest-guard

Functional specification for the fail-closed rewrite of `guard-harvest-before-delete.sh` and the anchored `Scope:` detection in `check-scope-field.sh`. Source: [REQ-harden-harvest-guard.md](REQ-harden-harvest-guard.md) **Version 1.7** (v1.3 approved with minor changes by SE and TE iteration-4 reviews — the three minor carry-forwards are resolved in this document, see § Carry-Forward Resolutions; v1.4 was the matrix extension driven by this FSPEC's iteration-1 review; v1.5 adds rows M80–M83 from the iteration-2 review and names the eager repo-check position in REQ-GUARD-01's decision-rule preamble; v1.6 adds rows M84–M86 and the `mv` destination-destruction rule from the iteration-3 review; v1.7 — landed in the same commit as this revision — adopts the `mv` static-destination destruction carve-out ahead of D3 (M87), promotes TSPEC rows T-01–T-03 as M87–M89, extends the redirection enumeration with the `N>` fd-digit truncation family (M90), and registers RR-6/RR-7, from the TSPEC iteration-1 reviews. Note: v1.4's original "no requirement semantics changed" framing was inaccurate and is corrected in v1.5 — rows M75–M76 flipped a v1.3-ALLOW command class to BLOCK `NO_REPO` under the eager repo-check ordering; SE FSPEC-v2 F2-03).
...
**Acceptance-test oracle:** the REQ's **Canonical Block/Allow Matrix (M01–M90, S01–S07)** is the acceptance-test suite for every flow below. This document does **not** duplicate the matrix — each FSPEC section cites its rows. Matrix rows are REQ-owned: the rows this FSPEC's and its downstream TSPEC's authoring and reviews produced (M66, closing TE v4 F4-01; M67–M79 and S07, closing the iteration-1 FSPEC findings; M80–M83, closing the iteration-2 findings; M84–M86, closing the iteration-3 findings; M87–M90, closing the TSPEC iteration-1 findings — M87–M89 promoted from the TSPEC-owned rows T-01–T-03) are promoted into the REQ matrix (v1.4, v1.5, v1.6, and v1.7, each in the same commit as the FSPEC revision that produced it) rather than held downstream — per the TE F-09 process rule that behavior-defining resolutions must amend the matrix, not just prose. § Acceptance Tests retains M66's full acceptance statement.
...
|---|---|---|---|
| FSPEC-GUARD-01 | Deletion-guard decision flow (parsing steps 1–5, decision points D1–D4) | REQ-GUARD-01, REQ-GUARD-02, REQ-GUARD-NFR-01 | M01–M08, M17–M32, M44–M51, M55, M60, M63–M66, M73, M80, M86 |
| FSPEC-GUARD-02 | `mv` resulting-path rule and redirection classification | REQ-GUARD-02 | M09–M16, M52–M54, M56, M61–M62, M67, M74, M79, M82, M84, M87–M90 |
| FSPEC-GUARD-03 | Git-state verification flow (G1–G10) | REQ-GUARD-03 | M33–M40, M57–M59, M75–M77, M83, M85 |
...
REQ-GUARD-05 (tests) has no behavioral flow of its own; it binds the matrix carried forward in § Acceptance Tests, including the FSPEC- and TSPEC-driven extension rows M66–M90 and S07.
...
6. **Step 4 — Effective-cwd resolution (REQ-GUARD-01 step 4, union rule).** For static relative operands and globs — in every segment, per below — resolve against **both candidate roots**: **(A)** the stdin `cwd` field when present, else `CLAUDE_PROJECT_DIR` (repo root); and **(B)** the hook process's own working directory. A hit through **either** root is a hit (conservative union; residual drift is RR-5). The initial effective cwd (the union of both candidate roots) applies to **every** segment — not only segment 1 — until a `cd` segment updates it: segment position never changes the resolution root (so the deletion in `echo done && rm docs/f/*.md`, M18, resolves against the same union as a segment-1 deletion; row M80 pins the every-segment rule discriminatingly — a segment-2 glob resolved via candidate root (B) alone, which a segment-1-only union implementation fails). Within the compound command, a `cd <static-path>` segment updates the effective cwd for subsequent segments; `cd <indeterminate>` makes all subsequent relative operands indeterminate. (Rows M17, M18, M64, M65, M66, M80.)
...
8. **Guarded-state lookup.** The lookup opens with FSPEC-GUARD-03's **G-DP1 repo check as an eager early exit**. G-DP1 consults **candidate root (A)** — the stdin `cwd` when present, else `CLAUDE_PROJECT_DIR`; when neither signal exists, the hook process cwd is the only remaining cwd notion and is tested instead (the neither-signal branch is pinned by row M85: no stdin `cwd`, `CLAUDE_PROJECT_DIR` unset, process cwd = the fixture repo → the deletion proceeds past G-DP1 and blocks `NOT_COMMITTED`, not `NO_REPO`): if the tested root is not a git repository, BLOCK `NO_REPO` immediately — D2–D4 are never evaluated. Only deletion-shaped commands reach this point (D1 has already passed), so the eager exit cannot touch REQ-GUARD-NFR-01. (Rows M37, M75, M76; mixed-root discriminator M83 pins that root (A), not the process cwd, is the one consulted; neither-signal row M85.) Otherwise enumerate guarded directories (top-level `docs/{feature}/` whose subtree contains a `CROSS-REVIEW-*.md` / `CODE_REVIEW-*.md`, per REQ Definitions) — enumeration is anchored at the **repo root**: `CLAUDE_PROJECT_DIR` when set, else the `git rev-parse --show-toplevel` of the root G-DP1 just tested (candidate root (A), or the process cwd in the neither-signal branch — one mechanism, so the fallback can never fail where G-DP1 passed); neither the process nor the stdin cwd is ever itself the anchor. The anchor choice is pinned discriminatingly by row **M86**: `CLAUDE_PROJECT_DIR` unset, stdin `cwd` = the fixture repo, process cwd = a second guarded-directory-free repo — an implementation that rev-parses the process cwd enumerates the wrong repo, finds no guarded directories, and allows via BR-01-2; the conformant anchor blocks `NOT_COMMITTED`. Classify each enumerated directory as Verified or unverified via the FSPEC-GUARD-03 flow. This state feeds decision points D2–D4.
...
2b. **Static-destination destruction test — destination-side certainty consumed before any indeterminacy jump (source guardedness and source *determinacy* irrelevant).** If the destination is **static**, its canonicalized path is not an existing directory and does not end in `/`, and it names an **existing** guarded file in an unverified guarded directory → BLOCK with the affected feature's FSPEC-GUARD-03 reason code (`NOT_COMMITTED` in the default fixture). The destination alone proves the destruction, so the agent gets the provable harvest/commit message rather than an unresolvable-operand report — the destination-side twin of step 2's source-side principle (M74). Row **M87** pins the carve-out: an implementation that jumps to step 3 first emits `INDETERMINATE` and fails it. M82 is unaffected (its destination is indeterminate — this step requires a static one); M84 and M88 block here with the same code as their step-4 routing. *(REQ v1.7 `mv` table carve-out row; PM TSPEC F-01 + TE TSPEC F-03.)*

3. If the source **or the destination** is indeterminate (resulting path genuinely unknowable) → decision point D3 applies (FSPEC-GUARD-01), matching the REQ v1.7 `mv` table and the general D3: no source-guardedness qualifier. Steps 2 and 2b have already consumed all static certainty — guarded source (M74) and statically-provable destination destruction (M87) — so this jump never masks a G-state reason code: every operand pair reaching this step is one the guard genuinely cannot resolve. An indeterminate docs-referencing destination alone still suffices whatever the source is, because the move can still **overwrite** (destroy) a guarded file — a static *unguarded* source with an indeterminate docs-referencing destination blocks `INDETERMINATE` (M82).
4. Otherwise (all operands static), compute the **resulting file path** for each source: if the destination is a directory (existing, or spelled with a trailing `/`), resulting path = `destination/basename(source)`; otherwise resulting path = destination. **Destination-destruction test — source guardedness irrelevant:** if the resulting path, or the static destination itself, resolves to an **existing** guarded file in an unverified guarded directory → BLOCK with the affected feature's FSPEC-GUARD-03 reason code (`NOT_COMMITTED` in the default fixture). Certain overwrite is deletion — the fully static twin of M82's indeterminate spelling, symmetric with truncation of the same file (M14/M16): the guard must not block when it cannot know (M82) yet allow when it knows for certain. The **existing** qualifier is what keeps M11/M52 allowed: their resulting paths name no existing file. (Row M84: `mv /tmp/notes.md docs/f/CROSS-REVIEW-x.md` → BLOCK `NOT_COMMITTED`.) The literal-destination arm is already exhausted by step 2b and is retained here for the trailing-slash-on-a-file corner; the **resulting-path arm** is pinned by **M89** (`mv /tmp/CROSS-REVIEW-x.md docs/f/` — the destination is a directory, so a literal-destination-only implementation misses and falls through to ALLOW), and the **source-guardedness-irrelevant clause** by **M88** (`mv docs/f/CROSS-REVIEW-x.md docs/f/CODE_REVIEW-f-v1.md` — a guarded-source-exempting implementation reaches step 5's same-subtree/pattern ALLOW and fails).
5. **Decision (guarded sources):** if the source is/globs a guarded file in an unverified guarded directory, ALLOW iff the resulting path is still under the **same feature's** `docs/{feature}/` subtree (any depth; feature = first path segment under `docs/`) **AND** the resulting basename still matches `CROSS-REVIEW-*.md` / `CODE_REVIEW-*.md`. Everything else → BLOCK (move-out, cross-feature move, and pattern-destroying rename are all deletion). An unguarded static source that survived the step-2b/step-4 destruction tests falls through to ALLOW. Rows M09–M13, M52.
...
2. **Destructive forms.** `>`, `>|`, `N>` for **any** fd digit-string `N` (`1>`, `2>`, `3>`, … — REQ v1.7 closes the family by rule, not by digit list; row **M90** pins the `N ∉ {1, 2}` membership), `&> <path>`, `>& <path>` (per rule 1) truncate: a static target resolving to a guarded file in an unverified guarded directory → BLOCK (rows M14, M53, M54, M61, M62, M90).
3. **Non-destructive forms.** `>>` (append) — and likewise the `N>>` append spellings — never deletion-shaped → no contribution to any decision (M15).
4. **Indeterminate target** → deletion-shaped; decision point D3 applies with its segment-scoped docs-reference test (M56).

### Business rules

- BR-02-1: The `mv` decision is a rule pair over the resulting path — the destination-destruction test (step 2b for static destinations, step 4 for resulting paths), then the step-5 same-feature-subtree + basename-pattern rule — not a case list; the REQ's `mv` table rows are consequences of it. **This flow, not plain D2, is the decision procedure for `mv` operands:** a guarded static source renaming in place ALLOWs (M11) where a literal D2(i) reading would block, and the step-2b/step-4 destruction test is the only destination-guardedness rule (SE/TE F3-01; carve-out ahead of D3 per PM TSPEC F-01).
...
| `mv /tmp/notes.md docs/f/CROSS-REVIEW-x.md` (static **unguarded** source; static destination = an **existing** guarded file) | Step 2 misses (source unguarded); step 2b's destination-destruction test fires → BLOCK `NOT_COMMITTED` — the statically provable overwrite blocks exactly as its indeterminate twin (M82) and truncation of the same file (M14) do (M84) |
| `mv "$SRC" docs/f/CROSS-REVIEW-x.md` (indeterminate source; static destination = an **existing** guarded file) | Step 2b fires before the step-3 indeterminacy jump → BLOCK `NOT_COMMITTED`, **not** `INDETERMINATE` — destruction is statically certain from the destination alone (M87) |
| `mv docs/f/CROSS-REVIEW-x.md docs/f/CODE_REVIEW-f-v1.md` (guarded source over an **existing** guarded destination) | Destruction test before step 5 — source guardedness irrelevant → BLOCK `NOT_COMMITTED`; a guarded-source-exempting implementation reaches step 5's same-subtree/pattern ALLOW and fails (M88) |
| `mv /tmp/CROSS-REVIEW-x.md docs/f/` (unguarded source; destination an **existing directory** → resulting path = existing guarded file) | Step-4 resulting-path arm fires → BLOCK `NOT_COMMITTED`; a literal-destination-only implementation misses the directory-destination collision (M89) |
...
| Redirection operators | `>` (any occurrence of the character) | Lexically covers the whole REQ-GUARD-02 truncation family (`>`, `N>` incl. `1>`/`2>`/`3>`…, `>|`, `&>`, `>&`) since each contains `>`; JSON uses `>` only inside string values, so an occurrence always comes from field content. Pinned by M70 |
...
| D2 hit (including the FSPEC-GUARD-02 `mv`-flow static tests — step 2 static-source, step 2b static-destination, and step 4 destination-destruction); feature state G2/G3/G5/G8/G9 | `NOT_COMMITTED` |
| D2 hit (same inclusion); feature state G4/G7/G10 | `NOT_PUSHED` |
| Git query failure after G-DP1 passes (state unprovable) | `NOT_COMMITTED` |
| D3 or D4 hit (reachable only inside a repo — G1 is exhausted by the eager exit above; the `mv` step-2b static-destination carve-out precedes D3, so a statically-certain destination destruction never lands here — M87) | `INDETERMINATE` |
...
The acceptance-test suite is the REQ v1.7 **Canonical Block/Allow Matrix** — deletion-guard rows **M01–M90** and scope-check rows **S01–S07** — carried forward by reference, under REQ-GUARD-05's one-test-per-row obligation (bulk row M33 expands to one asserting test per referenced row; parameterized acceptable).

Matrix rows are **REQ-owned**: the twenty-six rows this FSPEC's and its downstream TSPEC's authoring and reviews produced — **M66** (CF-3, TE v4 F4-01), **M67–M79, S07** (iteration-1 FSPEC findings, REQ v1.4), **M80–M83** (iteration-2 FSPEC findings, REQ v1.5), **M84–M86** (iteration-3 FSPEC findings, REQ v1.6), and **M87–M90** (TSPEC iteration-1 findings, REQ v1.7, same commit as this revision — M87–M89 promoted from the TSPEC-owned rows T-01–T-03 per the TE F-09 process rule; M90 the `N>` truncation pin) — live in the REQ matrix. M33's G6 re-run expansion set is extended there to include **M66, M67, M73, M74, M79, M80, M82, and M84–M90** (under G6 each resolves against a *Verified* guarded directory → ALLOW; for M82, D3(b) is unmet once no unverified guarded directory exists, so the `mv` falls through to ALLOW; for M84 and M87, the step-2b/step-4 destruction test is likewise unmet — no unverified guarded directory — and, with D3(b) also unmet for M87's indeterminate source, both `mv`s fall through to ALLOW; for M88, the destruction test is unmet and step 5's same-subtree/pattern rule allows; for M89, step 4 is unmet and the unguarded source falls through; for M90, the static truncation target no longer sits in an unverified guarded directory). The degraded rows M68–M72, the non-repo rows M75–M76, the mixed-root row M83 (root (A) is non-repo regardless of LEARNINGS state), the contract rows M77–M78, and the contract-boundary allow row M81 are git-state-independent or non-G6-constructible and are deliberately outside the re-run set.

M66 retains its full acceptance statement:

- **Who:** any agent / **Given:** default matrix fixture state; hook stdin carries no `cwd` field; `CLAUDE_PROJECT_DIR` is set to the fixture repo root (candidate root (A) exists and does not resolve the glob); the hook process is spawned with working directory `docs/f` / **When:** `rm *.md` / **Then:** exit 2, reason `NOT_COMMITTED` — candidate root (B) alone resolves the glob inside the unverified guarded directory, exercising the union rule's second disjunct (an (A)-only implementation must fail this row). M80's fixture states the same `CLAUDE_PROJECT_DIR` control — without it, both rows would degenerate to single-root tests (SE F3-02).

All twenty-six rows are hermetically writable in the existing jest harness (spawn cwd, stdin content — including the stdin `cwd` field for M83/M86 and the empty-string `command` for M81 — restricted `PATH`, `CLAUDE_PROJECT_DIR` — including unsetting it for M85–M86 — and fixture-repo git state — including a corruptible `origin/{branch}` ref file for M77 and a second guarded-directory-free fixture repo for M86 — are all controllable), per REQ-GUARD-05's test-environment requirements.
...
| Finding | Resolution |
|---|---|
| SE F3-01 / TE F3-01 (Medium, shared — the fully static `mv`-overwrite of an existing guarded file was undecided: the v1.2 `mv` flow fell through to ALLOW while a literal D2 reading blocked, no matrix row arbitrated, no RR entry registered the allow — contradicting the step-3 overwrite-is-deletion rationale that justifies M82 and the M14/M16 truncation family) | **Decided: BLOCK.** FSPEC-GUARD-02 step 4 gains the **destination-destruction test** (source guardedness irrelevant): a static resulting path — or static destination — that resolves to an **existing** guarded file in an unverified guarded directory blocks with the affected feature's G-state reason code; the *existing* qualifier preserves M11/M52 (their resulting paths name no existing file). Step 5 explicitly scoped to guarded sources, with unguarded-source fall-through-to-ALLOW stated. BR-02-1 now states which reading governs: the `mv` flow, not plain D2, is the decision procedure for `mv` operands (M11 forecloses the literal D2(i) reading), and step 4 is the only destination-guardedness rule. REQ v1.6 (same commit, per TE F-09): `mv` rule pair + decision-table row + AC; matrix row **M84** (`mv /tmp/notes.md docs/f/CROSS-REVIEW-x.md` → BLOCK `NOT_COMMITTED`), added to M33's G6 re-run set (destination Verified → destruction test unmet → ALLOW) |
| SE F3-02 (Low — G-DP1's neither-signal tertiary branch had no discriminating row; M66/M80's fixtures never stated `CLAUDE_PROJECT_DIR`, risking degeneration to single-root tests) | M66/M80 fixture text (REQ rows and § Acceptance Tests) now states `CLAUDE_PROJECT_DIR` = fixture repo root, preserving the root-(B)-alone discriminating purpose; the offered neither-signal row was cheap and is added as **M85** (no stdin `cwd`, `CLAUDE_PROJECT_DIR` unset, process cwd = fixture repo, `rm docs/f/CROSS-REVIEW-x.md` → BLOCK `NOT_COMMITTED`) — an implementation that blocks `NO_REPO`, or allows, whenever both signals are absent fails it; cited in FSPEC-GUARD-01 step 8 and FSPEC-GUARD-03 G-DP1 |
| TE F3-02 (Low — the unified enumeration-anchor fallback (`rev-parse` of the root G-DP1 tested) had no discriminating row; a process-cwd-rev-parsing implementation passed all 83 rows) | Matrix row **M86**: `CLAUDE_PROJECT_DIR` unset; stdin `cwd` = fixture repo (unverified guarded `docs/f`); process cwd = a **second** guarded-directory-free git repo; `rm docs/f/CROSS-REVIEW-x.md` → BLOCK `NOT_COMMITTED` — the wrong-anchor implementation enumerates the second repo, finds no guarded directories, and allows via BR-01-2; cited in step 8. M85–M86 both join M33's G6 re-run set |
| TE F3-03 (Low — REQ-GUARD-01 step-4 prose still scoped the union rule to "segment 1's" static operands, diverging from M80 and FSPEC step 6) | REQ v1.6 step 4 reworded: the union applies to static relative operands and globs in **every** segment — segment position never changes the resolution root until a `cd` segment updates it; M80 cited in the sentence |

---

## Reviewer Findings Disposition (v1.3 → v1.4)

Source reviews: **TSPEC iteration-1 cross-reviews** (PM and TE on TSPEC v1.0) — this revision resolves the findings those reviews routed to REQ/FSPEC ownership; TSPEC-internal findings are se-author's. REQ v1.7 lands in the same commit (TE F-09 process rule: every behavior decision below arrives with its Canonical Matrix amendment).

| Finding | Resolution |
|---|---|
| PM TSPEC F-01 (High) + TE TSPEC F-03 (Medium) — TSPEC step 2b changed the reason code of the indeterminate-source × static-existing-guarded-destination `mv` cell, contradicting this FSPEC's step-3 "unconditionally" gloss and the REQ `mv` table | **Hoist adopted upstream.** `mv` flow gains **step 2b** (static-destination destruction test ahead of the D3 jump — source guardedness and determinacy irrelevant); step 3 reworded: every operand pair reaching it is genuinely unknowable, so D3 never masks a G-state code; step 4's literal-destination arm noted as exhausted by 2b (retained for the trailing-slash corner); BR-02-1 and the FSPEC-GUARD-06 table updated. REQ v1.7: `mv` table carve-out row + AC + REQ-GUARD-07 annotation; row **M87** pins it |
| PM TSPEC F-02 (Medium) + TE TSPEC F-03 — T-01–T-03 held as a parallel TSPEC-owned row namespace | Promoted to the REQ Canonical Matrix as **M87–M89**, with the TSPEC-specified G6 re-run membership folded into M33's expansion set (extended to M84–M90); § Acceptance Tests updated. TE F-09 honored explicitly — the durable oracle stays single and REQ-owned |
| PM TSPEC F-04 (Medium) — TSPEC's `N>` destructive classification widened the closed BR-02-3 enumeration | REQ v1.7 **extends the enumeration**: `N>` for any fd digit-string joins the truncation family (`3>` truncates identically to `2>`; a digit-boundary allow would be incoherent with the M14/M16 family). Redirection rule 2 updated; the family stays closed by rule — `N>>` append and `N>&M` fd-forms remain outside; row **M90** pins the block, so BR-02-3's closed-enumeration discipline is preserved with the TSPEC's classification now sanctioned |
| PM TSPEC F-05 (Medium) — `echo $(rm docs/f/…)` command-substitution payload bypass unregistered | REQ v1.7 Residual Risk Register gains **RR-6** (D1-opacity rationale; deletion-operand substitutions remain D3 per M20). No FSPEC flow change — the allow already follows from step-2/D1 scoping |
| PM TSPEC F-07 (Low) — `pushd`/`popd` same-call drift and degraded `\/`-escaping accepted only in TSPEC prose | REQ v1.7: **RR-5 extended** (same-call directory-stack drift) and **RR-7 added** (degraded `\/` under-match, cross-referenced from REQ-GUARD-06); FSPEC-GUARD-04's truncation-family parenthetical aligned to the `N>` spelling |
| PM TSPEC F-03 (Medium) / TE TSPEC F-10 (Low) — `PARSE_ERROR` catch-all outside its catalog condition | Upstream answer recorded in REQ v1.7: the reason-code catalog stays closed and unchanged; the TSPEC-side fix (conformant message or routing) is se-author's |

---

## Open Questions

None. All reviewer questions across four REQ iterations are answered inline in REQ v1.7 (including TSPEC-iteration-1 PM Q-01, answered by the v1.7/v1.4 touch itself), the three v4 carry-forwards are resolved above, and all three FSPEC review iterations are fully dispositioned (SE Q-01 answered in the v1.0 → v1.1 disposition table; no iteration-2 or iteration-3 questions). Behavior is fully specified by the flows in this document plus the Canonical Matrix M01–M90 / S01–S07.
...
| 1.3 | 2026-07-02 | Addressed all iteration-3 FSPEC findings (SE: 1 Medium + 1 Low; TE: 1 Medium + 2 Low — the Medium shared): `mv` flow step 4 gains the **destination-destruction test** — a static resulting path or destination that resolves to an existing guarded file in an unverified guarded directory blocks with the G-state reason code, deciding the fully static overwrite cell as BLOCK, symmetric with M82 and the M14/M16 truncation family; step 5 scoped to guarded sources; BR-02-1 states the `mv` flow (not plain D2) governs `mv` operands (SE/TE F3-01). G-DP1's neither-signal branch and the enumeration-anchor fallback each gain a discriminating row (SE F3-02, TE F3-02); M66/M80 fixtures state `CLAUDE_PROJECT_DIR` = fixture repo root (SE F3-02). Matrix rows M84–M86 added to the REQ Canonical Matrix v1.6 in the same commit (M84 static `mv`-overwrite; M85 neither-signal G-DP1 discriminator; M86 enumeration-anchor discriminator); M33 G6 re-run set extended with M84–M86; REQ-GUARD-01 step-4 union prose aligned to every-segment application in REQ v1.6 (TE F3-03) |
| 1.4 | 2026-07-02 | TSPEC iteration-1 review deltas (REQ/FSPEC-owned findings: PM F-01/F-02/F-04/F-05/F-07, TE F-03; PM F-03/TE F-10 answered upstream by keeping the reason-code catalog closed): `mv` flow gains **step 2b** — the static-destination destruction test ahead of the D3 indeterminacy jump (source guardedness and determinacy irrelevant), adopting the TSPEC hoist upstream so the FSPEC step-3 gloss and REQ table are true again for every reachable cell; step 3/4, BR-02-1, and the FSPEC-GUARD-06 table aligned. Redirection rule 2 extended with the `N>` fd-digit truncation family per REQ v1.7's closed-enumeration extension. TSPEC rows T-01–T-03 promoted as REQ matrix rows **M87–M89** and the `N>` pin added as **M90** (REQ v1.7, same commit, per TE F-09); M33 G6 re-run set extended with M87–M90; § Acceptance Tests updated to M01–M90 with per-row G6 rationale |
