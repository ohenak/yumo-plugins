...
Functional specification for the fail-closed rewrite of `guard-harvest-before-delete.sh` and the anchored `Scope:` detection in `check-scope-field.sh`. Source: [REQ-harden-harvest-guard.md](REQ-harden-harvest-guard.md) **Version 1.6** (v1.3 approved with minor changes by SE and TE iteration-4 reviews — the three minor carry-forwards are resolved in this document, see § Carry-Forward Resolutions; v1.4 was the matrix extension driven by this FSPEC's iteration-1 review; v1.5 adds rows M80–M83 from the iteration-2 review and names the eager repo-check position in REQ-GUARD-01's decision-rule preamble; v1.6 — landed in the same commit as this revision — adds rows M84–M86 and the `mv` destination-destruction rule from the iteration-3 review. Note: v1.4's original "no requirement semantics changed" framing was inaccurate and is corrected in v1.5 — rows M75–M76 flipped a v1.3-ALLOW command class to BLOCK `NO_REPO` under the eager repo-check ordering; SE FSPEC-v2 F2-03).
...
**Acceptance-test oracle:** the REQ's **Canonical Block/Allow Matrix (M01–M86, S01–S07)** is the acceptance-test suite for every flow below. This document does **not** duplicate the matrix — each FSPEC section cites its rows. Matrix rows are REQ-owned: the rows this FSPEC's authoring and review produced (M66, closing TE v4 F4-01; M67–M79 and S07, closing the iteration-1 FSPEC findings; M80–M83, closing the iteration-2 findings; M84–M86, closing the iteration-3 findings) are promoted into the REQ matrix (v1.4, v1.5, and v1.6, each in the same commit as the FSPEC revision that produced it) rather than held here — per the TE F-09 process rule that behavior-defining resolutions must amend the matrix, not just prose. § Acceptance Tests retains M66's full acceptance statement.
...
| FSPEC-GUARD-02 | `mv` resulting-path rule and redirection classification | REQ-GUARD-02 | M09–M16, M52–M54, M56, M61–M62, M67, M74, M79, M82, M84 |
...
REQ-GUARD-05 (tests) has no behavioral flow of its own; it binds the matrix carried forward in § Acceptance Tests, including the FSPEC-driven extension rows M66–M86 and S07.
...
- BR-02-1: The `mv` decision is a rule pair over the resulting path — the step-4 destination-destruction test, then the step-5 same-feature-subtree + basename-pattern rule — not a case list; the REQ's `mv` table rows are consequences of it. **This flow, not plain D2, is the decision procedure for `mv` operands:** a guarded static source renaming in place ALLOWs (M11) where a literal D2(i) reading would block, and step 4's destruction test is the only destination-guardedness rule (SE/TE F3-01).
...
| `mv /tmp/notes.md docs/f/CROSS-REVIEW-x.md` (static **unguarded** source; static destination = an **existing** guarded file) | Steps 2–3 miss (all static, source unguarded); step 4's destination-destruction test fires → BLOCK `NOT_COMMITTED` — the statically provable overwrite blocks exactly as its indeterminate twin (M82) and truncation of the same file (M14) do (M84) |
...
| D2 hit (including the FSPEC-GUARD-02 `mv`-flow static tests — step 2 static-source and step 4 destination-destruction); feature state G2/G3/G5/G8/G9 | `NOT_COMMITTED` |
...
The acceptance-test suite is the REQ v1.6 **Canonical Block/Allow Matrix** — deletion-guard rows **M01–M86** and scope-check rows **S01–S07** — carried forward by reference, under REQ-GUARD-05's one-test-per-row obligation (bulk row M33 expands to one asserting test per referenced row; parameterized acceptable).

Matrix rows are **REQ-owned**: the twenty-two rows this FSPEC's authoring and reviews produced — **M66** (CF-3, TE v4 F4-01), **M67–M79, S07** (iteration-1 FSPEC findings, REQ v1.4), **M80–M83** (iteration-2 FSPEC findings, REQ v1.5), and **M84–M86** (iteration-3 FSPEC findings, REQ v1.6, same commit as this revision) — live in the REQ matrix. M33's G6 re-run expansion set is extended there to include **M66, M67, M73, M74, M79, M80, M82, and M84–M86** (under G6 each resolves against a *Verified* guarded directory → ALLOW; for M82, D3(b) is unmet once no unverified guarded directory exists, so the `mv` falls through to ALLOW; for M84, the step-4 destruction test is likewise unmet — no unverified guarded directory — and the unguarded-source `mv` falls through to ALLOW). The degraded rows M68–M72, the non-repo rows M75–M76, the mixed-root row M83 (root (A) is non-repo regardless of LEARNINGS state), the contract rows M77–M78, and the contract-boundary allow row M81 are git-state-independent or non-G6-constructible and are deliberately outside the re-run set.
