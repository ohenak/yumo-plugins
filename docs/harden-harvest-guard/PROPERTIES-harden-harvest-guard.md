...
> New/rewritten production modules **must** meet the pdlc DoD branch-coverage criterion (≥ 85%). **Measurement decision:** jest/istanbul instruments JavaScript only, and the production deliverables here are a bash script plus a Python program embedded in a quoted heredoc. A numeric branch-coverage figure is *not* strictly impossible — it is rejected on concrete grounds, not an absolute:
> - **bash is unreachable by the installed toolchain.** nyc/istanbul cannot see shell; `bashcov`/`kcov` would produce a bash figure but are **new non-npm toolchain dependencies** the repo does not carry and DEC-02's stdlib-/repo-tooling-only floor rejects. stdlib `trace` is line-only, not branch.
> - **the embedded Python could be measured, but only via measured-copy indirection.** `coverage run --branch` over a checksum-asserted heredoc extraction (verbatim copy → instrument → replay self-test + MATRIX payloads) would yield a branch %. It is rejected because it measures a *copy* the deployable never executes (the number attaches to extracted-`guard_parser.py`, not the shipped heredoc), and it introduces coverage.py + an extraction/replay harness as new deps — reintroducing exactly the split-artifact indirection DEC-01/DEC-02 chose the single-file heredoc to avoid.
>
> The criterion is therefore discharged by a **structural proxy**, asserted mechanically where possible:
>
...
>
> **Honest limitation (input to dod-verify, with documented waiver rationale):** no numeric branch-coverage percentage is produced for the two shell deliverables without adding dependencies and measured-copy indirection this feature's architecture decisions (DEC-01/DEC-02) deliberately reject; the proxy above is the coverage evidence offered in lieu of a number. The following surfaces are **known-untested by decision**: the wrapper's no-interpreter `--self-test` branch (three lines, unreachable from production — PLAN TE Q-02 accepted); the Python top-level internal-error catch-all (no constructible trigger; message contract carried by M41's template assertions); `check-scope-field.sh`'s grep-unavailable degradation beyond Δ4's stdout-silence check. This section states the waiver rationale for the verifier to weigh against the diff; it does not instruct the verifier to suppress a finding — dod-verify remains free to flag the missing number, and the harvest signal below records that the pipeline's coverage criterion lacks a sanctioned path for non-instrumentable shell artifacts. The JS test helpers (`guardFixtures.js`, `guardRowIds.js`) are test infrastructure, not production modules — the criterion does not apply to them.
>
> **Harvest signal (Process):** the pdlc DoD branch-coverage criterion (dod-verify criterion 4) assumes an instrumentable artifact. It has no defined discharge path for bash / heredoc-embedded deliverables. This waiver is a per-feature workaround; the durable fix belongs in the dod-verify skill (a sanctioned "non-instrumentable module" evidence class).
...
| 1.1 | 2026-07-02 | Address PM/SE iteration-1 review: Δ4 BASH_ABS spawn for degraded scope case (SE F-01); Δ6 P-SORT-01 mixed-state sorted-order discriminator with G4-origin builder discipline (SE F-02); Δ1–Δ6 landing-task bindings + Δ1/Δ5 reclassified as upstream-mechanism supersessions (SE F-04); PROP-COV-01 waiver re-grounded on real constraints (new deps + measured-copy indirection, not an impossibility absolute) and re-framed as input-to-dod-verify with a Process harvest signal (SE F-03, PM F-02); trace corrections — allow-silence source = TSPEC §§ 3.2–3.3 not FSPEC BR-01-4, M65/M50 remainder-class nits (SE F-05, PM F-01) |
