...
| Cross-Reviews | `CROSS-REVIEW-{software-engineer,test-engineer}-REQ-v{1..12}.md` (24 files, this branch) |
...
|---|---|---|---|---|
| pdlc | **approved (product scope)** | Claude + operator | 14.0 | 2026-07-28 |
...
today, "managed manually" (both orchestrator SKILLs say so). A workflow improvement can be
...
This REQ closes A′→B (REQ-DIST-06: build to a shippable path and ship it) and B→C (drift detection
+ explicit sync), and retires the legacy consumer `.js` copies. A→A′ is already closed by
`build-runtime.mjs --check` and `__tests__/runtimeBundle.test.js`. Refreshing B from the
marketplace is Claude Code's plugin updater and out of scope (D-DIST-05).
...
- **AC-6.4 — No document contradicts the manifest.** Who: the maintainer. Given the `.js` →
  `.bundle.js` migration, When the feature lands, Then a test asserts
  `coveredViolations(repoRoot) == ∅`, where the covered set is computed, not hand-listed:
  `grep` of four literal qualifier-free patterns (the two
  `.claude/workflows/orchestrate-{dev,queue}.js` forms, `.claude/workflows/*.js`, the phrase
  `managed manually`) minus a four-member path-rule exemption (generated trees; per-feature
  `docs/` artifact dirs; any `distribution-manifest.json`; any `__tests__/`). The checker is a
  pure function of a root directory. A false positive on `managed manually` is resolved by
  rephrasing the document — narrowing a pattern requires changing this AC in the same commit.
  Measured covered set today: 5 files; the three already-correct normative documents get their
  `dist/` path updates as ordinary in-scope edits. *(P0)*
...
| `sync-workflows.sh` exits | `0` verified green · `1` sync-fixable drift · `2` unverified provenance · `3` unverified/unresolved (incl. no write target) · `4` write attempted and failed |
...
- **NFR-2 — Latency observed, not gated.** §4's budget is a maintainer-owned observation; no
  test asserts wall-clock time. Anyone tempted to write a timing test should read this instead.
...
- `__tests__/runtimeBundle.test.js`: freshness repointed at `dist/` (AC-6.3); packaging oracle
  (AC-6.2); document-contradiction oracle (AC-6.4); fresh-clone bootstrap (AC-6.5).
...
- Execute bits (index + worktree) on this feature's two scripts **and** the three existing
  sibling hook scripts (class fix, §0 fact 11).
...
- Document corrections: whatever `coveredViolations(repoRoot)` returns (5 files today) plus
  `dist/` path updates to the three already-correct normative documents. Archived per-feature
...
| US-04 | REQ-DIST-00 (0.1, 0.2, 0.7), REQ-DIST-04 (4.1–4.3), REQ-DIST-06 (6.1–6.5) |
...
| O-7 | SE v12 F-06, TE v12 F-06 | TSPEC | If a trace seam is specified: pin delimiter and quoting; state whether non-row probes (manifest, sync manifest, `pdlc.config.json` reads) are traced |
...
archived in REQ v13.0 §10 (`9b66cdb`) and the 24 `CROSS-REVIEW-*` files on this branch. Phase R
