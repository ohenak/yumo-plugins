...
| # | File | Edit | Oracle (asserted in `documentOracles.test.js` by T-02, red until L-06) |
|---|---|---|---|
...
- The risk PM F-04 names is real but is a **queue-time** risk, not a landing-time one: a future
  ...
  **`docs/_queue/QUEUE.md` is §6.2 row 1 and *is* corrected by L-06**, and this PLAN adds a `Notes`
  line to both queue rows recording that their REQs predate `dist/` and must be re-read against
  `pdlc/README.md` before pickup. `orchestrate-queue`'s Phase-0 readiness triage reads the queue
  row, so the warning lands where it is acted on.
...
| **PQ-1** — the two pending-queue REQs (`pdlc-consolidation-agent`, `pdlc-review-loop-hardening`) reference `.claude/workflows/` and are exempt (ii); their REQs are **not** edited by this feature | §6.3 (PM F-04) | **L-06** — adds a `Notes` line to both `docs/_queue/QUEUE.md` rows (QUEUE.md is §6.2 row 1, already in L-06's work list) recording that the REQ predates `dist/` and must be re-read against `pdlc/README.md` before pickup |
...
- [ ] `CLAUDE.md` and `pdlc/README.md` document the two-command bootstrap and the manual-worktree limitation; **§6.3's D-1, D-2 and D-3 each meet their stated oracle** (the half of REQ §6's document-corrections bullet `coveredViolations` is blind to — PM F-04); `docs/_queue/QUEUE.md` carries the PQ-1 `Notes` lines.
