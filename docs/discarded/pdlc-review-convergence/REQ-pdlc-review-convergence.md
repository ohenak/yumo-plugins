...
>    *(Superseded in v1.8: the delete branch is withdrawn at every `H − A` — SE v9 G-26.)*
...
> finding — the third consecutive round of that — and every round-6 finding lies in text v1.4 added,
> seven of the **eight** in the two mechanisms v1.4 introduced. Round 6 produced eight findings, not
> seven: SE v6 filed four Medium and one Low, TE v6 filed one High, one Medium and one Low (SE v7 MF-1,
> TE v7 MF-19). §10.10 maps each one. Five things changed:
>
> 1. **Every halt writes a reset region, including the halt that creates the post-mortem (TE F-01,
>    High).** v1.4 scoped AC-1.4's clauses to *"every halt that finds an existing post-mortem"*, which
>    left the first halt of a phase governed by nothing: the file carried no `## Reset Region`, so
>    `H = 0`, so `A < H` was false, so the operator's **first** clearance granted no window and the phase
>    halted again on the budget path. AC-1.4 clause 1 is now stated over **every** halt — create, or
>    preserve-and-append — under one rule, because O-5's captured region of a non-existent file is the
>    empty region. `H` is exactly the number of halts taken, on every path.
> 2. **The counting rule validates the relation it rests on, and a corrupt region spends nothing
>    (SE G-14, TE F-02).** AC-1.5(4) gains a counts check — `H − A ∉ {0, 1}` ⇒ corrupt ⇒ `W` = 1, no
>    grant — because v1.4's algorithm rejected a bad *value* but passed a region with two halts and no
>    answer, which handed out `H − A − 1` unpaid windows: fail-**open**, on the budget this REQ exists to
>    make absolute. Validation is now a conjunct of the grant gate, so a region that fails it consumes no
>    clearance; the operator's sanctioned repair is stated; and the notice AC-1.5(4) has always required
>    gains a catalogue id (**S-16**), a closed reason enum, an AC-4.7 precedence slot and a §6 row.
> 3. **The answering lines are appended (SE G-13).** Step 2's validation reads *"every value before
>    it"*, so it is order-sensitive; a prepending implementation puts `WINDOW-RESUMED: 4` ahead of the
>    `WINDOW-START: 4` it answers, fails validation, and locks `W = 1` permanently — closed, but
>    absorbing. Document order is now event order for every line in the region, by the same rule AC-1.4
>    gives `HALT-REASON:`.
> 4. **The verifier is given its window rather than asked to derive it (SE G-16).** AC-3.2's required
>    `## Disposition` rows are a function of `W`, and `W` lives behind AC-1.5(4)'s validation and the
>    directory listing. The loop now passes the inclusive round range `{W … N−1}` in the dispatch; `W`'s
>    single reader stays the loop; O-3 and O-9(c) carry it.
> 5. **A `## Verdict` section with no `VERDICT: ` line is classified, and it is classified as HEAD
>    classifies it (SE G-15).** AC-2.7 gains the row ⇒ *malformed* — `parseVerdict`'s `malformed: true`
>    fallback, which is a different object from the genuine `0/0/0` truncated-output return — and AC-3.4
>    step 1, which had said *unavailable*, is corrected to match. The table is also stated as read in
>    order, and both normative clauses now write the **trailing space** into `VERDICT: `.
>
> Also: AC-1.5(5)'s receive-side table drops the *absent* row that its own gate makes unreachable
> (TE F-03); AC-1.4's strip is scoped to **unfenced** `RESOLVED:` lines, so the document has one scoping
> rule (SE G-17); `HALT-REASON:` gets its own catalogue id **S-15**; AC-2.6's reachable-sequence table is
> restated over `W`, `W+1`, `W+2` in its cells and not only in its lead-in; and §10.9's heading names its
> non-finding rows. Nine mechanical fixes applied (SE MF-1 … MF-5, TE MF-12 … MF-15). No citation in this
> document changed baseline: every row still resolves at `9486c81`, and the citations v1.5 adds
> (`:900-903`, `:906`, `:415-422`, `:424-428`, `:394-400`, `:451`, `:417`, `:902`, `:953-958`, `:569`)
> were read there in one pass.
...
| **reset region** | The section headed exactly `## Reset Region` (S-12) in `POSTMORTEM-{phase}-{feature}.md`, from that heading to the next top-level heading or end of file, **outside any fenced block** — the same scoping rule `scanLines` already applies. It is the only place `HALT-REASON:` (S-15), `WINDOW-START:` (S-13) and `WINDOW-RESUMED:` (S-14) lines are read from, so a line quoted in a post-mortem's prose or Recommendation counts for nothing (TE v5 Q-07). Every line in it is **appended at the end** by the event that writes it, so document order is event order (AC-1.4 clause 1, AC-1.5(4)). It is machine-written and machine-maintained (AC-1.4) — except for the sanctioned operator repair AC-1.5(4) describes **per reason**, taken only when the run report emits S-16: a value reason is repaired by **correcting** the named line, which is the only sanctioned value repair (deleting an answering line is forbidden at every `H − A`, SE v9 G-26), and `counts-mismatch` by deleting the whole section, which S-12 reads as the empty region (SE v6 G-19, TE v6 F-02, SE v8 G-24). The operator's `RESOLVED:` marker is never **counted**, wherever in the file it sits: that is a fact about the readers, not a placement constraint on the operator, and AC-1.4 clause 2 strips an unfenced marker wherever it sits (TE v6 MF-15). |
| **zero-delta** round | A round `N > W` — i.e. not the first round of the current window — whose reviewed document is byte-and-hash identical to the document reviewed at round N−1 — `bytes(t0 of round N) = DOC-BYTES(N−1)` **and** `sha256(t0 of round N) = DOC-SHA256(N−1)`. It is not a small revision; it is *no revision*, and it is a halt (AC-2.8), not a consumed round. |
| **unavailable** | A quantity that no reader can obtain from the branch — distinct from **malformed**, which is a quantity that was read and could not be parsed, **or that the structure carrying it says should be there and is not**. The second half of *malformed* is what AC-2.7 row 3 is stated over: a `## Verdict` section with no `VERDICT: ` line at all has read the structure and found the quantity missing from it, and the shipped reader returns `parseVerdict`'s `malformed: true` fallback for exactly that input — so it is *malformed*, not *unavailable*, and this definition is amended to say so rather than leaving a reader who reasons from §5 with the opposite answer (TE v6 F-04). *Unavailable* is the case where the **carrier** is absent: no file, or no `## Verdict` heading. Both break AC-2's comparison chain, and the run report distinguishes them (AC-2.3, AC-2.7). |
| **phase refusal** | A decision **not to enter** the phase, taken before any round of it opens. No halt is recorded, so the halt accounting is untouched (`H` and `A` both unchanged) and no post-mortem byte is written; the phase does not run and the invocation terminates on the same path step G takes for an unresolved post-mortem (AC-1.4, AC-1.5(4) step 4). It is **not** the *approval refusal* of AC-3.5 and AC-3.2, which is the opposite shape: there the round has already run, the loop records the refusal, the round remains owed an authoring pass, and **the window proceeds**. The two are distinguished by name wherever either is used, because a reader who applies one definition where the other is meant gets the wrong machine (SE v8 G-25). The full consequence set is AC-1.5(4) step 4's, and it includes the two invocation-level effects this row previously omitted: the phase's report row is **❌**, and the feature's `docs/_queue/QUEUE.md` row is rewritten to `halted` and committed on the shipped halt path (TE v9 MF-24). |
...
| **Which halt a POSTMORTEM records** | AC-1.5(4), AC-1.5(5), AC-2.8 | The **last** `HALT-REASON: {string}` line in the reset region (S-15) — one line per halt, on **every** halt including the one that creates the file, **appended to the end of the region**, so document order is halt order | Read as a convergence halt (S-3/S-4) — fail-closed, so an unreadable reason never converts a consuming reset into a free one |
...
before FSPEC authoring**. This REQ introduces **seventeen**. All seventeen are fixed here — id, exact
emitted form, emitter, receiver, and the receiver's behaviour on **every** input outside the catalogue.
FSPEC may not add an eighteenth without amending this table.
...
Ids are stable once assigned, so the ones added in v1.2, v1.4, v1.5 and v1.6 keep the next free numbers;
the table is therefore ordered **by kind** and not by id, so `S-10` … `S-17` sit beside their kin rather
than at the end. The six kinds, in the order the rows appear: **anchor lines** (S-1, S-2, S-10),
**halt reasons** (S-11, S-3, S-4), **report notices** (S-5, S-6, S-16), **section headings and fields**
(S-7, S-8, S-9, S-12), **reset-region lines** (S-15, S-13, S-14), **dispatch inputs** (S-17) — SE v4
MF-3. The S-16 row now sits with S-5 and S-6 rather than between two reset-region rows, so the
kind-adjacency this clause claims is true of the table as written (SE v6 MF-3).
...
receiver and a fail-closed receive side — all four columns — so v1.4 gave it a row and a §6 entry, and
v1.5 gives it its **own catalogue id, S-15**, rather than nesting it inside S-12's *Exact string* cell
alongside a section heading: S-13 and S-14 are one literal each and this line is no different (SE v5
MF-2, TE v5 F-06, SE v6 MF-4).
...
| **S-16** | Report notice `reset-region-corrupt: {reason} (H={h}, A={a}) {path}` where `{reason}` ∈ `{invalid-window-start, invalid-window-resumed, counts-mismatch}` — a closed three-member enum — `{h}` and `{a}` the two counts as decimal integers, and `{path}` the post-mortem's repo-root-relative path. On the two value reasons the offending **line** — the whole line as it appears in the region, not the value alone — follows the path in square brackets: `reset-region-corrupt: invalid-window-start (H=2, A=1) docs/f/POSTMORTEM-R-f.md [WINDOW-START: 99]`; on `counts-mismatch` there is no such suffix, because the evidence is the pair. **Exactly one S-16 notice is emitted per entry**, whatever the region's fault count: the reported `{reason}` is the **first** failing line in document order, and `counts-mismatch` only when every line passes step 2 (SE v8 G-22) | AC-1.5(4) step 4 | the run report row of AC-4.7, in the `notice` column | the enum is closed here; a reason outside it is a defect, not a fallback. The render is fixed **here and only here**, character for character, on the same footing as every sibling notice, because AC-4.7's bar is that a test author derives the exact cell from this document alone (TE v6 MF-17). The notice is the operator's only signal that the region needs the sanctioned repair AC-1.5(4) describes, so a corrupt region is diagnosable rather than a silent permanent halt (TE v6 F-02) |
...
| **S-12** | Section heading `## Reset Region`, exactly | every halt path (AC-1.4) — the first halt of a phase **creates** it, every later halt preserves it | AC-1.5(4)'s window-origin and counts resolution; a human reading the post-mortem | The heading is read outside fenced blocks (`scanLines`). An absent `## Reset Region` heading, or a present one containing no `HALT-REASON:` line, is read as an empty region: `H = A = 0`, `W = 1`, no reset in effect and no clearance outstanding. Every other input is a set of S-13/S-14/S-15 lines, which AC-1.5(4)'s ordered algorithm is total over |
| **S-15** | `HALT-REASON: {value}` — one line per halt, appended at the **end** of the reset region. `{value}` is the **`; `-joined render, in AC-4.7's precedence order, of every halt reason that halt raised** — `no-revision: …` alone, or `fixed-point: …`, or `budget-exhausted: …`, or `fixed-point: …; budget-exhausted: …` | every halt path (AC-1.4 clause 1, AC-1.5(5)) | AC-1.5(5)'s clearance decision; the `H` count; a human reading the post-mortem | AC-1.5(5) states all three reachable cases of the **last** such line: begins `no-revision:`; begins `fixed-point:` or `budget-exhausted:`; unparseable or anything else — the last treated as a convergence halt, fail-closed. *Absent* is not a case at that gate (it requires `H ≥ 1`); the empty region is S-12's case (TE v6 F-03) |
| **S-13** | `WINDOW-START: {N}` — one line, `{N}` a decimal integer ≥ 1, **appended to the end** of the reset region | the loop, on the entry that grants a convergence-halt clearance (AC-1.5(4)) | AC-1.5(4)'s window-origin resolution; the `A` count | AC-1.5(4)'s ordered algorithm is total over every region: any line that fails validation, or counts with `H − A ∉ {0, 1}`, ⇒ `W = 1`, fail-closed, no clearance consumed; otherwise `W` is the greatest value. The append is normative, because step 2's validation reads *"every value before it"* (SE v6 G-13) |
| **S-14** | `WINDOW-RESUMED: {W}` — one line, `{W}` a decimal integer ≥ 1 equal to the origin then in effect, **appended to the end** of the reset region | the loop, on the entry that clears an S-11 halt (AC-1.5(5)) | the `A` count; AC-1.5(4)'s validation | same algorithm and same append rule: a `WINDOW-RESUMED:` value that is not a decimal integer ≥ 1, or that does not equal the resolved `W`, ⇒ `W = 1`, fail-closed. It never moves the origin — it only answers a clearance, which is exactly what distinguishes resuming from resetting |
| **S-17** | Dispatch input `REVIEW-SCOPE-ROUNDS: {W}..{N−1}` — one line in the verifier's dispatch prompt, two decimal integers ≥ 1 separated by the two-character literal `..`, `{W}` ≤ `{N−1}`, naming the **inclusive** round range whose findings AC-3.2(1) puts in scope. The separator is `..` because AC-6.4 already fixes a range separator for this document and S-4 already renders a round range that way; an en dash, a hyphen or the set notation `{4 … 6}` is **not** this line | the loop, on every verifier dispatch (AC-3.2) | the verifier SKILL's disposition-check contract (O-9(c)) | AC-3.2's receive-side bullet for the range states all four non-canonical inputs — absent, empty, unparseable, and endpoints with `{W}` > `{N−1}` — under one behaviour: the verifier does **not** guess and does **not** fall back to the whole branch history; it omits `## Disposition` entirely and says which input was missing, which lands in AC-3.2's `## Disposition` receive-side table, first row — the already-stated absent-`## Disposition` case — so **approval** is refused (an approval refusal, not a phase refusal) without halting (SE v8 MF-22). New in v1.6 (TE v6 F-03) |
...
`notice` a **possibly-empty list** of S-3 … S-6, S-11 and S-16 in the precedence order AC-4.7 fixes. O-8
...
4. **the reset is anchored and consumed, in the POSTMORTEM, by the loop.** The **reset region** (§5) is
   ...
   **Step 2's range check is re-evaluated on every read, against the current listing**, not fixed at
   write time: *"one past the highest round on the branch"* is a predicate over mutable branch state by
   design, because the region is only meaningful alongside the rounds it describes. `harvest-learnings`
   deletes `CROSS-REVIEW-*` and `POSTMORTEM-*` together, so the ordinary path never sees a region
   outliving its rounds; any sequence that removes the cross-reviews while the post-mortem survives turns
   a previously-valid region invalid, which lands in the fail-closed case above — reported as S-16, with
   the sanctioned repair available and no clearance spent (TE v6 Q-10).
...
SKILL writes the heading (already in the file contract) without the trailer this REQ adds. **§5's
definition of *malformed* is amended to cover it** — a quantity the structure carrying it says should be
there and is not — so a reader who reasons from the definition and a reader who reads this row reach the
same answer, which is the document's own convention about where the authority lives (TE v6 F-04). Rows 1
and 2 are the *unavailable* cases by the same split: there the **carrier** is absent, not the quantity.
...
|---|---|
| `round` | **one past the highest round on the branch.** The definition is the whole rule, and it is total: on a *mid-window* refusal this is also the round that would have opened, but on the canonical exhausted-branch fixture **no** round would have opened — `W` = 1 admits none, which is why the entry has no round to report at all. Do not derive the cell from that gloss (SE v8 MF-21). It is derived from the directory listing alone (§5, `deriveRoundWindow`), not from `W`, which is 1 on this path by construction and says nothing about where the branch got to |
| `panel-shape` | **empty** |
| `blocking` | **empty** |
| `growth-bytes` | **empty** |
...
| `## Reset Region` | that exact heading | the halt path (AC-1.4) — **created by the first halt of a phase**, preserved by every later one | AC-1.5(4), AC-1.5(5), §5's *reset region* | S-12. The syntactic home of every line AC-1.5 counts or reads. Naming it is what stops `R`/`A`/`H` being attacker-writable by ordinary prose — a `HALT-REASON:` quoted in a post-mortem's Recommendation is outside the region and counts for nothing (TE v5 Q-07). Absent heading ⇒ empty region ⇒ `H = A = 0`, `W = 1`. v1.4 scoped the write to a halt that finds an existing post-mortem, which left the creating halt writing no region and the operator's first clearance silently swallowed (TE v6 F-01). |
| `HALT-REASON: {value}` | that exact literal; `{value}` is the `; `-joined render, in AC-4.7's precedence order, of every halt reason that halt raised. **One line per halt**, appended to the end of the reset region | the halt path — every halt, without exception (AC-1.4, AC-1.5(5)) | AC-1.5(5)'s clearance decision; the `H` count | S-15, its own catalogue id since v1.5 (SE v6 MF-4). v1.3 stated the grammar three ways — *"the same `; `-joined string"* in AC-2.2, *"the S-3, S-4 or S-11 string **verbatim**"* in §5 and AC-1.5(5) — so the line O-10 requires a test for had three candidate expectations (TE v5 F-06). One line, joined, in the report's order: the operator sees the identical string in both places, and AC-1.5(5)'s receive side reads its **leading** reason, which is exact because S-11 never co-occurs (AC-2.2). |
| `WINDOW-START: {N}` | that exact literal, `{N}` a decimal integer ≥ 1 | the loop, on the entry that grants a convergence-halt clearance (AC-1.5(4)) | AC-1.5(4)'s ordered algorithm | S-13. The durable home of the window origin `W`. Written by the loop, **never authored by a human**; it carries no authority of its own and only records where a reset the operator already granted began. That prohibition is scoped to **authoring**: it forbids the one hand-edit that could bank an unpaid window, and it does not forbid AC-1.5(4)'s sanctioned `counts-mismatch` repair, which **deletes** the whole `## Reset Region` section and can only cost windows, never grant them (SE v6 G-19, TE v6 F-02). **The *never by a human* clause is scoped to authoring, and exempts both sanctioned repairs of AC-1.5(4).** It forbids a human introducing a `WINDOW-START:` line where none stood — the one hand-edit that could bank an unpaid window. It does not forbid the `counts-mismatch` repair, which deletes the whole `## Reset Region` section and zeroes both counts, so it can only cost windows, never grant them; and it does not forbid the value repair, which **corrects in place** a line the loop itself wrote, leaving `H` and `A` untouched and therefore granting nothing that was not already paid for. Deleting a single answering line is forbidden at every `H − A`, because it lowers `A` alone (SE v9 G-26). Without this second exemption O-10's value-repair fixture — a corrected line, then a later entry that grants the window — would be unconstructible under the closed grammar (TE v9 F-02). |
| `WINDOW-RESUMED: {W}` | that exact literal, `{W}` a decimal integer ≥ 1 equal to the origin then in effect | the loop, on the entry that clears an S-11 halt (AC-1.5(5)) | AC-1.5(4)'s ordered algorithm; the `A` count | S-14. **New in v1.4.** It answers a clearance without moving the origin, which is precisely the difference between resuming an interrupted window and being granted a new one. Without it an S-11 clearance stayed unanswered forever and the next halt of any kind consumed it unattended (SE v5 G-10, TE v5 F-01). |
| `reset-region-corrupt: {reason} (H={h}, A={a}) {path}` | that exact literal and that exact render; `{reason}` ∈ `{invalid-window-start, invalid-window-resumed, counts-mismatch}`, `{h}`/`{a}` decimal integers, `{path}` repo-root-relative, and on the two value reasons a trailing ` [{line}]` carrying the offending line — **the specimen is §5's S-16 row and is not repeated here**, so the two homes cannot drift apart again (SE v8 MF-10) | the loop, from AC-1.5(4) step 4 | AC-4.7's `notice` column | S-16. **New in v1.5**, render fixed in v1.6 (TE v6 MF-17), and one notice per entry whatever the fault count — first failing line in document order (SE v8 G-22). AC-1.5(4) has required a corrupt region to be *reported* since v1.4, and the run report had no slot for it and the catalogue no member — so the one operator-visible half of the fail-closed behaviour was unassertable (TE v6 F-02). The enum names which of the two value checks or the counts check refused. |
| `REVIEW-SCOPE-ROUNDS: {W}..{N−1}` | that exact literal, two decimal integers ≥ 1 either side of the two-character `..` separator, `{W}` ≤ `{N−1}` | the loop, on every verifier dispatch (AC-3.2) | the verifier SKILL's disposition-check contract (O-9(c)) | S-17. **New in v1.6.** AC-3.2 has required the loop to pass the verifier its round range since v1.5, and the value crossed the boundary with no rendering, no catalogue id and no stated behaviour on an absent or unreadable input — the DC-01 defect the paragraph introducing it cites DC-01 to justify (TE v6 F-03). The separator matches S-4's `rounds {first}..{last}`; an en dash or set notation is not this line. Absent, empty, unparseable or `{W}` > `{N−1}` ⇒ the verifier omits `## Disposition` and names the missing input, which refuses **approval** without halting (AC-3.2). Because the emitter is the loop, a deterministic bad render recurs every round; AC-3.2 states the sequence that follows and O-10 asserts two garbled dispatches across that sequence — three rounds and one operator clearance, since the round carrying the byte-identical document halts at AC-2.8 before any reviewer is dispatched (TE v8 F-03, SE v9 G-27, TE v9 F-03). |
...
| **N-4** | Changing what a halt is. | AC-1.4: the POSTMORTEM path, the write confirmation, and the rule that **only a human ever writes `RESOLVED: yes`** are untouched, as is the shipped gate that reads it (`parseResolvedMarker` → `checkPostmortem` → the step-G refusal). This REQ changes *when* a halt happens and *what it says*. *Amended in v1.4 (SE v5 G-07, TE v5 F-02):* one thing about the marker's **lifecycle** does change — a halt that rewrites an existing post-mortem **strips** the spent `RESOLVED:` line, so the new post-mortem is unresolved on arrival and every halt demands a fresh human clearance. That is the fail-closed direction and it is what keeps the marker single-valued, which the shipped reader requires; v1.3's claim that the marker is simply "untouched" stopped being true the moment AC-1.4 made the file durable machine state. |
...
| **O-3** | Specify the verifier role's SKILL file and whether it is a new SKILL or a mode of an existing review SKILL (AC-3.7). **Its slug is not open**: §6 fixes it at `verifier`, and a rename is an amendment to §6, not an FSPEC decision. Specify also the **dispatch input AC-3.2 requires**: the loop passes the verifier the inclusive round range of the current window on one `REVIEW-SCOPE-ROUNDS: {W}..{N−1}` line (**S-17**, grammar fixed in §5 and §6), so the `## Disposition` row set is determinate without the verifier reading the post-mortem or re-running AC-1.5(4)'s validation. `W`'s single reader stays the loop (SE v6 G-16). **Neither the rendering nor the receive side is open** — AC-3.2 fixes the line's form and the one behaviour for all four non-canonical inputs (absent, empty, unparseable, `{W}` > `{N−1}`); O-3 specifies where in the dispatch the line is placed, not what it says (TE v6 F-03). | FSPEC |
...
| **O-12** | Specify where AC-2.8's byte-and-hash identity test is evaluated — after round N−1's anchors are readable and **before** round N's reviewers are dispatched — and how the S-11 halt reaches the post-mortem writer on the same path as S-3 and S-4 (AC-1.4). Specify the **single round-open read** AC-4.1 and AC-2.8 share, so the two never see different bytes. Note it must not consume a round of AC-1's budget, and that an S-11 halt cleared by the operator resumes the window rather than resetting it, writing `WINDOW-RESUMED: {W}` (AC-1.5(5)). Specify the **order of the two round-open derivations** over that shared read: whether AC-4.1 step 1's classification is computed before AC-2.8's identity test and discarded on a halt, or never computed at all. Nothing turns on it for correctness — the read is singular — but an implementer needs to know which (SE v5 G-12). | FSPEC → TSPEC |
| **O-5** | Specify where in the loop AC-2's comparison is evaluated so that it precedes the optimizer dispatch (AC-2.1), and how its halt reason reaches both the post-mortem prompt and the run report distinctly from budget exhaustion (AC-2.2). Specify the **reset-region read-modify-write** AC-1.4 now requires: before the halt dispatch the loop reads the existing post-mortem and captures its `## Reset Region` — **the captured region of a file that does not exist is the empty region**, so the first halt of a phase creates a one-line region by the same path (AC-1.4 clause 1, TE v6 F-01, SE v6 MF-3); after the write it re-applies that region — preserved `WINDOW-START:` / `WINDOW-RESUMED:` / `HALT-REASON:` lines in document order, this halt's `HALT-REASON:` appended last, any prior `RESOLVED:` line stripped — and confirms the result, reporting a lost or unwritable region rather than proceeding on a silently widened window. The region is loop-owned state, so it is **not** discharged by a prompt clause (O-9(d) is belt-and-braces); this is what removes TE v5 MR-05 from the critical path. | TSPEC |
...
| **O-9** | Write the SKILL amendments: (a) the three review SKILLs — AC-5.1, AC-5.2, AC-6.5, **and AC-3.4's requirement that the count trailer appear inside the file's `## Verdict` section as well as in the response** (the one file-grammar change, SE F-10); (b) the three author SKILLs — AC-5.3, AC-4.6, AC-6.5; (c) the verifier's disposition-check contract — AC-3.2, including the `## Disposition` section (S-8), the per-finding `New-mechanism:` field (S-9), **and the statement that the in-scope round range is supplied by the loop's dispatch on a `REVIEW-SCOPE-ROUNDS: {W}..{N−1}` line (S-17) and is not derived by the verifier** — a verifier whose range is absent, empty, unparseable or inverted does not guess and does not fall back to the whole history; it **omits `## Disposition` entirely** and names the missing or unreadable input in its findings table, which refuses approval without halting (AC-3.2), because `W` is unreadable to it (SE v6 G-16, TE v6 F-03); **(d) the post-mortem prompt — a belt-and-braces clause telling the agent that the `## Reset Region` section (S-12) is machine state and must be left alone.** At the Citation baseline that prompt is a bare `Write ${postmortemPath}.` plus a section list, built inline in **`reviewLoop`** (`pdlc/workflows/orchestrate-dev.js:1725-1730`, local `postmortemPrompt`), so nothing today tells the agent anything in that file is precious (SE v4 G-04, TE v4 F-02). **It is not the mechanism**: AC-1.4 requires the loop to re-apply the region deterministically after the dispatch (O-5), precisely so correctness does not rest on an agent's compliance (TE v5 MR-05). v1.3 cited this prompt as `writePostmortem` at `:1912-1918` — the range at `main`, and a symbol that exists at neither commit (SE v5 MF-1, TE v5 F-05). | FSPEC → implementation |
| **O-10** | Properties and tests for all six ACs, including the negative cases this REQ names explicitly: the `0 ≥ 0` non-firing (AC-2.5), the malformed-count chain break in **both** directions (AC-2.3), the *unavailable*-count chain break (AC-2.7), the unequal-panel-shape and crashed-round non-comparisons (AC-2.4), the lone-file-without-marker fail-closed (AC-3.5(b)), all **six** `REVIEW-MODE:` rows (AC-3.5(e)), the **four `DOC-BYTES:` inputs and their three reasons** (AC-4.1) with unmeasurable growth escalating rather than degrading (AC-4.5), a missing `## Disposition` refusing approval without halting (AC-3.2), and AC-6.4's C-3/C-4/en-dash/unparseable citation forms each being *reported* rather than skipped or fatal. Added in v1.2: **the crashed-round report row**, whose `notice` cell carries three co-occurring notices in AC-4.7's stated precedence order (TE F-04); **a failed verifier round comparing as `{verifier}` and not as *crashed*** (AC-2.4, SE F-02); **the zero-delta halt** and each of AC-2.8's three non-halting inputs, including the fail-open on an absent `DOC-SHA256:` (TE F-07); **an absent, unparseable and duplicated `WINDOW-START:` each yielding no reset** (AC-1.5(4)); **an anchor line following `VERDICT:` reading as *unavailable*, not *malformed*** (AC-2.7); and **AC-6.4's own C-3/C-4 example cells producing no report item** (TE F-06). Added in v1.3: **the two-halt row**, whose `notice` cell carries S-3 then S-4 on the last admitted round (AC-2.2, AC-4.7 — SE G-01/TE F-03); **the AC-2.8 halt row**, four empty cells and `notice` = S-11 alone (SE G-02); **round 2's panel selected from the growth into round 2**, both classifications, with round 1 raising no S-6 notice (TE F-01); **a second halt preserving the reset region**, and the counted one-shot rule over a region carrying two `RESOLVED: yes` lines and one `WINDOW-START:` (AC-1.4, AC-1.5(4) — TE F-02); **an S-11 halt cleared without consuming the reset** and **a first-round-of-window zero-delta not halting** (AC-1.5(5), AC-2.8 — TE F-04); **`VERDICT:` → anchor → valid trailer reading as a count, not as *unavailable*** (AC-3.4's algorithm, AC-2.7 — SE G-05/TE F-06); and **a line-endings-only revision not firing AC-2.8** (equal digest, unequal byte count — SE G-03). Added in v1.4, one obligation per bullet because v1.3's single sentence carried eight in one ~700-byte run-on (SE v5 MF-5): <br>• a second halt's post-mortem carrying **no** `RESOLVED:` line after the strip, so `checkPostmortem` returns `unresolved` and step G refuses (AC-1.4 — SE v5 G-07); <br>• a region with two `HALT-REASON:` lines and one `WINDOW-START:` granting **exactly one** further window, and a region with `A = H` granting none (AC-1.5(4)); <br>• an S-11 clearance writing `WINDOW-RESUMED: {W}`, leaving `W` unchanged, and a **subsequent** S-3 halt then **not** being auto-cleared (AC-1.5(5) — SE v5 G-10, TE v5 F-01); <br>• a halt appending its `HALT-REASON:` to the **end** of the region, so *"the last line"* is the most recent halt's (SE v5 G-11); <br>• AC-1.5(4)'s ordered algorithm on `WINDOW-START: 4` then `WINDOW-START: 9` with highest round 6 ⇒ `W = 1`, fail-closed (TE v5 F-04); <br>• a co-occurring S-3/S-4 halt writing **one** `HALT-REASON:` line whose value is the `; `-joined string, character-identical to the report's `notice` cell (AC-2.2, §6 — TE v5 F-06); <br>• the first round of a **reset** window dispatching the full panel, with no growth measurement and no S-6 notice, and **not** approving a byte-identical document under a single verifier (AC-3.1, AC-4.1 — TE v5 F-03); <br>• `VERDICT:` → prose line → valid trailer reading as *malformed* under the stopping scan (AC-3.4 — SE v5 G-08); <br>• a `## Verdict` section with two `VERDICT:` lines reading as *malformed*, not *unavailable* (AC-2.7, AC-3.4 step 1 — SE v5 G-09). Added in v1.5, one obligation per bullet: <br>• **the first halt of a phase** — no post-mortem on disk beforehand — leaving a file that carries `## Reset Region` with **exactly one** `HALT-REASON:` line, and the operator's **first** clearance then granting a window (AC-1.4 clause 1 — TE v6 F-01). This is the fixture the v1.4 text did not entitle a PROPERTIES author to; the existing "halt, clear, assert a window opens" test is written against a fixture that already has a region and passes under both readings, so it must be written separately; <br>• a region with two `HALT-REASON:` lines and **no** answering line ⇒ counts-mismatch ⇒ `W = 1`, **no** window granted on that entry, **and none on a later entry that has not performed the sanctioned repair** — the universal is scoped, because AC-1.5(4) now states a repair for this reason and the recovery leg below asserts that it works (TE v8 F-01), `RESOLVED: yes` **not** consumed, and `reset-region-corrupt: counts-mismatch (H=2, A=0) {path}` in the report row (AC-1.5(4) steps 3–4, S-16 — SE v6 G-14). The positive control is the same region with one answering line, which grants exactly one window. **The recovery leg is part of this obligation**: after the operator's sanctioned `counts-mismatch` repair — deleting the whole `## Reset Region` section — the next halt re-creates a one-line region and the clearance after it grants a window, so the fixture asserts a *recoverable* refusal and not a permanent one (SE v6 G-19, TE v6 F-02). **The two legs are a mutation pair**: an implementation that refuses permanently fails the recovery leg, and one that grants a window on a corrupt region fails the refusal leg, so neither leg is green on both. The recovery leg must be sequenced with **two** clearances, not one — deleting the section leaves `H = A = 0`, so the gate's `A < H` conjunct is false and no window is granted; the entry halts, appends one `HALT-REASON:` and strips the marker, and it is the operator's **second** `RESOLVED: yes` that opens the window (TE v8 Q-14); <br>• a region whose `WINDOW-START:` fails step 2 with `A < H` ⇒ **nothing written**, the clearance survives, and a later entry after the operator's sanctioned repair grants the window (AC-1.5(4) — TE v6 F-02). The v1.4 reading spent the clearance here, so the assertion is *the file is byte-unchanged apart from the report* — which is satisfiable because a refusal takes **no** halt (v1.6); <br>• **the entry that follows a refusal, and the refusal's own entry, on an exhausted branch.** On a branch whose highest round is 3 with a corrupt region and a fresh `RESOLVED: yes`: the refusing entry appends **no** `HALT-REASON:` (`H` unchanged), strips **no** marker (`checkPostmortem` still reads `resolved` afterwards), writes no post-mortem, and emits S-16 with the **same** reason on the next entry as on the first — the ratchet test, which fails against any implementation that lets AC-1.5(1)'s budget halt run after step 4 (AC-1.4, AC-1.5(4) — SE v6 G-18, TE v6 F-01); <br>• **the no-round-admitted report row** (AC-4.7 row B): `round` = one past the highest round on the branch, `panel-shape` / `blocking` / `growth-bytes` / `classification` all empty, and `notice` = S-16 **alone** — asserted character for character, with **no** S-4 reason present, since no halt was taken (SE v6 G-20); <br>• **a verifier dispatched with no `REVIEW-SCOPE-ROUNDS:` line, an empty one, an unparseable one and an inverted one** (`{W}` > `{N−1}`) each producing a cross-review with **no `## Disposition` section** and a findings-table row naming the missing input, and the loop then refusing approval **without halting** — the same enumerated case a missing `## Disposition` already has (AC-3.2, S-17 — TE v6 F-03). **Two garbled dispatches across the sequence**, not one, and the sequence spans **three rounds and one operator clearance**: S-17's emitter is the loop, so a deterministic bad render recurs on every round of the phase, and an obligation that asserts only the single round is green against an implementation that never converges. Sequenced as AC-3.2 states it — round `k` dispatches and reports `disposition-missing` (dispatch 1); the intervening authoring pass returns a byte-identical document; round `k+1` **opens and halts at AC-2.8's round-open read before any reviewer is dispatched**, so it asserts S-11 `no-revision:` and **no** second `disposition-missing`; the operator clears (AC-1.5(5), `WINDOW-RESUMED:` written, `W` unchanged); round `k+1` re-opens and emits the same garbled dispatch (dispatch 2). The fixture must make the authoring pass move at least one byte before the re-opened round — a cosmetic revision — because with the anchors unchanged the sequence is **absorbing at the zero-delta halt** and dispatch 2 is unreachable. The two assertions are therefore properties of two different rounds, never of one (TE v8 F-03, SE v9 G-27, TE v9 F-03); <br>• **the mid-window refusal — the positive control for step 4's unconditional refusal.** On a branch whose highest round is **2** (round 2 halted on AC-2.1's fixed point or AC-2.8's zero-delta, creating the region with `H = 1`, `A = 0`) with a corrupt region and a fresh `RESOLVED: yes`: the entry refuses — **no round-3 cross-review file is written**, the report carries AC-4.7 row B with `round` = 3 and S-16 alone, and the invocation terminates with the queue row `halted`. An implementation that skips step 4 and merely falls back to `W` = 1 runs round 3 and produces the round-3 file, so this is the one fixture on which honouring step 4 and falling back are distinguishable; the exhausted-branch row above cannot separate them (AC-1.5(4) step 4 — SE v9 G-28, TE v9 F-01); <br>• the loop **appending** its `WINDOW-START:` / `WINDOW-RESUMED:` line to the end of the region — asserted positionally, with a prepending implementation failing the test, because a prepended answering line inverts step 2's *"before it"* and locks `W = 1` permanently (AC-1.5(4) — SE v6 G-13); <br>• a `## Verdict` section carrying the heading and **no** `VERDICT: ` line reading as *malformed*, matched against `parseVerdict`'s `malformed: true` fallback and **not** against the genuine `0/0/0` truncated-output return (AC-2.7 row 3, AC-3.4 step 1 — SE v6 G-15); and a line reading `VERDICT:Approved` — no space — counting as **zero** `VERDICT: ` lines (SE v6 MF-2); <br>• a verifier round `N > W` whose dispatch carries `REVIEW-SCOPE-ROUNDS: {W}..{N−1}` (S-17), with the `## Disposition` row set derived from that range and **not** from the whole branch history (AC-3.2, O-3, O-9(c) — SE v6 G-16); <br>• a fenced `RESOLVED: yes` surviving AC-1.4 clause 2's strip untouched, while an unfenced one is removed (AC-1.4 — SE v6 G-17). | PROPERTIES |
...
| **R-9** | **A count-only fixed point cannot distinguish a plateau from complete finding turnover.** *New in v1.3 (SE v4 G-06); demonstration restated in v1.4 (TE v5 F-07).* Demonstrated by this document's own review, stated over counts that are actually obtainable from the branch: read as §5 defines `blocking(N)` — the sum over every reviewer of that round, from the file, by `extractFileVerdict` — rounds 1–3 are **unavailable** (rounds 1 and 2 carry no readable trailer in either file, and at round 3 only the SE file does — `3+2` — so the round has no blocking count at all), round 4 is `1+4` plus `2+2` = **9**, and round 5 is `1+4` plus `3+3` = **11**. *Extended in v1.7 (SE v8 MF-9):* round 6 is `(0+4)` plus `(1+1)` = **6**, and round 7 is `(1+2)` plus `(1+2)` = **6** — so `blocking(7) ≥ blocking(6) > 0` with both operands available and panel shape `{software-engineer, test-engineer}` on both rounds, which means **AC-2.1 would have halted this phase at round 7**. Round 8 is the round on which every blocking finding closed and none replaced it (SE v8: 0H/0M; TE v8: 0H/1M). *Extended again in v1.8 (SE v9 MF-13):* `blocking(8) = 1` — `(0+0)` from SE v8 plus `(0+1)` from TE v8 — so `1 < 6` and the rule does **not** fire at round 8. Round 9 is `(0+3)` from SE plus `(0+3)` from TE = **6**, a *rise* on `blocking(8) = 1`, which AC-2.1 also reads as "not a fixed point", so it does not fire at round 9 either — on a revision that closed all eight round-8 findings and left the document materially better. The two instances are mirrors and are worth more to the calibration successor together than either alone: at round 7 a **flat** count concealed collapsing severity and would have produced a false-positive halt; at round 9 a **rising** count conceals a document that is converging, and correctly produces no halt but for a reason the rule cannot see. Both directions confirm the same shape of coarseness, and neither direction produces a wrong approval — the failure mode stays a false halt or a missed stop. That is a sharper instance of this risk than the one above: the counts were *flat* while the severities collapsed from `1H+2M` and `1H+2M` to `0H+0M` and `0H+1M`, so the false positive lands immediately **before** approval rather than mid-phase — the first on-branch instance of that shape. Each panel's own trajectory note tells the same story from its side: SE records 10, 5, 5, 5, 5 and TE records 7, 4, 5, 4, 6. Either reading shows the same thing, and it is the thing the risk is about: at rounds 4 and 5 **zero** prior findings survived — every one was closed — and every new finding was a defect in text the immediately preceding revision had added. v1.3 cited "10, 5, 5, 5" and claimed AC-2.1 would fire on 3→4; those numbers are not derivable from the files by this REQ's own definition, and three of the four rounds are *unavailable* to AC-2 exactly as R-7 predicts (TE v5 F-07). AC-4.2 would have classified that revision `new-mechanism` and re-escalated to the full panel, so the two mechanisms disagree about the same round. | **Accepted, Low, and recorded rather than fixed here.** The cost is a false-positive halt — one operator interaction on a round that made large, correct progress — never a wrong approval, and R-2 already accepts the coarseness of count-only comparison (N-2). A finding-identity test would need a findings-table grammar N-3 declines to introduce. Successor: the same calibration REQ as R-2, which now carries *"does the fixed-point test need finding identity, not just count?"* as a fourth question. |
...
| **R-5** | **AC-5, AC-4.6, AC-3.2(2) and AC-3.2's S-17 receive side are prompt clauses, so they are directive rather than enforced.** An agent that ignores them is not detected. *Widened in v1.2 (SE F-03, TE F-03).* AC-3.2(2) joins the list explicitly: the verifier excludes an unbindable finding from the counts it writes, and no reader can check that it did, because checking would require parsing the findings table — a grammar N-3 declines to introduce. *Widened again in v1.7 (SE v8 MF-8):* **AC-3.2's S-17 receive side is a fourth**, and its failure direction is different in kind from the other three. A verifier that ignores the clause and **fabricates** a range still writes a `## Disposition` section, which passes clause 1's completeness check — nothing checks the row set against the range. A fabricated *wider* range is harmless; a fabricated **narrower** one is the unsafe direction, because it approves a document whose earlier-round findings are unresolved. The mechanical residue that keeps this Low rather than blocking is that S-17 is **loop-emitted and therefore always present**, so the case requires a misbehaving agent rather than a missing input. | Accepted, and now stated rather than implied. AC-4.2 and AC-2 remain the mechanical controls; AC-5's mechanical half is only the extraction (AC-5.4). The mechanical residue of AC-3.2(2) is that `blocking(N)` keeps **one** definition — the count trailer — so a verifier that ignores the clause produces a *higher* count, which can only make AC-2 halt earlier, never later. The failure direction is safe. A finding that any of the three is unenforceable is **correct and known** — file it as Low. |
...
**Convention (SE v5 MF-4).** Each §10.x map is **frozen** at the round it was written for. A later
revision that changes something an earlier map describes records the change in the **latest** §10.x
...
| Finding | Answered in |
|---|---|
| SE G-07, TE F-02 — AC-1.4's preserved `RESOLVED:` line collides with the shipped gate: one line makes the next halt read as resolved (fail-open), two read as `duplicated` and brick the phase | **AC-1.4** — the halt path **strips** any prior `RESOLVED:` line while preserving the rest of the region, so the file carries at most one marker and every halt demands a fresh clearance; **AC-1.5(4)** restates one-shot over `H` (halts) and `A` (answers) instead of counting the marker; §5's durability rows cite `parseResolvedMarker` (`:953`/`:961`) and `checkPostmortem` (`:2440`/`:2446-2447`); **N-4** amended; **O-10** asserts the unresolved-on-arrival case |
| SE G-10, TE F-01 — an S-11 clearance is never answered, so `A < H` persists and the next halt of any kind is auto-cleared with a fresh window | **AC-1.5(5)** — the S-11 path writes `WINDOW-RESUMED: {W}` (S-14): the origin does not move and the spent rounds stay spent, but the clearance **is** answered. It also gives the path a positive artifact, which O-10's S-11 obligation previously lacked; §6 and §5 carry the new literal |
| SE G-11 — "the last `HALT-REASON:`" is not determined, because the halt writes *around* the region | **AC-1.4** clause 1 and **AC-1.5(5)** — each halt **appends** its own `HALT-REASON:` to the end of the reset region, so document order is halt order; §5's durability row states it too |
| TE F-03 — the first round of a reset window is a single-verifier round and can approve the document a full panel just rejected | **AC-3.1** restated over windows: round `W` is the full panel, `N > W` is the verifier; **AC-3.2**'s *Given* and clause 1 scoped to the current window; **AC-4.1** does not measure growth at `N = W` (which answers **TE Q-08**: not deliberate, and now declined); §5's *current window* states the four ACs' shared boundary |
| TE F-04 — AC-1.5(4)'s receive side is total but not single-valued | **AC-1.5(4)** — the table is replaced by a four-step ordered algorithm: collect, validate every line, any failure ⇒ `W = 1`, else the greatest |
| TE F-05, SE MF-1 — v1.3's citations resolve at `main`, not at the declared baseline, and one names a symbol that exists at neither commit | **Header** *Citation baseline* and the four sites — `sha256Hex` `:696`, `canonicaliseForDigest` `:615`, its JSDoc `:600-614`, `approvalHashOf` `:797`, and the post-mortem prompt as `reviewLoop` / `postmortemPrompt` at `:1725-1730`. Every citation this document makes was re-verified at `9486c81` in one pass |
| TE F-06 — `HALT-REASON:`'s value grammar is stated three ways and has no §6 row | **§6** rows for `## Reset Region`, `HALT-REASON:`, `WINDOW-START:` and `WINDOW-RESUMED:`; **§5** S-12 … S-14; **AC-1.5(5)** states one line per halt carrying the `; `-joined render in AC-4.7's order, and reads its **leading** reason |
| SE G-08 — AC-3.4's step 2 stops but steps 4–5 collect | **AC-3.4** — the scan stops, step 4's *"two or more parsing candidates"* is deleted, and a second parsing trailer is stated to be unobserved |
| SE G-09 — a duplicated `VERDICT:` line is classified nowhere | **AC-2.7**'s new table row ⇒ *malformed*, and **AC-3.4** step 1, citing `extractFileVerdict` (`:888`, the count at `:904`) |
| SE G-12 — the AC-2.8 halt row's justification is false of two cells | **AC-2.8** and **AC-4.7** — the cells are empty **by choice**, not for want of a source; **O-12** carries the round-open ordering question |
| TE F-07 — R-9's demonstration is not obtainable from the branch | **R-9** — restated over `blocking(N)` as §5 defines it (rounds 1–3 *unavailable*, round 4 = 9, round 5 = 11) plus each panel's own trajectory note |
| SE MF-2, MF-3, MF-5; TE MF-08, MF-09, MF-10 | **§5** catalogue lead-in and S-12 (the render is a member); **AC-2.7** ("the anchors-only row"); **O-10** (bulleted); **§5** *current window* ("since the last **granted** window"); **AC-1.5(4)** ("on any entry"); **AC-2.6** lead-in (over the growth into the round, read across a window) |
...
finding, and all **eight** findings below lie in text v1.4 added — seven of them in the two mechanisms
v1.4 introduced to answer round 5 (the reset region's accounting, and the trailer reader). One is High.
The count is eight and not seven — SE v6's four Medium and one Low plus TE v6's one High, one Medium and
one Low, which is what the two verdict trailers and this table's own rows say (SE v7 MF-1, TE v7 MF-19).
...
| TE F-02 — a region that fails validation may or may not consume the clearance; one reading is a permanent dead end with no repair path and no report slot | **AC-1.5(4)** — validation becomes a **conjunct of the gate**, so a corrupt region consumes nothing; the **sanctioned operator repair** is stated (delete or correct the line the report names, nothing else, and a mis-repair fails closed on step 3); and the notice gains a catalogue id **S-16** (`reset-region-corrupt: {reason}`, closed three-member enum) with a home in **AC-4.7**'s `notice` column at precedence 8 and a **§6** row. *(v1.6 amends both halves per round 7: the refusal takes no halt, and the repair is stated per reason — §10.11.)* |
...
| TE MR-03, MR-04, MR-06 (round 6) | Carried, none blocking. MR-06 (is O-5's re-apply observable in the same invocation?) is recorded here rather than answered: AC-1.4 states the obligation either way, and the arithmetic TE F-01 raised is now settled by *"every halt writes one `HALT-REASON:`"* independently of which invocation the write lands in. All three remain bound to `docs/pdlc-runtime-measurement-spike/REQ-pdlc-runtime-measurement-spike.md` per AC-5.3 |

### 10.11 Round-7 finding, mechanical fix, question and measurement request → where it is answered

Round 7 is the **fourth** consecutive round with no carried finding: both panels closed every prior
finding. The two panels **filed seven** findings (SE v7: 1 High, 2 Medium; TE v7: 1 High, 2 Medium,
1 Low), which the table below carries on **five** rows because two pairs are the same finding filed
independently by both panels (SE G-18/TE F-01, SE G-19/TE F-02). All seven lie in text v1.5 added, and
**five of the seven filed** are in the **refusal path** v1.5 introduced to close round 6's dead end. The
two counts are stated on their stated bases here because v1.6 mixed them in one sentence (SE v8 MF-6,
TE v8 MF-20). One finding is High, and both panels filed it independently.

| Finding | Answered in |
|---|---|
| SE G-18 / TE F-01 (High) — the refusal path halts, and the halt strips the clearance the refusal declined to spend and increments the mismatch it reported, so the sanctioned repair never converges | **AC-1.5(4)** — the new *refusal is not a halt* paragraph and **step 4**: a refused entry takes no halt, evaluates no budget, writes no `HALT-REASON:`, strips no marker and leaves the file byte-unchanged; its only effect is the S-16 notice. **AC-1.4** states the same boundary from the halt side (*a halt is an event the accounting records; a refusal is a decision not to enter*) without weakening *"every halt, without exception"*. **AC-1.5(1)** notes it is not reached on a refused entry. §5's durability row and the invariant paragraph carry the consequence: `H` and `A` are both unchanged by a refusal, so the reported reason is stable. **O-10** gains the ratchet fixture — the refusing entry *and the entry after it* |
| SE G-19 / TE F-02 (Medium) — the sanctioned repair is stated only over an *offending line* and is inoperable for `counts-mismatch`, whose evidence is the pair `H`/`A`; both repairs that would work are forbidden elsewhere | **AC-1.5(4)** — the repair is now a **per-reason table**: value reasons are repaired by deleting or correcting the named line; `counts-mismatch` by deleting the **whole `## Reset Region` section**, which S-12 already reads as the empty region (`H = A = 0`, `W = 1`), at the cost of one further halt and the halt history. **§6**'s `WINDOW-START:` row scopes *"never by a human"* to **authoring** — the edit that could bank an unpaid window — so the deletion contradicts no rule. §5's *reset region* row carries the per-reason split; **O-10**'s counts-mismatch bullet gains the recovery leg it previously specified away |
| SE G-20 (Medium) — AC-4.7's precedence row 8 introduces a second dispatchless report row inside a notice-ordering table, contradicting the *"one row with no dispatch"* paragraph and leaving four cells underivable | **AC-4.7** — two paragraphs now, **row A** (the AC-2.8 halt row) and **row B** (the no-round-admitted row), each stating its six cells: `round` = one past the highest round on the branch, four empty cells licensed explicitly, `notice` = **S-16 alone** with no S-4 reason, because no halt was taken. Precedence row 8 keeps only its sort justification |
| TE F-03 (Medium) — the verifier's dispatched round range crosses a component boundary with no grammar, no catalogue id and no total receive side | **§5 S-17** — `REVIEW-SCOPE-ROUNDS: {W}..{N−1}`, the `..` separator S-4 already uses — plus a **§6** row and an **AC-3.2** clause fixing one behaviour for all four non-canonical inputs (absent, empty, unparseable, inverted): the verifier omits `## Disposition` and names the missing input, which lands in the already-enumerated case where approval is refused without halting. **O-3** and **O-9(c)** carry it; **O-10** asserts all four |
| TE F-04 (Low) — AC-2.7 row 3 assigns *malformed* to an input §5's definition calls *unavailable*, and §5 was not amended | **§5** — *malformed* now covers *"a quantity the structure carrying it says should be there and is not"*, with *unavailable* scoped to an absent **carrier**; **AC-2.7**'s row-3 paragraph points at the amended definition, so both readings agree |
| SE MF-1 … MF-5; TE MF-16 … MF-19 | **§10.10** lead-in and the *Revision note (v1.5)* both read **eight** round-6 findings (SE MF-1, TE MF-19); **AC-1.5(5)**'s lead-in re-wrapped (SE MF-2, TE MF-16); **§5**'s catalogue ordering fixed by moving the S-16 row beside S-5/S-6 and S-9 beside S-8, so the kind-adjacency clause describes the table (SE MF-3); **AC-1.5(4)**'s repair paragraph says the repair is the only hand-edit to **machine-written** state (SE MF-4); **O-10** bullet 3 re-checked and kept, now satisfiable because a refusal takes no halt (SE MF-5); **§5 S-16** and **§6** fix the notice's render character for character (TE MF-17); **step 3** records that `A > H` is hand-edit-only (TE MF-18) |
| SE Q-16 … Q-18, TE Q-11, TE Q-12 | Answered by the rows above: **Q-16** — the entry does **not** halt, so nothing strips the marker; **Q-17** — for `counts-mismatch` the operator deletes the section, not a line; **Q-18** — AC-4.7 row B states all six cells, S-4 does **not** co-occur, and `round` is one past the highest round on the branch. **TE Q-11** (does an S-11 clearance that resumes an already-spent window halt in the same entry?) — yes, and it is intended: the clearance is answered (`A` moves with `H`), so step 3 stays satisfied and the run report carries S-14's answer beside S-4's halt; a PROPERTIES author should read co-occurrence as a **pass**. **TE Q-12** (where does the creating halt place `## Reset Region`?) — at **end of file**, the only placement self-consistent with §5's *"to the next top-level heading or end of file"*; O-5's re-apply is the write, and O-10's first-halt fixture asserts it positionally |
| TE MR-07 (round 7) | Carried, non-blocking. Whether the shipped zero-admitted-rounds path reaches the post-mortem writer at all decides whether FSPEC is describing a change or a preservation, not what the REQ must state: AC-1.5(4) step 4 fixes the behaviour either way. Bound to `docs/pdlc-runtime-measurement-spike/REQ-pdlc-runtime-measurement-spike.md` per AC-5.3, alongside MR-03, MR-04 and MR-06 |

### 10.12 Round-8 finding, mechanical fix, question and measurement request → where it is answered

Round 8 is the **fifth** consecutive round with no carried finding: both panels closed every prior
finding at the mechanism, and every finding below is new and lies in text v1.6 added. The two panels
filed **eight** findings — SE v8: 0 High, 0 Medium, 5 Low; TE v8: 0 High, 1 Medium, 2 Low. **One
merged pair** is the same finding filed independently by both panels (SE G-21 / TE F-02); nothing else
merges, since SE G-23's shared answer is with **TE MR-08**, a measurement request rather than one of
the eight, and it keeps its own row at the foot of the table. So the row count is *filed minus merges*
= 8 − 1 = **seven** finding rows below (TE F-01; SE G-21/TE F-02; G-22; G-23; G-24; G-25; TE F-03).
The count and its derivation are stated in one place here, and the rows were counted after they were
written, because this arithmetic has needed a mechanical fix in three consecutive §10.N lead-ins
(MF-1 at round 7, MF-6 at round 8, SE v9 MF-11 / TE v9 F-04 here). **One** is blocking, and it is not in an
acceptance criterion: it is a stale assertion in an O-10 obligation that the AC above it already
contradicts. SE v8 filed no High and no Medium — the first round in this phase in which that is true.

| Finding | Answered in |
|---|---|
| TE F-01 (Medium) — O-10's `counts-mismatch` bullet asserts *"no window granted on that entry **or any later one**"* and, appended in v1.6, that the clearance after the sanctioned repair grants one; no implementation satisfies both, and the likelier reading is the permanent-halt oracle round 7 removed | **O-10** — the universal is scoped to *"a later entry that has not performed the sanctioned repair"*, and the recovery leg is restated as the refusal leg's **mutation pair** (an implementation that refuses permanently fails one; one that grants on a corrupt region fails the other). The leg is also sequenced with **two** clearances, per TE Q-14, because the whole-section deletion leaves `H = A = 0` and the re-creating halt strips the marker again |
| SE G-21 / TE F-02 (Low) — S-16's bracket content is called *the offending value* in §5's prose and AC-1.5(4) step 4, and *the offending line* in §6, with the specimen agreeing with §6, on a row whose own text declares the render fixed *"here and only here, character for character"* | **§5 S-16** and **AC-1.5(4) step 4** — both now read *the offending **line***, the specimen unchanged; **§6**'s row no longer restates the parts and points at §5's specimen instead, so the two homes cannot drift apart again (SE MF-10) |
| SE G-22 (Low) — `{reason}` is single-valued and precedence row 8 gives S-16 one slot, but step 4 fires on *any* failing line **or** the counts, so a two-fault region has no derivable `notice` cell | **AC-1.5(4) step 4** and **§5 S-16** — exactly one S-16 notice per entry: the **first failing line in document order**, and `counts-mismatch` only when every line passes step 2. Two S-16 notices never co-occur in the `; `-joined cell. Answers **Q-19** |
| SE G-23 (Low) — *"the only effect of the entry is the S-16 notice"* overstates a step-G-shaped refusal, which records a ❌ phase row, throws, and leaves the queue row `halted`; a literal `return` would let the pipeline advance past an unentered review phase | **AC-1.5(4)**'s *refusal is not a halt* block — *"only effect"* is scoped to the **post-mortem file**, and *returns* is stated as *the phase does not run and the invocation terminates on step G's path*, with the ❌ row and the `halted` queue row named as the intended outcome. **AC-1.4**'s paragraph carries the same scoping. Answers **Q-20** and records the answer TE **MR-08** asks for, which is now confirmation rather than a decision |
| SE G-24 (Low) — the value-reason repair offers *delete or correct* as equals, but deleting an answering line raises `H − A` by one and so converts `invalid-window-start` into `counts-mismatch` on a region already at `H − A = 1` | **AC-1.5(4)**'s repair table — **correct** is preferred and always safe; **delete** only at `H − A = 0`, a number S-16's `(H={h}, A={a})` already gives the operator — **superseded in v1.8**, which withdraws the delete branch entirely (SE v9 G-26, §10.13). The arithmetic is stated beneath the table. §5's *reset region* row carries the same preference. Answers **Q-21** |
| SE G-25 (Low) — *refusal* carries three senses, the new term of art is not in §5's meanings table, and AC-3.5 uses the word for something whose defining property is that *the window proceeds* | **§5** — new **phase refusal** row, defined against *approval refusal*; **AC-1.4**, **AC-3.5** and **AC-3.2** now name which is meant at each use |
| TE F-03 (Low, Cross-Feature) — *"a garbled range costs one round"* is unsupported: S-17's emitter is the **loop**, so a deterministic bad render recurs every round, and O-10 asserts only the single round | **AC-3.2** — the *costs one round* claim is replaced by the sequence it actually produces (refusal ⇒ `disposition-missing` ⇒ an authoring pass with no defect to fix ⇒ a plausible byte-identical revision ⇒ AC-2.8's zero-delta halt, attributed to the author for the loop's failure ⇒ clearance ⇒ the same dispatch again), with the two things that make it survivable named: the diagnostic repeats under one name, and the cap is absolute — both **restated in v1.8**: the diagnostic *alternates* with S-11 and the bound is the operator, not a cap (SE v9 G-29). **O-10** asserts **two** consecutive garbled dispatches |
| SE MF-6 … MF-10; TE MF-20 … MF-23 | **§10.11** lead-in now states both counts on their bases — seven filed, five rows, five of the seven in the refusal path (SE MF-6, TE MF-20); the header **Cross-Reviews** row gains both v8 files (SE MF-7); **§9 R-5** gains AC-3.2's S-17 receive side as a fourth unenforceable clause, with the fabricated-**narrower**-range direction named as the unsafe one (SE MF-8); **§9 R-9**'s demonstration is extended to rounds 6 and 7 (SE MF-9); **§6**'s S-16 row points at §5's specimen (SE MF-10); **AC-4.7 row B**'s `round` gloss is scoped to a mid-window refusal (TE MF-21); **§5 S-17**'s two pointers now resolve — *AC-3.2's receive-side bullet for the range*, and *AC-3.2's `## Disposition` receive-side table, first row* (TE MF-22); **AC-1.5(5)** clause 5 re-flowed (TE MF-23) |
| SE Q-19 … Q-21, TE Q-13, TE Q-14 | Answered by the rows above: **Q-19** — one S-16, first failing line in document order; **Q-20** — the invocation terminates on step G's path and the queue row becomes `halted`; **Q-21** — **correct** the line; v1.8 withdraws the delete branch entirely, because deleting an answering line converts the reason at `H − A = 1` and banks an unpaid window at `H − A = 0` (SE v9 G-26); **Q-13** — step 4's refusal is **unconditional**, and v1.8 restates its justification as *fail-closed, not costless*: on an exhausted branch the outcome is unchanged, on a mid-window branch (highest round 2) the refusal costs the round `W` = 1 would have admitted, which is the fixture O-10 now carries (SE v9 G-28, TE v9 F-01); **Q-14** — the `counts-mismatch` recovery costs one halt and **two** clearances, now stated in O-10's recovery leg |
| TE MR-08 (round 8) | Answered rather than carried. A phase refusal terminates the invocation on step G's path, so the shipped `orchestrate-dev` halt path rewrites the feature's `docs/_queue/QUEUE.md` row to `halted` and commits it — which is the outcome an unattended queue needs, since the alternative (`in-progress` retained) has `/loop` re-pick the feature and refuse again once per iteration forever. Stated in AC-1.5(4)'s *refusal is not a halt* block, so O-10 needs no separate queue-row obligation |

### 10.13 Round-9 finding, mechanical fix and question → where it is answered

Round 9 is the **sixth** consecutive round with no carried finding: both panels closed every prior
finding at the surface it named, and every finding below is new and lies in text v1.7 added. The two
panels filed **six** findings — SE v9: 0 High, 3 Medium, 1 Low; TE v9: 0 High, 3 Medium, 1 Low. **Two
merged pairs** are the same finding filed independently (SE G-28 / TE F-01 on step 4's justification;
SE G-27 / TE F-03 on O-10's garbled-range obligation), and one further pair is the same rule filed as a
mechanical fix by one panel and a Low by the other (SE MF-11 / TE F-04). Row count is *filed minus
merges* = 6 − 2 = **four** finding rows below, plus one row for the merged mechanical-fix pair. **Four**
are blocking (all Medium): three in AC-1.5(4)/AC-3.2/O-10, one in §6's grammar row. No panel contests
any mechanism this document installs; every blocking finding is in a clause *around* a rule v1.7
widened, which is the shape TE v9's trajectory note names.

| Finding | Answered in |
|---|---|
| SE G-26 (Medium) — the newly sanctioned *delete the answering line at `H − A = 0`* branch banks an unpaid window: it lowers `A` alone while the refusal leaves the `RESOLVED:` marker in place, so the next entry grants a window off a clearance already answered — the edit §6's prohibition exists to prevent | **AC-1.5(4)**'s repair table — the licence is withdrawn; **correct that line** is the only sanctioned value repair for both value reasons, at every `H − A`. The arithmetic paragraph now shows both unsafe directions in one table (`H − A = 1` converts the reason; `H − A = 0` grants a window) and states that correcting is safe everywhere, so the branch was never needed. **§5**'s *reset region* row carries the same rule |
| SE G-28 / TE F-01 (Medium) — step 4's justification for the unconditional refusal is false on a reachable branch (*"`W` falls back to 1, which admits no rounds"* is true only where the branch already carries three), and AC-4.7 row B, added in the same revision, states the opposite | **AC-1.5(4) step 4** — the justification is restated as **fail-closed, not costless**, with the case split: unchanged on an exhausted branch; on a mid-window branch (reachable at highest round 2 via AC-2.1's (1,2) fixed point or AC-2.8's zero-delta) the refusal costs the round `W` = 1 would have admitted, and that is intended. Row B's gloss is now the consistent statement. **O-10** gains the mid-window fixture as the refusal's positive control, which is the only branch separating step 4 from the `W` = 1 fallback |
| SE G-27 / TE F-03 (Medium) — O-10's *two consecutive garbled dispatches* obligation asserts two properties of one round that cannot both hold: AC-2.8 halts at round-open *before* dispatching, so the round carrying the byte-identical document runs no verifier and reports no second `disposition-missing` | **O-10** — the obligation is sequenced across **three rounds and one clearance** (dispatch 1 → byte-identical pass → S-11 halt on an undispatched round → clearance → dispatch 2), mirroring AC-3.2's prose. **AC-3.2**'s paragraph states the same sequence explicitly and adds the second-order point: with the anchors unchanged the pathology is **absorbing at the zero-delta halt**, so the fixture must move a byte for dispatch 2 to be reachable at all |
| TE F-02 (Medium) — §6's `WINDOW-START:` row forbids the corrected line AC-1.5(4) now prefers; its exemption was written for the old repair set, so O-10's value-repair fixture is unconstructible under the closed grammar | **§6**'s `WINDOW-START:` row — the *never by a human* clause is scoped to **authoring** and now exempts **both** sanctioned repairs, with the reason each grants nothing: the whole-section deletion zeroes both counts, and the in-place correction leaves `H` and `A` untouched. Deleting a single answering line stays forbidden at every `H − A` (SE G-26). AC-1.5(4)'s scoping paragraph names the same two exemptions |
| SE G-29 (Low) — *"the cap is absolute, so the sequence is finite"* contradicts §4 M-1d (*"a per-invocation budget … not an absolute cap"*) and AC-1.5(3)'s fresh window per clearance | **AC-3.2**'s sequence paragraph — the cap clause is deleted and the bound restated as **the operator**: every iteration costs an operator interaction and the operator sees the repeat, which was already the other half of the same sentence. The neighbouring claim is corrected too: the diagnostic **alternates** — `disposition-missing` on the dispatched rounds, S-11 `no-revision:` on the rounds between — rather than repeating under one name |
| SE MF-11 / TE F-04 (Low, Process) — the §10.N lead-in count has been wrong in three consecutive revisions; §10.12 says *six rows* where seven exist, and the second "pair" it names is a measurement request, which merges nothing | **§10.12** lead-in — restated as filed count, merge list, derived row count: eight filed, one merged pair (SE G-21 / TE F-02), **seven** rows. The rule is stated with the number rather than instead of it, and §10.13's own lead-in above follows the same pattern |
| SE MF-12 … MF-14; TE MF-24 … MF-27 | The header **Cross-Reviews** row lists both v9 files (MF-12); **§9 R-9**'s demonstration is extended to `blocking(8) = 1` and round 9's rise, recording both directions of the rule's coarseness — a flat count concealing collapsing severity at round 7, a rising count concealing convergence at round 9 (MF-13); **§5**'s *phase refusal* row carries the ❌ report row and the committed `halted` queue row, the two consequences AC-1.4 and AC-1.5(4) added in the same revision (MF-24); the queue-row claim in AC-1.5(4)'s *returns* bullet carries its two citations at the Citation baseline `9486c81` in AC-6.4's C-1 form, with the contrast against a literal early `return` stated (MF-25); §10.12's lead-in follows §10.11's pattern (MF-26 — see the row above); two over-long lines re-flowed as blocks (SE MF-14, TE MF-27) |
| SE Q-22 … Q-25, TE Q-15, TE Q-16 | Answered by the rows above and here. **Q-22** — after the withdrawal there is no delete branch, so the region in the question has one sanctioned repair (correct the value) and no entry grants a window off it. **Q-23** — the second dispatch lands on the **re-opened** round `k+1`, after the clearance, and is reachable only when the authoring pass moved at least one byte; O-10's fixture supplies that byte. **Q-24** — the entry **refuses**; row B's gloss is the correct statement and step 4's justification is corrected to match. **Q-25** — yes: re-invoking is the intended recovery of the diagnosis. A phase refusal is pure with respect to disk (§5), so the S-16 notice is re-emitted identically on every entry until the region is repaired — that stability is exactly why the reason is decidable from the region alone — and O-8's report-destination question is not constrained further here. **Q-15** — yes: step 4 sits inside `W`'s resolution, which runs on **every** entry, so a corrupt region refuses the phase whether or not a `RESOLVED:` marker is pending; the marker only decides whether a repair can then be honoured. Step 4 states this. **Q-16** — any step-2-valid value is acceptable: the operator picks the origin within the stated bounds (decimal integer ≥ 1, ≥ every value before it, ≤ one past the branch's highest round), and O-10's repair fixture may write any such value. The loop's original value is not recoverable from a corrupted region and is not required to be |
