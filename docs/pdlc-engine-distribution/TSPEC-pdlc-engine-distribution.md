...
| S-7 | `PluginRootResolver` — `resolvePluginRoot({devDeclared, env, override, home, fs})` → `{ok, root, source, reason, tried, notices}`, the shipped return plus the one `notices` key §6.5 adds; `notices` is `{id, text}` records, non-empty only on the ignore branch | `lib/skills.mjs` | real `fs` and `process.env`; catalogue text via `lib/catalogue.mjs`'s `message(id, params)` | `lib/startup.mjs`'s `runStartupChecks` already takes it as `resolveFn` (default `resolvePluginRoot`) and **surfaces** the returned `notices` without re-deriving them. AC-5.6's path-level oracle drives the function directly over all four rows of §6.5's table — no injection needed, which is the point of deciding the notice at the resolver rather than one level up (§3.1, §6.5, §12.1) |
...
| Level | Runs | Covers |
|---|---|---|
| Unit, in-process | `pdlc/engine/__tests__/`, the shipped `node --test` suite | Resolution ladder, store enumeration, provenance rendering, catalogue closure, config read discipline, handshake/env-var branch, and **AC-5.6's path-level notice oracle** over `resolvePluginRoot`'s four rows, asserted on the resolver's own extended return (`notices`, §6.5 / §10.1 S-7) — the honour direction on the `devDeclared: true` × set row (resolved root `===` the env value, `source` unchanged, `notices` empty), emission by catalogue id plus rendered text on the ignore branch, and `notices` empty on the two variable-unset rows, so the row is falsifiable in both directions (§6.5; the shipped catalogue equalities do not cover this row). All pure over injected seams — no temp dirs, no network, no spawn |
