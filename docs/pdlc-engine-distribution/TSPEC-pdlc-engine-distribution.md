...
|---|---|---|---|---|
| pdlc | Draft — in review (Phase T) | Claude | 0.14 | 2026-08-16 |
...
| 0.10 | **Erratum round (Phase D).** Targeted, versioned edit only — no round-9 cross-review finding is addressed here and nothing settled in rounds 1–8 is re-opened. Upstream re-grounded first: REQ is v0.10 and FSPEC v0.2 at HEAD, the same versions this TSPEC was last approved against, so there is nothing to absorb and no upstream decision this round routes back. Five items, all raised against **this** document while DECISIONS was in review: **(1)** §5.1 now names **O-8 blocker 1 as closed by this feature** — removing `"private": true` (`pdlc/engine/package.json:4`) is engineering work in the manifest table, asserted by PF-3 (§8.3), leaving blockers 2 (N-6, scope) and 3 (N-2, licence) as the only operator-owned ones, so downstream records cite the closure instead of repeating V-01's HEAD count of three. **(2)** §5.1's inventory and §5.2 now **create `pdlc/engine/.npmignore`**, one line `!vendor/workflows/`, authored in the same task as the `vendor/` git-ignore rule it exists to negate (DEC-EDIST-01); it is stated to be **never a packed member** — npm excludes `.npmignore` from every tarball — so PF-4's `PK-*` set is unchanged, and it can only readmit, never widen. **(3)** D-5's wording (§4) and §5.4 reconciled to that: the allow-list-over-deny-list choice is about *who decides the packed set*, not about whether the file exists; no deny-list entry is ever added to it. **(4)** §6.2 **decides the signalled child** rather than naming signal handling as a behaviour needing assertion — `spawnSync` returns `{status: null, signal}`, so a literal `process.exit(result.status)` exits 0 on a Ctrl-C'd run against the engine's shipped crash-1 / halt-2 exit-code invariant (`exitCodeFor`, `PROP-EXIT-1`); per DEC-EDIST-06 the launcher exits `128 + signum`, with a signalled-child leg asserting the **exact number**, not `!== 0` (§6.2, §12.1). **(5)** §6.5's claim that `lib/catalogue.mjs`'s registered-message set-equality "covers it for free" is **withdrawn as false at HEAD**: `__tests__/catalogue.test.js:71-74` compares `messageIds()` with `Object.keys(MESSAGES)` (the module against itself) and `:4-6` disclaims emitted-ids equality, while the suite-wide row 1 (`_assert-suite-wide.mjs:196-210`) is path-blind and runs the direction that creates an emission *obligation*, not coverage; AC-5.6 therefore gains a named **path-level oracle** over `resolvePluginRoot`'s four rows — emission by catalogue id plus rendered text on the ignore branch, absence on the other three (§6.5, §12.1) |
| 0.11 | **Round-10 confirmation findings addressed (TE F-45 High, TE F-46, TE F-47 / PM F-01, PM F-02, PM F-03).** A targeted edit only: nothing settled in rounds 1–9 is re-opened, no new requirement is absorbed, and no upstream document is touched — REQ is still v0.10 and FSPEC v0.2 at HEAD, unchanged by this round. **F-45 (the blocking one): AC-5.6's path-level oracle is now writable against a named seam.** §6.5 states where the notice is observable instead of leaving the implementer to choose: at HEAD `resolvePluginRoot` returns `{ok, root, source, reason, tried}` (`pdlc/engine/lib/skills.mjs`, its JSDoc `@returns`) with no notice channel, so the shipped return is **extended, not replaced** — one `notices` key of `{id, text}` records, the catalogue id plus the string rendered by `lib/catalogue.mjs`'s `message(id, params)`, non-empty only on the ignore branch. The resolver decides and renders because it owns the branch; `lib/startup.mjs`'s `runStartupChecks` **surfaces** the returned `notices` through the `resolveFn` seam it already calls the resolver by, re-deriving nothing — the same channel `bin/pdlc.mjs` already drains for `readEngineConfig`'s `notices`. §3.1's handshake/startup and `skills.mjs` rows are reconciled to that split (they disagreed, TE Q-23), and §10.1 gains the seam row it lacked, **S-7 `PluginRootResolver`** (§3.1, §6.5, §10.1, §12.1). **F-46: the honour direction is asserted, not only the absence one.** The oracle now drives all four rows of §6.5's table, and row 1 (`devDeclared: true` × variable set) asserts the resolved root `===` the env value, `source` unchanged and `notices` empty — so an implementation that ignores the variable unconditionally goes red, which the absence-only wording permitted. DEC-EDIST-04's assertion 2 is now transcribed in full alongside assertion 1 (§6.5, §12.1). **F-47 / PM F-01: the `AC-1.4's exit-code contract` citation is withdrawn as false.** REQ's AC-1.4 is the version-triple criterion and states nothing about exit codes; the invariant the signalled-child decision actually rests on is the engine's own — `exitCodeFor`'s refusal/crash-1, halt/block-2 mapping (`pdlc/engine/lib/run.mjs`, pinned by `PROP-EXIT-1`). §6.2 now names that and **defers the reasoning to DEC-EDIST-06** rather than restating it, so the constraint is stated right in one place instead of wrong in two (PM Q-02); the sibling DECISIONS §7 is corrected in the same edit, and the stale phrase inside the v0.10 row above is fixed (§6.2, changelog). **PM F-02: §5.1 stops pointing a correction downstream.** The paragraph now says blocker 1 is discharged by this feature's manifest edit, that O-8's owner field is unchanged (REQ's O-8 and FSPEC's F-5 step 7 / Q-8 are upstream and are not rewritten here), and that clearing O-8 still gates AC-3.1's real-channel leg. No FSPEC erratum is opened (§5.1). **PM F-03: the v0.10 row's count now reads five, matching its own numbered list** (changelog) |
| 0.13 | **Round-11 confirmation findings addressed (TE F-48 Medium, PM F-01, PM F-02) plus the upstream re-grounding they surfaced.** Both round-11 reviews returned **Approved** with no High, so nothing here re-opens a settled decision; the three recorded findings and both questions are closed against shipped code rather than against prose. **Upstream re-grounding first (DEC-ERR-01).** REQ moved **v0.11 → v0.12** and FSPEC **v0.2 → v0.8** since this TSPEC was last approved, and the lineage header now names them. The decision both moves carry, and the one with a TSPEC surface, is **absorbed ahead of the raised items**: PR-check-set membership is **trigger-derived**, not a fixed count and not a filename list (REQ v0.12's O-B, FSPEC v0.8's BR-7.5 and its §5.1 row 6). §3.2's **V-18 was outright false at HEAD** — it claimed `.github/workflows/` "holds exactly one workflow" with five jobs, where HEAD holds three files, two of them pull-request-triggered, rendering **six** checks; V-18 now states the trigger rule, enumerates the six jobs across `pr-tests.yml` and `fixture-machine.yml`, and excludes tag-triggered `publish.yml` for a reason true of it and false of `fixture-machine.yml`. **V-19 was stale in the safer direction** and now names what `ci-arrangement.test.js` actually ships — `PR_GATE_FILES` and a `§5.1's file scope` test that re-derives the key set from the files' own `on:` triggers, so a new PR-gating workflow cannot join the repo without joining §5.1. §8.5's membership bullet, which read "`pr-tests.yml` only", and its per-job matrix bullet ("four of the five jobs") are corrected to the same rule. Per DEC-DOC-01 the line-window citations in all four places are replaced by symbol and job-id names, since that is what drifted. **TE F-48 / PM F-02: the surfacing hop now names both ends, and the two `notices` channels are held apart.** §6.5 previously said startup "passes the notices into the run's notices … exactly as `bin/pdlc.mjs` already drains `readEngineConfig`'s" — a destination that was not a destination and a citation that is wrong at HEAD. It now states that `runStartupChecks`'s **own return carries `notices`**, the resolver's `{id, text}` records verbatim, neither folded into `banner` nor flattened on the way out; that the single render site is `lib/startup.mjs`'s **`formatStartup`**, which appends `notice.text` per record, and that rendering there rather than per CLI call site is what makes AC-5.6 true on `dev`, `queue`, `queue --loop`, both `--dry-run` surfaces and the refusal path at once; and that `readEngineConfig`'s channel is a **different** one — `string[]`, drained by `bin/cli.mjs`'s `tunablesFor`, not by `bin/pdlc.mjs`, which contains no notice code at all. §3.1's row and §10.1's S-7 are reconciled to the same three statements. **PM F-01: the honour row's equality is `path.resolve`-aware.** The explicit leg resolves its candidate before testing it, so `root === ` the raw env string is false for a relative or non-normalised fixture value for a reason AC-5.6 does not care about; §6.5 and §12.1 now assert against `path.resolve(env value)` and tell the PLAN author to hand the fixture an already-absolute root. **PM Q-01 and TE Q-26 answered in the document:** `notices` is initialised once and returned on **every** leg including both refusals, so it is never `undefined`, the three empty rows are deep equalities against `[]` rather than truthiness checks, and an implementer cannot pass by attaching `notices` to the success returns alone. **Absorption still owed, named not silently skipped:** FSPEC v0.3–v0.7 (the `PK-*`/`E-*` renumbering settlements, per-class count ownership, the `not found` literal alignment) were reviewed for TSPEC surface in this round and are already consistent with §5.4 as it stands, but they have not had a full section-by-section re-grounding pass; §5.1's own six-row expected set and BR-7.7 have not been transcribed into §5.1's narrative beyond the V-18/V-19 and §8.5 corrections above. |
...
| V-18 | **Membership is decided by a workflow's `on:` trigger, not by its filename or by there being one file** (FSPEC v0.8's BR-7.5). `.github/workflows/` holds **three** files at HEAD — `pr-tests.yml`, `fixture-machine.yml` and `publish.yml` — of which the first two are pull-request-triggered and the third is tag-triggered and therefore **out** of the rendered check set. The **six** in-scope job-level `name:` strings match FSPEC §5.1's authored column verbatim: five jobs in `pr-tests.yml` (`unit-tests`, `engine-tests`, `artifact-freshness`, `fresh-clone-bootstrap`, `script-syntax`) plus `fixture-machine` in its own file. The jobs do **not** share one matrix, and §8.5's expander is per-job for that reason: `unit-tests` declares `os` **and** `node`; `engine-tests` declares `os` only; `artifact-freshness`, `fresh-clone-bootstrap`, `script-syntax` and `fixture-machine` declare no matrix at all. | `.github/workflows/pr-tests.yml`, jobs `unit-tests`/`engine-tests`/`artifact-freshness`/`fresh-clone-bootstrap`/`script-syntax` and their `name:` keys; `.github/workflows/fixture-machine.yml`, job `fixture-machine`, `name: Fixture machine (install/upgrade, launcher, container, two-repo)`; matrix axes on `unit-tests` (`os`, `node`) and `engine-tests` (`os`) |
| V-19 | `ci-arrangement.test.js` is the shipped carrier for §5.1's arrangement, and it already encodes the two-file scope: a `PR_GATE_FILES` map from workflow filename to the job ids it owns (`pr-tests.yml` → the five gate jobs, `fixture-machine.yml` → `fixture-machine`), with a `§5.1's file scope` test that **re-derives that key set from the files' own `on:` triggers**, so a new PR-gating workflow cannot join the repo without joining §5.1 — and `publish.yml` is excluded for being tag-triggered, a reason true of it and false of `fixture-machine.yml`. FSPEC BR-7.6's "older overlapping assertions removed" has a concrete target. | `pdlc/engine/__tests__/ci-arrangement.test.js`, `GATE_JOB_IDS` / `FIXTURE_MACHINE_JOB_IDS` / `PR_GATE_FILES` and the `§5.1's file scope` test |
...
| Handshake / startup | `pdlc/engine/lib/handshake.mjs`, `lib/startup.mjs` | extended | Unchanged decision logic (V-09); startup gains the resolution announcement (BR-4.1) and **surfaces** the resolver's ignored-env notice into the run's notices — it does not decide the branch and does not re-render the text. `runStartupChecks` already calls `resolvePluginRoot` through its `resolveFn` seam (`lib/startup.mjs`, `runStartupChecks`'s `resolveFn` default), so the extra work is one added return key, `notices`, carrying the resolver's `{id, text}` records verbatim — not folded into `banner`, not flattened to strings. The flattening happens once at one render site, `formatStartup`, which appends `notice.text` per record and serves the five pipeline surfaces (`cmdDoctor` prints its own copy, §6.5); that is a different channel from `readEngineConfig`'s `string[]`, which `bin/cli.mjs`'s `tunablesFor` drains on its own path (§6.5, §10.1 S-7) |
| Plugin-root resolution | `pdlc/engine/lib/skills.mjs` | extended | `resolvePluginRoot` gains a `devDeclared` input so the env var stops being honoured on presence alone, **and its return gains one key**, `notices`, carrying the ignored-env entry — catalogue id plus the text rendered by `lib/catalogue.mjs`'s `message(id, params)` — on that branch only. The resolver decides and renders the notice because it owns the branch; startup surfaces it (V-11, §6.5, §10.1 S-7) |
...
**O-8 blocker 1 is discharged by this feature's manifest edit.** V-01 records three blockers
standing at HEAD. Blocker 1 is `"private": true` (`pdlc/engine/package.json:4`), and the row
above removes it as ordinary engineering work, with PF-3 asserting `private` is absent at
publish time (§8.3). What that does **not** do is change O-8's owner field: REQ's O-8 and
FSPEC's F-5 step 7 / Q-8 record all three blockers as operator-owned, those are upstream
documents, and nothing in this TSPEC rewrites them. Clearing O-8 still gates AC-3.1's
real-channel leg, and blockers 2 (npm scope, N-6) and 3 (`license`, N-2) remain operator
obligations. The narrow claim made here is only that no engineering work remains on blocker 1.

**Owed absorption, with an owner and a landing point (TE F-53).** The v0.13 changelog recorded
that FSPEC v0.3–v0.7 have not had a section-by-section re-grounding pass here, and that §5.1's
own six-row expected check set and BR-7.7 are not transcribed into this section's narrative
beyond §3.2's V-18/V-19 and §8.5. Round 13 showed the cost of leaving that untracked — TE F-51
was exactly this gap producing a wrong expected set one section over. The remainder is
therefore **owned by the PLAN task that builds §8.5's arrangement oracle** (the task that edits
`pdlc/engine/__tests__/ci-arrangement.test.js`), which must read FSPEC §5.1's expected rows and
BR-7.7 to author its constants: that task's Definition of Done includes transcribing the six-row
set and BR-7.7 into this section, or raising an FSPEC erratum if they disagree with HEAD. If
Phase T re-opens this TSPEC for any other reason before Phase P, the transcription lands in that
round instead. §5.4's own deferred items (TE F-49/F-50, PM F-02/F-03) stay parked on §5.4 and are
not folded in here.
...
(PK-5…PK-19 — V-03's twelve plus §3.1's three), three vendored workflow members
(PK-20…PK-22), `scripts/postinstall.mjs` (PK-23), and the licence (PK-3) at 0 before N-2's
decision is recorded and 1 after — 4 + 15 + 3 + 1 + 0/1, which is **23 members before N-2 and
24 after**, the same arithmetic FSPEC §5.2 states from its own side.
...
per-class counts** and this TSPEC owns its **member names**: any edit that adds, removes or
re-classes a `PK-*` row above updates FSPEC §5.2's per-class counts **in the same change** —
the mirror of the obligation FSPEC §5.2 already places on this table (its `TSPEC:386-389`
citation). The per-class counts and not the total are the change-control point, because a
member moved between classes leaves the total invariant and is still a co-change; PF-4's
member-for-member equality is what makes a missed co-change red rather than merely untidy.
...
does not flatten them to strings on the way out. The flattening happens **once for the pipeline
surfaces**, at one render site: `lib/startup.mjs`'s `formatStartup` appends one line per notice,
taking `notice.text` for a record (and the notice itself when it is already a string, so the key
...
surfaces and the refusal path are the **five `formatStartup` call sites** in `bin/cli.mjs`, so a
notice rendered at one call site would have been silently absent from the others.

**"One render site" is scoped to those five surfaces, and deliberately not stated as a
repo-wide set-equality.** `bin/cli.mjs`'s `cmdDoctor` does not print through `formatStartup` —
it prints `result.banner` itself and then flattens `result.notices` with its own
`typeof notice === "string" ? notice : notice.text` loop. That predates this feature, is
behaving correctly (the operator sees the notice text on `doctor` too), and AC-5.6 is about the
running pipeline (REQ's AC-5.6), of which `doctor` is not a surface. It is named here so a
later reader does not turn "single render site" into an oracle ("only `formatStartup` reads
`notice.text`") that would go red against a correct `doctor`. Routing `cmdDoctor` through
`formatStartup` would make the invariant literally true and is a cheap change, but no
acceptance criterion requires it and this TSPEC does not schedule it (PM Q-01).
...
shapes, one render site each for the pipeline surfaces; §3.1's two rows and §10.1's S-7 say this same split, so no
...
| S-7 | `PluginRootResolver` — `resolvePluginRoot({devDeclared, env, override, home, fs})` → `{ok, root, source, reason, tried, notices}`, the shipped return plus the one `notices` key §6.5 adds; `notices` is `{id, text}` records, non-empty only on the ignore branch | `lib/skills.mjs` | real `fs` and `process.env`; catalogue text via `lib/catalogue.mjs`'s `message(id, params)` | `lib/startup.mjs`'s `runStartupChecks` already takes it as `resolveFn` (default `resolvePluginRoot`) and **surfaces** the returned `notices` without re-deriving them — through one added key on its own return, rendered to `text` at one site, `formatStartup`, for the five pipeline surfaces `bin/cli.mjs` calls it from (`cmdDoctor` prints its own copy on its own path, §6.5), distinct from `readEngineConfig`'s `string[]` channel (§6.5). `notices` is carried on every return including both refusal legs, so it is never `undefined`. AC-5.6's path-level oracle drives the function directly over all four rows of §6.5's table — no injection needed, which is the point of deciding the notice at the resolver rather than one level up (§3.1, §6.5, §12.1) |
...
| Level | Runs | Covers |
|---|---|---|
| Unit, in-process | `pdlc/engine/__tests__/`, the shipped `node --test` suite | Resolution ladder, store enumeration, provenance rendering, catalogue closure, config read discipline, handshake/env-var branch, and **AC-5.6's path-level notice oracle** over `resolvePluginRoot`'s four rows, asserted on the resolver's own extended return (`notices`, §6.5 / §10.1 S-7) — the honour direction on the `devDeclared: true` × set row (resolved root `===` `path.resolve(env value)`, `source` unchanged, `notices` empty), emission by catalogue id plus rendered text on the ignore branch, and `notices` deep-equal `[]` on the two variable-unset rows, so the row is falsifiable in both directions (§6.5; the shipped catalogue equalities do not cover this row). All pure over injected seams — no temp dirs, no network, no spawn |
