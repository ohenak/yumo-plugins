...
|---|---|---|---|---|
| pdlc | Draft — in review (Phase T) | Claude | 0.8 | 2026-08-13 |
...
2. **Dev-mode kind equality (AC-5.3, §5.3).** Equality is over the kinds a run **actually
   ...
   **One leg reaches the module through the engine, not through a hand-built parameter object**
   ...
   at `run*()`** (TE v5 F-32): it calls `cli.mjs`'s exported `main(argv, deps)` with recorder
   runners in `deps` (the mechanism is §9.3's entry guard plus runner seam, spelled out in
   §12.1) and asserts all of `runDev` (`bin/pdlc.mjs:385`), `runQueueLoop` (`:434`) and
...
condition, two exported bindings — `main` and the default `deps` object, whose export exists so
the leg can pin the defaults against `run.mjs`'s real exports, §9.3, TE v7 F-41 — and one
defaulted parameter threaded to two command bodies) and
