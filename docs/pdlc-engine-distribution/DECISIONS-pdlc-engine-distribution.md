...
- **Resolution fails** — fall back to the launcher's **own** version, `mode: "unresolved"`, the
  ...
  instruction).
- **Exit code is 0 whether resolution succeeds or fails.** A diagnostic that exits non-zero because the thing it
