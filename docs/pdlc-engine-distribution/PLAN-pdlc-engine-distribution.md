...
|---|---|---|---|---|
| pdlc | Draft — in review (Phase P) | Claude | 0.2 | 2026-08-13 |
...
| AT-3.1 *(AC-3.1)* tag ⇒ built, checked, stub-recorded publish, no human step | T49, T52 |
...
**Counted against HEAD, §2 creates eighteen engine test files and extends five that already exist** (TE round-1 F-01 — the earlier "eighteen new" was off by the extensions). New: `preflight-baseline`, `store`, `resolve-version`, `provenance`, `plugin-root-notice`, `workflow-roots`, `update-probe`, `bin-guard-structure`, `launcher`, `version-doctor`, `packaging`, `docs-uniqueness`, `commit-sites`, `postinstall`, `startup-announce`, `provenance-path`, `publish-channel`, `fixture-machine`. **Extended, and declared as such in their rows**: `engine-config.test.js` (9 tests at HEAD, T10), `ci-arrangement.test.js` (T17), `run.test.js` (T33 and T41), plus `skills-composition.test.js` (12 tests at HEAD, T57) and `seam-contract.test.js` (T48). A whole-file write over any of those five deletes passing assertions **and leaves the suite green** — the one defect class a coverage gate cannot see, which is why each row states the extension rather than the file name alone.
...
14. `.github/workflows/pr-tests.yml` is byte-unchanged: its five rendered job names still satisfy FSPEC §5.1's set-equality (C-5, BR-7.5). The **new** `.github/workflows/fixture-machine.yml` is registered as a **required** check on the PR (TE round-1 F-16), so a fixture-machine failure halts Phase PUB rather than being polled and ignored: `gh pr view --json statusCheckRollup` sees six workflows where it saw five, and the DoD is not met while the sixth is red. Its job names are deliberately **outside** FSPEC §5.1's frozen set, which is about `pr-tests.yml` alone.
15. `docs/pdlc-engine-distribution/EVIDENCE-AT-6.2.md`, `EVIDENCE-AT-4.4.md` and `EVIDENCE-BR-3.9.md` exist, dated, each naming what was observed — for AT-6.2 the stated limit of the conjunction as a discriminator, and for AT-4.4 all three reported pairs (change **and** revert), since a constant fails on the second observation, not the first.
...
**Not in scope of this plan's done-ness.** N-1, N-3, N-4 and N-5 remain unbuilt by design (§1.2); AC-6.2 is therefore delivered as a limited manual observation and not as a discriminating test, which §1.2 now states in product terms rather than leaving to the evidence file. N-6 is discharged as a decision by T02; N-2 is discharged by T05 as a decision **and** as the two artefacts that decision makes expected.
