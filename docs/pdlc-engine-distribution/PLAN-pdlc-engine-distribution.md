...
| T07 | A | [red] [generated-input] Resolution-ladder totality over branches 0–7 including both corrupt-config cases of §6.4's table, and the announcement-never-empty property (AT-5.1, AT-5.2, AT-5.4, AT-5.5). Two of its legs are **generated, not enumerated** (TE round-1 F-12), drawn from T03's S-7: over `genVersionString()` × `genConfigShape()` the ladder is **total** (every input lands on exactly one of branches 0–7, none throws) and its announcement is **never empty**; the seed is printed on failure and the shrunk counter-example is pinned as a named example test | `pdlc/engine/__tests__/resolve-version.test.js` | — | 2 | T01, T03 | ⬚ |
...
| T18 | C | [red] AT-2.2 uniqueness over a **stated, tracked file set** — never a tree walk (TE round-1 F-05): the enumeration is `git ls-files -- 'README.md' 'pdlc/README.md' 'pdlc/engine/README.md' '*.md' ':(exclude)docs/'`, i.e. tracked markdown outside `docs/`, so the feature's own REQ/FSPEC/TSPEC/PLAN/cross-reviews (which quote every command) are out of scope by construction and an untracked `.claude/worktrees/` copy — **present on disk in this checkout** — can never redden it. Within that set, each engine install/upgrade/pairing command, keyed on `@{scope}/pdlc-engine` (the scope is T02's record), appears exactly once; the plugin's three `claude plugin install` occurrences (`README.md:115`, `pdlc/README.md:139,145`) are asserted **outside** the set as a positive, not by absence | `pdlc/engine/__tests__/docs-uniqueness.test.js` | — | 2 | T01, T02, T03 | ⬚ |
| T19 | D | [green] [standing guard] Green at authoring time and verified so at HEAD (`orchestrate-dev.js:2839` inside `buildA5SeamOps`, `:6736` inside `appendApprovalAnchors`, `:10429` inside `commitPaths`; `orchestrate-queue.js:1603` `commitQueueRow`, `:1645` `commitAdvisoryRecord`). Commit-site set-equality over both workflow modules' sources: the set of **enclosing named functions** containing a `git commit` invocation equals `{commitPaths, appendApprovalAnchors, commitQueueRow, commitAdvisoryRecord, buildA5SeamOps}`, stated unconditionally (AT-5.3). **Ships its own falsifier** (TE round-1 F-13), because a hand-transcribed expected set that is green on day one never demonstrates it can redden: the same scanner runs a second time over a **fixture source** carrying a sixth `git commit` inside a named function, and that run must report the sixth site and fail the equality; a scanner that returns the same set for both inputs fails the task | `pdlc/engine/__tests__/commit-sites.test.js` | — | 2 | T01 | ⬚ |
...
| T57 | A | [green] [standing guard] **AT-1.5 *(AC-1.2)*, which had no carrier** (PM round-1 F-03): extends the existing `skills-composition.test.js` (its twelve tests at HEAD survive verbatim; this appends) with the skills-source oracle — a fixture plugin root whose dispatched role's `SKILL.md` carries a **distinguishing marker string**, dispatched through `composePrompt`, asserting the **composed prompt contains that marker**; and the negative pair, a second fixture root whose marker differs, asserting the composed prompt carries the *second* marker and not the first, so the test cannot pass against engine-resident bytes. Green at HEAD by design: this is the load-bearing half of G-1's "no skills snapshot" promise (AC-1.3's packed-set equality proves the engine ships *no* skills; AT-1.5 proves it then reads the installed ones), and vendoring in T33 is exactly what could break it (AT-1.5) | `pdlc/engine/__tests__/skills-composition.test.js` | — | 2 | T01, T03 | ⬚ |
