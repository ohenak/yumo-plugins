...
| Field | Value |
|---|---|
| Upstream | `REQ → FSPEC → TSPEC → PLAN → **PROPERTIES**` — `docs/pdlc-engine-distribution/REQ-pdlc-engine-distribution.md` (v0.11), `FSPEC-pdlc-engine-distribution.md` (v0.7), `TSPEC-pdlc-engine-distribution.md` (v0.12), `DECISIONS-pdlc-engine-distribution.md` (v0.3), `PLAN-pdlc-engine-distribution.md` (v0.8) |
...
|---|---|---|---|---|
| pdlc | Draft — in review (Phase PT) | Claude | 0.5 | 2026-08-14 |
...
| 0.4 | **POSTMORTEM-PR follow-up, literal alignment only.** PROP-LAUNCH-3's discriminator no longer names the `"none installed"` message — the string exists in no shipped module — and pins the unparseable-manifest refusal as **not AT-1.1's `not found` message** instead, matching FSPEC v0.6's AT-1.4 and `pdlc/engine/lib/handshake.mjs`. Upstream cells re-pinned to FSPEC v0.6 and PLAN v0.7. No property added, removed, or re-scoped; §4's set-equality unchanged. |
| 0.5 | **Phase-PT erratum round, alignment only.** Nothing settled in rounds 1–2 is re-opened, no round-3 cross-review is addressed, and no scope is expanded. **Upstream re-grounding first (DEC-ERR-01).** Two upstream documents moved after v0.4 was written: FSPEC **v0.6 → v0.7** (`a57e0547`, round-6) and PLAN **v0.7 → v0.8** (the PLAN's own erratum round absorbing that FSPEC); REQ (v0.11), TSPEC (v0.12) and DECISIONS (v0.3) are unmoved. The Upstream cell now names v0.7 and v0.8. **Decisions absorbed, four of them, all no-ops here by construction:** (a) FSPEC §5.2's vendored-module class is renamed **Workflow members** because `PK-22` is a JSON manifest, not a module — this document never transcribes class names (reading rule 3 gives *classes and per-class counts* to FSPEC §5.2 and *member names* to TSPEC §5.4), and PROP-PACK-5 asserts the `modules` array of `VENDOR-MANIFEST.json` (two `.js` modules), which is the manifest's content and not the class's cardinality, so the rename touches nothing asserted; (b) FSPEC §5.2's CLI-entry and engine-module rows now anchor `PK-4`/`PK-4b` and `PK-5`…`PK-19`, and the CLI-entry note no longer calls its cardinality a downstream-only choice — PROP-PACK-1 and PROP-PACK-2 already source member names from TSPEC §5.4 and classes/counts from FSPEC §5.2, so the anchors change nothing they transcribe; (c) FSPEC v0.7 states AT-3.8a's count conjunct positively — PROP-PACK-2 already asserts the count against the **transcribed** `PK-*` list (23, or 24 once N-2's licence record lands) and forbids reading it off the tarball, so the conjunct is already in the positive form; (d) PLAN v0.8 retitles its §2.1 trace row for AT-1.1 to "refusal, plugin reported `not found`" with no task, batch or ownership-manifest change, so every carrier cell in §2 and §4 stands. **The one raised item, the same residue on this side:** PROP-LAUNCH-9's headline clause still said the refusal must "state that none is installed", the label FSPEC v0.6 retired — its conjunct (b) has pinned the shipped literal `not found` since v0.3, so the headline was drawing a reader back to a string no module emits. The clause now reads as FSPEC v0.7's AT-1.1 does: the refusal names the declared range and reports the plugin version as the literal `not found`. PROP-LAUNCH-3's discriminator (already re-pinned in v0.4) and PROP-LAUNCH-5's `not found` triple member need no change. No property added, removed or re-scoped; §4's set-equality against FSPEC §8, §5's coverage accounting and all counts are unchanged. |
...
|---|---|---|---|---|---|
| PROP-LAUNCH-1 | With no engine version installed, an invocation **must refuse** with a message that (a) names the store root it searched, (b) names the reason id `store.empty`, and (c) exits non-zero with the pipeline's own refusal code — never a bare `!= 0` against an unstated baseline. This is the **engine-store** refusal, not the plugin-handshake one: it says nothing about `pdlcPluginCompat`, and AT-1.1's criterion is carried by PROP-LAUNCH-9. **It is a resolver-shape property with no acceptance criterion of its own**, stated the way PROP-CAT-1/-2/-4 are: it traces to TSPEC §6.2's ladder branch for the empty store, *not* to AC-5.5. AC-5.5 is *"a pin naming a version that is not installed"* (`REQ:427-429`), whose message id is `version.pin-missing` and whose sole carrier is PROP-VER-5 — a different branch from this one's `store.empty`. The empty store is the precondition state that sits upstream of AC-5.5's pin case, and this row must never be read as a second AC-5.5 carrier. | Error Handling | Unit | TSPEC §6.2 | T15, T14 → `version-doctor.test.js`, `launcher.test.js`; T46 → `bin/cli.mjs` |
