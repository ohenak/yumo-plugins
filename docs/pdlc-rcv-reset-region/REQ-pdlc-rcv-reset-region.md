...
|---|---|---|---|
| **X-07** | **owed to** `pdlc-rcv-budget-stop` | AC-1.5(4)'s *region validates* predicate, its S-16 notice, and the phase refusal that follows a failure. `REQ-RCV-01` states the conjunct and the fail-closed outcome and cites AC-7.1 for the algorithm | **Paired edge — revise with `REQ-RCV-01` X-06 and R-14 in the same commit, in the same words** (`docs/_constraints/pdlc-rcv-split.md` §5). Until this REQ ships, `REQ-RCV-01`'s step-4 conjunct has no decision procedure, so **that REQ ships with the conjunct unwired**: its gate is the two decidable conjuncts (a readable `RESOLVED: yes`, `A < H`), the call site exists as an injected seam the interim composition does not consult, and no interim entry is refused. **This REQ wires the conjunct**, in the commit that lands AC-7.1. An interim *procedure* was considered and rejected, and the argument is stated once for both ends at `pdlc-rcv-split.md` §5.1: one refusing what it cannot decide refuses on non-emptiness — which is *the phase has halted* — making each phase's first halt terminal with `RESOLVED: yes` unable to clear it and no repair shipped, including on this row's own Phase R; one granting what it cannot decide is the fail-open AC-1.5(4) exists to close; and a **narrower** one deciding only what `REQ-RCV-01` specifies escapes both horns but must still disagree with AC-7.1 on ordering and highest round, unclearable in the refusing direction until AC-7.4. The queue orders this row at **`Order 18`** (net pickup 10 → 12 → 18), not immediately behind row 10, and `REQ-RCV-01`'s `depends-on` is empty because nothing it needs *as a requirement* is owed here |
...
| **R-16** | **`REQ-RCV-01` is deliverable as a requirement but has no decision procedure for its step-4 conjunct until this REQ ships** (X-07), and this row is a whole intervening feature away — `Order 18`, net pickup 10 → 12 → 18. | **Mitigated by leaving the conjunct unwired on row 10, not by sequencing and not by a stub** (paired with `REQ-RCV-01` R-14; `pdlc-rcv-split.md` §5). Sequencing is too weak at that distance. A stub returning *invalid* is **not** safe: the predicate is resolved on every entry and a region is non-empty exactly when the phase has halted, so it would refuse every document-typed review phase after its first halt — this row's own Phase R included — with `RESOLVED: yes` unable to clear it. A stub returning *valid* is the fail-open this REQ exists to close, and a defect once the conjunct **is** wired. A **narrower** interim procedure — deciding only what `REQ-RCV-01` AC-1.5(4) specifies — escapes both, but must still disagree with AC-7.1 on ordering and highest round, and a disagreement in the refusing direction is unclearable until AC-7.4 (`pdlc-rcv-split.md` §5.1, where the argument is stated once for both ends). So row 10 ships the two decidable conjuncts behind an unconsulted seam, and this REQ wires the third with AC-7.1. Residual until then: `REQ-RCV-01` R-10's hand-edited-region fail-open, no wider than HEAD's. |
