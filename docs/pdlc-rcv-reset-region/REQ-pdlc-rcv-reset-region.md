...
| **O-12** | Specify the **answering-line append and its confirmation** on the granting path: append one `WINDOW-START:`/`WINDOW-RESUMED:` line at the end of the region, re-read and confirm it **by byte comparison against what was written**, re-running AC-7.1 steps 1–3, and **dispatch no round until it is confirmed**; on an unconfirmable write take AC-7.5's fail-closed exit (no window, no round, phase refused, row B's unconfirmable-append variant). Specify how the comparison is performed so that a value tear is caught — a length or presence test is explicitly insufficient. The read-modify-write on the **halt** path is `REQ-RCV-01` O-5's and is not restated here. **The *validate* seam's contract is fixed here, by this obligation, and nowhere else** — corrected at `REQ-RCV-01` v2.7 (round 1) because the two O-12s previously cited each other and no document fixed it. It is arity 2 (the region, the branch listing), returning a result carrying the verdict and, when false, AC-7.1 step 4's `{reason}` from the closed set of three; the notice itself is emitted by the caller's run-report builder, so the seam takes no report sink. Row 18 therefore wires the conjunct without changing the contract `REQ-RCV-01`'s interim 0-call leg is asserted against (that REQ's O-10, restored at its v2.7). | TSPEC |
...
| **R-16** | **`REQ-RCV-01` is deliverable as a requirement but has no decision procedure for its step-4 conjunct until this REQ ships** (X-07), and this row is **three** intervening features away — `Order 18`, net pickup **10 → 12 → 13 → 17 → 18**, with row 17 (the S-11 emitter) ahead of it. | **Mitigated by leaving the conjunct unwired on row 10, not by sequencing and not by a stub** (paired with `REQ-RCV-01` R-14; `pdlc-rcv-split.md` §5). Sequencing is too weak at that distance, and all three interim procedures — the *invalid* stub (which would refuse every document-typed phase after its first halt, this row's own Phase R included), the *valid* stub (the fail-open this REQ exists to close) and the narrower procedure (which must still disagree with AC-7.1 in the unclearable direction until AC-7.4) — are rejected once for both ends at `pdlc-rcv-split.md` §5.1, and not restated here. So row 10 ships the two decidable conjuncts behind an unconsulted seam — plus `REQ-RCV-01` §6's S-13/S-14 **grammar**, which is in force there and which AC-7.1 step 2 re-checks — and this REQ wires the third conjunct's **analysis** layer with AC-7.1. **Two residuals until then**, both stated identically at `REQ-RCV-01` R-14: (i) R-10's hand-edited-region fail-open, no wider than HEAD's; (ii) from row 17 onward, machine-written `WINDOW-RESUMED:` lines land in regions nothing validates — **not** covered by "no wider than HEAD's", since HEAD writes no region lines at all, and bounded only by the two live conjuncts' accounting until this row's AC-7.1 and AC-7.4 land. If row 17 is picked up before this row, that is the moment to reconsider moving this row ahead of it by `Order`. |
