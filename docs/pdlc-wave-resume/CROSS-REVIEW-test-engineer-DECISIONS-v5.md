...
Two findings are `delta`: the round-5 TSPEC erratum created them, by moving §3.1's value tally under
O-8 (F-01) and by demoting the second conjunct of §2.4's rule under DEC-WVR-03's criterion (F-03).
Both are `local` — they sit in the DECISIONS material whose upstream counterpart this edit changed.
The remaining six are `inherited`: DECISIONS' bytes have not changed since `a0cb8d32`, and this
erratum introduced none of them; all six are `nonlocal`, sitting in material the delta never
reached. One prior finding (v4 F-06) is **not** re-filed: upstream adopted its position.

| ID | Severity | Provenance | Locality | Finding | Section anchor |
|----|----------|-----------|----------|---------|----------------|
| F-01 | Medium | delta | local | O-8 asserts “the correct count is three reasons carrying **four** interpolated values”, and DEC-WVR-06 repeats “three interpolate run-specific values”. TSPEC §3.1 at v1.4 now says three reasons “carrying **five** interpolated values between them”, because `feature-mismatch`'s renderer names both the recorded feature and the run's — which DECISIONS' own enumeration (`it records feature "X", not "Y"`) contains but counts as one. An approved document now contradicts upstream on a counted fact (DEC-ERR-01). Non-gating: the tally lives only in a rejected alternative's rationale; DEC-WVR-06's oracle is set equality over the **seven codes**, unchanged, and neither PLAN nor PROPERTIES transcribes either number (verified by grep). Update four → five, or drop the tally and keep the reason count of three. | O-8; DEC-WVR-06 |
| F-02 | Medium | inherited | nonlocal | Carried from v2/v3/v4 F-01, unresolved and now doubly stale. O-5's closing parenthetical says “TSPEC §2.4's announcement table omits the invalid-pointer notice entirely rather than excluding it by rule; that is an upstream gap”. §2.4 has closed the catalogue by rule and named the excluded notice in a one-row table since v1.2, and v1.4 goes further, re-deriving the exclusion and crediting TE. Non-gating: DECISIONS held the winning side and DEC-WVR-03's set-equality oracle transcribes identically either way. Delete the parenthetical. | O-5, closing parenthetical (→ DEC-WVR-03) |
| F-03 | Low | delta | local | DEC-WVR-03's criterion is stated as a two-conjunct biconditional — “a notice carries a provenance token **iff the resume decision emits it about a resolved start point**” (lines 146–148, restated at 288). TSPEC §2.4 v1.4 now says the second conjunct “does **not** discriminate here”, because a clamped past-the-end pointer is also a rejected value yet still carries a token. The biconditional remains extensionally true, so this is not a correctness defect — but a PROPERTIES author who builds the announcement filter on the second conjunct rather than on DECISIONS' own enumerated literals will wrongly exclude the past-the-end row. Strike or footnote the second conjunct, citing §2.4 v1.4. | DEC-WVR-03 criterion (lines 146–148, 288) |
| F-04 | Low | inherited | nonlocal | Carried from v3/v4 F-02, unresolved. O-8's closing parenthetical says “TSPEC §3.1 says 'four of the seven reasons interpolate'”. §3.1 has said **three** reasons since v1.2 and says three at v1.4; the “four” it once carried was the value tally, not the reason count. Same class as F-02, and the same fix: delete the parenthetical. | O-8, closing parenthetical (→ DEC-WVR-06) |
| F-05 | Low | inherited | nonlocal | `Process`. Carried from v3/v4 F-03, unresolved and now demonstrated in both directions. Erratum raises written into the durable document body have no expiry — O-5's and O-8's parentheticals have survived four confirmations as stale text, one acquiring a second error this round. By contrast the raise I kept in the cross-review (v4 F-06) was retired the round upstream adopted it. File errata in the cross-review; state only the position taken in the body. | Cross-cutting (O-5, O-8) |
| F-06 | Medium | inherited | nonlocal | Carried from v2 F-01 / v3 F-04 / v4 F-04, unresolved. DEC-WVR-04's write-side key-set oracle conditions `head`'s presence on “the transport being injected”; the shipped write site fills `waveHead` only when the transport is injected **and** `rev-parse HEAD` returns a sha, so a strict key-set equality reds on a fixture where `rev-parse` fails. Untouched by this delta, though TSPEC AT-05's new write-side conjunct and §5.5 mutation 5 now strengthen the same seam. State the two admissible key-sets instead of one. | DEC-WVR-04, Consequences |
| F-07 | Medium | inherited | nonlocal | Carried from v2 F-02 / v3 F-05 / v4 F-05, unresolved. DEC-WVR-05's re-evaluation trigger is stated observably, but no Consequences row obliges anyone to build the observer — a trigger with no detector can never fire, which makes the decision effectively irreversible in practice while documented as reversible. | DEC-WVR-05, trigger vs. Consequences |
| F-08 | Low | inherited | nonlocal | Carried from v2 F-04 / v3 F-07 / v4 F-07, unresolved. The v1.1 revision-history row ends “No decision, alternative disposition or downstream obligation changed”, but v1.1 added four downstream obligations. A PLAN or PROPERTIES author who trusts that line skips four obligations. | Revision history, v1.1 row |

FINDING: Medium | delta | local | O-8 / DEC-WVR-06 interpolated-value tally | O-8 asserts "three reasons carrying four interpolated values" but TSPEC §3.1 at v1.4 says five (feature-mismatch names both the recorded and the run's feature) — an approved document contradicting upstream on a counted fact. Non-gating: the tally sits in a rejected alternative's rationale, DEC-WVR-06's oracle is set equality over the seven codes (unchanged), and neither PLAN nor PROPERTIES transcribes the number.
FINDING: Medium | inherited | nonlocal | O-5, closing parenthetical (→ DEC-WVR-03) | v2/v3/v4 F-01, still open and now doubly stale: O-5 raises §2.4's missing exclusion as an outstanding upstream gap, but §2.4 has excluded by rule since v1.2 and v1.4 re-derives the exclusion crediting TE. DECISIONS holds the winning side and the set-equality oracle transcribes identically either way.
FINDING: Low | delta | local | DEC-WVR-03 criterion, lines 146–148 and 288 | The criterion keeps the conjunct "about a resolved start point", which TSPEC §2.4 v1.4 now says does not discriminate — a clamped past-the-end pointer is a rejected value yet still carries a token. Extensionally still true, but it invites a PROPERTIES author to build the announcement filter on the non-discriminating conjunct instead of the enumerated literals, wrongly excluding the past-the-end row.
FINDING: Low | inherited | nonlocal | O-8, closing parenthetical (→ DEC-WVR-06) | v3/v4 F-02, still open: O-8 cites §3.1 as saying "four of the seven reasons interpolate"; §3.1 has said three reasons since v1.2 and still says three at v1.4. No oracle transcribes the count.
FINDING: Low | inherited | nonlocal | Cross-cutting (O-5, O-8), Process | v3/v4 F-03, still open and now demonstrated both ways: erratum raises in the durable body have no expiry — O-5's and O-8's parentheticals have survived four confirmations as stale text — while the raise kept in the cross-review (v4 F-06) retired the round upstream adopted it.
FINDING: Medium | inherited | nonlocal | DEC-WVR-04, Consequences | v2 F-01 / v3 F-04 / v4 F-04, still open: the write-side key-set oracle conditions `head` on transport injection alone, while the shipped site also requires `rev-parse HEAD` to succeed — strict key-set equality reds on a fixture where `rev-parse` fails. State two admissible key-sets.
FINDING: Medium | inherited | nonlocal | DEC-WVR-05, trigger vs. Consequences row | v2 F-02 / v3 F-05 / v4 F-05, still open: the observable re-evaluation trigger has no Consequences row obliging anyone to build the observer, so the trigger cannot fire and the reversibility claim is untestable.
