...
|----|----------|-----------|----------|---------|----------------|
| F-01 | Medium | delta | local | The replacement wording calls the worktree include list "consumer-local ... a consumer fact and not a repo fact". Upstream says otherwise: `.worktreeinclude` was a **tracked** repo artifact, deleted from the default branch by `pdlc-plugin-retirement` as retirement class **M-11j** (`origin/main:docs/_constraints/pdlc-retirement-baseline.md`). What is genuinely consumer-local is `.claude/workflows/` and `.claude/pdlc-wave-state.json`, both anchored in `origin/main:.gitignore` under "Machine-local pdlc consumer runtime + wave ledger — never shared". The "untracked on the default branch" half is true; the reason welded to it is not. Conclusion unaffected — cite the `.gitignore` block, or attribute the absence to M-11j. | OB-1, worktree clause |
| F-02 | Medium | inherited | local | The same sentence closes "consistent with the D-DIST-07 deferral". Upstream no longer records a deferral: `origin/main:docs/_decisions/DECISIONS-plugin-distribution.md:209` says D-DIST-07 "dissolves with the consumer copy that `pdlc-plugin-retirement` removes", and `origin/main:docs/_queue/QUEUE.md:230` says it "closes by construction". Retirement has landed (zero tracked `.claude/workflows/` entries on `origin/main`), so the REQ hands TSPEC a closed decision described as open. Say "closed by construction" and the fail-open conclusion stands unchanged. | OB-1, D-DIST-07 clause |
| F-03 | Medium | inherited | nonlocal | Carried from v5 F-01, partially addressed. §5's row now names the outcome, but the header "Note on this branch's base" still calls the rebase "a branch-management step owed before FSPEC authoring" while §10 says "It is owed before implementation". FSPEC authoring is complete and BL-04 is unmet, so the header still reads as a missed deadline and §10 as a pending one. Align the header note to §10's "before implementation", or state plainly that the FSPEC-authoring deadline passed unmet. | "Note on this branch's base" vs §10 |

FINDING: Medium | delta | local | OB-1, worktree clause | The new wording labels the worktree include list "consumer-local ... not a repo fact"; upstream says `.worktreeinclude` was a tracked repo artifact deleted by pdlc-plugin-retirement as class M-11j (docs/_constraints/pdlc-retirement-baseline.md). The genuinely consumer-local artifacts are `.claude/workflows/` and `.claude/pdlc-wave-state.json`, both gitignored on origin/main under "never shared". Right fact, wrong reason — cite the .gitignore block or M-11j; the fail-open conclusion is unaffected.
FINDING: Medium | inherited | local | OB-1, D-DIST-07 clause | OB-1 cites "the D-DIST-07 deferral", but upstream now records D-DIST-07 as closed: DECISIONS-plugin-distribution.md:209 ("dissolves with the consumer copy that pdlc-plugin-retirement removes") and QUEUE.md:230 ("closes by construction"), and retirement has landed. Describe it as closed, not deferred.
FINDING: Medium | inherited | nonlocal | "Note on this branch's base" vs §10 | BL-04 still carries two deadlines — the header note's "owed before FSPEC authoring" against §10's "owed before implementation". §5's edit fixed the outcome mismatch but not this one; align the header note to §10 or say the FSPEC-authoring deadline passed unmet.
