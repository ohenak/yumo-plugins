...
| TSPEC §5.8 states the c8 `include` set as three modules | **Open, and newly raised this round (PM F-02).** `TSPEC:838` reads `include: ["orchestrate-dev.js", "orchestrate-queue.js", "build-runtime.mjs"]`, but `pdlc/workflows/package.json`'s `c8.include` carries **four** `**/`-anchored entries, the fourth being `**/scripts/capture-learnings-baseline.mjs`. § 11 above corrects the fact locally, which is not enough on its own: TSPEC §5.8 keeps a stale include list that the next document derived from it inherits. | **Yes** — one `ERRATUM: TSPEC` line. § 11's local correction stands in the meantime. |
