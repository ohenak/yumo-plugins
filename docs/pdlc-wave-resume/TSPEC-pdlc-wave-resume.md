...
| 1.4 | **Erratum round (round 5, Phase PR).** Corrections only; no decision re-litigated and no scope change. §5.7: the generative run count is **pinned at `numRuns: 500`** on the `advisoryHelperProperties.test.js` precedent, matching PLAN T-08 and PROPERTIES, instead of being left to fast-check's default (PM, TE). §5.8: `c8.include` carries **four** entries, not three — the fourth, `**/scripts/capture-learnings-baseline.mjs`, is external to `pdlc/workflows/` and is why `allow-external` is set; `--per-file` keeps a red there off this feature's module (PM, TE, SE). §3.1 and §6.1 DEC-WVR-06: the interpolated-value count is **five**, not four — the `feature-mismatch` renderer interpolates both the recorded feature and this run's (PM, TE). §2.5 restated as a **ratification of FSPEC §3.4**, which states the operator-pointed write at HEAD, and §6.3 items 1–4 recorded as **landed upstream** (REQ v1.7 / FSPEC v1.2) with current version labels; §6.2 OB-F1's re-raise justification struck and §1.3 repointed at REQ OB-1's current framing — OB-F1's substance (BL-04 unmet, AT-14 red, wave sequencing) unchanged (PM). §5.4 AT-05 gains a **write-side conjunct** and §5.5 a fifth mutation, killing a suppressed write while `explicitPointer` is true (PM). §5.2 H-1 restated as a reuse-and-consistency choice, not a harness expressiveness limit (PM). §2.4: the exclusion column now names the **first** conjunct as the discriminating one, since a clamped past-the-end pointer is also a rejected value yet does carry a token (TE). |
| 1.5 | **Erratum round (round 6, Phase DOD).** Corrections only; no decision re-litigated and no scope change. §5.4: the `feature-mismatch` and `plan-changed` fixture rows now state **`head` present** and why, matching the shipped fixtures (`head: DISREGARDED_HEAD`) and PROPERTIES v1.6's twin table, and AT-03's gloss carries the same clause — a head-less fixture makes the `toEqual([])` zero-probe conjunct vacuous (CODE_REVIEW v2 B-5). §5.5 mutation 4: the kill condition is restated as the conjunction of `toEqual` on the filtered call list **and** an ancestry-independent fixture carrying a `head`, citing the survival and subsequent kill measured in `MUTATION-EVIDENCE-pdlc-wave-resume.md` (CODE_REVIEW v2 B-6). |
...
|---|---|---|
| `Notice: implementation.startWave in {cfg} is not a valid value — using the default.` | **No** | **The discriminating conjunct is the first one — *the resume decision emits it*.** This notice is emitted by config validation, *before* any resume decision runs, so the resume decision never emits it at all. The second conjunct does **not** discriminate here and is not what excludes it (TE): a past-the-end `implementation.startWave` is also a rejected value, is clamped, and still yields a start point that carries ` (provenance: operator-set)` in the table above — so "about a rejected value rather than a resolved start point" would exclude a row that is in fact included. What is true of this notice specifically is that the pointer it names is discarded and no operator pointer is in force for the run that follows, so FSPEC BR-07's "full run reached by an operator pointer" does not describe it. That run is an ordinary automatic run, and whatever it announces afterwards is one of the six rows above — including the silent IG-6 row when there is no record. |
...
|---|---|---|---|
| H-1 | An **ordered event sink** that both the `_runCommand` and `_git` doubles append to, so relative order is observable. `makeLedgerArgs` gains an optional `events` array; when supplied, each double pushes `["runCommand", cmd]` / `["git", …argv]` to it in addition to its own log, and the existing per-double logs are unchanged so no shipped assertion moves. | `makeLedgerArgs` gives `runCommand` and `git` two **independent** call logs; nothing records their interleaving, and AT-04's oracle is precisely "the gate ran before the first commit". **This is a reuse-and-consistency choice, not an expressiveness limit (PM):** the doubles are caller-supplied, so a single test could append both to one local array with no harness change at all. The extension is preferred because it gives the whole ledger block *one* ordered sink with a single shape, rather than one bespoke array per test that needs ordering — the same reason the per-double logs live in `makeLedgerArgs` and not in each test. | AT-04 |
...
| AT-03 ordering of disregard causes | unit + integration | *Unit:* a record failing **both** ancestry and over-count classifies `head-unreachable`, not `over-count` — the one pair where §3.2's order diverges from the REQ's IG numbering. *Integration (the laziness oracle, TE F-02/PM F-05):* on a feature-mismatch fixture and on a plan-hash-mismatch fixture, the `_git` spy's calls filtered to `merge-base` are asserted `toEqual([])` — **zero**, equality not containment, so an extra probe reds. **Both mismatch fixtures must themselves carry a `head`** (the shipped fixtures use `head: DISREGARDED_HEAD`): `headCorroborated` returns before touching the transport when the record has no `head`, so a head-less fixture makes the zero-probe conjunct vacuous and an eagerly-probing build indistinguishable from a lazy one — measured, in `MUTATION-EVIDENCE-pdlc-wave-resume.md`. Paired positive: the same filtered list on the ancestry fixture is asserted `toEqual([["merge-base", "--is-ancestor", HEAD_SHA, "HEAD"]])` — exactly one. |
...
| AT-05 operator override wins | integration | With both a valid record and `startWave: 2`: resume at 2 and **no** log line matching `wave ledger … was ignored` — the record was never consulted. The provenance conjunct **names its announcement** (TE F-05): the single log line beginning `Resuming at wave 2 of 3 (implementation.startWave)` — the operator banner of §2.3/V-18, not any other line — ends with the literal ` (provenance: operator-set)`, asserted on that filtered element. A token appearing anywhere else does not satisfy it. **Write-side conjunct (PM):** after the run the operator-pointed run must itself have *written* a record — `ledgerWrites` is non-empty and the written `lastGreenWave` is the **plan-absolute** number of the last wave this run completed, not a run-relative count. Without it, a mutation that suppresses the write while `explicitPointer` is true stays green here and everywhere else (§5.5 mutation 5), removing resume from the recovery path §2.5 ratifies the write for. |
...
Five mutations this suite is specifically designed to kill, because each would otherwise pass:
...
4. **Resolving the ancestry probe eagerly** for every well-formed record (§2.2, DEC-WVR-08). Killed
   only by AT-03's and AT-11's `merge-base` call-count assertions, and only under a **conjunction**:
   `toEqual` on the filtered call list **and** an ancestry-independent fixture that itself carries a
   `head`. Neither conjunct suffices alone. `toContainEqual` — what the shipped ancestry test used —
   passes under the mutation; so does `toEqual([])` on a head-less fixture, because `headCorroborated`
   returns before reaching the transport and there is no probe to observe either way. This is measured,
   not asserted: `MUTATION-EVIDENCE-pdlc-wave-resume.md` records the mutation **surviving** on first
   application (212 passed, 0 failed) against head-less fixtures, and killed (2 failed / 210 passed)
   once `feature-mismatch` and `plan-changed` carried `head: DISREGARDED_HEAD` with their expected
   `merge-base` list still `[]`. Every behavioural test in §5.4 stays green under the mutation — the
   announced reason and the resolved outcome are identical either way. This is the mutation that makes
   both the choice of matcher and the shape of the fixture load-bearing rather than stylistic.
5. **Suppressing the record write while `explicitPointer` is true** (writing only on automatic
   runs). Killed only by AT-05's write-side conjunct. Without it the mutation leaves AT-05, AT-07,
   AT-15 and AT-18 green — those three drive automatic-provenance runs, and AT-05 formerly asserted
   only the resume point, the operator banner's provenance token and that the record was never
   *consulted* — while removing resume from exactly the recovery path §2.5 ratifies the write for.
