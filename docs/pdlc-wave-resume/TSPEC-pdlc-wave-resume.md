...
| DEC-WVR-06 | **Reason codes, not rendered sentences, are the closed catalogue.** | *Set equality over rendered strings*: rejected — three of the seven interpolate run-specific values (four values in total, §3.1), so the assertion would be over fixture data rather than over the catalogue. |
