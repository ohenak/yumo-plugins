...
| T-10 | `pdlc/workflows/__tests__/waveResume.test.js`, `pdlc/workflows/__tests__/waveExecution.test.js`, `pdlc/workflows/__tests__/waveResumeRepoState.test.js`, `pdlc/workflows/scripts/check-wave-resume-delta-coverage.mjs`, `pdlc/workflows/package.json`, `pdlc/workflows/__tests__/coverageInstrumentation.test.js` | 4 | the three suites existing by then (second owner of each — `waveResume.test.js` T-02 batch 2, `waveResumeRepoState.test.js` T-03 batch 2, `waveExecution.test.js` T-07 batch 3, all ≠ batch 4, and `T-10 → T-02/T-03/T-07` are real `Deps` edges); the script **new**; `package.json` and `coverageInstrumentation.test.js` existing and tracked, sole owner of each in this plan — both are oracle (ii)'s wiring, added in Phase CR round 1 (TE F-04) |
