...
double and golden fixture, and every task that *consumes* one depends on it (rule 4). **R1 is the one
exception and deliberately so** — it is F1's wave-1 peer with no dependency, because it consumes
neither a double nor a golden: its tests extend the existing `haltAndQueue` / `runtimeBundle` /
`seams.js` harnesses. Read the rule as "every consumer is downstream of F1", not as a universal over
all 17 tasks (TE F-04).
...
| D1 | Adapter transport — TSPEC §11.3. `rtGhRun` with `rtGit`'s JSON-object reply (`runtime-adapter.js:932`–`:938`), the at-most-once mutation sentence and the do-not-retry clause, plus `_ghRun` in `rtDevInjections` **and the rename of the `_recordHalt is deliberately ABSENT` comment at `runtime-adapter.js:1004` to `_recordQueueRow`** — the one site R1 cannot reach, and the one the §11 "no seam named `_recordHalt` remains anywhere" box would otherwise trip over (TE F-05). Red first: the injection key is present; the prompt interpolates only the command it was handed; **both prompt clauses**; and the three reply arms — `ok:true` ⇒ `{ok,stdout,stderr:""}`, `ok:false` **preserving `stderr`**, unparseable ⇒ `"unparseable adapter response"` | `pdlc/workflows/__tests__/mergeAdapter.test.js` | `pdlc/workflows/runtime-adapter.js` | 7 | A5 | ⬚ |
...
| A8 | Pipeline wiring and the report — TSPEC §10.1, §10.4. `main()` gains `_phaseMergeEnabled` and `_ghRun`, calls `phaseMerge` after Phase PUB inside the same guarded body, forwards `_git`/`_readFile`/`_recordQueueRow`/`_now`/`_sleep`, pushes escalations then notes onto `notices`, and `buildFinalReport` emits `mergeStatus`/`mergeSha`/`mergeMethod` unconditionally with `queueRow` carrying the disposition on a merged run. **Adds `_ghRun` to `runtimeBundle.test.js:212`'s `AT19_SEAM_NAMES`** so the new transport is inside the await-discipline scan (`_phaseMergeEnabled` stays out — a boolean never called, `_phaseDodEnabled`'s rationale). **Also owns `RLH-AT-32-orch-merged`**, the sibling case, because A8 is the task that builds the behaviour and can therefore genuinely red it (TE F-02). Red first: the sibling case; the three fields present on **every** report including the halt path (row 23); RLH-AT-64 green for both new seams; RLH-WIRE-01 still green | `pdlc/workflows/__tests__/mergePhase.test.js`, `pdlc/workflows/__tests__/haltAndQueue.test.js`, `pdlc/workflows/__tests__/pipelineWiring.test.js`, `pdlc/workflows/__tests__/reportTemplates.test.js`, `pdlc/workflows/__tests__/runtimeBundle.test.js` | `pdlc/workflows/orchestrate-dev.js` | 9 | A7, B3 | ⬚ |
| A9 | `RLH-AT-32-orch` **re-expression only** — TSPEC §13.4, FSPEC §7.5 (F-13). Restate the existing case (`haltAndQueue.test.js:809`–`:821`) as "a successful direct run **that did not merge** records no status", pinning `mergeStatus` to a non-merged value so the premise is explicit, and preserving all **four** of its assertions: `outcome === "success"`, the three `not.toContain` on `recordQueueRow.statuses`, **and `result.queueRow === "none"`** — the fourth is the one A8's `mergeOutcome.queueRow ?? "none"` change makes load-bearing, so it must be re-asserted deliberately rather than inherited. The merged sibling belongs to A8. **Declared deviation:** this task has no red phase by construction — it retro-fits a premise onto behaviour A8 already built, so "observe it fail first" is unexecutable at wave 10 (§2 rule 1; TE F-02). Its gate is instead that the re-expressed case passes **and** that reverting A8's `queueRow` line reds it | `pdlc/workflows/__tests__/haltAndQueue.test.js` | *(none — test-only)* | 10 | A8 | ⬚ |
