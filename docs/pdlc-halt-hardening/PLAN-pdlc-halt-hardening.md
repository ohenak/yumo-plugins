...
| R3 | ≥1 High tagged `delta`, **and** every finding (any severity) is `local` to the prior round's edited sections, **and** the follow-up budget for this doc is unspent | dispatch **one** follow-up erratum round (`MAX_ERRATUM_FOLLOWUP_ROUNDS = 1`, shipped constant, per upstream doc per phase invocation), its item list = the confirmers' findings verbatim; then re-confirm. A second non-approving confirmation → R4 |
