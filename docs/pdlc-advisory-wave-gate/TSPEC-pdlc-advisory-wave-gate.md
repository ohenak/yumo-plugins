...
| Bare row-count assertions | **Four** sites assert `rows).toHaveLength(5)` on a report's advisory rows: `advisoryDisabled.test.js`'s `T-10-5 / PROP-DIS-05 — enabled-but-quiet reports five zero rows (S-1)` block (whose per-row `invocations === 0` loop already covers the new row unchanged), `advisoryQueueSeams.test.js`'s `ADVISORY_SEAMS drives the row list (S-1)` assertion, and **both** sites in `advisoryHarvest.test.js` — the one immediately above that file's `seamNames` literal, and the one whose neighbourhood is a member *lookup* (`rows.find((r) => r.seam === "A1")`) rather than a member list, so an instruction to retarget seam literals never reaches it | `5` → `6` |
| `.enabled` occurrence count | `advisoryDisabled.test.js`'s `PROP-DIS-06 — exactly three \`.enabled\` reads outside parseAdvisoryConfig` block counts `/\.enabled\b/` over `orchestrate-dev.js` + `orchestrate-queue.js` raw source with `parseAdvisoryConfig`'s body excised, and requires exactly **three** — today `runAdvisorySeam`'s disabled-tier early return, the run-level `const advisoryTierOn = advisoryConfigResult.config.enabled`, and `orchestrate-queue.js`'s `finish` closure | **unchanged at three** — a constraint on A6, not an edit: §3.2 step 2's tier gate must receive the resolved boolean and add no fourth `.enabled` token |
...
| File | Status | Carries |
|---|---|---|
| `pdlc/workflows/__tests__/advisoryWaveGate.test.js` | new | A6's parsers, envelope classification, invocation ordering, snapshot/restore, wave budget, the prohibition tests of §5.5, the capture-failure disposition, the post-gate un-skip halt fields, **and both arms of BR-14's co-located overwrite warning — `AT-06-4` (capture succeeded) and `AT-06-4b` (E-34, `snapshotRef: null`), §5.6** |
...
| AT-06-4 | `advisoryWaveGate.test.js` | halt report following an escalation that captured a pre-A6 tree state carries **all three** of FSPEC v1.7's conjuncts on one run: (1) the diagnosis, (2) the root-cause class, and (3) the overwrite statement rendered from a non-`null` `snapshotRef` (§4.5's halt fields). Conjunct (3)'s oracle is **co-location within one rendered report string** — the ref pointer and the overwrite sentence found in the *same* `haltError` report text, not merely both present somewhere in the run, since two independent `toContain` assertions over separate strings cannot falsify a split. Presence of the statement, never a verbatim sentence pin (TE Q-01: FSPEC pins co-location as the observable, so no sentence is transcribed into §5.5) and never the capture's name (O-1) |
| AT-06-4b | `advisoryWaveGate.test.js` | the **negative arm**, on §5.2's existing E-34 capture-failure fixture (`snapshotRef: null`, §4.5's literal field values): the halt report carries the diagnosis and the root-cause class, points at **no** ref, and carries **no** overwrite sentence anywhere in it. This is what makes AT-06-4's conjunct (3) falsifiable rather than a string that is always present — an implementation emitting the warning unconditionally passes AT-06-4 and fails here. No new file and no new double: `advisoryWaveGate.test.js` already owns the capture-failure disposition |
