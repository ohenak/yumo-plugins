...
| E-25 | `advisory.seamBudgetMinutes` is exceeded on one invocation | Escalate with the tier's `budget-exhausted`. The budget is measured dispatch→verdict on that one dispatch (BR-11); the gate command runs outside that window, so NFR-4's carve-out needs no subtraction here. |
...
| E-33 | `advisory.waveBudgetPerRun` is absent, malformed, or zero | Absent or malformed falls back to the shipped default `1`, per key independently like the other advisory keys, with the key named in the tier's existing invalid-key report; a malformed value never disables the seam or the tier. An explicit `0` is **honoured as written** — a legitimate operator setting, not a misconfiguration: the tier stays enabled, every red wave escalates with no dispatch, and the sixth summary row reads zero. This key therefore validates as a **non-negative** integer, a distinct variant from the shipped positive-integer validator, which rejects `0` and substitutes the default; reusing the shipped variant would silently turn an operator's `0` into `1`, the one failure direction the tier's safety case cannot absorb (E-32, BR-11, REQ C-2, AT-07-2b). |
...
| E-30 | The escalation log cannot be written | The escalation still happens and the operator is still told; the carrier is the halt report, the same artifact BR-14 already makes carry the diagnosis and its root-cause class, which here also states that the log write failed. A failed log write never upgrades an escalation to a resolution and never changes the halt (BR-13, BR-14, AT-06-6). |
...
- **AT-04-3** — *Who:* the workflows suite. *Given* an A6 invocation of any outcome. *When* the run
  completes. *Then* the set of committing writer **identities** is unchanged from the pre-A6 baseline — the pathspec-scoped per-task commit and the post-wave-pathspec build-output commit, both still reached only past a green gate — and no commit is attributable to A6. The assertion is over writer identity and the green-gate precondition, not the pathspec scope those writers pass: that scope may widen under O-8's E-6 resolution, which AT-04-5 asserts. *(BR-8, AC-4.2.)*
...
- **AT-07-1** — *Who:* the workflows suite. *Given* each **agent-proposable** boundary in §4, enumerated rather than inferred: E-5's scope rule, E-6's two halves, the tier's exclusions (BR-5), each prohibition in BR-6, and the no-commit rule (BR-8). *When* a stub agent double returns a violating proposal — no live model, no prompt, the form AT-03-5 already uses. *Then* the proposal is refused by the workflow script, the shipped refusal reason is reported, and the working tree is unchanged. BR-13, BR-14 and BR-16 are excluded by construction: no agent proposes against them. *(BR-16, NFR-1.)*
