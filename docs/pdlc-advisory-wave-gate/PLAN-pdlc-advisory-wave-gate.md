...
| Batch | Gate wording |
|---|---|
| 1 (RED-terminal) | A6-02's and A6-03's retargeted transcriptions **fail for their named reason** — the five-member seam literal, the four-member envelope literal, the absent `waveBudgetPerRun` key, the absent `A6` registry row, and the four bare `toHaveLength(5)` row counts now reading `6` (`advisoryDisabled.test.js:622`, `advisoryQueueSeams.test.js:627`, `advisoryHarvest.test.js:571`, `advisoryHarvest.test.js:726`) — and every other pre-existing test is green. A6-00's pre-flight assertions and A6-01's fixtures are green from the start; red there means the baseline moved and this PLAN is invalid, not that the wave failed. A6-04's engine-channel red is **not** observed by this gate (`implementation.testCommand` scopes to `pdlc/workflows`); it is checked by the task's own `cd pdlc/engine && npm ci && npm test` step. |
