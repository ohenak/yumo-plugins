...
| PROP-CFG-03 | `.claude/pdlc.config.example.json` must carry the **whole** `advisory` section — `{"enabled": false, "waveBudgetPerRun": 1}` — and that section must parse, carry both keys, and hold `waveBudgetPerRun` as a non-negative integer. Asserted in the purpose-named new engine file, never in `ci-arrangement.test.js`, whose stated scope is FSPEC §5.1's CI arrangement alone | Contract | Unit (engine channel) | C-2, TSPEC §4.4, §5.1 | `pdlc/engine/__tests__/advisory-config-example.test.js` (A6-04) |
