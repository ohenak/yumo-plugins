...
**Scope.** Properties derive from REQ v1.15 (AC-1.1…AC-6.4, NFR-1…NFR-6), FSPEC v1.6 (BR-1…BR-16,
E-01…E-34, AT-01-1…AT-07-5), TSPEC v1.11 (§2–§5) and DECISIONS (DEC-A6-01…DEC-A6-04). Every property
...
| PROP-SEAM-02 | Every transcription surface coupled to the catalogue's cardinality must read six, as one set — anchored by symbol or block title per DEC-DOC-01, as TSPEC §1.3 re-anchored the same surfaces: `advisoryDriver.test.js`'s module-scope `GATE_EXCLUSIVITY_REGISTRY` declaration and the set-equality `it` that compares `Object.keys(GATE_EXCLUSIVITY_REGISTRY)` against `ADVISORY_SEAMS`; `advisoryRecord.test.js`'s ordered full-catalogue equality (`expect(rows.map((r) => r.seam)).toEqual([…])`) and its per-seam `test.each` seam list; the harvest and consolidation seam literals (`advisoryHarvest.test.js`'s `seamNames` equality, `consolidationProperties.test.js`'s `rng.pick([…])` seam list); `helpers/advisoryDoubles.js`'s `SEAMS` literal; and the four **bare row-count** assertions named by block title in the Overview's derivation rule 1. At HEAD every one of these reads six already except `advisoryRecord.test.js`'s `rows.map((r) => r.seam)` equality, which still reads `["A1" … "A5"]` — the one test-side literal TSPEC §1.3 records as untranscribed; the set-equality this property fixes is the end state, not the edit list | Contract | Unit | AT-07-2, R-5, BL-06, TSPEC §1.3 | `advisoryDriver`, `advisoryRecord`, `advisoryHarvest`, `consolidationProperties`, `advisoryDisabled`, `advisoryQueueSeams` `.test.js` (A6-03) |
...
| PROP-ENV-13 | Given a repair that writes **only** `.gitignore`d paths, on one run: `producedPaths()` must return `[]`, `apply` must return `{ok:false}`, the refusal reason must be the exact literal `post-action-verification-failed`, an escalation entry must be written, one attempt must be consumed, and the tree must be carried no further — no re-gate invocation may be appended to the ledger after the anchor. The positive control on the same fixture: an otherwise identical repair writing one **non-ignored** in-envelope path returns a non-empty `producedPaths()` and `{ok:true}`, so the refusal is pinned to the ignored-ness and not to the fixture being broken | Error handling | Integration | AC-3.4, BR-9, TSPEC §3.3 (`apply` row), §5.5 (*Ignored-path-only repair*), §6 OQ-11 | `advisoryWaveGate.test.js` (A6-15) |
...
| PROP-CFG-03 | `.claude/pdlc.config.example.json` must carry the **whole** `advisory` section — `{"enabled": false, "waveBudgetPerRun": 1}` — and that section must parse, carry both keys, and hold `waveBudgetPerRun` as a non-negative integer. The file is not a new assertion surface: `pdlc/engine/__tests__/ci-arrangement.test.js`'s module-scope `const configPath` already resolves it from `repoRoot`, and that file's test titled `ci arrangement — .claude/pdlc.config.example.json's implementation.testCommand` already parses it, so A6-06's edit lands under two live assertions (anchored by symbol and test title per DEC-DOC-01; the earlier `:39` / `:799`–`:819` line pins have drifted). The paired blast-radius conjunct is therefore that `implementation.testCommand` still matches both `/cd pdlc\/workflows\s*&&\s*npm test/` and `/cd pdlc\/engine\s*&&\s*npm test/` **after** the advisory key is added — a sibling-key edit that reflowed or truncated `testCommand` would otherwise be caught only incidentally. Asserted in the purpose-named new engine file, never in `ci-arrangement.test.js`, whose stated scope is FSPEC §5.1's CI arrangement alone; extending a second file rather than that one is the batch-safety answer, and `ci-arrangement.test.js` stays unowned by PLAN | Contract | Unit (engine channel) | C-2, TSPEC §4.4, §5.1 | `pdlc/engine/__tests__/advisory-config-example.test.js` (A6-04) |
...
| A6-15 | `advisoryWaveGate.test.js` | PROP-SEAM-07, PROP-SEAM-08, PROP-CTR-03, PROP-CTR-04, PROP-CTR-05, PROP-CTR-06, PROP-CTR-08, PROP-CTR-09, PROP-CTR-11, PROP-CTR-12, PROP-CTR-13, PROP-ENV-03, PROP-ENV-04, PROP-ENV-05, PROP-ENV-08, PROP-ENV-09, PROP-ENV-10, PROP-ENV-12, PROP-ENV-13, PROP-GATE-01…-06, PROP-REST-07, PROP-REST-08, PROP-REST-09, PROP-REC-05, PROP-NFR-03 |
