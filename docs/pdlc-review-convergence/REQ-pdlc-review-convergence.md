...
| `reset-region-corrupt: {reason}` | that exact literal; `{reason}` ∈ `{invalid-window-start, invalid-window-resumed, counts-mismatch}` | the loop, from AC-1.5(4) step 4 | AC-4.7's `notice` column | S-16. **New in v1.5.** AC-1.5(4) has required a corrupt region to be *reported* since v1.4, and the run report had no slot for it and the catalogue no member — so the one operator-visible half of the fail-closed behaviour was unassertable (TE v6 F-02). The enum names which of the two value checks or the counts check refused. |
