...
filed **eight** — SE v8: 0 High, 0 Medium, 5 Low; TE v8: 0 High, 1 Medium, 2 Low — carried on **six**
rows below, because two pairs are the same finding filed independently (SE G-21/TE F-02, and SE G-23's
invocation question shares its answer with TE MR-08). **One** is blocking, and it is not in an
...
| SE Q-19 … Q-21, TE Q-13, TE Q-14 | Answered by the rows above: **Q-19** — one S-16, first failing line in document order; **Q-20** — the invocation terminates on step G's path and the queue row becomes `halted`; **Q-21** — **correct** the line, because deleting it at `H − A = 1` reports `counts-mismatch` next; **Q-13** — step 4's refusal is **unconditional**, and the mid-window outcome is unchanged (`W` falls back to 1, which admits no rounds), stated in step 4; **Q-14** — the `counts-mismatch` recovery costs one halt and **two** clearances, now stated in O-10's recovery leg |
