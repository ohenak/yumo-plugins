...
| **current window** | The rounds admitted by AC-1 since the last operator reset: `{W … W+2}`, where `W` is the `WINDOW-START:` value AC-1.5(4) resolves (1 when no reset is in effect). AC-2.1 and AC-2.8 compare round N with round N−1 **only when both are in the current window**; a round whose predecessor is in an earlier window has no comparable predecessor, exactly as round 1 has none. |
...
| **First round of the current window** (the post-reset offset) | AC-1.5(4), AC-1.1, AC-2.1, AC-2.8 | The `WINDOW-START: {N}` lines in `POSTMORTEM-{phase}-{feature}.md`'s reset region — the region AC-1.4 forbids any halt to rewrite. The window origin is the **greatest** value present | Treated as **1** — no reset is in effect and AC-1.1's absolute cap applies from round 1. Fail-closed: an absent, unparseable, non-increasing or out-of-range marker never widens the window. **Survives a second halt** because AC-1.4 requires the halt path's rewrite to preserve the region verbatim |
| **Reset consumed** (the reset is one-shot) | AC-1.5(4) | The **counts** of `RESOLVED: yes` and `WINDOW-START:` lines in that region: a reset is unconsumed iff `count(RESOLVED: yes) > count(WINDOW-START:)` | Equal counts ⇒ every granted reset is spent; the loop writes nothing and grants nothing. Counting rather than presence-testing is what keeps one-shot true on a file that has accumulated more than one halt (TE v4 F-02) |
...
| **Which halt a POSTMORTEM records** | AC-1.5(4), AC-2.8 | The `HALT-REASON: {string}` line AC-1.5(5) requires every halt to write, carrying the S-3, S-4 or S-11 string verbatim, outside any fenced block | Read as a convergence halt (S-3/S-4) — fail-closed, so an unreadable reason never converts a consuming reset into a free one |
...
therefore ordered **by kind** and not by id, so `S-10` and `S-11` sit beside their kin rather than at
the end. The four kinds, in the order the rows appear: **anchor lines** (S-1, S-2, S-10), **halt
...
report and by the loop itself on the next entry (AC-1.5(4) must know which halt it is clearing). Each is
therefore written into the post-mortem on a line of its own, `HALT-REASON: {the string}`, outside any
fenced block — the same convention `RESOLVED:` already uses. This is a **rendering** of the three
strings above, not a twelfth catalogue member: its receive side is AC-1.5(5)'s, and it introduces no new
vocabulary.
...
| `DOC-SHA256: {64 hex}` | that exact literal, `{64 hex}` lower-case | **`appendRoundAnchors`** — every round, same instant, same read (AC-4.1) | AC-2.8 | The durable home of AC-2.8's identity test (S-10). It exists so *"the document did not change"* is decided exactly rather than by byte length alone; it reuses the hashing the tier-1 approval anchors already perform over the reviewed document (M-4a). |
...
| **O-12** | Specify where AC-2.8's byte-and-hash identity test is evaluated — after round N−1's anchors are readable and **before** round N's reviewers are dispatched — and how the S-11 halt reaches the post-mortem writer on the same path as S-3 and S-4 (AC-1.4). Note it must not consume a round of AC-1's budget. | FSPEC → TSPEC |
