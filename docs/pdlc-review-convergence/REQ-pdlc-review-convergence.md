...
before FSPEC authoring**. This REQ introduces **sixteen**. All sixteen are fixed here — id, exact
emitted form, emitter, receiver, and the receiver's behaviour on **every** input outside the catalogue.
FSPEC may not add a seventeenth without amending this table.
...
Ids are stable once assigned, so the ones added in v1.2, v1.4 and v1.5 keep the next free numbers; the
table is therefore ordered **by kind** and not by id, so `S-10` … `S-16` sit beside their kin rather
than at the end. The five kinds, in the order the rows appear: **anchor lines** (S-1, S-2, S-10),
**halt reasons** (S-11, S-3, S-4), **report notices** (S-5, S-6, S-16), **section headings and fields**
(S-7, S-8, S-9, S-12), **reset-region lines** (S-13, S-14, S-15) — SE v4 MF-3.
...
| **S-16** | Report notice `reset-region-corrupt: {reason}` where `{reason}` ∈ `{invalid-window-start, invalid-window-resumed, counts-mismatch}` — a closed three-member enum. The notice carries the post-mortem's path and, per reason, the offending value or the pair `H`/`A` | AC-1.5(4) step 4 | the run report row of AC-4.7, in the `notice` column | the enum is closed here; a reason outside it is a defect, not a fallback. The notice is the operator's only signal that the region needs the sanctioned repair AC-1.5(4) describes, so a corrupt region is diagnosable rather than a silent permanent halt (TE v6 F-02) |
