...
FINDING: High | delta | local | Obligations table K-3 row unterminated — two pipes not five; `Owner` and `Falsified by` cells stranded in the following divergence paragraph, terminating the table at K-3 and leaving K-4..K-9 outside it
FINDING: Low | inherited | nonlocal | v1.4 changelog entry retains the `pdlc/README.md:231` raw anchor and the superseded "not a tenth site-table row" framing
