...
|----|----------|-----------|----------|---------|----------------|
| F-01 | Medium | delta | nonlocal | K-3's *"Upstream divergence, owed to TSPEC and not resolved here (TE F-05)"* clause states that TSPEC §2.1's row "describes P9-02's title count as moving *six → seven*". TSPEC v1.7 repaired that row: it now states the title and comment are already stale at HEAD, measures the literal at seven, and moves it seven → eight. The divergence is discharged and the erratum satisfied, so the clause — and the matching sentences in the v1.5 and v1.6 changelogs — cite upstream text upstream no longer says (DEC-ERR-03). Non-gating: every engineering conjunct of K-3 (four entries, `4 + 1 + 2` = seven, feature makes eight, title and comment stale at HEAD and moving as a clause) re-measured true at HEAD, and TSPEC has converged onto them. Only the disagreement bookkeeping rotted. Owed to the next DECISIONS touch, not to this round. | Obligations table, K-3; changelog v1.5 *"carried unresolved by design"*; changelog v1.6 *"erratum owed upstream"* |
| F-02 | Medium | inherited | nonlocal | The v1.6 changelog's grounding attestation (*"Upstream re-grounded first and did not move: TSPEC HEAD is v1.4 … FSPEC HEAD v1.5 … REQ HEAD (v1.4) matches its pin"*) is now three versions stale — HEAD is TSPEC v1.7, FSPEC v1.7, REQ v1.6. True when written, false now. I performed the absorption check independently this round and nothing is owed, so no load-bearing claim is falsified; the risk is that a later round reads the attestation to decide whether re-grounding is needed. Carried unchanged from v9 F-01, now staler by one TSPEC version. | Changelog, v1.6 grounding attestation |
| F-03 | Low | inherited | nonlocal | The sentence introducing the site table still reads *"Four hold the enumerations; five pin them from two other packages"*, which contradicts the document's own 5 + 4 + 1 breakdowns two paragraphs below and in TSPEC §7.3 — `pdlc/workflows/package.json`'s `c8.include` is a fifth *holder* whose transcribed members pin nothing (`coverageInstrumentation.test.js` pins it), so there are four pinning files. Totals ten either way; no count word downstream is wrong, no falsifier or K-row is affected. Carried unchanged from v9 F-02 (originally v8 F-01). | Options Considered / DEC-STATS-01, paragraph introducing the site table |
| F-04 | Low | inherited | nonlocal | Process defect in the cascade mechanism, not in this document. Two consecutive rounds have recorded an `UPSTREAM-STATE` TSPEC pin that resolves to no version in this branch's history for that path — v8's `512a9fcf…` and v9's `235fd3dd…` — while each round's body measured a different, real hash (v9's body: `37422160…`). Both rounds recovered manually by trusting the body over the trailer. An unresolvable pin silently turns a cascade confirmation into a guess about the baseline; a reviewer who does not cross-check body against trailer will diff from the wrong base. Recommend the trailer pin be emitted from the same measurement the body reports, and that a non-resolving pin be treated as fail-closed by the round that reads it. | v9 / v8 `UPSTREAM-STATE` trailers (mechanism, not document bytes) |

FINDING: Medium | delta | nonlocal | Obligations table, K-3 — "Upstream divergence, owed to TSPEC (TE F-05)" clause | K-3 states TSPEC §2.1 "describes P9-02's title count as moving six → seven"; TSPEC v1.7 repaired that row to seven → eight and the erratum is discharged, so this clause and the matching v1.5/v1.6 changelog sentences cite upstream text upstream no longer says (DEC-ERR-03). Non-gating: all of K-3's engineering conjuncts re-measured true at HEAD and TSPEC converged onto them; only the disagreement bookkeeping is stale. Owed to the next DECISIONS touch.
FINDING: Medium | inherited | nonlocal | Changelog, v1.6 grounding attestation | "Upstream did not move: TSPEC v1.4 / FSPEC v1.5 / REQ v1.4" is three versions stale at HEAD (v1.7 / v1.7 / v1.6); the absorption check was done independently this round and nothing is owed, but a later round may read the attestation to decide whether to re-ground.
FINDING: Low | inherited | nonlocal | Options Considered / DEC-STATS-01, site-table preamble | "Four hold the enumerations; five pin them" contradicts the document's own 5 + 4 + 1 breakdowns and TSPEC §7.3; total is still ten and no falsifier or K-row is affected.
...
REVIEWED-COMMIT: 10963e85dcf2d62fb869f704f02d9d2c76484ba7
