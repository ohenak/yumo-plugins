...
104 properties in thirteen domains. Categories are the skill's table (Functional, Contract, Error
...
| PROP-ERR-10 | The set of `error.reason` values the implementation can emit must be **set-equal**, in both directions, to a hand-transcribed literal `["not_found","no_docs_root","unreadable_feature"]` — never containment. Collected behaviourally by driving every refusal scenario of FSPEC §5's table through `runStats` under `--json` (the corpus PROP-ERR-09 already builds) and gathering the distinct `error.reason` strings, never by reading a module constant. A fourth reason released without an FSPEC edit fails, and deleting any one of the three fails — the protection REQ R-5's field-drift class already gives the success document's key set (PROP-JSON-03) and BR-30's enum lacked. | Contract | integration-fake | REQ R-5, BR-30, AT-23, AT-27; PLAN T-07 |
...
| **Reason-catalogue equality** | drive every refusal row of FSPEC §5's table through `runStats` under `--json`, collect the distinct `error.reason` strings, assert set-equality with the hand-transcribed literal `["not_found","no_docs_root","unreadable_feature"]` in both directions | a fourth reason ships without an FSPEC edit, or one of the three is deleted and the case-by-case assertions of PROP-ERR-01/-04/-05 leave the enum itself unpinned (PROP-ERR-10) |
