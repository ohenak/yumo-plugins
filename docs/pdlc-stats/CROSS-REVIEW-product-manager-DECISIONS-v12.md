...
|----|----------|-----------|----------|---------|----------------|
| F-01 | Medium | delta | local | The v1.7 entry attests *"Every count, option, obligation and falsifier below re-measured true at HEAD."* The options, obligations and falsifiers do re-measure true; the **counts** do not. This feature's implementation landed on this branch before the attestation was written (`c3acd694d` T-16 at 15:32 through T-20 at 15:51, all ancestors of the changelog commit `f3ab46e72` at 16:43), so at HEAD `prepack.mjs`'s `MODULE_NAMES` holds **five** including `lib/stats.mjs` and `package.json`'s `c8.include` holds **eight** — not the four and seven the site table and K-3 record as HEAD. Non-gating: the decision content is untouched (still ten sites, same partition, same falsifiers), the moved digits are exactly the `4 → 5` and `7 → 8` deltas the document specified, and no downstream consumer reads the baseline digits. Fix by scoping the attestation — *"re-measured against the pre-implementation baseline at commit X"* — rather than by restating the table | v1.7 changelog, re-grounding paragraph |
| F-02 | Low | inherited | nonlocal | The site table's *Members at HEAD* column, and K-3's *"`c8.include` is seven at HEAD to match"*, now describe a baseline the branch has moved past. Pre-existing at v1.6 and not created by this edit — the edit only made the attestation above assert over it. Reading the column as a live claim about the post-implementation tree would make any DECISIONS document self-falsifying once its own feature merges, which is why this is Low and not the same finding as F-01. A *"measured at commit X"* stamp on the table header resolves both | *Option A's ten sites* table, *Members at HEAD* column |
| F-03 | Low | delta | local | The changelog credits the K-3 repair as *"closes PM F-01 (carried from v6–v9)"*. I carried F-01 through **v11**, not v9 — it appears in my v10 and v11 cross-reviews as an open Low. The repair itself is complete and correctly described; only the provenance range is short by two rounds. Matters only because harvest reads these ranges to judge how long a finding sat open, and a two-round understatement makes the channel look faster than it was | v1.7 changelog, record correction (1) |

FINDING: Medium | delta | local | v1.7 changelog, re-grounding paragraph | The new attestation "Every count, option, obligation and falsifier below re-measured true at HEAD" is false for the counts: the feature's own implementation (T-16 c3acd694d at 15:32 through T-20 at 15:51) landed before the changelog commit f3ab46e72 at 16:43, so at HEAD MODULE_NAMES holds five including lib/stats.mjs and c8.include holds eight, not the four and seven the site table and K-3 record as HEAD. Non-gating — no decision, option, obligation or falsifier moves, and the changed digits are exactly the deltas this document specified — but the attestation should be scoped to the pre-implementation baseline commit rather than claiming HEAD.
FINDING: Low | inherited | nonlocal | Option A's ten sites table, Members at HEAD column | The Members at HEAD column and K-3's "c8.include is seven at HEAD to match" now describe a baseline the branch has moved past, since the implementation landed on this branch. Pre-existing at v1.6, not created by this edit; a design document's at-HEAD column is conventionally the pre-feature baseline, so the fix is a "measured at commit X" stamp on the table header rather than restating the digits.
FINDING: Low | delta | local | v1.7 changelog, record correction (1) | The K-3 repair is credited as closing "PM F-01 (carried from v6-v9)", but I carried F-01 as an open Low through v10 and v11 as well; the repair is complete and correctly described, only the provenance range is short by two rounds, which understates how long the finding sat open when harvest reads the range.
