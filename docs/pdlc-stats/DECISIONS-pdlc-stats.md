...
- **Co-change surface roughly the size of the feature's own logic.** One ~300-line module costs
  **nine** edit sites — five enumerations plus the four test files that pin their membership or size
  (`loop-distribution.test.js`, `coverageInstrumentation.test.js`, `run.test.js`,
  `learningsPremises.test.js`) — and an amendment to a completed feature's frozen table. The number is
  sweep-derived; it grew from five to six to nine over three review rounds, and the reason it stopped
  growing is that it is now produced by a query — repo-scoped and run with `git grep -l`, after round
  4 showed that a `__tests__/`-scoped `grep -rln` both missed production-side copies and silently
  dropped NUL-containing files — rather than by reading files. Plus `pdlc/README.md:231`'s prose
  count, which no oracle pins: it is owed as an edit under K-9, it drifts silently if forgotten, and
  it is corrected by review, not by a red. Accepted because every
