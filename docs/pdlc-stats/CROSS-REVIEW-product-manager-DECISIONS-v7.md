...
|----|----------|-----------|----------|---------|----------------|
| F-01 | Medium | delta | local | The count moved to **ten** but two prose breakdowns still name **nine** items, omitting `pdlc/README.md`: *What the sweep found* (*"exactly the ten in the table above: the five enumeration holders … plus `loop-distribution.test.js`, `coverageInstrumentation.test.js`, `run.test.js` and `learningsPremises.test.js`"* = 5 + 4) and *Reversibility: hard* (*"all ten sites — the five enumerations and the four test files"* = 5 + 4). Both were updated in their count word and not in their list. *Standing costs accepted* already carries the correct form to copy. Non-gating: the site table, K-1's partition and K-9's ownership all cover site 10, so no obligation is unowned. | *What the sweep found, and why two rounds of per-file reading kept missing sites* / `DEC-STATS-01` *Reversibility: hard* |
| F-02 | Low | inherited | nonlocal | TSPEC §2.1's `coverageInstrumentation.test.js` row still states P9-02's title moves *"six → seven"*. Re-measured at HEAD: `REQUIRED_INCLUDES` holds four entries, so `c8.include` holds seven today and this feature makes it **eight**. DECISIONS carries the correct arithmetic and books the difference as an erratum owed upstream — the right handling. Repair belongs in **TSPEC**; this document must not be edited into agreement with a number known to be wrong. Carried from v6 F-05, unresolved upstream. | K-3 / `DEC-STATS-01` site table, `coverageInstrumentation.test.js` row |
| F-03 | Low | inherited | nonlocal | The **v1.4 changelog entry** still carries raw `file:line` anchors of the class `DEC-DOC-01` forbids — `pdlc/README.md:231`, `publish-preflight.mjs:205-219`, `loopProperties.test.js:370` — including the very anchor this round removed from K-9. Historical changelog prose, untouched by this edit, and the `loopProperties.test.js:370` case arguably qualifies for the runtime-measured-evidence carve-out (the NUL bytes at that position are the measurement). The v1.5 entry's own mention of the removed anchors is mention-not-use and is fine. Cosmetic; fold in only if the section is touched. | *v1.4 (cross-review round 4)* changelog entry |

FINDING: Medium | delta | local | What the sweep found / DEC-STATS-01 Reversibility: hard | The co-change count moved to ten but two prose breakdowns still enumerate nine items, omitting `pdlc/README.md` — "exactly the ten in the table above: the five enumeration holders … plus loop-distribution.test.js, coverageInstrumentation.test.js, run.test.js and learningsPremises.test.js" (5+4), and "all ten sites — the five enumerations and the four test files" (5+4). Count word updated, list not. Standing costs accepted carries the correct ten-item form to copy. Non-gating: site table, K-1's partition and K-9's ownership all cover site 10.
FINDING: Low | inherited | nonlocal | K-3 / DEC-STATS-01 site table coverageInstrumentation.test.js row | TSPEC §2.1 still says P9-02's title moves six → seven; re-measured at HEAD c8.include holds seven entries already (REQUIRED_INCLUDES is four), so this feature makes it eight. DECISIONS is correct and records the divergence as owed upstream — erratum belongs in TSPEC, this document must not be corrected into agreement. Carried from v6 F-05.
FINDING: Low | inherited | nonlocal | v1.4 changelog entry | The v1.4 changelog paragraph still carries raw file:line anchors (pdlc/README.md:231, publish-preflight.mjs:205-219, loopProperties.test.js:370) of the class DEC-DOC-01 forbids, including the anchor this round removed from K-9; historical prose untouched by this edit, and the NUL-byte case arguably falls under the runtime-measured-evidence carve-out. Cosmetic.
