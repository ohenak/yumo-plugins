...
Addresses every finding of `CROSS-REVIEW-product-manager-TSPEC-v2.md` (1 High, 2 Medium, 3 Low) and
`CROSS-REVIEW-test-engineer-TSPEC-v2.md` (1 High, 3 Medium, 1 Low). **Both Highs were the same
failure — a v1.1 fix that moved its defect rather than removing it — and both are now stated as
invariants over *every* path rather than as behaviour at an enumerated step.**
...
Addressed every High and Medium of `CROSS-REVIEW-{product-manager,test-engineer}-TSPEC-v1.md`
(5 High, 8 Medium, 2 Low) and settled four open questions. Both shared defects were fixed once, at
the root, **by deleting an over-generalised clause rather than adding a reconciling one** (lesson
R-5). Round-2 review verified 4/5 PM and 9/10 TE fixes as complete; the two partial fixes are v1.2's
PM F-01 and TE N-01 above.

| Finding | Sev | Resolution | Where |
|---|---|---|---|
| **PM F-01** | High | POSTMORTEM gate demoted from a global pre-step-1 gate to a per-path query (partial — see v1.2) | §2.5, §5.8 |
| **PM F-02 ≡ TE F-02** | High | The mode-blind trailer conjunct **deleted** from the greenfield path; `isTerminal(mode, …)` introduced | §5.6.2, §4.3 |
| **PM F-03 ≡ TE F-01** | High | `selectMode` specified as the only producer of `EpisodeKey.mode`; §5.6.3's rule demoted to *prompt kind* (partial — see v1.2) | §5.6.1, §5.6.3 |
| **TE F-03** | High | AT-19 scoped to FSPEC's closed thirteen-name list, not `main()`'s parameters, which reds on correct source | §8.5 |
| **TE F-04** | Medium | AT-64's hand-list replaced by the E-1/E-2/E-3 predicate; the reviewer's own predicate measured too wide and declined | §8.5 |
| **TE F-05** | Medium | §5.3's pre-count became a count **and** a comparison; E-14 separated from E-15 | §5.3 |
| **TE F-06** | Medium | `deriveRoundWindow` keeps its rejects and returns `skipped: { basename, reason }[]` | §5.2, §3.7 |
| **TE F-07** | Medium | §7.2 edit 2 split into 2a (`_git` only) and 2b (`_recordHalt` in the `_runPipeline` closure) | §7.2, §3.5 |
| **TE F-08** | Medium | The shipped `driftGenerators.js` reused rather than a second generator written; seven-row property table | §8.2, §9.4 |
| **PM F-04** | Medium | Carried items bound to a `QUEUE.md` row instead of prose owners (`DC-08`) | §10.2, §10.3 |
| **PM F-05 ≡ T-Q-01** | Medium | `PR` missing from the skip-eligible set: ruled a drafting slip and corrected upstream — **REQ v1.6** (AC-4.7) and **FSPEC v1.6** (§10.7, §11.1's catalogue where `all` expands to six, §11.3). Downstream here: §5.5's scope, §5.7's `valid` array and its derived rejection message, §6.2 row 12, AT-29. Amending approved upstream documents is deliberate and authorised, and recorded in all three changelogs because it is not routine | REQ, FSPEC, §5.5, §5.7 |
| **TE F-09** | Low | `parseApprovalHash`'s `reviewedCommit` is `"unavailable"` when absent/duplicated/non-hex, never a `HashFailure` | §3.7, §4.3 |
...
| Symbol | Change |
|---|---|
| `export async function reviewLoop({ doc, phase, reviewers, optimizer, feature, iteration = 1, docType, _agent, _parallel, _checkFile, _listFiles, _readFile })` | the `iteration = 1` default stays (it is the correct value for a virgin branch), but **all seven existing call sites now pass the branch-derived `startIndex`**; the gate `if (iteration > 5)` becomes `if (iteration > endIndex)`; the return shape gains `postmortemWritten` and `trailerReason`. **Three new parameters, all for §5.6.1's S-INV**: `docType`, plus the two seams `refreshReviewState` needs to read the review record at every episode entry. No seed `present` / `reviewFiles`: the first episode refreshes like every other one, so a seed would be an unread parameter (§5.6.1). Without `_listFiles` and `_readFile` in this scope the mode of every episode after the first is decided from a snapshot taken before the loop, which is TE-v2 N-01 |
...
Every row below is either **closed here** or **bound to a named successor surface**, per `DC-08` — no row
is owned by prose intent alone. **FSPEC v1.7 records the same three dispositions in §20**, so the two
documents no longer disagree about who owns them (PM-v2 F-06). The successor surface for the two carried
rows is **`docs/_queue/QUEUE.md` Order 9, `pdlc-authoring-contract`** — `blocked`, added 2026-07-30 in the
same convention as rows 6 and 7, depending on this feature, whose scope is exactly these items: the six
author/review SKILLs declaring machine-readably what they produce.

| FSPEC Q | Owner | Status |
|---|---|---|
| **Q-05** — if a future `harvest-learnings` revision adds a sixth prose section, does the approval record renumber to `## 7`, or is the heading pinned by name? | **QUEUE.md Order 9** | **Carried, bound.** No product exposure today: §5.9's heading matcher normalises numeric prefixes, so this implementation tolerates either answer. The open part is the SKILL's contract, which is Order 9's scope. |
| **Q-06** — should the `RESOLVED:` marker also record who resolved it and when? | — (closed) | **Declined, closed — not carried.** Nothing branches on it: §5.8 parses the boolean only (C-5), and no AT, report field or halt message consumes provenance, so the field would be write-only — and git already carries author and timestamp on the commit that flipped the marker, retrievably (`git log -S 'RESOLVED: yes' -- {postmortemPath}`). Reopen only if a consumer appears that must branch on *who* resolved a POSTMORTEM. |
| **Q-09** — should the six author/review SKILLs declare the top-level heading template §16.2 measures against? | **QUEUE.md Order 9** | **Carried, bound, and the acute one of the two.** §5.9's per-class heading lists live only in the workflow script; the templates authors follow live in the SKILLs, so the two can drift and the failure mode is a correct document scored incomplete — **a false halt**. Not closed here because closing it means editing six SKILLs, widening this feature's blast radius from the workflow scripts to the prompts driving every phase. Mitigation until Order 9 lands: §8.3's `completeness.test.js` fixtures are copied from the SKILL templates verbatim, so a drift reds the suite rather than a run. |

### 10.3 New questions this TSPEC raises

| # | Question | Owner | Disposition |
|---|---|---|---|
| **T-Q-01** | **The skip-eligible phase set omitted `PR` (PROPERTIES).** AC-4.7, FSPEC §10.7 and §11.3 named the set as five phases while `PHASE_DISPATCH` carries six document-review phases, and `PR` satisfies AC-4.7's own functional criterion (named creator, the same reviewer pair, convergence through a cross-review artifact) exactly like the other five. | REQ / pm-author | **Settled — ruled a slip, corrected at REQ altitude.** Raised independently by both round-1 cross-reviews (PM F-05, TE T-Q-01); decisive evidence was that AC-4.7a's unread-record consequence applied to a PROPERTIES approval record no differently from a PLAN one, so the enumeration contradicted the criterion it claimed to enumerate. **REQ v1.6** widens AC-4.7 to `R, F, T, P, D, PR` and **FSPEC v1.6** follows (§10.7, §11.1, §11.3). Here: §5.5's scope, §5.7's `valid` array, `parseForcePhases`' catalogue and rejection message, and AT-29's operator text. One token in a closed enumeration — the mechanism was already total over six phases. |
...
| **T-Q-03** | **`MAX_AUTHORING_WRITE_BYTES` has no oracle, and §6.6's commit-diff proxy is advisory**, so the constant is enforced only by agent compliance. The only stronger control available under C-2 is to halt the *pipeline* on an oversized commit — which converts a stylistic violation into an outage and would fire on a legitimately large section. | **QUEUE.md Order 9** | **Accepted as-is, and bound.** Recorded rather than hidden: a constant that looks enforced but is not is worse than one documented as advisory. Same class of gap as Q-09 — a contract the script states and the SKILL does not — so bound to the same row rather than to "post-implementation observation", which nobody owns (`DC-08`). Revisit there with measured non-compliance data. |
...
| **T-Q-05** | **Adding `queueModule` to the dev bundle (§7.2 edit 4) grows `orchestrate-dev.bundle.js` by the whole queue module** (~1,150 source lines) for the sake of `_recordHalt`'s row helpers, and no bundle size budget is documented anywhere in the repo. | — (closed) | **Closed, not carried.** Measured, not estimated (`DC-02`) — the numbers are in §7.2: `orchestrate-queue.bundle.js` already inlines the whole of `orchestrate-dev` and is ~1.5× the bundle this edit grows, in production since `pdlc-workflow-distribution` landed with no observed size problem. There is no budget to breach. The duplicate-row-helpers alternative is **declined on the merits, not deferred**: a second copy of the queue's table grammar can diverge, and a divergence is a silent wrong-row rewrite — strictly worse than bytes. |
