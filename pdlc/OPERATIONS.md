...
A **decision ledger** block can be appended to reviewer prompts, surfacing hand-authored `DEC-*` decision records from project- and feature-level DECISIONS files so reviewers see prior decisions instead of re-litigating them (REQ NG-1…NG-6). It ships **off by default**: `.claude/pdlc.config.json` → `decisionLedger.enabled` defaults to `false`; with it `false` (or the section absent/malformed) `report.decisionLedger` is not set and every reviewer dispatch receives the identical, block-free prompt (parity with the shipped `learningsInjectionField` discipline).

- **Recognition rule.** Decision headings are found line by line via a frozen heading regex applied to each DECISIONS file's text; on an id collision between a project-level and a feature-level record, the project-level record wins, keyed on origin rather than on path order (TSPEC §3.4).
- **Bounds and omission order.** Two independent thresholds jointly cap the rendered block, `maxEntries` (default `70`) and `maxBytes` (default `12500`). Candidates are dropped one at a time from the tail of the ordered set until both bounds are satisfied, and at each drop the **entries** bound is checked before the **bytes** bound: a drop caused by exceeding `maxEntries` is tagged `RSN-ENTRIES`, otherwise `RSN-BYTES`.
- **Omission reasons:** `RSN-ENTRIES`, `RSN-BYTES`.
- **Fail-open legs.** `parseDecisionLedgerConfig` is pure and total, structurally cloned from `parseLearningsConfig`: a malformed or absent `decisionLedger` section degrades the whole block to `DECISION_LEDGER_DEFAULTS` and emits `NTC-DECLEDGER-MALFORMED`; independently, a single wrong-typed key falls back to that key's own default without retuning any other key and emits `NTC-DECLEDGER-KEYTYPE` — per-key independent fallback, never a whole-block failure for a one-key defect.
- **Notices:** `NTC-DECLEDGER-MALFORMED`, `NTC-DECLEDGER-KEYTYPE`.
- **Config keys:** `enabled`, `maxEntries`, `maxBytes`.
