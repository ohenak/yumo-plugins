...
const LOOP_SOURCES = Object.freeze(["pipeline-halt", "merge-refusal"]);
...
const ADVISORY_ROOT_CAUSE_MEANINGS = Object.freeze({
  "plan-ordering-defect":
 ...
 * @returns {string[]}
 */
function a6ProhibitedPaths(feature) {
...
const A6_MIN_CITATION_CHARS = 24;
...
const PIN_CHECK_DEFAULTS = Object.freeze({ enabled: false });
const DERIVATIVE_STOP_DEFAULTS = Object.freeze({ enabled: false, rounds: 2 });
 ...
 * @returns {{ config: object, sectionMalformed: boolean, invalidKeys: string[] }}
 */
function parsePinCheckConfig(text) {
  const degraded = (sectionMalformed) => ({
 ...
 * @returns {{ config: object, sectionMalformed: boolean, invalidKeys: string[] }}
 */
function parseDerivativeStopConfig(text) {
  const degraded = (sectionMalformed) => ({
...
function selectDecisions({ entries, thresholds }) {
  const failedSources = [];
  const emptySources = [];
...
const DECISION_LEDGER_CORPUS_OUTCOMES = Object.freeze({
...
async function gatherDecisionCorpus({ feature, _git, _readFile }) {
  try {
...
function buildDecisionLedgerInjector({ config, sink, _git, _readFile, _log }) {
 ...
 * @returns {boolean}
 */
function citesGateOutput(evidence, gateOutput) {
...
function gateSequenceFor(implConfig) {
 ...
 * @returns {string}
 */
function redactEntryText(value) {
 ...
 */
function classifyRoundFindings(prev, curr) {
 ...
 * @returns {boolean}
 */
function derivativeStopReached(history, rounds) {
 ...
 */
function refreshUpstreamStateText(fileText, rows) {
 ...
 */
function parsePinCheckVerdicts(text) {
 ...
 * @returns {string}
 */
function pinCheckPrompt({ feature, target, upstreamPath, targetHash, docs = [] }) {
  const rows = docs
    .map(
      (d) =>
        `DOC: ${d.docType}\n` +
 ...
 * @returns {number}
 */
function deriveDodRoundIndex(basenames, feature) {
 ...
 */
function classifyDodFindings(codeReviewText, feature) {
 ...
 * @returns {string[]}
 */
function waveOwnedPaths(waves, waveIndex) {
 ...
 * @returns {string[]}
 */
function laterOwnedPaths(waves, waveIndex) {
  const out = new Set();
  for (let j = waveIndex + 1; j < (waves ? waves.length : 0); j++) {
    for (const task of waves[j]) {
      for (const f of task.files || []) out.add(f);
 ...
 * @returns {boolean}
 */
function ownedSetCovers(ownedSet, path) {
...
const RESUME_OUTCOMES = Object.freeze(["full-run", "resume", "skip-phase"]);

/** FSPEC BR-07 — the provenance vocabulary, closed at two (TSPEC §3.1). */
const RESUME_PROVENANCE = Object.freeze(["operator-set", "automatic"]);
 ...
 */
const WAVE_IGNORE_REASONS = Object.freeze({
 ...
 * ancestry.
 */
const ANCESTRY_INDEPENDENT_CODES = Object.freeze([
  null,
  "unreadable-json",
  "not-an-object",
  "wrong-shape",
 ...
 */
function classifyWaveLedger({ parsed, feature, planHash, waveCount, headOk }) {
 ...
 */
async function captureTreeSnapshot({ feature, waveNum, _git, _sleep, emit, failure }) {
