...
# Splits "$1" on TAB into the global FIELDS array, preserving empty fields (TE F-10). Manual
# prefix-stripping — not `IFS=$'\t' read -a`/`read -r ... a b c` — because tab is an
# IFS-whitespace character even when it is the sole IFS member: consecutive tabs collapse and a
# trailing empty field is dropped, silently shifting every LATER field left (e.g. an empty `id`
# case's `stamp`/`nn` would be read into `a`/`b`, and `c` would come back empty). Same technique
# as the sibling driver's `bin/lib-probe.sh` `split_tab_fields`.
split_tab_fields() {
  FIELDS=()
  local remaining="$1"
  while [[ "$remaining" == *$'\t'* ]]; do
