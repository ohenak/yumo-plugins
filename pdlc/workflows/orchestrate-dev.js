...
export const DECISION_LEDGER_DEFAULTS = Object.freeze({
  enabled: false,
  maxEntries: 70,
  maxBytes: 12500,
});
...
// emits neither.
export const DECISION_LEDGER_NOTICES = Object.freeze({
 ...
 * @returns {{ config: object, sectionMalformed: boolean, invalidKeys: string[] }}
 */
export function parseDecisionLedgerConfig(text) {
  const degraded = (sectionMalformed) => ({
 ...
 */
export function recogniseDecisionRecords(text, sourcePath) {
...
export const DECISION_LEDGER_PREAMBLE =
...
export const DECISION_LEDGER_RULE_TEXT =
 ...
 * @returns {string}
 */
export function renderDecisionLedgerBlock({ selected }) {
...
export const DECISION_LEDGER_OMIT_REASONS = Object.freeze(["RSN-ENTRIES", "RSN-BYTES"]);
 ...
 */
export function planLintFeedForwardClause(diagnostics) {
 ...
 */
export function headingFeedForwardClause(docType) {
  const rows = REQUIRED_HEADINGS[docType];
 ...
 */
export function nearestHeadingMisses(fileText, docType) {
  const rows = REQUIRED_HEADINGS[docType];
  if (!Array.isArray(rows) || rows.length === 0) return [];
  const sections = topLevelSections(fileText);
 ...
 */
export function nearestHeadingMissClause(misses) {
