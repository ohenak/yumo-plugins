...
6. **The task table is machine-parsed — parse-verify the document you wrote.** The orchestrator's parser accepts a task-table header only when it carries an exact id cell (`#`, `ID`, `Task ID`) and an exact dependencies cell (`Deps`, `Dependencies`, `Depends on`, `Prerequisites`) — substring look-alikes in other tables (a risk register's `Depends on what…` column) are deliberately not matched. Write task ids identically in the id column and in `Deps` cells (no `**bold**` in one and bare in the other — the mismatch reads as a missing task and fails the DAG). A PLAN whose table the parser cannot read, or whose dependencies contain a cycle, is refused at Phase P — it never reaches implementation.
...
5. Address only what is **not already reflected** in the document as it stands. A finding already
   applied needs no new write — writing something gratuitously to "show progress" is an error.
6. Observe the [Authoring Pacing Contract](#authoring-pacing-contract) while you edit, and end your
