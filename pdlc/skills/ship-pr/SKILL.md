---
name: ship-pr
description: Ship role. Raises (or reuses) the pull request for a completed feature branch and reports the current GitHub Actions check status back to the orchestrator. Invoked by orchestrate-dev in Phase PUB, after harvest, once all implementation and test automation is done. The orchestrator owns the polling cadence and the pass/fail/timeout decision — this skill only performs one discrete action per invocation and reports state via a trailer.
---
...
1. **Create the PR** for `feat-{feature}` against the default branch.
2. **Report CI status** — inspect the PR's GitHub Actions checks once and report the current state.

**Scope:** You raise/reuse a PR and report CI state. You do NOT merge the PR, you do NOT loop or sleep waiting for checks (the workflow owns the polling cadence and the timeouts), and you do NOT edit feature artifacts or code. One discrete action per invocation.
...
1. Ensure `feat-{feature}` is pushed to `origin` (push it if needed).
...
Terse and factual. The orchestrator parses only the trailer line; keep any prose above it short. Never invent a PR URL or a check result — if you cannot determine the state, report `CI_STATUS: pending` and let the workflow's timeout handle it.
